...
public class BaseCompanyServiceImpl implements BaseCompanyService {
    ...
    @Override
    public List<Integer> getAllDealerCompanyId(Integer companyId) {
        return baseCompanyDao.getAllDealerCompanyId(companyId);
    }

    @Override
    public List<BaseCompanyVo> getAllDealerCompany(Integer companyId) {
        return baseCompanyDao.getAllDealerCompany(companyId);
    }

    @Override
    public List<BaseCompanyVo> getAllCompany(SysUser sysUser) {
        BaseCompany baseCompany = baseCompanyDao.selectByPrimaryKey(sysUser.getBusinessAdministrationId());

        List<BaseCompanyVo> allCompanyList = new ArrayList<>();
        Integer value = CompanyTypeEnum.MANUFACTURER.getValue();
        Integer businessType = baseCompany.getBusinessType();
        logger.info(value.equals(businessType) ? "相等" : "不等");
        if (CompanyTypeEnum.MANUFACTURER.getValue().equals(baseCompany.getBusinessType())) {
            allCompanyList = baseCompanyDao.getAllDealerCompany(baseCompany.getId());
            if (CollectionUtils.isEmpty(allCompanyList)) {
                allCompanyList = new ArrayList<>(1);
            }
        }
        BaseCompanyVo baseCompanyVo = new BaseCompanyVo();
        BeanUtils.copyProperties(baseCompany, baseCompanyVo);
        allCompanyList.add(0, baseCompanyVo);
