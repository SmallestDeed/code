...
public class FullHouseDesignPlanController {
    ...
    @PostMapping(value = "/addFullHouse")
    public Object addFullHouse(@RequestBody FullHouseDesignPlanAdd fullHouseDesignPlanAdd, SysUser user){
        if (fullHouseDesignPlanAdd == null || fullHouseDesignPlanAdd.getHouseId() == null) {
            return new ResponseEnvelope<>(false,"参数错误");
        }
        Integer fullHouseId = fullHouseToCBizService.addTempFullHouseDesignPlan(user.getId().intValue(), null,fullHouseDesignPlanAdd.getHouseId());
        if (fullHouseId != null) {
            Integer mainTaskId = autoRenderTaskService.addMainTask(fullHouseId, fullHouseDesignPlanAdd.getHouseId(), user);
            if (mainTaskId != null) {
                autoRenderTaskStateService.addMainTask(fullHouseId, fullHouseDesignPlanAdd.getHouseId(), user, mainTaskId);
                Map<String, Integer> result = new HashMap<>();
                result.put("fullHouseId", fullHouseId);
                result.put("mainTaskId", mainTaskId);
                return new ResponseEnvelope<>(true, result);
            }
        }
        return new ResponseEnvelope<>(false, "添加方案失败");
    }

    @GetMapping(value = "/{name}/{action}")
    public Object getFlag(@PathVariable("name") String name,
                          @PathVariable("action") String action,
                          SysUser user){
        boolean flag = true;
        if ("close".equals(action)) {
            flag = redisService.bloom(name, user.getId() + "");
        } else if ("query".equals(action)) {
            flag = redisService.bloomWithoutPut(name, user.getId() + "");
        }
        if (flag) {
            return new ResponseEnvelope<>(true, "用户已关闭过提示，不需要显示提示", false);
        } else {
            return new ResponseEnvelope<>(true, "用户未关闭过提示，显示提示", true);
        }
    }
