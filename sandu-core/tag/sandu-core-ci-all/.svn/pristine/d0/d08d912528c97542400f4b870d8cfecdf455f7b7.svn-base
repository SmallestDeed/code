...
public class FullHouseToCBizServiceImpl extends AbstractFullHouseBizService implements FullHouseToCBizService {
    ...
    public String copyFullHouseDesignPlan(FullHouseDesignPlanCopy fullHouseDesignPlanCopy, Integer userId) throws BizException {
        ...
        if (!CollectionUtils.isEmpty(subTaskList)) {
            ...
            AutoRenderTaskState subTaskState = null;
            for (AutoRenderTask subTask : subTaskList) {
                // 这里必须要单独每条分开插入，因为必须要把子任务状态和子任务一一对应，所以就是每条先复制子任务，再复制子任务状态
                ...
                newTaskId = autoRenderTaskService.insertSelective(subTask);
                if (null == newTaskId || 0 >= newTaskId) {
                    throw new BizException("复制子任务失败，mainTaskId="+mainTaskId+",subTaskId="+taskId);
                }

                subTaskState = autoRenderTaskStateService.getMainTaskStateByTaskId(taskId);
                if (null != subTaskState) {
                    subTaskState.setId(null);
                    date = new Date();
                    subTaskState.setGmtCreate(date);
                    subTaskState.setGmtModified(date);
                    subTaskState.setCreator(nickName);
                    subTaskState.setModifier(nickName);
                    subTaskState.setOrderNumber(null);
                    subTaskState.setTaskId(newTaskId);
                    subTaskState.setMainTaskId(newMainTaskId);
                    subTaskState.setOperationUserId(userId);
                    subTaskState.setIsValid(0);
                    subTaskState.setFullHousePlanId(newFullHouseDesignPlanId);
                    subTaskState.setNewFullHousePlanId(newFullHouseDesignPlanId);
                    subTaskState.setSupplyDemandId(supplyDemandId);
                    influenced = autoRenderTaskStateService.insertSelective(subTaskState);
                    if (influenced != 1) {
                        throw new BizException("复制子任务状态失败，newMainTaskId=" + newMainTaskId + ",subTaskId="+ taskId);
                    }
                }
            }
        }
    ...
    private FullHouseDesignPlan setBaseAttribute(FullHouseDesignPlan fullHouseDesignPlan,
        ...
        this.setBaseAttribute(fullHouseDesignPlan,
    ...
    private FullHouseDesignPlan setBaseAttribute(FullHouseDesignPlan fullHouseDesignPlan,
        ...
        int n = autoRenderTaskService.getUserHouseCount(user.getId().intValue(), houseId, houseName);
        houseName = houseName + " [" + n + "]";
