...
public class SysUserServiceImpl implements SysUserService {
	...
	@Override
	public Map<Long, String> id2headPic(List<Long> ids)  {
        if(ids ==null ||ids.isEmpty()){
            return Collections.emptyMap();
        }
        List<SysUser> sysUsers = sysUserMapper.listByIds(ids);
        return sysUsers.stream().collect(Collectors.toMap(SysUser::getId,it-> it.getHeadPic() == null ? "": it.getHeadPic()));
	}
}
