...
//				 value = jedis.brpop(2,key);
    ...
    public boolean lock(String key, String value, long ms) {
        ...
        try {
            shardedJedis = shardedJedisPool.getResource();
            String result = shardedJedis.set(key, value, "NX", "PX", ms);
            if ("OK".equals(result)) {
                return true;
            }
            return false;
    ...
    @Override
    public boolean lock(String key, String value, int s) {
        ShardedJedis shardedJedis = null;
        try {
            shardedJedis = shardedJedisPool.getResource();
            String result = shardedJedis.set(key, value, "NX", "EX", s);
            if ("OK".equals(result)) {
                return true;
            }
            return false;
    ...
    @Override
    public boolean eval(String script, List<String> list, List<String> list1) {
        Jedis jedis =  null;
        Long success = 1L;
        try {
            ...
            jedis.connect();
            Object result = jedis.eval(script, list, list1);
            log.info(CLASS_LOG_PREFIX + "Jedis.eval(), result={}", result);
            if (success.equals(result)) {
                return true;
            }
        } catch (Exception e) {
            logger.error("Jedis.eval() error,{}", e);
        } finally {
            if (jedis != null) {
                jedis.disconnect();
            }
    ...
    @Override
    public boolean bloom(String name, String value) {
        Object obj = SpringContextHolder.getBean(name);
        if (obj == null || !(obj instanceof BloomFilter)) {
            throw new IllegalArgumentException("错误的NAME参数  name:" + name);
        }
        return ((BloomFilter) obj).existsAndPut(value);
    }

    @Override
    public boolean bloomWithoutPut(String name, String value) {
        Object obj = SpringContextHolder.getBean(name);
        if (obj == null || !(obj instanceof BloomFilter)) {
            throw new IllegalArgumentException("错误的NAME参数  name:" + name);
        }
        return ((BloomFilter) obj).exists(value);
    }
}
