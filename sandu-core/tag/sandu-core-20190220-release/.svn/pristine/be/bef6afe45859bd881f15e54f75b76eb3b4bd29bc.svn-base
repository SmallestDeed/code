...
public class CompressImageServiceImpl implements CompressImageService{
    ...
    public final static String rootPath = "/data001/mfs";
    ...
    private void comPressAction(List <GroupProductImageInfo> list)  {
        ...
        for(GroupProductImageInfo info : list) {
            ...
            String newSmallPath = fileUrlStr + "_small.jpg";
//                    fileUrlStr.replace(".jpg", "_small.jpg");
//            File thumbnailFile = new File(folder + "/" + thumbnailName);
            if(info.getResPicId().intValue() ==  133400){
                String rr = newSmallPath.replace(rootPath, "");
                log.info("batchUpdateResSmallPicPath : newSmallPath  {} rr {}",newSmallPath,rr);
            }
            try {
                ...
                if(file.exists()) {
                    ...
                    Thumbnails.of(is)
                            ...
                            .size(512, 512)
                            .toFile(newSmallPath);
                    info.setNewSmallPath(newSmallPath.replace(rootPath, ""));
                    commonMapper.updateResSmallPicPath(info);
                }
