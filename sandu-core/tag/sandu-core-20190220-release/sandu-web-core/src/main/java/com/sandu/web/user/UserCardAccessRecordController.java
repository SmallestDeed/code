    ...
    public ResponseEnvelope getAccessRecord(@RequestParam(required = false, name = "userAccessId") Integer userAccessId,
      ...
      LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        ...
        try {
            ...
            if (count == 0) {
                log.info("获取用户访问记录为空");
                //应前端要求，没有值的时候返回长度为0的集合，吐血ing。。。
                return new ResponseEnvelope(true, "获取用户名片访问记录数量为0", 0, userCardAccessRecordVoList);
            }
        } catch (Exception e) {
            log.error(CLASS_LOG_PREFIX + e);
        ...
        // 推送清空未读访问的消息
        try {
            userCardAccessService.sendUserCardAccessRecordWebSocket(userAccessId);
        } catch (Exception e) {
            log.error(CLASS_LOG_PREFIX + "推送消息异常：{}", e);
        }

        return new ResponseEnvelope(true, "获取用户名片访问记录成功", count, userCardAccessRecordVoList);
    }
       ...
       LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        ...
        try {
            ...
            if (count == 0) {
                ...
                //应前端要求，没有值的时候返回长度为0的集合，吐血ing。。。
                return new ResponseEnvelope(true, "获取用户访问名片操作记录数量为0", 0, userCardAccessRecordVoList);
            }
        } catch (Exception e) {
            log.error(CLASS_LOG_PREFIX + e);
            return new ResponseEnvelope(false, "获取用户访问名片操作记录count数据异常");
        }
        ...
        try {
            ...
            log.error(CLASS_LOG_PREFIX + e);
            return new ResponseEnvelope(false, "获取用户访问名片操作记录list数据异常");
        }
        ...
        // webSocket推送消息
        userCardAccessService.sendUserCardAccessOperationWebSocket(userAccessId);

        log.info(CLASS_LOG_PREFIX + "获取名片操作记录，userCardAccessRecordVoList={}", GsonUtil.bean2Json(userCardAccessRecordVoList));
        return new ResponseEnvelope(true, "获取用户访问名片操作记录成功", count, userCardAccessRecordVoList);
