...
public class DesignPlanJob {
    ...
    // 刷店铺浏览量的相关数据
    public static List<String> aShopList = new ArrayList<>(0);
    public static List<String> bShopList = new ArrayList<>(0);
    public static Map<String, String> shopMap = new HashMap<>();

    public void run() {
        if (running) {
            if (this.getRunning(this.map)) {
                // redis锁
    ...
    private void increaseShopView(){
        if (this.getRunning(this.shopMap)) {
            ...
            List<Integer> ab = new ArrayList<>(a.size() + b.size());
            ab.addAll(a);
            ab.addAll(b);
