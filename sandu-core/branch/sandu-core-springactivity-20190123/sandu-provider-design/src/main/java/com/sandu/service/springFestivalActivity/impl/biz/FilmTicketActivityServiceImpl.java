...
public class FilmTicketActivityServiceImpl implements FilmTicketActivityService {
    ...
    public GiveMeFiveVo giveMeFive(SysUser giveUser, SysUser getUser, Long activityId) {
        if (this.check(activityId)) {
            ...
            Long cardId = 0L;
            String remark = null;
            boolean isNewUser = this.isNewUser(giveUser, activityId);
            if (isNewUser) {
                try {
            ...
            return GiveMeFiveVo.builder()
                    ...
                    .cardFlag(cardFlag)
                    .sendTemplateMsg(new SendTemplateMsg(getUser, 10,
                            this.buildData(format.format(dateVo.getEndDate()), "邀好友领取拼图，可以免费拿电影票哦", "您已成功通过好友助力获得拼图"),
                            new Object[]{detail.getId(), detail.getLuckyWheelId()}))
                    .build();
        }
        return null;
    }
    ...
    public UserTaskVo getUserTaskState(SysUser user, Long activityId) {
        ...
        if (this.getLock(key, value, 3, 1, 50)) {
