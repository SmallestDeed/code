...
public class FullHouseDesignPlanController {
    ...
    @ApiModelProperty(value = "单空间装进我家时临时创建的全屋方案，用于在移动端创建全屋方案")
    @GetMapping("/addTempFullHouseDesignPlan")
    public ResponseEnvelope addTempFullHouseDesignPlan(Integer userId){
        // 校验参数
        if (userId == null){
        ...
        Integer fullHouseId;
        try {
            fullHouseId = fullHouseDesignPlanBizService.addTempFullHouseDesignPlan(userId);
        } catch (BizException e) {
            log.error("", e);
            return new ResponseEnvelope(false, e.getMessage());
        }
