...
public interface FullHouseDesignPlanService {
     ...
     * 获取全屋方案渲染空间数据
     *
     * @param fullHouseDesignPlanId
     * @return
     */
    List<BaseHousePicFullHousePlanRel> getBaseHousePicFullHousePlanRel(Integer fullHouseDesignPlanId);

    /**
     * 批量插入渲染空间数据
     *
     * @param baseHousePicFullHousePlanRelList
     * @return
     */
    int insertBatchBaseHousePicFullHousePlanRel(List<BaseHousePicFullHousePlanRel> baseHousePicFullHousePlanRelList);

    Integer checkHistoryCopyHouseExits(String historyMsgId, Integer userId);

    Integer selectMainTaskId(Integer fullHouseId);
}
