...
public class InteractiveZoneTopicServiceImpl implements InteractiveZoneTopicService {
    ...
    @Override
    public PageInfo<InteractiveZoneTopicVO1> list(InteractiveZoneTopicQuery interactiveZoneTopicQuery) {
        ...
        List<InteractiveZoneTopic> list1 = interactiveZoneTopicMapper.getList(interactiveZoneTopicQuery);

        PageInfo<InteractiveZoneTopicVO1> result = list1.stream().map(it -> {
            InteractiveZoneTopicVO1 tmp = new InteractiveZoneTopicVO1();
            BeanUtils.copyProperties(it, tmp);
            return tmp;
        }).collect(Collectors.collectingAndThen(Collectors.toList(), PageInfo::new));


        Map<Long,String> id2Path = supplyDemandPicMapper.selectByIds(
                result.getList().stream().map(InteractiveZoneTopic::getCoverPicId).filter(Objects::nonNull).collect(Collectors.toList())
        )
            .stream().collect(Collectors.toMap(SupplyDemandPic::getId,SupplyDemandPic::getPicPath));


        for (InteractiveZoneTopicVO1 it : result.getList()) {
            //封面图
            if(it.getPlanCoverId() != null){
                it.setCoverPicPath(id2Path.get(it.getCoverPicId().longValue()));
            }
            //回复数
            ...
            if (nodeData != null) {
                it.setViewCount(Integer.valueOf(Objects.toString(nodeData.get("setViewNum"), "0")));
                it.setViewVirtualCount(Integer.valueOf(Objects.toString(nodeData.get("setVirtualViewNum"), "0")));
            }

            //处理作者信息
            if (it.getBlockType().equals(4)) {
                it.setDesignType(it.getHouseId() != null && it.getHouseId() > 0 ? "户型":"方案");
            ...
            SysUser user = sysUserService.getUserById(it.getPublishUserId().longValue());
            it.setPublishUserName(user.getNickName());
        }
...
//            List<SysDictionary> lists = sysDictionaryService.findListByType(NODE_TYPE);
    ...
    private void addCommonParam(InteractiveZoneTopic interactiveZoneTopic, SysUser sysUser) {
        ...
        Date date = new Date();
        if (null == interactiveZoneTopic.getId() || 0 == interactiveZoneTopic.getId()) {
            interactiveZoneTopic.setCreator(sysUser.getNickName());
            ...
            if (null == interactiveZoneTopic.getPublishTime()) {
    ...
    public InteractiveZoneTopicDetail getDetails(Integer id) {
        ...
        //用户头像
		SysUser user = sysUserService.getUserById(detail.getCreateUserId().longValue());
        if(user != null){
        	detail.setAuthorPic(user.getHeadPic());
		}
