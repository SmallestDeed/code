...
public class FullHouseDesignPlanController {
    ...
    public Object getMatchInfo(
            @ApiParam(name = "houseId", value = "户型id")
                    Integer houseId,
            @ApiParam(name = "fullHousePlanId", value = "全屋方案id")
    ...
    public ResponseEnvelope addFullHouseDesignPlan(@RequestBody FullHouseDesignPlanAdd fullHouseDesignPlanAdd){
        // 制作全屋方案
        log.info("装进我家创建全屋方案,fullHouseDesignPlanAdd:{}", gson.toJson(fullHouseDesignPlanAdd));
        String idAndUuid;
        try {
            idAndUuid = fullHouseDesignPlanBizService.addFullHouseDesignPlan(fullHouseDesignPlanAdd);
        } catch (BizException e) {
            log.error("",e);
            return new ResponseEnvelope(false, e.getMessage());
        }
        log.info("装进我家创建全屋方案成功,全屋方案ID&&UUID:{}", idAndUuid);
