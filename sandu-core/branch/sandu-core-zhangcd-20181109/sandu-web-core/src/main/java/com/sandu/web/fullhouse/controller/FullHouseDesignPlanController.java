...
public class FullHouseDesignPlanController {
    ...
    public ResponseEnvelope createFullHouseDesignPlan(@RequestBody @Validated FullHouseDesignPlanAdd fullHouseDesignPlanAdd,
        ...
        try {
            fullHouseDesignPlanBizService.createFullHouseDesignPlan(fullHouseDesignPlanAdd, user.getId(), user.getToken());
    ...
    public ResponseEnvelope deleteFullHouseDesignPlan(Integer fullHouseId, String msgId) {
        ...
        try {
            ...
            fullHouseDesignPlanBizService.deleteFullHouseDesignPlan(fullHouseId);
        } catch (BizException e) {
            log.error("", e);
    ...
    public ResponseEnvelope updateFullHouseDesignPlan(@RequestBody @Validated FullHouseDesignPlanUpdate update, BindingResult bindingResult) {
        ...
        try {
            fullHouseDesignPlanBizService.updateFullHouseDesignPlan(update, user.getId(), user.getToken());
        } catch (BizException e) {
            log.error("", e);
            return new ResponseEnvelope(false, e.getMessage());
        }
    ...
    public Object getMatchInfo(@ApiParam(name = "houseId", value = "户型id") Integer houseId,
        ...
        try {
            result = fullHouseDesignPlanBizService.getMatchInfo(houseId, fullHousePlanId);
    ...
    public ResponseEnvelope updateFullHouseDesignPlanScene(@RequestBody @Validated FullHouseDesignPlanSceneUpdate fullHouseDesignPlanSceneUpdate,
        ...
        String uuid;
        try {
            uuid = fullHouseDesignPlanBizService.updateFullHouseDesignPlanScene(fullHouseDesignPlanSceneUpdate);
        } catch (BizException e) {
            log.error("", e);
            // 释放锁，不是原子性的
            if (value.equals(redisService.get(key))) {
    ...
    public ResponseEnvelope addTempFullHouseDesignPlan(Integer userId, Integer oldFullHouseDesignPlanId, Integer houseId){
        ...
        Integer fullHouseId;
        try {
            fullHouseId = fullHouseDesignPlanBizService.addTempFullHouseDesignPlan(userId,oldFullHouseDesignPlanId, houseId);
        } catch (BizException e) {
            log.error("", e);
            return new ResponseEnvelope(false, e.getMessage());
        }
    ...
    public ResponseEnvelope copyFullHouseDesignPlan(@RequestBody @Validated FullHouseDesignPlanCopy fullHouseDesignPlanCopy,
        ...
        // if (null == fullHouseDesignPlanCopy) {
        //     return new ResponseEnvelope(false,"参数全部为空");
        // }
        // if (null == fullHouseDesignPlanCopy.getFullHouseDesignPlanId() || 0 == fullHouseDesignPlanCopy.getFullHouseDesignPlanId()) {
        //     return new ResponseEnvelope(false,"全屋方案id为空");
        // }
        // if (null == fullHouseDesignPlanCopy.getMainTaskId() || 0 == fullHouseDesignPlanCopy.getMainTaskId()) {
        //     return new ResponseEnvelope(false,"主任务id为空");
        // }
        // if (null == fullHouseDesignPlanCopy.getSupplyDemandId() || 0 == fullHouseDesignPlanCopy.getSupplyDemandId()) {
        //     return new ResponseEnvelope(false,"供求信息id为空");
        // }
        ...
        try {
            idAndMainTaskId = fullHouseDesignPlanBizService.copyFullHouseDesignPlan(fullHouseDesignPlanCopy,userId);
        } catch (BizException e) {
