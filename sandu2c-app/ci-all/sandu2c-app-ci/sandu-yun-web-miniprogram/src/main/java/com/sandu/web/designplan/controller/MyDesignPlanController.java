    ...
    public ResponseEnvelope getMydecorationPlan(@ModelAttribute MydecorationPlanAdd mydecorationPlanAdd, HttpServletRequest request) {
        ...
        try {
            ...
            if (count == 0) {
                ...
                return new ResponseEnvelope(true, "还没有装修的方案", -1);
            }
        } catch (Exception e) {
        ...
        for (MydecorationPlanVo mydecorationPlanVo : mydecorationPlanVoList) {
            ...
            // 取户型的空间定位类型
            List<String> spaceTypeList = houseSpaceNewService.getSpaceTypeListByHouseId(mydecorationPlanVo.getHouseId());
            ...
            mydecorationPlanVo.setHouseTypeStr(((elementsCount.containsKey("3") ? elementsCount.get("3") : 0) + "厅"
        ...
        if (mydecorationPlanVoList != null && mydecorationPlanVoList.size() == 1 && mydecorationPlanVoList.get(0).getHouseId() != null &&
                mydecorationPlanAdd.getProbationHouseId() != null && mydecorationPlanVoList.get(0).getHouseId().equals(mydecorationPlanAdd.getProbationHouseId())) {
            ResponseEnvelope r = new ResponseEnvelope();
            r.setStatus(true);
            r.setDatalist(mydecorationPlanVoList);
