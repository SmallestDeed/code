...
public class NodeInfoBizServiceImpl implements NodeInfoBizService {
    ...
    @Autowired
    public NodeInfoBizServiceImpl(RedisService redisService,
    ...
    public synchronized boolean incrementDataToDatabase() {
        ...
        if (redisService.set(key, value, "NX", "EX", 600)){
            if (this.version == 0) {
                ...
                if (versionStr != null && !"".equals(versionStr) && StringUtils.isNumeric(versionStr)) {
    ...
    @Override
    public Integer updateNodeInfoDetailValue(Integer contentId, Integer nodeType, Integer detailType, Integer value) {
        if (contentId == null || nodeType == null || detailType == null || value == null) {
            throw new RuntimeException("缺失参数！！！请确认传参是否正确！！！");
        }
        Integer nodeId = this.registerNodeInfo(contentId, nodeType.byteValue());
        // 节点详情key和桶key
        String nnKey = detailType + ":" + nodeId;
        String nnBucketKey = NodeInfoConstant.BUCKET_KEY_PREFIX_NODE_INFO_DETAIL + getBucketId(nnKey, NodeInfoConstant.BUCKET_KEY_BIT_LENGTH);
        boolean flag = redisService.setHSet(nnBucketKey, nnKey, value.intValue() + "");
        // 添加到同步set中
        redisService.addSet(NodeInfoConstant.NODE_INFO_DATA_SYNCHRONIZED_SET, 0 + ":" + nodeId);
        if (flag) {
            return value;
        } else {
            return 0;
    ...
    private void syncToDatabase() {
        if (this.isDataSecurity()) {
            // 更新数据版本
            this.version += 1;
            redisService.set(NodeInfoConstant.NODE_INFO_DATA_VERSION_KEY, this.version + "");
            // 获取同步集合
    ...
    private void incrementDataToDatabase(int userId, int nodeId) throws NoSuchAlgorithmException {
        ...
        for (SysDictionary sysDictionary : sysDictionaryList) {
            ...
            if (userId > 0) {
                this.updateUserNodeInfoRel(detailType, userId, nodeId);
            }
            // 更新node_info_detail表
     ...
     * @throws NoSuchAlgorithmException
     */
    private int updateUserNodeInfoRel(Integer detailType, int userId, Integer nodeId) throws NoSuchAlgorithmException {
        // 从缓存中获取用户-节点数据
        String unKey = detailType + ":" + userId + ":" + nodeId;
        ...
        if (statusStr != null && !"".equals(statusStr)) {
            ...
            userNodeInfoRel.setModifier(userId + "");
    ...
    public static void main(String[] args) {
        NodeInfoBizServiceImpl nodeInfoBizService = new NodeInfoBizServiceImpl(null, null, null);
        int i = 2;
        String sdfs = nodeInfoBizService.getBucketId(2 + ":51512", 16);
