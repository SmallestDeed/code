...
public class DesignPlanFavoriteController {
  ...
  /*  @Autowired
    ...
    public ResponseEnvelope setFavoriteOrUnfavorite(@RequestBody DesignPlanRecommendFavoriteRef favoriteRef, HttpServletRequest request) {
        ...
        if (favoriteRef.getRecommendId() == 0) {
            return new ResponseEnvelope(false, "推荐方案为空");
        }
        if (null == favoriteRef.getStatus()) {
            return new ResponseEnvelope(false, "收藏状态为空");
        }
        if (null == favoriteRef.getDesignPlanType()) {
            return new ResponseEnvelope(false, "方案类型为空");
        }
        // 平台编码
        ...
        // 公司
        BaseCompany baseCompany = baseCompanyService.getCompanyByDomainUrl(request.getHeader("Referer"), request.getHeader("Custom-Referer"));
        if (null == baseCompany) {
            return new ResponseEnvelope(false, "未获取到公司信息");
        }
        UserSO userSo = null;
        ...
        if (favoriteRef.getDesignPlanType() == 1){
            if (miniProgram.equals(platformCode) && companyCode.equals(baseCompany.getCompanyCode())){
                // 获取节点ID
                Integer nodeId = nodeInfoBizService.registerNodeInfo(favoriteRef.getRecommendId(), NodeInfoConstant.SYSTEM_DICTIONARY_NODE_TYPE_RECOMMENDED);
                // 处理操作
                favoriteNum = nodeInfoBizService.updateUserNodeRelStatus(loginUser.getId(),
                        nodeId,
                        favoriteRef.getRecommendId(),
                        NodeInfoConstant.SYSTEM_DICTIONARY_NODE_TYPE_RECOMMENDED,
                        NodeInfoConstant.SYSTEM_DICTIONARY_DETAIL_TYPE_FAVORITE,
                        favoriteRef.getStatus().byteValue());
            }else {
                try {
            ...
            if (miniProgram.equals(platformCode) && companyCode.equals(baseCompany.getCompanyCode())){
                // 获取节点ID
                Integer nodeId = nodeInfoBizService.registerNodeInfo(favoriteRef.getRecommendId(), NodeInfoConstant.SYSTEM_DICTIONARY_NODE_TYPE_FULL_HOUSE);
                // 处理操作
                favoriteNum = nodeInfoBizService.updateUserNodeRelStatus(loginUser.getId(),
                        nodeId,
                        favoriteRef.getRecommendId(),
     ...
     * @return
     */
    @RequestMapping(value = "/syncDeginplanFavorite",method = RequestMethod.GET)
    @ResponseBody
    public ResponseEnvelope syncDeginplanFavorite( HttpServletRequest request) {
        try {
            designPlanRecommendFavoriteService.syncDeginplanFavorite();
        } catch (Exception e) {
            logger.error("推荐方案点赞收藏同步数据-----" + e);
        }
        return new ResponseEnvelope(true, "同步方案点赞成功");
    }
