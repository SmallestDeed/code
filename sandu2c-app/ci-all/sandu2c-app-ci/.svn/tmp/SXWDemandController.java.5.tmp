...
public class SXWDemandController {
    ...
    public ResponseEnvelope publishSupplyDemandInfo(@RequestBody DemandAdd demandAdd, HttpServletRequest request){
        ...
        log.info("1111111111111111111发布供求信息，参数demandAdd={}" ,gson.toJson(demandAdd));
        /*检查用户是否登录*/
        ...
        //校验两个内容字段不能同时为空
        if (StringUtils.isBlank(demandAdd.getDescription()) && StringUtils.isBlank(demandAdd.getContent())) {
        ...
        //处理表情
        if(StringUtils.isNotBlank(demandAdd.getDescription())){
            String newDescription = EmojiUtil.emojiConverterToAlias(demandAdd.getDescription());
            log.info("将表情解码:   " + newDescription);
            demandAdd.setDescription(newDescription);
        }
        if(StringUtils.isNotBlank(demandAdd.getTitle())){
            String newTitle = EmojiUtil.emojiConverterToAlias(demandAdd.getTitle());
            log.info("将表情解码:   " + newTitle);
            demandAdd.setTitle(newTitle);
        }
    ...
    @ResponseBody
    public ResponseEnvelope checkIfSupplyDemandInfoDeleted(@PathVariable("id") Integer id) {
        Integer i = supplyDemandService.checkIfSupplyDemandInfoDeleted(id);
        return new ResponseEnvelope(true, i);
    }
    ...
    @RequestMapping(value = "/delsupplydemand", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEnvelope delSupplyDemand(@RequestParam(required = false) Integer supplyDemandId, HttpServletRequest request) {
         /*检查用户是否登录*/
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        if (loginUser == null) {
            log.warn(CLASS_LOG_PREFIX+"用户登录失败:"+gson.toJson(loginUser));
    ...
    /*     LoginUser loginUser = new LoginUser();
        ...
        int id = 0;
        try {
            id = supplyDemandService.delSupplyDemandById(supplyDemandId,loginUser.getId());
            if (id == 0) {
                log.warn(CLASS_LOG_PREFIX+"删除帖子失败:" + supplyDemandId);
                return new ResponseEnvelope(false, "删除帖子失败");
            }
        }catch (Exception e){
            log.error(CLASS_LOG_PREFIX+"删除帖子数据异常:", e);
            return new ResponseEnvelope(false, "删除帖子数据异常");
        }
        return new ResponseEnvelope(true, "删除帖子成功", supplyDemandId);
    }
