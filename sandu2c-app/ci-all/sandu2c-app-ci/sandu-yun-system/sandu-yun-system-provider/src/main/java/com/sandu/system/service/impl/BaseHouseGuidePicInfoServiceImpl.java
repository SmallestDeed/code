...
public class BaseHouseGuidePicInfoServiceImpl implements BaseHouseGuidePicInfoService {
	...
	public List<GuideInfoDTO> getGuideInfoDTOListByPicId(
		...
		baseHouseGuidePicInfoSearch.setHouseId(houseId);
		...
		boolean flag = false;
		Map<Integer, BaseHousePicFullHousePlanRel> renderMap = null;
		if(renderStatusMap == null) {
			renderMap = baseHousePicFullHousePlanRelService.getRenderMap(fullHousePlanId);
		}else {
			flag = true;
		}
		...
		if(baseHouseGuidePicInfoList != null && baseHouseGuidePicInfoList.size() > 0) {
			for(BaseHouseGuidePicInfo item : baseHouseGuidePicInfoList) {
				...
				if(StringUtils.isNotBlank(item.getUniqueId())){
					guideInfoDTO.setUniqueId(item.getUniqueId());
				}
				...
				if(flag) {
					...
					if(renderMap.containsKey(guideInfoDTO.getDesignTempletId())) {
						BaseHousePicFullHousePlanRel rel = renderMap.get(guideInfoDTO.getDesignTempletId());
						if (rel != null) {
							guideInfoDTO.setRenderStatus(rel.getState().intValue());
							guideInfoDTO.setPlanPic(rel.getPlanPic());
						}
					}
				}
	...
	public List<BaseHouseGuidePicInfo> getListBySearch(BaseHouseGuidePicInfoSearch baseHouseGuidePicInfoSearch) {
		...
		// 参数验证 ->end
		List<BaseHouseGuidePicInfo> list = baseHouseGuidePicInfoMapper.selectBySearch(baseHouseGuidePicInfoSearch);
		Map<Integer, List<BaseHouseGuidePicInfo>> map = list.stream().collect(Collectors.groupingBy(BaseHouseGuidePicInfo::getOrigin));
		return map.get(1) == null || map.get(1).size() == 0 ? map.get(0) : map.get(1);
	}
