...
public class MimeUtility {
    ...
    static {
        ...
        try {
            ...
            // default to true
            decodeStrict = s == null || !"false".equalsIgnoreCase(s);
            s = System.getProperty("mail.mime.encodeeol.strict");
            // default to false
            encodeEolStrict = s != null && "true".equalsIgnoreCase(s);
            s = System.getProperty("mail.mime.foldencodedwords");
            // default to false
            foldEncodedWords = s != null && "true".equalsIgnoreCase(s);
            s = System.getProperty("mail.mime.foldtext");
            // default to true
            foldText = s == null || !"false".equalsIgnoreCase(s);
     ...
     *
     * @paramdsDataSource
     * @returnthe encoding. This is either "7bit", "quoted-printable" or
     *            "base64"
     */
...
//	}
    ...
    /**
     * Same as <code>getEncoding(DataSource)</code> except that instead of
     * reading the data from an <code>InputStream</code> it uses the
     * <code>writeTo</code> method to examine the data. This is more efficient
     * in the common case of a <code>DataHandler</code> created with an object
     * and a MIME type (for example, a "text/plain" String) because all the I/O
     * is done in this thread. In the case requiring an <code>InputStream</code>
     * the <code>DataHandler</code> uses a thread, a pair of pipe streams, and
     * the <code>writeTo</code> method to produce the data.
     * <p>
     *
     * @sinceJavaMail 1.2
     */
...
//
//		/*
//		 * Try to pick the most efficient means of determining the encoding. If
//		 * this DataHandler was created using a DataSource, the
//		 * getEncoding(DataSource) method is typically faster. If the
//		 * DataHandler was created with an object, this method is much faster.
//		 * To distinguish the two cases, we use a heuristic. A DataHandler
//		 * created with an object will always have a null name. A DataHandler
//		 * created with a DataSource will usually have a non-null name.
//		 * 
//		 * XXX - This is actually quite a disgusting hack, but it makes a common
//		 * case run over twice as fast.
//		 */
//		if (dh.getName() != null)
...
//	}
     ...
     *
     * @paramisinput stream
     * @paramencodingthe encoding of the stream.
     * @returndecoded input stream.
     */
    public static InputStream decode(InputStream is, String encoding)
        ...
        } else if ("uuencode".equalsIgnoreCase(encoding)
                || "x-uuencode".equalsIgnoreCase(encoding)
                || "x-uue".equalsIgnoreCase(encoding)) {
     ...
     * is also supported.
     *
     * @paramosoutput stream
     * @paramencodingthe encoding of the stream.
     * @returnoutput stream that applies the specified encoding.
     */
     ...
     * @paramtextUnicode string
     * @returnUnicode string containing only US-ASCII characters
     */
     ...
     * 822 headers.
     *
     * @paramtextthe header value
     * @paramcharsetthe charset. If this parameter is null, the platform's
     * default chatset is used.
     * @paramencoding the encoding to be used. Currently supported values are
     * "B" and "Q". If this parameter is null, then the "Q"
     * encoding is used if most of characters to be encoded are
     * in the ASCII charset, otherwise "B" encoding is used.
     * @returnUnicode string containing only US-ASCII characters
     */
     ...
     * <p>
     *
     * @parametextthe possibly encoded value
...
//		 * 2047, Section 6.1.1) RFC 822 defines "linear-white-space" as SPACE |
//		 * HT | CR | NL.
//		 */
//		String lwsp = " \t\n\r";
//		StringTokenizer st;
...
//	}
     ...
     *
     * @throws UnsupportedEncodingException if the encoding fails
     * @paramwordUnicode string
     * @returnArray of Unicode strings containing only US-ASCII characters.
     */
     ...
     * is being encoded. This is because the "Q" encoding defined in RFC 2047
     * has more restrictions when encoding "word" tokens. (Sigh)
     */
     ...
     * "encoded-word". If the parse fails, a ParseException is thrown.
     ...
     * <p>
     *
     * @paramewordthe possibly encoded value
...
//	}
     ...
     * header fields.
     *
     * @paramwordword to be quoted
     * @paramspecials the set of special characters
     * @returnthe possibly quoted word
     * @seejavax.mail.internet.HeaderTokenizer#MIME
     * @seejavax.mail.internet.HeaderTokenizer#RFC822
     */
     ...
     * be.
     *
     * @paramusedcharacters used in line so far
     * @paramsthe string to fold
     * @returnthe folded string
     */
	/* public */
     ...
     *
     * @paramsthe string to unfold
     * @returnthe unfolded string
     */
	/* public */
     ...
     * <p>
     *
     * @param charsetthe MIME charset name
     ...
     * <p>
     *
     * @returnthe default charset of the system's default locale, as a Java
    ...
    public static String getDefaultJavaCharset() {
        ...
        if (defaultJavaCharset == null) {
			...
			/*
			 * If mail.mime.charset is set, it controls the default Java charset
			 * as well.
			 */
            String mimecs = null;
            try {
     ...
     *
     * @paramsstring
     * @returnALL_ASCII if all characters in the string belong to the US-ASCII
     * charset. MOSTLY_ASCII if more than half of the available
     * characters are US-ASCII characters. Else
     * MOSTLY_NONASCII.
     */
     ...
     * the end of stream.
     *
     * @paramisthe input stream
     * @parammaxmaximum bytes to check for. The special value ALL indicates that
     * all the bytes in this input stream must be checked.
     * @parambreakOnNonAscii if <code>true</code>, then terminate the the check
