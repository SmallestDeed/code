...
public class BaseGoodsSPUServiceImpl implements BaseGoodsSPUService {
    ...
    public List<GoodsVO> getGoodsList(GoodsListPO goodsListPO) {
        ...
        // 拼团活动数据
        Map<Integer, ActivityVO> activityVOMap = mapGoodsActivity(goodsBOList);
        ...
        for (GoodsBO goodsBO : goodsBOList) {
            ...
            if (priceList == null || priceList.size() == 0) {
                ...
                vo.setSalePrice(salePrice == null ? "0" : salePrice.setScale(2, BigDecimal.ROUND_DOWN).toString());
            }

            // added by songjianming@sanduspace.cn on 2018/12/13
            // 拼团活动数据
            vo.setActivity(activityVOMap.get(goodsBO.getId()));
    ...
    private Map<Integer, ActivityVO> mapGoodsActivity(List<GoodsBO> listGoodsBO) {
        ...
        if (listActivityVO == null || listActivityVO.isEmpty()) {
            log.info("拼团商品：未有商品参加了拼团活动, " + listGoodsBO.toString());
            return new HashMap<>(0);
        }

        log.info("拼团商品：找到商品的拼团信息，" + listActivityVO.toString());
    ...
    public GoodsDetailVO getGoodsDetail(Integer id) {
        ...
        try {
            ...
            throw new RuntimeException("BaseGoodsSPUService.getGoodsDetail() row:103\n" + e.toString());
        }
        // 商品详情的轮播图，按后台保存的顺序展示
        if (picIdList.size() > 0) {
        ...
        // 最低优惠价的索引
        int priceIndex = NumberUtil.findMinIndex(prices, new BigDecimal("0"), false);

        goodsDetailVO.setLimitation(goodsDetailBO.getLimitation());
        goodsDetailVO.setSalePrice(priceIndex == -1 ? "0" : salePrices.get(priceIndex).setScale(2,BigDecimal.ROUND_DOWN).toString());
        goodsDetailVO.setPrice(priceIndex == -1 ? "0" : prices.get(priceIndex).setScale(2,BigDecimal.ROUND_DOWN).toString());
    ...
    public GoodsSkuVO getSpuAttrList(Integer id) {
        ...
        if (paList == null || paList.size() == 0){
            throw new RuntimeException("该商品下的产品列表为空");
        }
        // 所有产品的价格集合
