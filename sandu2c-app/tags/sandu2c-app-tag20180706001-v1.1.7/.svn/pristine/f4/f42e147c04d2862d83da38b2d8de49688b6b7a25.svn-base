...
public class BaseGoodsSPUServiceImpl implements BaseGoodsSPUService {
        ...
        for (GoodsBO goodsBO : goodsBOList) {
            ...
            if (priceList == null || priceList.size() == 0) {
                ...
                vo.setSalePrice(salePrice == null ? "0" :
                        (salePrice.compareTo(new BigDecimal("0")) == 0 ? "0" : salePrice.toString()));
            }
            goodsVOList.add(vo);
        }
        // 倒序排列
    ...
    public GoodsDetailVO getGoodsDetail(Integer id) {
        ...
        goodsDetailVO.setPrice(priceIndex == -1 ? "0" :
                prices.get(priceIndex).compareTo(new BigDecimal("0")) == 0? "0" : prices.get(priceIndex).toString());
        ...
        goodsDetailVO.setTransportPay(goodsDetailBO.getTransportPay() == null ? "0.00" : goodsDetailBO.getTransportPay().toString());
    ...
    public GoodsSkuVO getSpuAttrList(Integer id) {
        ...
        if (spuAttrList == null || spuAttrList.size() == 0) {
            ...
            goodsSkuVO.setPrice(pa.getPrice() == null ? "0" :
                    pa.getPrice().compareTo(new BigDecimal("0"))== 0? "0" : pa.getPrice().toString());
            ...
            goodsSkuVO.setTransportMoney(pa.getTransportMoney() == null ? "0.00" : "" + pa.getTransportMoney().intValue());
            return goodsSkuVO;
        } else {
            // 返回属性列表
            ...
            goodsSkuVO.setSkuId(pa.getSkuId());
            goodsSkuVO.setAttr(spuAttributeVOList);
            goodsSkuVO.setTransportMoney(pa.getTransportMoney() == null ? "0.00" : "" + pa.getTransportMoney().toString());
            return goodsSkuVO;
        }
    }
