...
//        }
     ...
     */
    @Override
    public DesignPlanRecommendedResult getRecommendedDesignPlanDetail(PlanRecommendedListModel model, BaseCompany baseCompany) throws BizException {
        LoginUser loginUser = model.getLoginUser();
        // 更新方案浏览次数
        int success = designPlanSummaryInfoService.updateViewNumberPlusOne(model.getId());
        if (success <= 0){
            DesignPlanSummaryInfo info = new DesignPlanSummaryInfo();
            Date now = new Date();
            info.setDesignId(model.getId());
            info.setViewNum(1);
            info.setGmtCreate(now);
            info.setGmtModified(now);
            info.setCreator(model.getLoginUser().getLoginName());
            info.setModifier(model.getLoginUser().getLoginName());
            designPlanSummaryInfoService.add(info);
        }
        // 查询推荐方案详情
        ...
        // 获取推荐方案详情的描述文件ID
        logger.info("查找推荐方案详情结果:{}", GSON.toJson(designDetail));
