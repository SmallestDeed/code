...
public class MallBaseOrderServiceImpl implements MallBaseOrderService{
	...
	public MallBaseOrder getOrderByOrderId(MallBaseOrder order)
	{
        ...
        {
            mallBaseOrder.setSpuTotalPrice(mallBaseOrder.getSpuTotalPrice()==null?0+orderProduct.getProductPrice().intValue():
                    mallBaseOrder.getSpuTotalPrice()+orderProduct.getProductPrice().intValue());
            mallBaseOrder.setTotalTransportCost(orderProduct.getTransportationExpenses()==null?0:
                    (mallBaseOrder.getTotalTransportCost()==null?0+orderProduct.getTransportationExpenses().intValue():
                    mallBaseOrder.getTotalTransportCost()+orderProduct.getTransportationExpenses().intValue()));
        }
    ...
    public ResponseEnvelope updateOrderStatusByOrderList(MallBaseOrder mallBaseOrder) {
        ...
        {
            baseGoodsSKUService.changeInventory(orderProduct.getProductId(), orderProduct.getProductNumber());
        }
        // modified by zcd end*/
    ...
    public int createOrder(MallBaseOrder mallBaseOrder) {
        ...
        for(OrderProduct orderProduct :mallBaseOrder.getOrderProductList()){
            ...
            if(goodsInfo != null)
            {
                ...
                orderAmount += (orderProduct.getProductNumber()*orderProduct.getProductPrice().doubleValue()
                        + orderProduct.getProductNumber()*orderProduct.getTransportationExpenses().doubleValue());
            }
        ...
        Date date = new Date();
        //向订单表插入数据
        mallBaseOrder.setOrderAmount(new BigDecimal(orderAmount));
        ...
        /*// modified by zcd start
        // 生成订单后删除购物车内相关产品
        mallCartProductRefService.delete(cartProductIdList);
        //modified by zcd end*/
