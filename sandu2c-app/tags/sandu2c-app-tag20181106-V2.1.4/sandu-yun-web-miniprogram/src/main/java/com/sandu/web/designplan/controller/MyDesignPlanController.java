...
public class MyDesignPlanController {
    ...
    public ResponseEnvelope getMydecorationPlan(@ModelAttribute MydecorationPlanAdd mydecorationPlanAdd, HttpServletRequest request) {
        ...
        int count;
        try {
            count = fullHouseDesignPlanService.getMydecorationPlanCount(loginUser.getId());
            if (count == 0) {
    ...
    @ResponseBody
    public ResponseEnvelope delPlan(@RequestParam("planType") Integer planType,
        ...
        //校验参数不为空
        if (null == planType || 0 == planType
                || null == taskId || 0 == taskId
                || null == renderState || 0 == renderState) {
            log.warn("必传参数为空" + "---------" + planType + "-----------" + planId);
            return new ResponseEnvelope(false, "必传参数为空");
        }
        ...
        int fullHouserId;
        try {
            fullHouserId = fullHouseDesignPlanService.delMyDecorationPlan(loginUser, planType, planId,taskId,renderState);
            if (0 == fullHouserId) {
                log.warn("删除我装修的方案失败" + "---------" + planType + "-----------" + planId + "-------" + gson.toJson(loginUser));
                return new ResponseEnvelope(false, "删除我装修的方案失败");
            }
        } catch (Exception e) {
        ...
        return new ResponseEnvelope(true, "删除我的方案成功", fullHouserId);
