...
public class PlanDecoratePriceController extends BaseController {
    ...
    public ResponseEnvelope queryPlanDecoratePriceList(Integer renderSceneId,String msgId) {
        ...
        if (results != null && results.size() > 0) {
            final List<PlanDecoratePriceVO> planDecoratePrices = getDecoratePriceVOS(results);
    ...
    @ApiOperation(value = "查询PlanDecoratePrice列表", response = PlanDecoratePriceVO.class)
    @PostMapping(value = "/fullHouseList")
    public ResponseEnvelope queryFullHousePlanDecoratePriceList(Integer fullHouseId,String msgId) {
        ResponseEnvelope response = new ResponseEnvelope();
        response.setMsgId(msgId);
        if (fullHouseId == null || fullHouseId == 0) {
            response.setStatus(false);
            response.setMessage("全屋方案id为空");
            return response;
        }

        PlanDecoratePriceQuery query = new PlanDecoratePriceQuery();
        query.setFullHouseId(fullHouseId);
        final List<PlanDecoratePrice> results = planDecoratePriceBizService.query(query);
        log.debug("Result: {}", results);
        if (results != null && results.size() > 0) {
    ...
    private List<PlanDecoratePriceVO> getDecoratePriceVOS(List<PlanDecoratePrice> results) {
