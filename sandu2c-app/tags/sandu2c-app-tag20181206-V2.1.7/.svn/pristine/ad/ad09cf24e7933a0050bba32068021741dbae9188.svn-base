...
import com.sandu.common.cache.utils.SerializeUtil;
...
//            System.out.println(String.format("key:%s--->map:%s",mapName,objectMap));
            return true;
        } catch (Exception ex) {
...
//            returnBrokenResource(shardedJedis);
     ...
     */
    @Override
    public boolean setHSet(Map<String, Map<String, String>> map) {
        if (map == null) {
            return false;
        }
        ShardedJedis shardedJedis = null;
        try {
            ...
            map.forEach((bucketId, v) -> {
                v.forEach((key, value) -> {
                    pipeline.hset(bucketId, key, value);
                });
            });
            pipeline.sync();
            return true;
     ...
     */
    @Override
    public long delMatch(String match) {
        ShardedJedis shardedJedis = null;
        try {
            int num = 0;
            shardedJedis = shardedJedisPool.getResource();
            ...
            logger.error("delMatch error.", ex);
            returnBrokenResource(shardedJedis);
        } finally {
            returnResource(shardedJedis);
        }
    ...
    public boolean set(String key, String value, String nxxx, String expx, long time) {
        ...
        try {
            shardedJedis = shardedJedisPool.getResource();
            String result = shardedJedis.set(key, value, nxxx, expx, time);
            if ("OK".equals(result)) {
                return true;
            }
            return false;
...
//				 value = jedis.brpop(2,key);
    ...
    public void releaseLock(String lockKey, String uuid) {
        ...
        try {
            jedis = getResource();
            String[] s = jedis.get(lockKey).split("_");
            if (s[1].equals(uuid)) {
                //release lock
                jedis.del(lockKey);
//                logger.trace("{}'s lock is released...", uuid);
            } else {
                logger.error("release lock error, lock is be used...");
            }
        } catch (Exception e) {
    ...
    public boolean tryLock(String lockKey, String uuid, long expire) {
        ShardedJedis jedis = null;
        try {
            jedis = getResource();
            long now = LocalDateTime.now().toInstant(ZoneOffset.of("+8")).toEpochMilli();
            String value = now + expire * 1000 + "_" + uuid;
            if (jedis.setnx(lockKey, value) == 0) {
                String oldValue = jedis.get(lockKey);
                String[] values = oldValue.split("_");
                //过期处理
                if (Long.valueOf(values[0]) < now) {
//                    logger.trace("lock {} was expired", values[1]);
                    String validValue = jedis.getSet(lockKey, now + expire * 1000 + "");
                    if (validValue.equals(oldValue)) {
//                        logger.trace("{} is get lock", uuid);
                        return true;
                    }
                }
                return false;
