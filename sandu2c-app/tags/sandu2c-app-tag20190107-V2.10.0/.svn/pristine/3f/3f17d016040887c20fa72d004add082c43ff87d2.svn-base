...
//        trigger.setCronExpression("0 0/1 * * * ?  ");
    ...
    public MethodInvokingJobDetailFactoryBean fourJobDetail(UserCommissionTopJob  fourJobDetail) {
        MethodInvokingJobDetailFactoryBean jobDetail = new MethodInvokingJobDetailFactoryBean();
        // 是否并发执行
        jobDetail.setConcurrent(false);
        // 为需要执行的实体类对应的对象
        jobDetail.setTargetObject(fourJobDetail);
        // 需要执行的方法名称
        jobDetail.setTargetMethod("autoReviews");
        return jobDetail;
    }
    ...
    public CronTriggerFactoryBean fourTrigger(JobDetail fourJobDetail) {
        CronTriggerFactoryBean trigger = new CronTriggerFactoryBean();
        trigger.setJobDetail(fourJobDetail);
        // 设置任务启动延迟
        trigger.setStartDelay(0);
        //
        trigger.setCronExpression("0 0/60 * * * ? ");
//        trigger.setCronExpression("0 0/1 * * * ?  ");
        return trigger;
    }
    ...
    // 配置Scheduler
    @Bean(name = "scheduler")
    public SchedulerFactoryBean schedulerFactory(Trigger firstTrigger, Trigger secondTrigger, Trigger thirdTrigger,Trigger fourTrigger) {
        SchedulerFactoryBean bean = new SchedulerFactoryBean();
        // 延时启动，应用启动1秒后
        bean.setStartupDelay(1);
        // 注册触发器
        bean.setTriggers(firstTrigger,secondTrigger,thirdTrigger,fourTrigger);
        return bean;
    }
} 