...
public class MallBaseCartController {
    ...
    public ResponseEnvelope getDetail(HttpServletRequest request, HttpServletResponse response){
        ...
        {
            cartProductRef.setTransportMoney(cartProductRef.getFixTransportExpense()==null?"0.00":
                    cartProductRef.getFixTransportExpense().multiply(
                            new BigDecimal(cartProductRef.getProductNumber())).setScale(2, BigDecimal.ROUND_DOWN).toString());
