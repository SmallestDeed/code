...
public interface SupplyDemandService {
    ...
    DemandOut publishSupplyDemandInfo(DemandAdd demandAdd, UserVo loginUser);

    SupplyDemandDetailVo getSupplyDemandDetailVo(Integer supplyDemandId, Integer id);

    Integer updateSupplyDemandInfoInMiniProgram(BaseSupplyDemand baseSupplyDemand);

    List<DemandInfoRel> selectDemandInfoRelBySupplyDemandId(Integer businessId);

    List<SupplyDemandPic> selectSupplyDemandPic(List<Integer> coverPicIdList);

    int delSupplyDemandById(Integer supplyDemandId, Integer userId);

    Integer checkIfSupplyDemandInfoDeleted(Integer id);
}
