 ...
 */
@Controller
@RequestMapping("/v1/tocmobile/product/baseCompany")
public class CompanyController {
    ...
    public ResponseEnvelope companyDetails(HttpServletRequest request,
    	...
    	//接收验证域名参数
                    String domainUrl = null;            try {                domainUrl = request.getHeader("Referer");                if(domainUrl==null){                    domainUrl = request.getHeader("Custom-Referer");                }            }catch (Exception e){                domainUrl = request.getHeader("Custom-Referer");            }            BaseCompany baseCompany = baseCompanyService.getCompanyByDomainUrl(domainUrl);
        //获取企业logo图片地址
        if(baseCompany!=null) {
        	 ...
        	 if (baseCompany.getCompanyLogo() != null) {
                 ...
                 if (resPic != null) {
                     resultMap.put("picPath", Utils.getPropertyName("app", "app.resources.url", "") + resPic.getPicPath());
                 }
             }
