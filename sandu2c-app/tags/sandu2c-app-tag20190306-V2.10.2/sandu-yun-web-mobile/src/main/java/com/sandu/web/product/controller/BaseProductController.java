 ...
 */
@Controller
@RequestMapping("/v1/tocmobile/product/baseproduct")
public class BaseProductController {
    ...
    public ResponseEnvelope productDetailsWeb(@ModelAttribute("baseProduct") BaseProduct baseProduct,
        ...
        logger.info(CLASS_LOG_PREFIX + "根据产品ID查询产品详情完成:{}", baseProduct.toString());
    ...
    public ResponseEnvelope getAllProductCategory(HttpServletRequest request) {
        ...
        List<List<ProCategoryPo>> categories = baseProductService.getAllProductCategory(baseCompany.getId());
//        logger.info(CLASS_LOG_PREFIX + "根据公司分类查询分类完成:{}", gson.toJson(categories));
    ...
    public ResponseEnvelope searchallproduct(@ModelAttribute ProductCategoryRel productCategoryRel, PageModel pageModel,
        ...
        productCategoryRel.setLimit(pageModel.getLimit());
        //获取当前平台
        BasePlatform basePlatform = basePlatformService.getBasePlatform(mobile2cPlatformCode);
        if (basePlatform == null) {
            return new ResponseEnvelope(false, "未知的平台");
        ...
        } else if (productCategoryRel.getCompanyId() != null && productCategoryRel.getCompanyId().intValue() > 0) {
            // 查询公司品牌
            List<Integer> brandList = baseBrandService.queryBrandListByCompanyId(productCategoryRel.getCompanyId());
...
//            logger.info(CLASS_LOG_PREFIX + "根据公司分类查询分类下产品列表:{}", gson.toJson(list));
            list = productCategoryRelService.findAllProductResultByCode(productCategoryRel, userSo);
//            logger.info(CLASS_LOG_PREFIX + "根据公司分类查询分类下产品列表完成:{}", gson.toJson(list));
        } else {
