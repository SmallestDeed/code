...
public class UserReviewsServiceImpl implements UserReviewsService {
    ...
    public List<UserReviewsVo> getAllUserReviews(Integer supplyDemandId, Integer userId, int start, int limit, String order, Integer isRead) {
        ...
        if (null != userReviewsVos && userReviewsVos.size() > 0) {
            ...
            for (UserReviewsVo userReviewsVo : userReviewsVos) {
                ...
                //处理表情
                String reviewsMsg = userReviewsVo.getReviewsMsg();
                if(StringUtils.isNotBlank(reviewsMsg)){
                    String newReviewsMsg = EmojiUtil.emojiConverterUnicodeStr(reviewsMsg);
                    userReviewsVo.setReviewsMsg(newReviewsMsg);
                    log.info("处理表情成功"+newReviewsMsg);
                }

                //处理该登录用户是否具备删除和编辑此信息权限
                if(userReviewsVo.getUserId()== null || userReviewsVo.getUserId() == 0 ||  userReviewsVo.getUserId()!=userId.intValue()){
                    userReviewsVo.setDelAuthor(false);
                } else{
                    userReviewsVo.setDelAuthor(true);
                }

                //处理该登录用户是否具备置顶该帖子的评论权限
                if(userReviewsVo.getSupplyDemandPublisherId()== null || userReviewsVo.getSupplyDemandPublisherId() == 0 ||  userReviewsVo.getSupplyDemandPublisherId().intValue()!=userId.intValue()){
                    userReviewsVo.setTopReviewsAuthor(false);
                } else{
                    userReviewsVo.setTopReviewsAuthor(true);
                }

                //查询是否能进入店铺
                if(userReviewsVo.getUserPlatformType()!= null &&userReviewsVo.getUserPlatformType()==2){
                    List<CompanyShop> companyShopList = userReviewsMapper.selectShopIdByUserId(userReviewsVo.getUserId());
                    if(companyShopList != null && companyShopList.size()>0 ){
                        CompanyShop companyShop = companyShopList.get(0);
                        userReviewsVo.setShopId(companyShop.getId());
                    }else {
                        userReviewsVo.setShopId(0);
                    }
                }else {
                    userReviewsVo.setShopId(0);
                }
                ...
                //取默认头像
                if (StringUtils.isEmpty(userReviewsVo.getUserPicPath())) {

                    if (userReviewsVo.getSex() == null || userReviewsVo.getSex() == 0 || userReviewsVo.getSex() == 1) {
                        String senderPic = sysUserService.getUserDefaultPic(1);
                        userReviewsVo.setUserPicPath(senderPic);
                    } else if (userReviewsVo.getSex() != null && userReviewsVo.getSex() == 2) {
            ...
            if (signleSpaceScenePlanIds != null && signleSpaceScenePlanIds.size() > 0) {
                ...
                if(mydecorationPlanVoList == null || mydecorationPlanVoList.size() ==0){}{
                    mydecorationPlanVoList = designPlanRenderSceneService.selectPcDesignPlanRenderScene(signleSpaceScenePlanIds);
                }
                if(mydecorationPlanVoList !=null && mydecorationPlanVoList.size() >0){
                    ...
                    if (mydecorationPlanVoList != null && mydecorationPlanVoList.size() > 0) {
                        for (UserReviewsVo userReviewsVos1 : userReviewsVos) {
                            for (MydecorationPlanVo mydecorationPlanVo : mydecorationPlanVoList) {
                                if (null != userReviewsVos1.getPlanId() && userReviewsVos1.getPlanId().intValue() == mydecorationPlanVo.getBusinessId().intValue()) {
            ...
            if (supplyDemandId != null) {
                ...
                userReviewsVos.forEach(it -> it.setLikeNum(it.getIsLike() == null ? 0 : it.getIsLike() + (it.getVirtualLikeNum() == null ? 0 : it.getVirtualLikeNum())));
                // 根据置顶次数.点赞数再次排序
                if ("nd.value".equals(order)) {
                    userReviewsVos.sort(Comparator.comparing(UserReviewsVo::getIsTop).thenComparing(UserReviewsVo::getLikeNum).reversed());

                }else {
                    //根据置顶次数,创建时间排序
                    userReviewsVos.sort(Comparator.comparing(UserReviewsVo::getIsTop).thenComparing(UserReviewsVo::getGmtCreate).reversed());
                }

                if(userReviewsVos.get(0).getIsTop()>0){
                    userReviewsVos.get(0).setTopSign(true);
                }
        ...
        try {
            if (userReviews.getId().intValue() > 0) {
                ...
                nodeInfoBizService.updateNodeInfoDetailValue(loginUser.getId(),
                        ...
                        NodeInfoConstant.SYSTEM_DICTIONARY_DETAIL_TYPE_COMMENT, new Byte("1"), true);
            }
        }catch (Exception e){
            log.info("保存评论成功,写入缓存数据失败");
        }
    ...
    @Override
    public int delReviewMsg(Integer reviewsId, LoginUser loginUser, Integer businessId) {
        UserReviews reviews = new UserReviews();
        reviews.setId(reviewsId);
        reviews.setUserId(loginUser.getId());
        int count = userReviewsMapper.delByIdAndUserId(reviews);
        //删除评论成功,修改评论总数
        if(count>0){
            Integer nodeId = nodeInfoBizService.registerNodeInfo(businessId, NodeInfoConstant.SYSTEM_DICTIONARY_NODE_TYPE_SUPPLY_DEMAND);
            nodeInfoBizService.updateNodeInfoDetailValue(loginUser.getId(),
                    nodeId, NodeInfoConstant.SYSTEM_DICTIONARY_NODE_TYPE_SUPPLY_DEMAND,
                    NodeInfoConstant.SYSTEM_DICTIONARY_DETAIL_TYPE_COMMENT, new Byte("0"), true);
        }
        return reviews.getId();
    }

    @Override
    public int updateReviewsIsTop(Integer reviewsId, Integer businessId, Integer isTop) {
        //查询该帖子是否置顶过,如果置顶过评论将之前的置顶次数全部置为0
        userReviewsMapper.delReviewsIsTopByBusinessId(businessId);
        if(isTop == null || isTop == 0 ){
            //取消置顶
            return userReviewsMapper.delReviewsIsTopById(reviewsId);
        }else {
            //置顶
            return userReviewsMapper.updateReviewsIsTopById(reviewsId,businessId);
        }

    }

    @Override
    public BaseSupplyDemand getPaivateDemandInfo() {
        return userReviewsMapper.getPaivateDemandInfo();
    }

    @Override
    public UserReviewsVo getPaivateuserReviewsInfo() {
        return userReviewsMapper.getPaivateuserReviewsInfo();
    }

    @Override
    public Integer getTopReviewIdByBusinessId(Integer businessId) {
        List<Integer> idList = userReviewsMapper.getTopReviewIdByBusinessId(businessId);
        log.info("获取置顶的评论，帖子businessId：{}，置顶评论id集合：{}", businessId, GsonUtil.bean2Json(idList));
    ...
    private UserReviews buildUserReviews(UserDemandReviewsAdd userDemandReviewsAdd, LoginUser loginUser) {
        ...
        userReviews.setContent(userDemandReviewsAdd.getContent());
