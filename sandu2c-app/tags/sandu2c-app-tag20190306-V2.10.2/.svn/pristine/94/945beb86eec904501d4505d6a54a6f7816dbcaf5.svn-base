...
public class SupplyDemandServiceImpl implements SupplyDemandService {
    ...
    public List<BaseSupplyDemand> getAllSupplyDemandInfo(BaseSupplyDemandAdd baseSupplyDemandAdd) {
        ...
        for (BaseSupplyDemand baseSupplyDemand : baseSupplyDemandList) {
            ...
            if(StringUtils.isNotBlank(baseSupplyDemand.getDescription())){
                String newDescription = EmojiUtil.emojiConverterUnicodeStr(baseSupplyDemand.getDescription());
                baseSupplyDemand.setDescription(newDescription);
            }
            if(StringUtils.isNotBlank(baseSupplyDemand.getTitle())){
                String newTitle = EmojiUtil.emojiConverterUnicodeStr(baseSupplyDemand.getTitle());
    ...
    public DemandOut publishSupplyDemandInfo(DemandAdd demandAdd, UserVo userVo) {
        ...
        nodeInfoBizService.registerNodeInfo(baseSupplyDemand.getId(), NodeInfoConstant.SYSTEM_DICTIONARY_NODE_TYPE_SUPPLY_DEMAND);
    ...
    public SupplyDemandDetailVo getSupplyDemandDetailVo(Integer supplyDemandId, Integer userId) {
        ...
        if(StringUtils.isNotBlank(baseSupplyDemand.getDescription())){
            String newDescription = EmojiUtil.emojiConverterUnicodeStr(baseSupplyDemand.getDescription());
            supplyDemandDetailVo.setDescription(newDescription);
        }
        if(StringUtils.isNotBlank(baseSupplyDemand.getTitle())){
            String newTitle = EmojiUtil.emojiConverterUnicodeStr(baseSupplyDemand.getTitle());
            supplyDemandDetailVo.setTitle(newTitle);
        }
      ...
      Map<String, Object> map = nodeInfoBizService.getNodeData(supplyDemandDetailVo.getId(), NodeInfoConstant.SYSTEM_DICTIONARY_NODE_TYPE_SUPPLY_DEMAND, userId);
        if(null !=map && map.size() > 0){
            ...
            // xxx : 供求详情浏览量，没有加上虚拟浏览量
            // create by zhoujc at  2019/3/11 17:36.
            Object num = map.get("setVirtualViewNum");
            num = num == null ? 0 : num;
            supplyDemandDetailVo.setViewNum(supplyDemandDetailVo.getViewNum() == null ? 0 : supplyDemandDetailVo.getViewNum()
                    + (Integer) num);

            num = map.get("setVirtualLikeNum");
            num = num == null ? 0 : num;
            supplyDemandDetailVo.setLikeNum(supplyDemandDetailVo.getLikeNum() == null ? 0 : supplyDemandDetailVo.getLikeNum()
                    + (Integer) num);

            num = map.get("setVirtualFavoriteNum");
            num = num == null ? 0 : num;
            supplyDemandDetailVo.setFavoriteNum(supplyDemandDetailVo.getFavoriteNum() == null ? 0 : supplyDemandDetailVo.getFavoriteNum()
                    + (Integer) num);
            // log : 供求详情浏览量，没有加上虚拟浏览量
        }
        ...
        supplyDemandDetailVo.setContent(baseSupplyDemand.getContent());
    ...
    private BaseSupplyDemand buildBaseSupplyDemandInfo(DemandAdd demandAdd, UserVo userVo) {
        ...
        baseSupplyDemand.setContent(demandAdd.getContent());
        return baseSupplyDemand;
    }
    ...
    @Override
    public Integer checkIfSupplyDemandInfoDeleted(Integer id) {
        BaseSupplyDemand record = baseSupplyDemandMapper.selectByPrimaryKey(id);
        return (record == null || record.getIsDeleted() == 1) ? 1 : 0;
    }
}
