...
public class DesignPlanController {
    ...
    /**
     * 获取设计方案筛选条件元数据
     * I think this is the common data that the changed frequency is very low. So I cached this.
     * Updated by steve on 2019/09/10
     *
     * @param request
     * @return
     */
    ...
    public ResponseEnvelope designplanStyleList(HttpServletRequest request) {
        ...
        String designMetadataStr = redisService.get(DESIGN_PLAN_CONDITION_METADATA);
        if(org.apache.commons.lang3.StringUtils.isNotEmpty(designMetadataStr)) {
            newDesignPlanStyleAllDic = gson.fromJson(designMetadataStr,new TypeToken<List<DesignProductStyleDicVo>>(){}.getType());
        }else{
        ...
        /*----------------Setp 2:获取设计方案空间面积数据-----------------*/
            ...
            sysDictionaryCondition.setOrder("ordering");
            sysDictionaryCondition.setOrderNum("asc");
            ...
            String newDesignPlanStyleAllDicStr = gson.toJson(newDesignPlanStyleAllDic,new TypeToken<List<DesignProductStyleDicVo>>(){}.getType());
            redisService.set(DESIGN_PLAN_CONDITION_METADATA, newDesignPlanStyleAllDicStr,60*60*12);
        }
        ...
        if (baseCompany != null && companyCode.equals(baseCompany.getCompanyCode())){
            newDesignPlanStyleAllDic = newDesignPlanStyleAllDic.stream().filter(a -> a.getHouseType() != 8).collect(Collectors.toList());
        }
