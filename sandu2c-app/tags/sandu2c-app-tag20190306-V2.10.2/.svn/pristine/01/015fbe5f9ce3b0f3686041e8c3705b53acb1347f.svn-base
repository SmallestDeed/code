 ...
 */
@Controller
@RequestMapping("/v1/miniprogram/sysUserMessage")
...
//            sysUserMessageService.updateIsRead(sysUserMessage.getUserId());
            sysUserMessageService.updateIsRead(sysUserMessage.getUserId(),sysUserMessage.getMessageType());
    ...
    @ResponseBody
    public Object getSystemList(@ModelAttribute SysUserMessage sysUserMessage, HttpServletRequest request) {
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        logger.info("getSystemList的传入参数:{}",gson.toJson(sysUserMessage));
        logger.info(CLASS_LOG_PREFIX + "获取登录用户信息:getUserFromCache:{}.", null == loginUser ? null : loginUser.toString());
        if (loginUser == null) {
            return new ResponseEnvelope(false, "请登录!");
        }
        sysUserMessage.setUserId(loginUser.getId());
        sysUserMessage.setMessageType(2);
        Integer isRead = sysUserMessage.getIsRead();
        sysUserMessage.setIsRead(null);
        logger.info("sysUserMessage的值:{}",sysUserMessage);
        int count = sysUserMessageService.getCount(sysUserMessage);
        if (count == 0) {
            return new ResponseEnvelope<>(true, "无系统消息");
        }
        List<SysUserMessage> allMessages = sysUserMessageService.getList(sysUserMessage);
        logger.info("isRead!=null && isRead == 1:{}",(isRead!=null && isRead == 1));
     ...
     * @return
     */
    @RequestMapping("/findSystemUnread")
    @ResponseBody
    public Object findSystemUnread(HttpServletRequest request) {
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        logger.info(CLASS_LOG_PREFIX + "获取登录用户信息:getUserFromCache:{}.", null == loginUser ? null : loginUser.toString());
        if (loginUser == null) {
            return new ResponseEnvelope(false, "请登录!");
