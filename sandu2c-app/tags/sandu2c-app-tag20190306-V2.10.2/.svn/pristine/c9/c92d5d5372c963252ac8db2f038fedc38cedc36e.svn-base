...
import com.sandu.common.util.ClassReflectionUtils;
...
public class DesignPlanProductRenderSceneServiceImpl implements DesignPlanProductRenderSceneService {
     ...
     */
    @Override
    public List<ProductsCostType> costList(UserSO userSo, DesignPlanProductRenderScene designPlanProductRenderScene){
		/*查询用户关联的序列号list*/
        AuthorizedConfig authorizedConfig = new AuthorizedConfig();
        authorizedConfig.setUserId(userSo.getUserId());
        authorizedConfig.setState(1);
        //authorizedConfig.setTerminalImei(terminalImei);
        List<AuthorizedConfig> list = authorizedConfigService.selectList(authorizedConfig);
		/*查询用户关联的序列号list->end*/
		/*add品牌,大类,小类,产品查询条件*/
        List<BaseProduct> baseProductList=new ArrayList<BaseProduct>();
        if( 3 == userSo.getUserType() ){
        ...
        if( total > 0 ){
            ...
            for(ProductsCostType costType : costTypeList){
                ...
                costType.setUserId(userSo.getUserId());
