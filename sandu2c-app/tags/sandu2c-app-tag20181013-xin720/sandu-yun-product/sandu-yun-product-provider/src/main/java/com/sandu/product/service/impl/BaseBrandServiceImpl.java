...
public class BaseBrandServiceImpl implements BaseBrandService {
    ...
    @Override
    public Map<Integer, String> brandIdAndNameMap(List<Integer> brandIds) {
        brandIds = brandIds.stream().filter(Objects::nonNull).distinct().collect(Collectors.toList());
        if (isEmpty(brandIds)) {
            return Collections.emptyMap();
        }
        List<BaseBrand> baseBrands = baseBrandMapper.selectBrandByIds(brandIds);
        return baseBrands.stream().collect(Collectors.toMap(BaseBrand::getId, BaseBrand::getBrandName));
    }
}
