...
//        }
        ...
        for (ProductsCost productsCost : productsCostList) {
            ...
            for (ProductCostDetail productCostDetail : productCostDetailList) {
                ...
                if ("mobile2b".equals(panoramaVo.getPlatformCode()))
                {
                    // B端显示所有价格
                }else
                {
                    //不是本公司的产品则产品总价和单价都为0
                    if (null == productCostDetail.getTotalPrice() ||
                            (isSandu.intValue() == 0 && !companyId.equals(productCostDetail.getCompanyId())))
                    {
