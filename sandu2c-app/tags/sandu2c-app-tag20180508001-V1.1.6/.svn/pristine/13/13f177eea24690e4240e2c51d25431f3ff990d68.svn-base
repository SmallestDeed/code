...
@Controller
@RequestMapping("/v1/tocmobile/product/collectcatalog")
public class CollectCatalogController {
    ...
    public Object addCollectCatalog(HttpServletRequest request, String catalogName) {
        ...
        try {
           ...
           /* UserSO userSO = userSessionService.getUserSessionObjectBySessionId(request.getRequestedSessionId(), request.getSession().getMaxInactiveInterval());*/
            Long userid= (Long) request.getAttribute("tokenUserId");
            com.nork.common.model.LoginUser loginUser=userSessionService.getUserFromCache(LoginUserCommonConstant.LOGIN_FROM_TOCMOBILE+userid);
            //LoginUser loginUser = new LoginUser();
            /*if (userSO == null) {
                loginUser = null;
            } else {
                loginUser = UserObject.parseUserSoToLoginUser(userSO);
            }
