package com.sandu.analyze.service.impl;

import com.sandu.analyze.common.constant.elasticsearch.IndexConstant;
import com.sandu.analyze.common.constant.elasticsearch.IndexInfoQueryConfig;
import com.sandu.analyze.common.tool.JsonUtil;
import com.sandu.analyze.entity.elasticseatch.dto.*;
import com.sandu.analyze.entity.elasticseatch.index.*;
...
public class ElasticSearchServiceImpl implements ElasticSearchService {
    ...
    @Override
    public BulkStatistics bulk(List<Object> bulkRequestList) throws ElasticSearchException {
        return this.bulk(bulkRequestList, null);
    }
    ...
    @Override
    public SearchObjectResponse search(String index, List<QueryBuilder> matchQueryBuilderList, int start, int dataSize) throws ElasticSearchException {
        return this.search(index, matchQueryBuilderList, null, null, null, start, dataSize);
    }

    @Override
    public SearchObjectResponse search(String index, List<QueryBuilder> matchQueryBuilderList, List<QueryBuilder> noMatchQueryBuilderList, int start, int dataSize) throws ElasticSearchException {
        return this.search(index, matchQueryBuilderList, noMatchQueryBuilderList, null, null, start, dataSize);
    }

    @Override
    public SearchObjectResponse search(String index, List<QueryBuilder> matchQueryBuilderList, List<QueryBuilder> noMatchQueryBuilderList, List<ShouldMatchSearch> shouldMatchSearchList, int start, int dataSize) throws ElasticSearchException {
        return this.search(index, matchQueryBuilderList, noMatchQueryBuilderList, shouldMatchSearchList, null, start, dataSize);
    }

    @Override
    public SearchObjectResponse search(String index, List<QueryBuilder> matchQueryBuilderList, List<QueryBuilder> noMatchQueryBuilderList, List<ShouldMatchSearch> shouldMatchSearchList, List<AggregationBuilder> aggregationBuilderList, int start, int dataSize) throws ElasticSearchException {
        return this.search(index, matchQueryBuilderList, noMatchQueryBuilderList, shouldMatchSearchList, aggregationBuilderList, null, start, dataSize);
    }

    @Override
    public SearchObjectResponse search(String index, List<QueryBuilder> matchQueryBuilderList, List<QueryBuilder> noMatchQueryBuilderList, List<ShouldMatchSearch> shouldMatchSearchList, List<AggregationBuilder> aggregationBuilderList, SortOrderObject sortOrderObject, int start, int dataSize) throws ElasticSearchException {

        if (StringUtils.isEmpty(index)) {
            log.error(CLASS_LOG_PREFIX + "搜索失败，必填参数为空.index:{}.", index);
            throw new ElasticSearchException(CLASS_LOG_PREFIX + "搜索失败，必填参数为空.index:" + index + ".");
        }
        ...
        if (null != matchQueryBuilderList && 0 < matchQueryBuilderList.size()) {
            matchQueryBuilderList.forEach(matchQueryBuilder -> boolQueryBuilder.must(matchQueryBuilder));
        }
        //排除指定数据
        if (null != noMatchQueryBuilderList && 0 < noMatchQueryBuilderList.size()) {
            noMatchQueryBuilderList.forEach(nomMatchQueryBuilder -> boolQueryBuilder.mustNot(nomMatchQueryBuilder));
        }
        //或匹配数据
        ...
        if (null != aggregationBuilderList && 0 < aggregationBuilderList.size()) {
            aggregationBuilderList.forEach(aggregationBuilder -> sourceBuilder.aggregation(aggregationBuilder));
        }
        ...
        switch (index) {
            //用户
            case IndexConstant.INDEX_USER_RECORD:
                clazz = UserMappingData.class;
                break;
            //产品
            case IndexConstant.INDEX_PRODUCT_RECORD:
                clazz = ProductMappingData.class;
                break;
            //设计方案
            case IndexConstant.INDEX_DESIGNPLAN_RECORD:
                clazz = DesignPlanMappingData.class;
                break;
            //户型
            case IndexConstant.INDEX_HOUSE_RECORD:
                clazz = HouseMappingData.class;
                break;
            //渲染
            case IndexConstant.INDEX_RENDER_RECORD:
                clazz = RenderMappingData.class;
                break;
            //接口
            case IndexConstant.INDEX_INTERFACE_RECORD:
                clazz = InterfaceMappingData.class;
                break;
            default:
                log.error(CLASS_LOG_PREFIX + "无法识别的索引对象类型,index:{}.", index);
                throw new ElasticSearchException(CLASS_LOG_PREFIX + "无法识别的索引对象类型,index:" + index + ".");
        }

        //组装结果
        List indexMappingDataList = new ArrayList<>();
        ...
        while (searchHitIterator.hasNext()) {
            ...
            if (!StringUtils.isEmpty(searchHit.getSourceAsString())) {
                ...
                indexMappingDataList.add(object);
            }
        }

        return new SearchObjectResponse(indexMappingDataList, searchResponse.getHits().totalHits, searchResponse.getAggregations());
    }
}