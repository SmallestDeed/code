...
import com.sandu.designshare.model.DesignPlanJsonData;
...
public class QRCodeController {
    ...
    @Value("${company.access.token.dynamic.url}")
    private String accessTokenDynamicUrl;
     ...
     * @date 2018/8/27 0027PM 9:02
     */
    @RequestMapping(value = "/noticecode", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEnvelope getNoticeQRCode(@RequestParam(value = "location", required = false) String location,
                                            @RequestParam(value = "jsonData", required = false) String jsonData,
                                            @RequestParam(value = "appId", required = false) String appId,
                                            HttpServletRequest request) {
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        DesignPlanJsonData designPlanJsonData = new DesignPlanJsonData();
        designPlanJsonData.setJsonData(jsonData);
        if (loginUser != null){
            designPlanJsonData.setCreator(loginUser.getName());
            designPlanJsonData.setModifier(loginUser.getName());
        }
        Integer id = designPlanService.addDesignPlanJsonData(designPlanJsonData);
        String sceneStr = id + "";
        log.info("生成微信分享参数:" + sceneStr);

        //根据appId动态获取秘钥，拼接accessTokenUrl，最终获取微信accessToken
        log.info("动态生成海报分享的二维码,appId="+appId);
        String finalAccessTokenUrl;
        if (StringUtils.isNotBlank(appId)) {
            BaseCompanyMiniProgramConfig miniProgramConfig = baseCompanyService.getBaseCompanyMiniProgramConfigByAppId(appId);
            if (null == miniProgramConfig || null == miniProgramConfig.getAppSecret()) {
                log.error("动态生成二维码,根据appId查询配置为空！appId=" + appId);
                return new ResponseEnvelope(false, "根据appId查询配置为空，请检查参数！appId=" + appId);
            }
            String appSecret = miniProgramConfig.getAppSecret();
            //拼接格式 accessTokenDynamicUrl + &appid=wx42e6b214e6cdaed3&secret=db38a182abfaaa2204f16375588f399c
            finalAccessTokenUrl = accessTokenDynamicUrl + "&appid=" + appId + "&secret=" + appSecret;
        } else {
            finalAccessTokenUrl = accessTokenUrl;
        }
        log.info("动态生成二维码,accessTokenUrl=" + finalAccessTokenUrl);

        //获取微信accessToken
        String accessTokenJson = WXChatRequest.httpsRequest(finalAccessTokenUrl, "GET", null);
        String accessToken = null;
        if (accessTokenJson.contains("access_token")) {
            Type type = new TypeToken<AccessTokenSuccess>() {
            }.getType();
        ...
        //生成二维码保存路径
        String absolutePath = FilePathUtil.absolutePath(storagePath, "xzmini", "shareSign", "image", LocalDateTime.now(), QRCodeShareTypeConstant.WEI_XIN_P_Y_QUAN+"_"+LocalDateTime.now().format(FormatterUtil.nameFormatter) + ".png");
        log.info("二维码图片路径:" + absolutePath);
        //封装微信图片参数
        QRCode qrCode = new QRCode();
        qrCode.setQRUrl(QRCodeUrl);
    ...
    @ResponseBody
    public ResponseEnvelope getJsonData(Integer id){
        return new ResponseEnvelope(true, "", designPlanService.getJsonData(id));
    }
}
