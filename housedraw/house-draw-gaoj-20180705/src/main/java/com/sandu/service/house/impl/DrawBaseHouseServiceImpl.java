...
public class DrawBaseHouseServiceImpl implements DrawBaseHouseService {
        ...
        if (list != null && !list.isEmpty()) {
            ...
            for (DrawBaseHouseBO house : list) {
                if (StringUtils.isNoneBlank(house.getAreaLongCode())) {
                    ...
                    buf.append(baseLivingService.getDetailAddress(split)).append(house.getLivingName());
    ...
    public Map<Long, String> mapResource(List<DrawBaseHouseBO> drawHouses) {
        ...
        // 图片资源优先级 ： 截图 > 户型图
        // 查询snap pic
        List<DrawBaseHouseBO> listResource = drawBaseHouseMapper.listDrawHouseResource(1, drawHouses);

        // 查询res_pic
        if (listResource.size() < drawHouses.size()) {
            List<DrawBaseHouseBO> snap = drawHouses.stream().filter(p -> p.getSnapPicId() == null || p.getSnapPicId() <= 0).collect(Collectors.toList());
    ...
    public Map<String, Object> saveDrawBaseHouse(String data, String userId) throws BusinessException {
        Map<String, Object> map = new HashMap<>();
        // 解析请求参数
    ...
    public void handlerCheck(DrawBaseHouseCheck check, DrawBaseHouse drawHouse) {
        ...
        if (updateCount == 1) {
            ...
            if (check.getCheckStatus().intValue() == DrawBaseHouseConstant.HOUSE_BAKE_WAIT) {
                ...
                log.info("添加户型{}(houseId)到烘焙队列中", drawHouse.getId());
    ...
    private String getQueueName(Long houseId) {
        // 数据类型
        Integer dataType = drawBaseHouseMapper.getDataType(houseId);
        log.debug("添加烘焙任务处理，dataType={}", dataType);
        return (Objects.isNull(dataType) || dataType.equals(DrawBaseHouseConstant.DATA_PLATFORM))
                ? BakeTaskQueue.INTERNAL.toString() : BakeTaskQueue.EXTERNAL.toString();
    }
    ...
    private void handlerJumpHouseCheck(DrawBaseHouseSubmitDTO dtoNew, UserLoginBO loginUser) {
        if (!DrawBaseHouseConstant.DATA_PERSONAL.equals(dtoNew.getDataType())) {
            log.info("{}(houseId)非个人户型数据，需要审核户型流程", dtoNew.getHouseId());
            return;
        }

        if (!HouseAuthUtils.hasJumpHouseCheckPermission()) {
            log.info("{}(userId)用户没有权限，无法进行审核户型", loginUser.getId());
            return;
        }
        ...
        log.info("普通、内部演示角色跳过审核户型的流程，直接进入烘焙队列，{}(houseId)", drawHouse.getId());
    ...
    public Map<String, Object> getHouses(DrawBaseHouseSearch query) {
        ...
        if (count > 0) {
            ...
            for (DrawBaseHouseBO drawBaseHouseBO : list) {
                ...
                // 检查样板房是否发布
//				if (checkDesignTemplet(drawBaseHouseBO.getId())) {
//					drawBaseHouseBO.setDealStatus(DrawBaseHouseConstant.DEAL_STATUS_DESIGN_PUTAWAY);
//				}
            }
    ...
    boolean checkDesignTemplet(Long houseId) {
        if (houseId == null || houseId <= 0) {
            return false;
        }
        return baseHouseMapper.countPutwayDesignTemplet(houseId) > 0;
    }
    ...
    public DrawHouseAndAreaVO facsimileHouse(Long houseId, String type) {
        ...
        BaseAreaBO baseAreaBO = getBaseAreaBO(drawBaseHouseBO);
        return setDrawHouseAndAreaVO(drawBaseHouseBO, baseAreaBO);
    }

    private DrawHouseAndAreaVO setDrawHouseAndAreaVO(DrawBaseHouseBO drawBaseHouseBO, BaseAreaBO baseAreaBO) {
        DrawHouseAndAreaVO drawHouseAndAreaVO = new DrawHouseAndAreaVO();
        drawHouseAndAreaVO.setDrawBaseHouseBO(drawBaseHouseBO);
        drawHouseAndAreaVO.setBaseAreaBO(baseAreaBO);
        return drawHouseAndAreaVO;
    }

    private BaseAreaBO getBaseAreaBO(DrawBaseHouseBO drawBaseHouseBO) {
        // Fix NullPointerException
        if (drawBaseHouseBO == null) {
            return null;
        }
        String areaId = drawBaseHouseBO.getAreaId();
        // 所有的地理区域
        List<BaseAreaBO> areaBos = baseLivingMapper.listAreas();
        Map<Integer, Object> map = new HashMap<>();
        Map<Integer, Object> dataMap = getParentAreaBO(areaBos, map, 1, areaId);
        // 将map中的节点去出来,组装成树状结构
        return constructionTree(dataMap);
    }

    // 递归查询省市区,从后往上查
    private Map<Integer, Object> getParentAreaBO(List<BaseAreaBO> areaBos, Map<Integer, Object> map, int i, String areaId) {
        for (BaseAreaBO baseAreaBO : areaBos) {
            if (Objects.toString(areaId, "").equals(baseAreaBO.getAreaCode())) {
                map.put(i, baseAreaBO);
            }
        }
        if (map.size() == 3) {
            return map;
        }
        return getParentAreaBO(areaBos, map, i + 1, ((BaseAreaBO) map.get(i)).getPid());
    }

    // 将省市区组装成树状返回
    private BaseAreaBO constructionTree(Map<Integer, Object> map) {
        BaseAreaBO baseAreaBO = (BaseAreaBO) map.get(DrawBaseHouseConstant.AREA_THIRD);
        List<BaseAreaBO> secondAreaBO = createList();
        BaseAreaBO secondBaseAreaBO = (BaseAreaBO) map.get(DrawBaseHouseConstant.AREA_SECOND);
        secondAreaBO.add(secondBaseAreaBO);
        List<BaseAreaBO> thirdAreaBO = createList();
        BaseAreaBO thirdBaseAreaBO = (BaseAreaBO) map.get(DrawBaseHouseConstant.AREA_FIRST);
        thirdAreaBO.add(thirdBaseAreaBO);
        secondBaseAreaBO.setChildAreas(thirdAreaBO);
        baseAreaBO.setChildAreas(secondAreaBO);
        return baseAreaBO;
    }

    private List<BaseAreaBO> createList() {
...
//			dto = new ObjectMapper().readValue(jsonStr, DrawBaseHouseControllerCallBackDTO.class);
    ...
    private void updateHouseResource(DrawBaseHouse drawHouse) {
        ...
        if (drawHouse.getBaseHouseId() == null || drawHouse.getBaseHouseId() <= 0) {
            log.error("户型 {}(baseHouseId)不存在", drawHouse.getBaseHouseId());
            throw new BusinessException(false, ResponseEnum.HOUSE_IS_NOT_FOUND);
        }
        ...
        DrawHouseResouceBO drawResource = drawBaseHouseMapper.getHouseFileResource(drawHouse.getId());
    ...
    public void handlerSubmitHouse(Long houseId) {
        ...
        List<Long> emptySpaces = drawSpaceCommonService.getDealDrawSpaceCommon(houseId);
    ...
    void handlerBakeTaskFailed(DrawBaseHouseControllerCallBackDTO dto) {
        ...
        if (dto != null) {
            ...
            // ip 记录
            record.setMessage("Bake Client IP => " + HttpContextUtils.getBakeIpAddress() + "\n" + dto.getMessage());
