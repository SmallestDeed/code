...
//        map.put("total", count);
    ...
    public Map<String, Object> listBaimoProduct(BaseProductQuery query, boolean useCache) {
        if (useCache) {
            ...
            String value = redisService.getCacheOfHash(key, hashKey, redisSoftProductExpire,
                    TimeUnit.MINUTES, () -> {
                        Map<String, Object> map = baseProductService.listBaimoProduct(query);
                        try {
    ...
    public Map<Long, ProductTypeBO> listProductType(List<BaseProductBO> products) {
        ...
        Map<Long, ProductTypeBO> map = new HashMap<>();
        ...
        if (proTypes.isEmpty()) {
            log.warn("产品的分类数据可能出现异常");
            return Optional.of(map).orElse(new HashMap<>());
        }

        // 去重
        proTypes = proTypes.stream().distinct().collect(Collectors.toList());

        List<ProductTypeBO> types = systemDictionaryMapper.getProductTypes(proTypes);
        if (types != null && !types.isEmpty()) {
            map = types.stream().collect(Collectors.toMap(ProductTypeBO::getProId, t -> t, (p, n) -> n));
        }
