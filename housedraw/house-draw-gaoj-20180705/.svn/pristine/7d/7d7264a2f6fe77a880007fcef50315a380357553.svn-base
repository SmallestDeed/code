...
public class Utils {
	...
	public final static String _LINE_SPARTA = "_";
	...
	public static String getValue(String configKey, String valueKey) throws BusinessException {
		...
		if (cacheConfigMap.containsKey(cacheKey)) {
			String value = cacheConfigMap.get(cacheKey);
			LOG.debug("从CacheConfigMap缓存里取值, {}(cacheKey), {}(value)", cacheKey, value);
			return value;
		}
		...
		try {
			...
			if (app.containsKey(valueKey)) {
				value = replaceDate(app.getString(valueKey));
			} else {
				value = replaceDate(app.getString(valueKey + ".upload.path"));
			}

			// 添加到本地缓存里
			if (StringUtils.isNoneBlank(value)) {
				cacheConfigMap.put(cacheKey, value);
			}
		} catch (Exception e) {
