...
public class ProductAttributeServiceImpl implements ProductAttributeService {
    ...
    public ProductAttribute getProAttr(String proAttrKey, String proAttrValKey) {
        ...
        if (!doorAttr.containsProAttr(proAttrKey) || !doorAttr.containsProAttrVal(proAttrValKey)) {
            log.info("{}(proAttrKey), {}(proAttrValKey)不是需要处理门类型", proAttrKey, proAttrValKey);
            return null;
        }

        ProductAttribute proAttr = this.handlerProprops(proAttrKey, proAttrValKey);
        if (proAttr == null) {
            return null;
        }
     ...
     * @return
     */
    private ProductAttribute handlerProprops(String proAttrKey, String proAttrValKey) {
