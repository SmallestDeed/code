...
public class DrawBakeTaskServiceImpl implements DrawBakeTaskService {
    ...
    @Value("${'${mail.bake.fail.subscribe}'.split(','):songjianming@sanduspace.cn}")
    ...
    public List<DrawBakeTaskBO> listBakeTask(String queueName, Long taskId) {
        List<DrawBakeTaskBO> tasks = drawBakeTaskMapper.list(taskId, queueName, failCount, resetCount);

        if (tasks != null && !tasks.isEmpty()) {
            // 主任务状态 ==> 烘焙中
            DrawBakeTaskBO task = tasks.get(0);
            // drawBakeTaskMapper.updateDrawBakeTaskStatus(DrawBaseHouseConstant.BAKE_TASK_STATUS_PROCESSING, "", task.getId());
            ...
            // 更改户型状态 ==> 烘焙中
            drawBaseHouseMapper.updateDrawHouseStatus(DrawBaseHouseConstant.HOUSE_BAKE_PROCESSING, null, task.getHouseId());

            // base_house烘焙中
            baseHouseMapper.updateHouseDealStatus(DrawBaseHouseConstant.DEAL_STATUS_BAKE, task.getHouseId());

            log.info("{}(ip), {}(queueName)获取烘焙子任务{}(taskDetailId)", HttpContextUtils.getBakeIpAddress(), queueName, taskDetails != null ? taskDetails.get(0).getId() : null);
        }
    ...
    public Long getBakeTaskId(String queueName) {
        ...
        String ipAddress = HttpContextUtils.getBakeIpAddress();
        if (tasks != null && !tasks.isEmpty()) {
