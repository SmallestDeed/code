...
//        }
    ...
    public void handlerSpacePic(DrawSpaceCommon drawSpaceCommon) {
        ...
        if (pic != null) {
            Long effectPicId = drawResHousePicService.copyFile(pic, SystemConfigEnum.SPACE_PIC_UPLOAD.getFileKey(), false);
            drawSpaceCommon.setPicId(effectPicId == null ? -1 : effectPicId.intValue());
        }
    }

    @Override
    public List<Long> getDealDrawSpaceCommon(Long houseId) {
        if (houseId == null) {
            return null;
        }
        return drawSpaceCommonMapper.getDealDrawSpaceCommon(houseId);
    }
