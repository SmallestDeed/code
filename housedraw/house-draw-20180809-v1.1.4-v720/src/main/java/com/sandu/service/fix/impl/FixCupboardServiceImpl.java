...
import com.sandu.service.fix.dao.DrawFixCupboardRecordMapper;
...
public class FixCupboardServiceImpl implements FixCupboardService {
    ...
    private final static String DEFAULTS_KITCHEN_PREFIX = "kitchen";
    ...
    @Autowired
    DrawFixCupboardRecordMapper drawFixCupboardRecordMapper;
    ...
    @Override
    public Map<String, Object> listFixCupboardHouseOfRedis(FixCupboardQuery query) {
        String value = redisService.rightPopByQueue(RedisKeyConstant.HOUSE_DRAW_FIX_CUPBOARD_QUEUE.toString(), null);
        if (value == null) {
            log.info("Redis队列里没有需要的修复的任务");
            return null;
        }
        ...
        log.info("查询到需要修复的户型 {}(houseId)", houseId);

        query.setHouseId(houseId);
        Map<String, Object> map = listFixCupboardHouse(query);

        if (map != null && map.get("list") != null && !((List) map.get("list")).isEmpty()) {
            // 更新bugfix status
            DrawBaseHouse drawHouse = new DrawBaseHouse();
            drawHouse.setId(houseId);
            drawHouse.setGmtModified(new Date());
            drawHouse.setBugfixStatus(BugFixStatus.FIX_PROCESS.getStatus());
    ...
    public Map<String, Object> saveV2(FixCupboardSaveBO fixCupboard) {
        try {
            log.info("修复橱柜保存：{}, {}, {}", HttpContextUtils.getIpAddress(),
                    fixCupboard.getHouseId(), fixCupboard.getMessage());
            return this.save(fixCupboard);
        } catch (Exception e) {
            // 异常时，删除上传的文件资源
            this.handlerErrorWithSave(fixCupboard);
    ...
    public Map<String, Object> save(FixCupboardSaveBO fixCupboard) {
        ...
        // 修复数据异常
        if (HouseSaveHandlerStatus.FAILURE.getStatus().equals(fixCupboard.getStatus())) {
            log.warn("修复橱柜失败，{}(houseId), {}(message)", fixCupboard.getHouseId(), fixCupboard.getMessage());
            // 删除上传的文件资源
            this.handlerErrorWithSave(fixCupboard);

            // 添加修复橱柜异常记录
            this.handlerFixCupboardErrorBySave(fixCupboard);

            // 修复异常
            drawHouse.setGmtModified(new Date());
            drawHouse.setBugfixStatus(BugFixStatus.FIX_ERROR.getStatus());
        } else {
            // 处理修复成功的户型
            response = handlerDrawHouse(fixCupboard, drawHouse);
        }
     ...
     * @return
     */
    private Map<String, Object> handlerDrawHouse(FixCupboardSaveBO fixCupboard, DrawBaseHouse drawHouse) {
        Map<String, Object> response = new HashMap<>();

        if (HouseSaveHandlerStatus.NORMAL.getStatus().equals(fixCupboard.getStatus())) {
            log.warn("修复橱柜正常，{}(houseId), {}(message)", fixCupboard.getHouseId(), fixCupboard.getMessage());

            // 不需要修复
            drawHouse.setGmtModified(new Date());
            drawHouse.setBugfixStatus(BugFixStatus.FIX_SUCCEED.getStatus());
        } else {
            ...
            // 处理保存的资源
            this.handlerDrawHouseResourceBySave(response, drawHouse, fixCupboard);
     ...
     * @param fixCupboard
     */
    private void handlerDrawHouseResourceBySave(Map<String, Object> response, DrawBaseHouse drawHouse,
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public void handlerFixCupboardErrorBySave(FixCupboardSaveBO fixCupboard) {
        DrawFixCupboardRecord record = new DrawFixCupboardRecord();
        record.setStatus(fixCupboard.getStatus() == null ? 0 : fixCupboard.getStatus().byteValue());
        record.setHouseId(fixCupboard.getHouseId());
        record.setMessage(fixCupboard.getMessage());

        UserLoginBO loginBO = HouseAuthUtils.getUserLoginBO();
        record.setCreator(loginBO.getLoginName());
        record.setModifier(loginBO.getLoginName());

        drawFixCupboardRecordMapper.insertSelective(record);

        // 修复异常
        DrawBaseHouse drawHouse = new DrawBaseHouse();
        drawHouse.setGmtModified(new Date());
        drawHouse.setId(fixCupboard.getHouseId());
        drawHouse.setBugfixStatus(BugFixStatus.FIX_ERROR.getStatus());
        drawBaseHouseMapper.updateByPrimaryKeySelective(drawHouse);
    }
     ...
     * @param fixCupboard
     */
    private void handlerErrorWithSave(FixCupboardSaveBO fixCupboard) {
        DrawBaseHouseNew houseNew = new DrawBaseHouseNew();
        houseNew.setDatas(fixCupboard.getDatas());
        resHandlerErrorService.handlerErrorWithSave(houseNew);
    }
    ...
    private DrawBaseHouse getSaveHouseByValid(FixCupboardSaveBO fixCupboard) {
        ...
        // 已被修复，不需要重复修复
        if (BugFixStatus.FIX_SUCCEED.getStatus().equals(drawHouse.getBugfixStatus())) {
            throw new BusinessException(false, ResponseEnum.FIX_CUPBOARD_SUCCEED);
        }
     ...
     * @return
     */
    private DrawResHousePic getDrawHousePicResource(DrawBaseHouse drawHouse, DrawFileDataNew drawFile) {
     ...
     * @return
     */
    private DrawResHousePic addDrawHousePic(DrawBaseHouse drawHouse, DrawFileDataNew drawFile) {
        DrawResHousePic pic = new DrawResHousePic();
     ...
     * @return
     */
    private DrawResFile getDrawFileResource(DrawBaseHouse drawHouse, DrawFileDataNew drawFile) {
     ...
     * @return
     */
    private DrawResFile addDrawHouseFile(DrawBaseHouse drawHouse, DrawFileDataNew drawFile) {
    ...
    @Override
    @Transactional(propagation = Propagation.REQUIRED)
    public void submitV2(FixCupboardSubmitBO fixCupboard) {
        try {
            this.submit(fixCupboard);
        } catch (Exception e) {
            // 异常时，删除上传的资源文件
            DrawBaseHouseSubmitDTO dtoNew = new DrawBaseHouseSubmitDTO();
            dtoNew.setSpaceCommonDTOList(fixCupboard.getSpaceCommonDTOList());
            resHandlerErrorService.handlerErrorWithSubmit(dtoNew, null);

            throw e;
        } finally {
            // 清除其它多余的资源（厨房的排查在外）
            List<DrawSpaceCommonDTO> drawSpaceCommonDTOS = fixCupboard.getSpaceCommonDTOList();
            if (drawSpaceCommonDTOS != null && !drawSpaceCommonDTOS.isEmpty()) {
                // 找到除厨房的其它空间
                List<DrawSpaceCommonDTO> clearDrawSpaces = drawSpaceCommonDTOS.stream()
                        .filter(sc -> !DEFAULTS_KITCHEN_PREFIX.equals(sc.getSpaceCommonFunctionValueKey()))
                        .collect(Collectors.toList());

                // 开始清除
                if (clearDrawSpaces != null && !clearDrawSpaces.isEmpty()) {
                    DrawBaseHouseSubmitDTO dtoNew = new DrawBaseHouseSubmitDTO();
                    dtoNew.setSpaceCommonDTOList(clearDrawSpaces);
                    resHandlerErrorService.handlerErrorWithSubmit(dtoNew, null);
                    log.info("修复橱柜：清除其它不需要修复的空间的资源");
                }
            }
        }
    }
    ...
    public void submit(FixCupboardSubmitBO fixCupboard) {
        ...
        if (isFixCupboard) {
            // submit
            List<DrawSpaceBO> drawSpaceBOS = drawBaseHouseServiceV2.submitByFixCupboard(loginBO, fixCupboard, (s) -> !s.getSpaceCode().startsWith(DEFAULTS_CF_PREFIX));

            if (drawSpaceBOS != null && !drawSpaceBOS.isEmpty()) {
                // 发起申请审核
                DrawBaseHouse drawHouse = new DrawBaseHouse();
                drawHouse.setId(fixCupboard.getHouseId());
                drawHouse.setHouseStatus(DrawBaseHouseConstant.CHECK_APPLY.toString());
                // fix succeed
                drawHouse.setBugfixStatus(BugFixStatus.FIX_SUCCEED.getStatus());
                drawBaseHouseMapper.updateByPrimaryKeySelective(drawHouse);
    ...
    private boolean clearCupboardSpaceBySubmit(Long houseId) {
        ...
        // 请旧数据的资源文件
        // drawResFileService.clearDrawResFileResource(clearCupboards, null);
    ...
    public String callback(FixCupboardCallbackBO fixCupboard) {
        DrawBaseHouseCallbackDTO dtoNew = new DrawBaseHouseCallbackDTO();
        dtoNew.setStatus(fixCupboard.getStatus());
        dtoNew.setMessage(fixCupboard.getMessage());
        dtoNew.setTaskDetailId(fixCupboard.getTaskDetailId());
        dtoNew.setDrawDesignTempletDTO(fixCupboard.getDrawDesignTempletDTO());

        return drawBaseHouseServiceV2.callback(dtoNew, new ClearCupboardSpaceHandler(), new UpdateBugfixStatusCallback());
    }
...
//
//
//        DrawBakeTaskDetail subTask = drawBaseHouseServiceV2.getDrawBakeTaskDetailByValid(dtoNew);
//
//        // 任务被删除
...
//            log.warn("{}(taskDetailId)处理烘焙任务异常", fixCupboard.getTaskDetailId());
//            // failed
...
//    }
    ...
    @Override
    @Transactional
    public void addFixTaskToQueue(List<String> houseIds, boolean isReset, FixCupboardQuery fixQuery) {
        // 没有houseIds, 默认从数据库取1000条数据添加到队列中
        if (houseIds == null || houseIds.isEmpty()) {
            fixQuery.setSpaceCode(DEFAULTS_CF_PREFIX);
            fixQuery.setHouseStatus(DrawBaseHouseConstant.HOUSE_BAKE_SUCCESS.toString());
            fixQuery.setPageSize(fixQuery.getPageSize() == null ? 1000 : fixQuery.getPageSize());
            fixQuery.setGmtCreate(StringUtils.isEmpty(fixQuery.getGmtCreate()) ? DEFAULTS_GMT_CREATE : fixQuery.getGmtCreate());
            List<DrawBaseHouseBO> drawBaseHouseBOS = fixCupboardMapper.listFixCupboardHouse(fixQuery);

            // 获取数据
            houseIds = drawBaseHouseBOS.stream().map(h -> h.getId().toString()).collect(Collectors.toList());
        }
        ...
        if (isReset) {
            drawBaseHouseMapper.batchResetFixHouse(BugFixStatus.APPLY_FIX.getStatus(), houseIds);
            log.info("重置修复的户型，houseIds => {}", houseIds);
        }

        redisService.leftPushAllByQueue(RedisKeyConstant.HOUSE_DRAW_FIX_CUPBOARD_QUEUE.toString(), houseIds, null);

        log.info("添加户型到修复队列中，houseIds => {}", houseIds);
    }

    @Override
    public Long getFixQueueCount() {
        return redisService.getListCount(RedisKeyConstant.HOUSE_DRAW_FIX_CUPBOARD_QUEUE.toString());
    }
     ...
     * 清除旧数据
     */
    public class ClearCupboardSpaceHandler implements CleanDrawHouseHandler {
        @Override
        public Integer cleanOldDrawHouse(DrawBaseHouse drawHouse, DrawBakeTaskDetail subTask) {
            return clearCupboardSpaceByCallback(drawHouse, subTask.getTaskId());
        }
    }
     ...
     * 更新bugfixStatus
     */
    public class UpdateBugfixStatusCallback implements BakeTransformCallback {
        @Override
        public void apply(CallbackTransformBO transformBO) {
            // 更新bugfix_status
            DrawBaseHouse drawHouse = new DrawBaseHouse();
            drawHouse.setGmtModified(new Date());
            drawHouse.setId(transformBO.getDrawHouse().getId());
            drawHouse.setBugfixStatus(BugFixStatus.FIX_BAKE_SUCCEED.getStatus());
            drawBaseHouseMapper.updateByPrimaryKeySelective(drawHouse);
        }

        @Override
        public void apply(DrawBaseHouse drawHouse,
    ...
    private Integer clearCupboardSpaceByCallback(DrawBaseHouse drawHouse, Long taskId) {
        // 是否第一个空间烘焙
...
//        List<Long> emptySpaces = drawSpaceCommonService.getEmptyDealSpaceCommon(drawHouse.getBaseHouseId());
        ...
        // 空间
        Integer updateCount = drawSpaceCommonService.emptySpaceCommon(DrawBaseHouseConstant.IS_DELETED, emptySpaces);
        ...
        // 清除assetbundle资源文件、draw_res_file表相关的assetbundle文件
        List<Long> clearDrawCupboards = drawSpaceCommonMapper.getClearCupboardSpaceBySubmit(drawHouse.getId());
        if (clearDrawCupboards != null && !clearDrawCupboards.isEmpty()) {
            updateCount += drawResFileService.clearDrawResFileResource(clearDrawCupboards, 1);
        }
