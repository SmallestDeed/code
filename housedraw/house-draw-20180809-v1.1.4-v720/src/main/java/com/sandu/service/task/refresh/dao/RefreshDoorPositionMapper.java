...
public interface RefreshDoorPositionMapper {
    ...
    int insertBatch(List<DesignTempletJumpPositionRel> list);

    int updateBatch(List<Long> list);

    List<FilePathBO> getErrorFilePath();

    int selectInJumpPosition(DesignTempletJumpPositionRel positionRel);

    int updateBatchByUniqueId(List<DesignTempletJumpPositionRel> list);
}
