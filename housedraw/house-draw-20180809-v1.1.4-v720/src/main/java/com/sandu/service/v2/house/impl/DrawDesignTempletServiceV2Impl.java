...
public class DrawDesignTempletServiceV2Impl implements DrawFlowGeneralService {
    ...
    private DrawDesignTemplet getDrawDesignTemplet(DrawSpaceCommon drawSpaceCommon,
        ...
        // v1.1.1 添加图片验证
        // Utils.requireGreaterZero(drawSpaceCommon.getPicId(), "样板房截图不能为空");
        ...
        drawDesignTemplet.setDesignName(drawSpaceCommon.getSpaceName());
...
//        return DesignTempletStatusConstant.TESTING;
    ...
    public void handlerCallbackTransform(CallbackTransformBO transformBO) {
        Long spaceCommonId = transformBO.getDrawSpace().getSpaceCommonId();
        DrawDesignTemplet drawDesignTemplet = transformBO.getDrawDesignTemplet();
        ...
        // 处理样板房的配置文件、模型文件
        this.handlerTransformDesignTempletResource(drawDesignTemplet, transformBO.getBakeFiles());

        // 处理样板房图片
        drawDesignTempletService.handlerDesigmTempletPic(drawDesignTemplet, transformBO.getBakeFiles());
     ...
     * 处理样板房的配置文件、模型文件
     */
    private void handlerTransformDesignTempletResource(DrawDesignTemplet drawDesignTemplet, List<String> bakeFiles) {
        ...
        // 检索需要处理的文件（配置文件、pc模型文件）
        List<Integer> filesIds = Lists.newArrayList(drawDesignTemplet.getConfigFileId(), drawDesignTemplet.getPcModelU3dId());
        List<DrawResFile> drawResFiles = drawResFileService.listDrawResFileById(filesIds);
        if (drawResFiles == null || drawResFiles.isEmpty()) {
            throw new DrawBusinessException(false, ResponseEnum.DESIGN_TEMPLET_CONFIG_EMPTY);
        }

        Map<Long, DrawResFile> fileMaps = drawResFiles.stream().collect(Collectors.toMap(DrawResFile::getId, f -> f, (p, n) -> n));

        // 样板房配置文件
        DrawResFile drawResFileConfig = fileMaps.get(drawDesignTemplet.getConfigFileId().longValue());
        ResFile configFile = resFileService.createByDrawResFile(drawResFileConfig, SystemConfigEnum.DESIGNTEMPLET_CONFIGFILE_UPLOAD.getFileKey(), true, true);
        if (configFile == null || configFile.getId() == null) {
            throw new DrawBusinessException(false, ResponseEnum.DESIGN_TEMPLET_CONFIG_EMPTY);
        }
        bakeFiles.add(configFile.getFilePath());
        drawDesignTemplet.setConfigFileId(configFile.getId().intValue());

        // 样板房pc模型文件
        DrawResFile drawResFileModel = fileMaps.get(drawDesignTemplet.getPcModelU3dId().longValue());
        ResModel pcModelU3d = resModelService.createByDrawResFile(drawResFileModel, SystemConfigEnum.DESIGNTEMPLET_U3DMODEL_WINDOWSPC_UPLOAD.getFileKey(), true, true);
        if (pcModelU3d == null || pcModelU3d.getId() == null) {
            throw new DrawBusinessException(false, ResponseEnum.DESIGN_TEMPLET_U3DMODLE_EMPTY);
        }
        bakeFiles.add(pcModelU3d.getModelPath());
        drawDesignTemplet.setPcModelU3dId(pcModelU3d.getId().intValue());
    }
    ...
    private void handlerDesignTempletResourceCallback(DrawDesignTemplet drawDesignTemplet) {
        ...
        drawDesignTemplet.setPicId(null);
        drawDesignTemplet.setEffectPic(null);
        drawDesignTemplet.setEffectPlanIds(null);
