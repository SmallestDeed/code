...
public class DrawDesignTempletServiceImpl implements DrawDesignTempletService {
	...
	@Override
	public List<DrawDesignTemplet> findAllBySpaceCommonId2(Long drawSpaceCommonId) {
		if (drawSpaceCommonId == null) {
			return null;
		}

		return drawDesignTempletMapper.findAllBySpaceCommonId2(drawSpaceCommonId);
	}
...
//			}
	...
	public void handlerDesigmTempletPic(DrawDesignTemplet designTemplet) {
		this.handlerDesigmTempletPic(designTemplet, null);
	}
	...
	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public void handlerDesigmTempletPic(DrawDesignTemplet designTemplet, List<String> bakeFiles) {
		log.debug("同步样板房图片处理，designTempletId => {}", designTemplet.getId());

		Long drawEffectPicId = Long.valueOf(Objects.toString(designTemplet.getEffectPic(), "-1"));
		Long drawEffectPlanId = Long.valueOf(Objects.toString(designTemplet.getEffectPlanIds(), "-1"));

		List<Long> picIds = Lists.newArrayList(drawEffectPicId, drawEffectPlanId);
		List<DrawResHousePic> drawResHousePics = drawResHousePicService.listDrawHousePicByIds(picIds);
		if (drawResHousePics == null || drawResHousePics.isEmpty()) {
			// v1.1.1不加图片验证
//			throw new BusinessException(false, ResponseEnum.DESIGEN_TEMPELT_PIC_IS_EMPTY);
			log.warn("{}(designTempletId)样板房没有图片数据", designTemplet.getId());
			return;
		}

		Map<Integer, DrawResHousePic> picMaps = drawResHousePics.stream().collect(Collectors.toMap(DrawResHousePic::getId, f -> f, (p, n) -> n));

		// 样板房picId
		ResPic resPic = this.getDesignTempletPic(picMaps, drawEffectPicId.intValue(), false);
		if (bakeFiles != null) {
			bakeFiles.add(resPic.getPicPath());
		}
		designTemplet.setPicId(resPic.getId().intValue());

		// 样板房effectPic
		ResPic effectResPic = this.getDesignTempletPic(picMaps, drawEffectPicId.intValue(), true);
		if (bakeFiles != null) {
			bakeFiles.add(effectResPic.getPicPath());
		}
		designTemplet.setEffectPic(Objects.toString(effectResPic.getId(), ""));

		// 样板房effectPlanPic
		ResPic effectPlanResPic = this.getDesignTempletPic(picMaps, drawEffectPlanId.intValue(), true);
		if (bakeFiles != null) {
			bakeFiles.add(effectPlanResPic.getPicPath());
		}
		designTemplet.setEffectPlanIds(Objects.toString(effectPlanResPic.getId(), ""));
	}
	 ...
	 * @return
	 */
	private ResPic getDesignTempletPic(Map<Integer, DrawResHousePic> picMaps, Integer drawPicId, boolean isSmall) {
		DrawResHousePic pic = picMaps.get(drawPicId);
		if (pic == null || StringUtils.isEmpty(pic.getPicPath())) {
			throw new BusinessException(false, ResponseEnum.DESIGEN_TEMPELT_PIC_IS_EMPTY);
		}

		ResPic resPic = drawResHousePicService.copyFile(pic, SystemConfigEnum.DESIGN_TEMPLET_PIC_UPLOAD.getFileKey(), isSmall, true);
		if (resPic == null || resPic.getId() == null || resPic.getId() <= 0
				|| StringUtils.isEmpty(resPic.getPicPath())) {
			throw new BusinessException(false, ResponseEnum.PIC_COPY_FAILED);
		}
