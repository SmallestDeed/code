...
public class KCDrawVolumeRoomServiceImpl implements DrawVolumeRoomService {
    ...
    // 文件回收器，异常时处理
    private ThreadLocal<List<String>> fileRegeneracy;
    ...
    public void saveVolumeRoomOfErrorable(RequestBody requestBody, MultipartFile zipFile) {
        try {
            saveVolumeRoom(requestBody, zipFile);
        } catch (Exception e) {
            if (fileRegeneracy != null && fileRegeneracy.get() != null) {
                // 异常时，删除处理过的文件
                log.error("保存量房异常，删除处理过的文件.begin");
                FileUtils.delete(fileRegeneracy.get().toArray(new String[]{}));
                log.error("保存量房异常，删除处理过的文件.end");
            }
...
//        }
        ...
        VolumeRoomData volumeRoomData = JSON.parse(requestBody.getData(), VolumeRoomData.class);
     ...
     * @return
     */
    private BaseLiving getBaseLiving(DrawVolumeRoom drawVolumeRoom, SysUser sysUser) {
...
//        baseLivingMapper.getLivingByName(baseAreas.get(0).getAreaCode(), drawVolumeRoom.getHouseName());
    ...
    private DrawResLfFile handlerZipFile(MultipartFile file, DrawVolumeRoom drawVolumeRoom, String defaultsCreator) {
        if (!file.getOriginalFilename().matches(Regex.SUFFIX.getValue(FileType.ZIP))) {
            throw new BizException("上传的不是zip格式文件, file=" + file.getOriginalFilename());
        }
        ...
        return this.saveVolumeRoomFile(filePath, drawResLfFile, drawVolumeRoom, defaultsCreator, FileKey.KECHUANG_ZIP_FILE);
    }
     ...
     * @return
     */
    private Long handlerLfFile(File file, DrawVolumeRoom drawVolumeRoom, String defaultsCreator) {
        // create file
        String lfFilePath;
        ...
        // save
        DrawResLfFile lfFile = this.saveVolumeRoomFile(lfFilePath, drawResLfFile, drawVolumeRoom, defaultsCreator, FileKey.KECHUANG_LF_FILE);
     ...
     * @throws IOException
     */
    private InputStream getStream(ZipFile zipFile, String key) throws IOException {
        if (zipFile == null || key == null) {
            throw new BusinessException(false, ResponseEnum.ERROR);
        }
        ...
        while (entries.hasMoreElements() && (zipEntry = entries.nextElement()) != null) {
            ...
            if (zipEntry.getName().endsWith(fileSuffix)) {
                log.info("量房资源defaults value => {}", defStream);
    ...
    private DrawResLfFile saveVolumeRoomFile(String newFilePath, DrawResLfFile drawResLfFile,
                                             DrawVolumeRoom drawVolumeRoom, String defaultsCreator, FileKey fileKey) {
        if (drawResLfFile == null || drawResLfFile.getId() == null) {
            drawResLfFile =  new DrawResLfFile();
            // file name
            drawResLfFile.setFilePath(newFilePath);
            String fileName = UploadUtils.getFileName(newFilePath);
            ...
            // 删除原先旧的文件
            FileUtils.delete(drawResLfFile.getFilePath());
     ...
     * @return
     */
    private Long handlerHousePic(File file, DrawVolumeRoom drawVolumeRoom,
        ...
        if (drawResHousePic == null || drawResHousePic.getId() <= 0) {
            ...
            // 删除原先旧的文件
            FileUtils.delete(drawResHousePic.getPicPath());
    ...
    private String createFile(InputStream stream, FileType fileType) {
        ...
        // 回收异常时需要处理的文件
        this.recovery(volumeFileDir + fileName);
     ...
     * @param file
     */
    private void recovery(String file) {
        if (fileRegeneracy == null) {
            fileRegeneracy = new ThreadLocal<>();
        }

        if (!StringUtils.isEmpty(file)) {
            if (fileRegeneracy.get() == null) {
                fileRegeneracy.set(Lists.newArrayList(file));
            } else {
                fileRegeneracy.get().add(file);
            }
        }
    }
}
