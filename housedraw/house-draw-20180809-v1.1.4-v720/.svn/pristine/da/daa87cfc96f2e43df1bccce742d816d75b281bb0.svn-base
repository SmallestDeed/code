...
public class UploadController extends BaseController {
    ...
    @Autowired
    private ResHandlerErrorService resHandlerErrorService;
    ...
    public ReturnData upload(MultipartHttpServletRequest files, String drawUploadBOS,
                             HttpServletRequest request, String uploadProperty) {
        ...
        try {
            ...
            // 批量处理删除文件时，某个文件处理失败，删除已处理成功的资源文件
            resHandlerErrorService.handlerErrorWithUpload(responseFileDatas);
            throw e;
        } finally {
            log.info("#################### 处理上传资源耗时：{}(ms) ####################", System.currentTimeMillis() - startup);
        }
