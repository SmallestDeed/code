...
import com.sandu.api.v2.house.bo.*;
...
public class DrawDesignTempletProductServiceV2Impl implements DrawFlowGeneralService {
    ...
    private void handlerChangedSoftProduct(DrawDesignTempletProduct drawDesignTempletProduct, DrawBaseProductDTO drawBaseProductDTO) {
        ...
        log.debug("处理拉伸的软装白膜产品, {}(productId)", drawBaseProductDTO.getProductId());
        ...
        // 设置copy那份资源来的产品id
        newProduct.setOriginProductId(drawBaseProductDTO.getProductId());
        ...
        // copy 一份拉伸过的产品的属性
        this.handlerChangedProductAttr(newProduct);
    }
     ...
     * @param newProduct
     */
    private void handlerChangedProductAttr(BaseProduct newProduct) {
        List<ProductAttribute> proAttrs = productAttributeService.listProductAttrByProductId(newProduct.getOriginProductId());
        proAttrs.forEach(attr -> attr.setProductId(newProduct.getId().intValue()));
        log.debug("处理拉伸的软装白膜产品属性, {}(originProductId)", newProduct.getOriginProductId());
    ...
    public void handlerCallbackTransform(CallbackTransformBO transformBO) {
        ...
        Long designTempletId = transformBO.getDrawDesignTemplet().getDesignTempletId();
        List<DrawDesignTempletProduct> drawDesignTempletProducts = transformBO.getDrawDesignTempletProducts();
...
//            }
    ...
    private void handlerDrawDesignTempletProductCallback(List<DrawDesignTempletProduct> drawDesignTempletProducts) {
        ...
        if (drawDesignTempletProducts.size() <= MysqlExecutor.BATCH.UPDATE.getCount()) {
        ...
        for (DrawDesignTempletProduct dtp : drawDesignTempletProducts) {
            updates.add(dtp);
            if (updates.size() < MysqlExecutor.BATCH.UPDATE.getCount()) {
                continue;
            }
