...
public class ResModelServiceImpl implements ResModelService {
	...
	public Long createByDrawResFile(DrawResFile drawResFile, String fileKey, boolean needEncrypt) {
		return createByDrawResFile(drawResFile, fileKey, needEncrypt, false).getId();
	}

	@Override
	@Transactional(propagation = Propagation.REQUIRED)
	public ResModel createByDrawResFile(DrawResFile drawResFile, String fileKey, boolean isEncrypt, boolean isAADir) {
		if (drawResFile == null) {
...
//		String targetPath = Utils.getValue(fileKey);
		// v1.1.1 复制到正式表,目录以 AA打头,所以需要加多一个AA目录的配置项,原来都是放到AA2目录下
