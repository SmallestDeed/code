...
public class SysUserFeedbackManageController extends BaseController {
     ...
     * 问题反馈消息类型
     */
    private static  final Integer TEMPLATE_TYPE_FEEDBACK_REPLY=6;
    ...
    public ResponseEnvelope feedbackReply(@Valid @RequestBody SysFeedbackReply reply,BindingResult validResult) {
        ...
        try{
            ...
            if (result>0){
                //随选网、小程序发送模板消息
                if (feedback.getPlatformId()==16||feedback.getPlatformId()==14){
                    logger.error("反馈问题答复成功，开始发送模板消息");
                    this.sendFeedbackTemplateMsg(feedback);
                    logger.error("反馈问题答复发送模板消息结束");
                }
                //添加系统消息
     ...
     * @param feedback 反馈
     */
    private void sendFeedbackTemplateMsg(SysUserFeedback feedback){
        SysUser user = sysUserService.get(feedback.getUserId());
        DateFormat format=new SimpleDateFormat("yyyy-MM-dd mm:HH:ss");
        String feedbackTimeStr = format.format(feedback.getGmtCreate());
        Map templateData = this.buildFeedbackReplyTempalteData(feedbackTimeStr, feedback.getFeedbackContent(), feedback.getReplyContent());
        logger.error("反馈问题答复成功，调用异步方法发送模板消息");
        asyncCallService.sendTemplateMsg(user,TEMPLATE_TYPE_FEEDBACK_REPLY,templateData,feedback.getId());
    }


    /**
     * 构造反馈消息模板内容
     * @param feedbackTimeStr 反馈时间
     * @param feedbackContent 反馈内容
     * @param replyContent    答复内容
     * @return map
     */
    private Map buildFeedbackReplyTempalteData(String feedbackTimeStr, String feedbackContent,String replyContent){
        ...
        keywordTempMap.put("value", feedbackTimeStr);
        keywordsMap.put("keyword1", keywordTempMap);

        keywordTempMap = new HashMap<String, String>();
        keywordTempMap.put("value", feedbackContent);
        keywordsMap.put("keyword2", keywordTempMap);

        keywordTempMap = new HashMap<String, String>();
        keywordTempMap.put("value", replyContent);
