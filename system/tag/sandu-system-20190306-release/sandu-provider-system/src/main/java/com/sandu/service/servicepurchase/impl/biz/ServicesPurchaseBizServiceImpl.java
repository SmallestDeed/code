...
public class ServicesPurchaseBizServiceImpl implements ServicesPurchaseBizService {
	...
	private void activeAccount(String account, ServicesAccountRel servicesAccountRel) {
		...
		//同步用户到期时间
		sysUserManageService.dealUserFailureTimeAndValidTime(servicesAccountRel.getUserId(), null,temRel.getEffectiveBegin(),temRel.getEffectiveEnd());
	}
	...
	public ServicesPurchaseRecord initServiceOrder(ServiceRecordInitBO bo) {
		...
		if (BUSSINESS_TYPE_0.equals(bo.getBusinessType())) {
			...
			if (account.getEffectiveBegin() == null && !PURCHASE_SOURCE_2.equals(bo.getPurchaseSource())) {
				//三度后台购买、续费无此校验
	...
	public boolean syncServiceAccount(String orderNum, String recordStatus) {
		...
		//套餐升级续费
		ServicesAccountRel oldAccount;
		if (BUSSINESS_TYPE_1.equals(record.getBusinessType()) || BUSSINESS_TYPE_4.equals(record.getBusinessType())) {
			oldAccount = servicesAccountRelService.getAccountByUserId(Integer.valueOf(record.getUserid()));
			...
			//计算套餐使用时长
			if (Objects.nonNull(curAccount.getEffectiveEnd())){
				log.info("同步套餐到期时间到用户表-----Start : userId:{},EffectiveEnd:{}", oldAccount.getUserId(), curAccount.getEffectiveEnd());
				boolean result = sysUserManageService.dealUserFailureTimeAndValidTime(oldAccount.getUserId(), new Date().compareTo(oldAccount.getEffectiveEnd()) >= 0 ? oldAccount.getEffectiveBegin() : new Date(),oldAccount.getEffectiveBegin(),curAccount.getEffectiveEnd());
	...
	private long computeOldPackageUseTime(ServicesAccountRel oldAccount) {
		//判断账号是否过期
		if (new Date().compareTo(oldAccount.getEffectiveEnd()) >= 0){
			//账号过期
			return 0;
		}
		return oldAccount.getEffectiveEnd().getTime() - oldAccount.getEffectiveBegin().getTime();
	}
	...
	private ServicesAccountRel insertUserAccountForAlter(ServicesPurchaseRecord record, ServicesAccountRel oldAccount) {
		...
		// 激活账户，升级处理账户有效期（三度后台存在未激活账户更换套餐的操作）
		if (Integer.parseInt(oldAccount.getStatus()) == 1) {
			dealWithNewAccountEffective(record, oldAccount, curAccount);
		}
	...
	private void dealWithNewAccountEffective(ServicesPurchaseRecord record, ServicesAccountRel oldAccount, ServicesAccountRel curAccount) {
		//更新作用时间,不然再次更换套餐有问题
...
//        }
	...
	public List<ServicesRoleRel> handlerCompanyUserPackageInfo(Long servicesId, SysUser sysUser,Integer priceId) {
		ServicesPurchaseRecord add = new ServicesPurchaseRecord();
			...
			roleList = servicesRoleRelService.getByServiceId(info.getId());
			if (roleList == null || roleList.size() == 0) {
				throw new RuntimeException("该套餐没有配置权限功能!!!");
			}
	...
	public ServicesAccountRel renewServiceForInner(InnerServiceRenew renew) {
		...
		//同步用户套餐 时长
		if (Objects.nonNull(account.getEffectiveEnd())){
			log.info("同步套餐到期时间到用户表-----Start : userId:{},EffectiveEnd:{}", oldAccount.getUserId(), account.getEffectiveEnd());
			boolean result = sysUserManageService.dealUserFailureTimeAndValidTime(oldAccount.getUserId(), null,account.getEffectiveBegin(),account.getEffectiveEnd());
			log.info("同步套餐到期时间到用户表-----END,reslut:{}", result);
		}
