...
import com.sandu.api.banner.input.*;
 ...
 */
@Api(tags = "miniBanner", description = "企业小程序首页广告")
@RestController
@RequestMapping("/v1/banner/manage/miniprogram")
public class MiniProgramBannerController  extends BaseController{
    private final static Logger logger = LoggerFactory.getLogger(MiniProgramBannerController.class);
    @Autowired
    ...
    @PostMapping("/pic/add")
    public ResponseEnvelope addBannerAd(@RequestPart("file") MultipartFile file, String skipPath, Integer type, Integer companyId){
        try{
            if (type == null || (type != 2 && type != 3) || (type == 3 && companyId == null)) {
                return new ResponseEnvelope(false, "参数错误");
            }
            //获取登录用户
            LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
            logger.error("当前登录用户:{}",loginUser);
            if (null!=loginUser.getBusinessAdministrationId()){
                ...
                if(null!=baseCompany){
                    ...
                    if (result>0){
                        ...
                        switch (type){
                            case 2:
                                isExist.setCode("company:home:page:top");
                                break;
                            case 3:
                                isExist.setCode("shop:home:page:top");
                        }
                        ...
                        if (position==null){
                            ...
                            switch (type){
                                case 2:
                                    positionAdd.setPositionCode("company:home:page:top");
                                    positionAdd.setPositionName("企业小程序首页Banner");
                                    break;
                                case 3:
                                    positionAdd.setPositionCode("shop:home:page:top");
                                    positionAdd.setPositionName("装企小程序首页Banner");
                            }
                            //1:平台--选装网,同城联盟;2:企业--企业小程序;3.店铺
                            positionAdd.setPositionType(type);
                        ...
                        if (type == 2) {
                            ...
                            if (shopId == null){
                                return new ResponseEnvelope(false,"还没创建企业店铺!!!");
                            }
                            bannerAdAdd.setRefModelId(shopId);
                            int count = baseBannerAdService.getBannerCountByModelPosition(positionId, shopId);
                            bannerAdAdd.setSequence(count + 1);
                        }
    ...
    public ResponseEnvelope updaterBannerSkipPath(@RequestBody MiniProgramBannerVO bannerVO){
        try{
            ...
            LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
            /**
             * add by qtq 对带有url参数的进行encode编码
             */
            String encodeUrl = this.encodeUrl(bannerVO.getSkipPath());
            BaseBannerAdUpdate update=new BaseBannerAdUpdate();
            update.setBannerId(bannerVO.getBannerAdId());
            update.setSkipPath(Strings.nullToEmpty(encodeUrl));
    ...
    @ApiOperation(value = "修改轮播图名称", response = MiniProgramBannerVO.class)
    @PutMapping("/name/update")
    public ResponseEnvelope updaterBannerName(@RequestBody MiniProgramBannerVO bannerVO){
        try{
            if (bannerVO.getBannerAdId() == null|| bannerVO.getName() == null){
                return new ResponseEnvelope(false,"参数异常");
            }
            //获取登录用户
            LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
            BaseBannerAdUpdate update=new BaseBannerAdUpdate();
            update.setBannerId(bannerVO.getBannerAdId());
            update.setBannerName(Strings.nullToEmpty(bannerVO.getName()));
            logger.error("修改广告跳转路径：修改baseBannerAd的跳转路径入参{}",update);
            int result = baseBannerAdService.updateBanner(update, loginUser);
            logger.error("修改广告跳转路径：修改baseBannerAd的跳转路径结果{}",result);
            if (result>0){
                return new ResponseEnvelope(true,"修改名称成功",bannerVO);
            }
            return new ResponseEnvelope(false,"修改名称失败");
        }catch (Exception e){
            logger.error("系统异常",e);
            return new ResponseEnvelope(false,"系统异常");
        }
    ...
    @ApiImplicitParam(name = "positionCode",value = "位置编码(类似：company:home:page:top)",paramType = "query",dataType = "String",required = true)
    @GetMapping("/get/list")
    public ResponseEnvelope getHomeBannerList(@RequestParam("positionCode") String positionCode){
        try{
            ...
            //获取登录用户
            LoginUser loginUser=LoginContext.getLoginUser(LoginUser.class);
            logger.info("查询首页广告列表：登录用户:{}",loginUser);
           /* LoginUser loginUser=new LoginUser();
            loginUser.setLoginName("dd");
            loginUser.setBusinessAdministrationId(18L);
    ...
    @ApiOperation(value = "查询banner列表", response = MiniProgramBannerVO.class)
    @GetMapping("/shop/bannerList")
    public ResponseEnvelope getBannerList(String positionCode, Integer type, Integer companyId){
        try{
            if (positionCode==null || type == null){
                return new ResponseEnvelope(false,"必传参数不全");
            }
            BaseBannerPositionIsExist isExist=new BaseBannerPositionIsExist();
            isExist.setCode(positionCode);
            ...
            if (position!=null){
                Integer shopId = baseCompanyService.getMainCompanyShopId(companyId);
                if (shopId != null) {
                    List<MiniProgramBannerVO> bannerList = baseBannerManageService.getBannerList(shopId, position.getId());
                    logger.info("查询首页广告列表：查询结果：{}", bannerList);
                    if (bannerList != null && bannerList.size() > 0) {
                        return new ResponseEnvelope(true, "列表查询成功", bannerList);
                    }
    ...
    @ApiOperation(value = "设置banner图的发布状态", response = ResponseEnvelope.class)
    @PostMapping("/pic/state")
    public ResponseEnvelope setBannerState(Integer bannerAdId, Integer state){
        if (bannerAdId == null || state == null) {
            return new ResponseEnvelope(false, "参数错误");
        }
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        if (loginUser == null) {
        ...
        int num = baseBannerAdService.setBannerState(bannerAdId, state, loginUser);
        if (num == 0) {
    ...
    @ApiOperation(value = "设置banner图的排序状态", response = ResponseEnvelope.class)
    @PostMapping("/pic/setSequence")
    public ResponseEnvelope setBannerSequence(Integer bannerAdId, Integer sequence){
        if (bannerAdId == null || sequence == null) {
            return new ResponseEnvelope(false, "参数错误");
        }
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        if (loginUser == null) {
        ...
        List<BaseBannerAd> bannerList = baseBannerAdService.setBannerSequence(bannerAdId, sequence, loginUser);
        if (bannerList == null || bannerList.size() <= 0) {
            return new ResponseEnvelope(false, "修改失败");
        }
        return new ResponseEnvelope(true, "修改成功", bannerList);
    }

    @ApiOperation(value = "新增合作品牌", response = ResponseEnvelope.class)
    @PostMapping("/brandPic/add")
    public ResponseEnvelope addFriendBrandBannerAd(@RequestPart("file") MultipartFile file, Integer companyId){
        try{
            ...
            if (null!=loginUser.getBusinessAdministrationId()){
                ...
                if(null!=baseCompany){
                    ...
                    if (result>0){
                        pic.setId(result);
                        //2. 判断首页顶部广告位置信息是否存在
                        Integer positionId = this.getPositionId("shop:home:page:brandPic",
                                "装企小程序合作品牌图片",
                                3,
                                loginUser);
                        //4. 添加广告信息(base_banner_ad)
                        int bannerId = this.addBaseBanner(shopId, positionId, result, "", "", loginUser);
                        //完成新增将MiniProgramBannerVO返回
                        MiniProgramBannerVO bannerVO=new MiniProgramBannerVO();
                        bannerVO.setImageId(result);
                        bannerVO.setImagePath(pic.getPicPath());
    ...
    @ApiOperation(value = "新增企业资质", response = ResponseEnvelope.class)
    @PostMapping("/qualification/add")
    public ResponseEnvelope addCompanyQualificationBannerAd(@RequestPart("file") MultipartFile file, Integer companyId){
        try{
            if (file == null ||  companyId == null) {
                return new ResponseEnvelope(false, "参数错误");
            }
            ...
            if (null!=loginUser.getBusinessAdministrationId()){
                ...
                if(null!=baseCompany){
                    ...
                    if (result>0){
                        pic.setId(result);
                        //2. 判断首页顶部广告位置信息是否存在
                        Integer positionId = this.getPositionId("shop:home:page:qualification",
                                "装企小程序企业资质图片",
                                3,
                                loginUser);
                        //4. 添加广告信息(base_banner_ad)
    ...
    private Integer getPositionId(String positionCode, String positionName, Integer type, LoginUser loginUser){
        BaseBannerPositionIsExist isExist=new BaseBannerPositionIsExist();
        isExist.setCode(positionCode);
        //1:平台--选装网,同城联盟;2:企业--企业小程序;3.店铺
        isExist.setType(type);
        ...
        if (position==null){
            BaseBannerPositionAdd positionAdd=new BaseBannerPositionAdd();
            positionAdd.setPositionCode(positionCode);
            positionAdd.setPositionName(positionName);
            //1:平台--选装网,同城联盟;2:企业--企业小程序;3.店铺
            positionAdd.setPositionType(type);
            positionId= positionService.addBannerPosition(positionAdd, loginUser);
            logger.info("新增广告信息：位置信息不否存在时新增位置信息结果{}",position);
            if (0==positionId){
                throw new RuntimeException("添加position信息失败");
            }
        }else {
            positionId = position.getId();
        }
    ...
    private Integer addBaseBanner(Integer refModelId, Integer positionId, Integer resBannerPicId, String bannerName, String url, LoginUser loginUser){
        BaseBannerAdAdd bannerAdAdd=new BaseBannerAdAdd();
        //用户为经销商时，refModelId为查询到的企业的Pid；不是经销商用户时，refModelId就是登录用户的BusinessAdministrationId
        bannerAdAdd.setRefModelId(refModelId);
        bannerAdAdd.setRefModelId(refModelId);
        int count = baseBannerAdService.getBannerCountByModelPosition(positionId, refModelId);
        bannerAdAdd.setSequence(count + 1);
        bannerAdAdd.setPositionId(positionId);
        bannerAdAdd.setResBannerPicId(resBannerPicId);
        bannerAdAdd.setBannerName(bannerName);
        bannerAdAdd.setStatus(1);
        bannerAdAdd.setUrl(Strings.nullToEmpty(url));
        logger.info("新增广告信息：向baseBannerAd添加数据入参{}",bannerAdAdd);
        int bannerId = baseBannerAdService.addBaseBanner(bannerAdAdd, loginUser);
        logger.info("新增广告信息：向baseBannerAd添加数据结果{}",bannerId);
        if (bannerId==0){
            throw new RuntimeException("添加BannerAd失败");
        }
        return bannerId;
    }
