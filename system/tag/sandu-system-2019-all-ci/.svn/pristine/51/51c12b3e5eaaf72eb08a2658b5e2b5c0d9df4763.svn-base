...
public class ServicesPurchaseBizServiceImpl implements ServicesPurchaseBizService {
	...
	private void activeAccount(String account, ServicesAccountRel servicesAccountRel) {
		...
		//同步用户到期时间
		sysUserManageService.dealUserFailureTimeAndValidTime(servicesAccountRel.getUserId(), 0,temRel.getEffectiveEnd());
	}
	...
	public boolean syncServiceAccount(String orderNum, String recordStatus) {
		...
		if (BUSSINESS_TYPE_1.equals(record.getBusinessType()) || BUSSINESS_TYPE_4.equals(record.getBusinessType())) {
			...
			//计算套餐使用时长
			long useTime = computeOldPackageUseTime(oldAccount);
			log.info("同步套餐到期时间到用户表-----Start : userId:{},EffectiveEnd:{}", oldAccount.getUserId(), curAccount.getEffectiveEnd());
			boolean result = sysUserManageService.dealUserFailureTimeAndValidTime(oldAccount.getUserId(), useTime,curAccount.getEffectiveEnd());
	...
	private long computeOldPackageUseTime(ServicesAccountRel oldAccount) {
		Date now = new Date();
		long l = oldAccount.getEffectiveEnd().getTime() - now.getTime();
		if (l < 0){
			//套餐已经过期
			return 0;
		}
...
//		long useTime = computeOldPackageUseTime(oldAccount);
		...
		boolean result = sysUserManageService.dealUserFailureTimeAndValidTime(oldAccount.getUserId(), 0,account.getEffectiveEnd());
		log.info("同步套餐到期时间到用户表-----END,reslut:{}", result);
