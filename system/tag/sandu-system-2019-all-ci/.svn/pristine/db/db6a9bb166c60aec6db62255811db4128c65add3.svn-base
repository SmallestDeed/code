...
public interface ServicesAccountRelMapper {
	...
	int batchInsertServiceAccount(@Param("accountList")List<ServicesAccountRel> accountList);
	...
	List<ServicesAccountRel> selectByServicesRecordId(Integer servicesId);

	List<ServicesAccountRel> queryByOptionForPage(ServiceInnerUserQuery query);
}