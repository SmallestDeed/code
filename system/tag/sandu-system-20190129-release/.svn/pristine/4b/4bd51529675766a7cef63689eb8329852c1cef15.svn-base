...
public class ExcelUtil {
            ...
            for (int j = 0, c = 0, r = 0; j < fields.length; j++) {
                ...
                if (isAnnotation) {
                    try {
                        ...
                        c++;
                        // 设置行样式
                        boolean isRowAnnotation = field.isAnnotationPresent(com.sandu.api.annotation.HSSFRow.class);
                        if (isRowAnnotation) {
                            com.sandu.api.annotation.HSSFRow hssfRow =
                                    field.getAnnotation(com.sandu.api.annotation.HSSFRow.class);
    ...
    private static List<HSSFCellStyle> getColStyles(HSSFWorkbook book, Field[] fields) {
        ...
        for (int j = 0; j < fields.length; j++) {
            ...
            if (colFlag) {
                ...
                com.sandu.api.annotation.HSSFFont font = column.font();
    ...
    private static List<HSSFCellStyle> getRowStyles(HSSFWorkbook book, Field[] fields) {
        ...
        for (int j = 0; j < fields.length; j++) {
            ...
            boolean colFlag = field.isAnnotationPresent(HSSFColumn.class);
            if (colFlag) {
                boolean rowFlag = field.isAnnotationPresent(com.sandu.api.annotation.HSSFRow.class);
                if (rowFlag) {
                    HSSFCellStyle hssfStyle = book.createCellStyle();
                    com.sandu.api.annotation.HSSFRow row = field.getAnnotation(com.sandu.api.annotation.HSSFRow.class);
                    com.sandu.api.annotation.HSSFFont  font = row.font();
                    HSSFFont hssfFont = book.createFont();
                    hssfFont.setBoldweight(font.bold());
                    hssfFont.setCharSet(font.charset());
                    hssfFont.setFontHeightInPoints(font.height());
