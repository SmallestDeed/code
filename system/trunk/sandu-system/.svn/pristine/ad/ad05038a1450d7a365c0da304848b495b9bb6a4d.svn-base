...
public class CompanyShopServiceImpl implements CompanyShopService {
    ...
    public int deleteShop(Integer shopId, LoginUser loginUser) {
        ...
        if (!loginUser.getId().equals(companyShop.getUserId())) {
            throw new SystemException("您不是店铺的创建者，不能删除店铺");
        }
        // 删除对应的工程案例
...
//            detailsVO.setIntroducedFilePath(resFile != null ? resFile.getFilePath() : "");
    ...
    @Override
    public List<CompanyShop> selectShopByUserIds(Set<Long> userIds) {
        return companyShopDao.selectShopByUserIds(userIds);
    }

    @Override
    public void delCompanyShopByUserIds(Set<Long> userIds, String loginName) {
        companyShopDao.delCompanyShopByUserIds(userIds, loginName);
    }

    @Override
    public List<CompanyShop> listShopByCompanyId(Long companyId) {
        if (companyId == null) {
            logger.error("参数companyId 不能为空！");
            return null;
        }

        return companyShopDao.listShopByCompanyId(companyId);
    }

    @Override
    public Integer deleteShopById(List<Integer> listShopId, String loginName) {
        if (listShopId == null || listShopId.isEmpty()) {
            logger.error("参数 listShopId 不能为空！");
            return null;
        }

        return companyShopDao.deleteShopById(listShopId, loginName);
    }
}
