...
import com.nork.common.async.*;
import com.nork.common.constant.SystemCommonConstant;
import com.nork.onekeydesign.cache.*;
...
import com.nork.onekeydesign.cache.DesignPlanCommentCacher;
import com.nork.onekeydesign.cache.DesignPlanLikeCacher;
import com.nork.onekeydesign.cache.DesignPlanProductCacher;
import com.nork.onekeydesign.cache.UsedProductsCacher;
import com.nork.onekeydesign.common.RecommendedDecorateState;
import com.nork.onekeydesign.dao.DesignPlanMapper;
import com.nork.onekeydesign.dao.DesignPlanProductMapper;
import com.nork.onekeydesign.exception.IntelligenceDecorationException;
import com.nork.onekeydesign.model.constant.ContextType;
import com.nork.onekeydesign.model.constant.DesignPlanBusinessTypeConstant;
...
import com.nork.onekeydesign.model.search.DesignPlanLikeSearch;
import com.nork.onekeydesign.model.search.DesignPlanSearch;
import com.nork.onekeydesign.model.unity.BindPointDataEx;
import com.nork.onekeydesign.model.unity.ItemExt;
import com.nork.onekeydesign.model.unity.RoomConfig;
import com.nork.onekeydesign.model.unity.RoomExt;
import com.nork.onekeydesign.model.unity.Root;
import com.nork.onekeydesign.service.DesignPlanCommentService;
import com.nork.onekeydesign.service.DesignPlanLikeService;
...
import com.nork.onekeydesign.service.DesignPlanRenderService;
...
public class DesignPlanServiceImpl implements DesignPlanService {
	...
	private static Logger logger = Logger
			.getLogger(DesignPlanServiceImpl.class);
	
	private static final String LOGPREFIX = "[设计方案模块]:";
...
//			}
			...
			if(newProduct.getProductCode() != null && !newProduct.getProductCode().startsWith("baimo_")){
				...
				/*UsedProfiles usedProfiles = new UsedProfiles();
				usedProfiles.setUsedId(id);
				usedProfiles.setTextContent(context);//配置文件文本信息
				int filesId = usedProfilesService.add(usedProfiles);*/
...
//				logger.info("usedProfiles add:filesId=" + filesId);
				...
				//已使用配置内容存储路径
//				String usedConfigPath = Utils.getValue("onekeydesign.designPlan.usedConfig.upload.path","/onekeydesign/designPlan/[code]/usedConfig/");
//				usedConfigPath = usedConfigPath.replace("[code]",desPlan.getPlanCode());
				/*String usedConfigPath = Utils.getValue("onekeydesign.designPlan.usedConfig.upload.path","/AA/e_userlogs/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/usedConfig/");*/
				String usedConfigPath = Utils.getValueByFileKey(ResProperties.RES, ResProperties.DESIGNPLAN_USEDCONFIG_FILEKEY, "/AA/e_userlogs/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/usedConfig/");
				...
				resUsedProducts.setFileKey("onekeydesign.designPlan.usedConfig");
...
//				usedConfigPath = usedConfigPath.replace("[code]",desPlan.getPlanCode());
    ...
    //                      String occu = Utils.getValue("onekeydesign.designPlan.maxrender.designPlan.upload.path", "/MaxRender/");
	...
	public boolean updatePlanProductByConfig(String context,Integer planId, String contextType){
		if(ContextType.CONTEXT_TYPE_OF_STRUCTURE.equals(contextType)){
			...
			try {
				...
				logger.error("配置文件 context ===>"+context);
			} catch (Exception e1) {
...
//		modelU3dnewId = this.copyFile(modelU3dId == null ? "" : modelU3dId.toString(), "model","onekeydesign.designPlan.u3dmodel", null, request, designPlan.getPlanCode());
//		designPlan.setModelU3dId(modelU3dnewId == null ? new Integer(-1) : modelU3dnewId);
		...
		Integer resFileId = this.planCopyFile(designTemplet.getConfigFileId() == null ? "" : designTemplet.getConfigFileId().toString(), "file","onekeydesign.designPlan.u3dconfig", null, request, designPlan.getPlanCode());
	...
	public Integer planCopyFile(String resId, String resType, String fileKey, Integer bussniess, HttpServletRequest request,
		...
		if (!StringUtils.isEmpty(resId)) {
			...
			if (!StringUtils.isEmpty(resFilePath)) {
				...
				String newPath = Utils.getPropertyName("config/res", fileKey + ".upload.path", "/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/u3dconfig/").trim();
...
//				newPath = newPath.replace("[code]", code);
	...
	* @throws
	...
	public Map<String, String> saveRenderPic(HttpServletRequest request) throws Exception {
		...
		/*应用根节点目录*/
		/*渲染图存放相对路径*/
		//String storePath = Utils.getValue("onekeydesign.designPlan.render.upload.path", "/onekeydesign/designPlan/render/").trim();
		String storePath = Utils.getPropertyName("config/res", "design.designPlan.render.upload.path", "/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/render/").trim();
	...
	public Object getDesignPlanInfoForOneKey(Integer designPlanId,Integer newFlag,String houseId,String livingId,String residentialUnitsName,Boolean isRelease, LoginUser loginUser, String mediaType, String needOrNoCache) {
		...
		if (designProductList != null && designProductList.size() > 0) {
			...
			for (DesignPlanProduct planProduct : designProductList) {
				logger.debug("productId= " + planProduct.getProductId() + ";posName = " + planProduct.getPosName());
...
//		resRenderPic.setFileKey(ResProperties.DESIGNPLAN_RENDER_PIC_FILEKEY);
	...
	public Integer saveRenderPicOf720(DesignPlan designPlan,
		...
		String storePath = Utils.getPropertyName("config/res",
				...
				"/onekeydesign/designPlan/render/").trim();
	...
	public DesignPlan createDesignPlanForOneKey(DesignPlan designPlan,DesignPlanRecommended designPlanRecommended,DesignTemplet designTemplet,
...
//			response.setContentType("text/html;charset=utf-8");
			...
			if (flag) {
				...
				try {
					String smallFileName = Utils
							...
							+ Utils.getCurrentDateTime(Utils.DATETIMESSS)
							+ fileName.substring(fileName.indexOf("."));
					String storePath = Utils.getPropertyName("config/res", map.get(FileUploadUtils.UPLOADPATHTKEY)+"", "/onekeydesign/designPlan/[code]/render/").replace("[code]", code)
                    + "/small/" + smallFileName;
					storePath = Utils.replaceDate(storePath);
	...
	public List<DesignPlan> getMyDesignPlanList(LoginUser loginUser, DesignPlan designPlan, int total, String mediaType) {
		...
		if (total > 0) {
			logger.info("=====================total:" + total + ".数据响应行数大于0.");
	...
	public Integer saveRenderPicOfVideo(DesignPlan designPlan,
	    ...
	    String storePath = Utils.getPropertyName("config/res",
                ...
                "/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/render/video/").trim();
        String storePath2 = Utils.getPropertyName("config/res",
                "design.designPlan.render.video.cover.upload.path",
                "/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/render/video/cover/").trim();
		/*String uploadRoot =Tools.getRootPath(storePath,"D:\\nork\\Resource");*/
	...
	public int saveRoam720(ResRenderPic resRenderPic, String storePath, String storeRealPath, MultipartFile mf,
		...
		try{
			/* 生成渲染图片 */
			/*storeRealPath = FileUploadUtils.UPLOAD_ROOT + Utils.getPropertyName("config/res",ResProperties.DESIGNPLAN_RENDER_ROAM_PIC_FILEKEY,"/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/render/roam/config/");*/
			storeRealPath = Utils.getAbsolutePath(Utils.getPropertyName("config/res",ResProperties.DESIGNPLAN_RENDER_ROAM_PIC_FILEKEY,"/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/render/roam/config/"), null);
			storeRealPath = Utils.replaceDate(storeRealPath);
			FileUtils.copyInputStreamToFile(mf.getInputStream(),
					new File(storeRealPath, bigFileName));
		}catch(IOException e){
    ...
    /* (non-Javadoc)    
     * @see com.nork.onekeydesign.service.DesignPlanService#getTempDesignPalnId(int, int)
     */
    @Override
	...
	public Integer createPlanByConfig(
		...
		logger.error("日志planId="+planId+"|filePath="+filePath);
	...
	@Override
	public ResponseEnvelope<DesignPlan> unityUpdate(DesignPlan designPlan,Integer designPlanId,Integer planProductId,Integer productId,String materialPicId,String context,String splitTexturesChoose,HttpServletRequest request,String msgId){
		boolean flag = false;
		try {
			flag = designPlanService.updateDesignPlan(designPlan, designPlanId, planProductId, productId,
		...
		if (flag) {
			UnitUpdateParameter parameter = new UnitUpdateParameter(designPlanId, context, planProductId, productId,
	...
	public Map<String,String> saveSpellingFlowerFile(String spellingFlower,String spellingFlowerProduct,DesignPlan designPlan) {
		...
		String fileKey = "onekeydesign.designPlan.spellingFlowerFile.upload.path";
		if(StringUtils.isEmpty(filePath)) { //如果拼花路径为空  代表第1次增加拼花信息
			String defaultPath = "/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/onekeydesign/designPlan/spellingFlowerFile/"; //如果res配置文件中没有该filekey,用的默认地址
	...
	public Map<String,String> transformAndCopyPlan(TransformAndCopyPlanModel model) {	
		...
		if (designPlanRecommended.getConfigFileId() != null) {
			Integer resFileId = this.planCopyFileFromResDesignScene(designPlanRecommended.getConfigFileId().toString(), 
				"onekeydesign.designPlan.u3dconfig","/AA/d_userdesign/[YYYY]/[MM]/[dd]/[HH]/onekeydesign/designPlan/u3dconfig/",
				null, designPlanRecommended.getPlanCode(),loginUser);
...
//						+ resourcesName.substring(resourcesName.indexOf("."));
	...
	@Override
	public Boolean batchAddPlanProduct(Integer planId, List<DesignProductResult> unmatchProductList ,LoginUser loginUser, Integer isDeleted, Integer opType) {
		Long startTime = System.currentTimeMillis();
		try{
			List<DesignPlanProduct>planProductList=new ArrayList<DesignPlanProduct>();
			// 存储组合对应的唯一值
			for (DesignProductResult dtp : unmatchProductList) {
				DesignPlanProduct planProduct = new DesignPlanProduct();
				planProduct.setPlanId(planId);
				/* 拷贝挂节点*/
				planProduct.setPosIndexPath(dtp.getPosIndexPath());
				...
				planProduct.setInitProductId(dtp.getProductId());
				planProduct.setBindParentProductId(dtp.getBindParentProductId());
				planProduct.setSameProductTypeIndex(Integer.parseInt(dtp.getSameProductTypeIndex()));
				/*天花、地面、地面结构拆分*/
				planProduct.setIsStandard(dtp.getIsStandard());
				planProduct.setCenter(dtp.getCenter());
				planProduct.setRegionMark(dtp.getRegionMark());
				planProduct.setStyleId(dtp.getStyleId());
				planProduct.setMeasureCode(dtp.getMeasureCode());
				planProduct.setDescribeInfo(dtp.getMeasureCode());
				planProduct.setProductIndex(dtp.getProductIndex());
				...
				sysSave(planProduct, loginUser);
				planProduct.setIsDeleted(isDeleted == null ? 0 : isDeleted);
				planProductList.add(planProduct);
			}
			if (opType.intValue() == DesignPlanConstants.AUTO_RENDER.intValue()) {
				optimizePlanService.batchAdd(planProductList);
			} else {
				designPlanProductService.batchAdd(planProductList);
			}
			Long endTime = System.currentTimeMillis();
			...
			e.printStackTrace();
			logger.warn("方法名：batchAddPlanProduct："+e);
			return false;
		}
	}

	@Override
	public Integer copyFromDesignPlanTemplateInfo(DesignPlanTemplate designPlanTemplate, Integer opType, Integer userId) throws IntelligenceDecorationException {
		String exceptionLogPrefix = "拷贝一键装修副本到设计方案失败";
		
		// 参数验证 ->start
		if(designPlanTemplate == null) {
			String log = "params error: designPlanTemplate = null";
		...
		// copy DesignPlanTemplate ->DesignPlan ->start
		Integer designPlanId = this.copyFromDesignPlanTemplate(designPlanTemplate, opType, userId);
		// copy DesignPlanTemplate ->DesignPlan ->end
		
		// copy ResDesignOnekeyTemplate to ResDesign->start
		Integer resDesignId = resDesignService.copyFromResDesignOnekeyTemplate(designPlanTemplate.getConfigFileId(), designPlanId, opType);
		// copy ResDesignOnekeyTemplate to ResDesign->end
		
		// copy DesignPlanTemplateProduct ->DesignPlanProduct ->start
		designPlanProductService.copyFromDesignPlanTemplateProduct(designPlanTemplate.getId() == null ? null : designPlanTemplate.getId().intValue(), designPlanId, opType);
		// copy DesignPlanTemplateProduct ->DesignPlanProduct ->end
		
		// 回填部分designPlan信息 ->start
		this.updateSomeInfo(designPlanId, resDesignId, opType);
		// 回填部分designPlan信息 ->end
		
		return designPlanId;
	}

	/**
	 * 回填某些design_plan/onekey_decoration_design_plan中的信息
	 * 
	 * @author huangsongbo
	 * @param designPlanId
	 * @param resDesignId
	 ...
	 */
	private void updateSomeInfo(Integer designPlanId, Integer resDesignId, Integer opType) throws IntelligenceDecorationException {
		String exceptionLogPrefix = "更新设计方案数据失败";
		
		// 参数验证 ->start
		if(designPlanId == null) {
			String log = "designPlanId = null";
			logger.error(LOGPREFIX + log);
			throw new IntelligenceDecorationException(exceptionLogPrefix + "(" + log + ")");
		}
		if(resDesignId == null) {
			String log = "resDesignId == null";
			logger.error(LOGPREFIX + log);
			throw new IntelligenceDecorationException(exceptionLogPrefix + "(" + log + ")");
		}
		if(opType == null) {
	 ...
	 * @throws IntelligenceDecorationException
	 */
	private Integer copyFromDesignPlanTemplate(DesignPlanTemplate designPlanTemplate, Integer opType, Integer userId) throws IntelligenceDecorationException {
		String exceptionLogPrefix = "拷贝副本到设计方案失败";
		...
		DesignPlan designPlan = EntityCopyUtils.copyData(designPlanTemplate, DesignPlan.class);
		designPlan.setDesignTemplateId(designPlanTemplate.getDesignTemplateId() == null ? null : Integer.valueOf(designPlanTemplate.getDesignTemplateId()));
		designPlan.setId(null);
		designPlan.setUserId(userId);
		// generate a new designPlan code
		designPlan.setPlanCode(designPlanService.getNewCode(designPlan.getPlanCode()));
		return this.add(designPlan, opType);
	}

	@Override
	public Integer add(DesignPlan designPlan, Integer opType) throws IntelligenceDecorationException {
		String exceptionLogPrefix = "新增设计方案失败";
		
		// 参数验证 ->start
		if(designPlan == null) {
			String log = "params error: designPlan = null";
			logger.error(LOGPREFIX + log);
			throw new IntelligenceDecorationException(exceptionLogPrefix + "(" + log + ")");
		}
		if(opType == null) {
		...
		if(DesignPlanConstants.USER_RENDER.intValue() == opType.intValue()) {
			// 通用版本一键装修
			return this.add(designPlan);
		}else {
			// 渲染机一键装修
			return optimizePlanService.add(designPlan);
		}
	}
	...
	public UnityDesignPlan getUnityDesignPlan(
		...
		String exceptionLogPrefix = "获取设计方案信息失败";
		...
		if(loginUser == null) {
			String log = "params error: loginUser = null";
			logger.error(LOGPREFIX + log);
			throw new IntelligenceDecorationException(exceptionLogPrefix + "(" + log + ")");
		}
		if(designTempletId == null) {
			String log = "params error: designTempletId = null";
			logger.error(LOGPREFIX + log);
			throw new IntelligenceDecorationException(exceptionLogPrefix + "(" + log + ")");
		}
		// 参数验证 ->end
		...
		if(DesignPlanConstants.AUTO_RENDER != opType) {
			...
			ResponseEnvelope<UnityDesignPlan> responseEnvelopeInfo = (ResponseEnvelope<UnityDesignPlan>) 
				designPlanService.getDesignPlanInfoForOneKey(designPlanId, null, null, null, null, null, loginUser, loginUser.getMediaType(), Constants.NO_CACHE);
			if(responseEnvelopeInfo.isSuccess()) {
				...
				throw new IntelligenceDecorationException("进入设计方案失败");
			}
			unityDesignPlan = designPlanService.wrapperData(designPlanId, unityDesignPlan);
			...
			ResponseEnvelope<UnityDesignPlan> responseEnvelopeInfo = (ResponseEnvelope<UnityDesignPlan>) optimizePlanService
					.getDesignPlanInfoForOneKey(designPlanId, null, null, null, null, false, loginUser, loginUser.getMediaType());
			if (responseEnvelopeInfo.isSuccess()) {
				unityDesignPlan = (UnityDesignPlan) responseEnvelopeInfo.getObj();
			} else {
			...
			unityDesignPlan = optimizePlanService.wrapperData(designPlanId, unityDesignPlan);
			unityDesignPlan.setDesignTempletId(designTempletId);
		}
	...
	@Override
	public String getNewCode(String planCode) throws IntelligenceDecorationException {
		String exceptionLogPrefix = "生成新编码失败";
		
		// 参数验证 ->start
		if(StringUtils.isEmpty(planCode)) {
			String log = "params error: StringUtils.isEmpty(planCode) = true";
			logger.error(LOGPREFIX + log);
			throw new IntelligenceDecorationException(exceptionLogPrefix + "(" + log + ")");
		}
		// 参数验证 ->end
		...
		String newCode = "";
		if(planCode.indexOf("_") != -1) {
			newCode = planCode.substring(0, planCode.lastIndexOf("_"));
			newCode += "_" + Utils.generateRandomDigitString(6);
		}else {
			// append "_${6位随机数}"
			newCode = planCode + "_" + Utils.generateRandomDigitString(6);
		}
		return newCode;
	}
	 ...
	 * 获取一键装修 方案资源信息
	 * @author chenqiang
	 * @param unityDesignPlanRecommended  一键装修匹配后 资源信息
	 * @return
	 * @date 2018/10/31 0031 16:42
	 */
	@Override
	public Object getDesignRecommendedInfoByMath(UnityDesignPlanRecommended planRecommended,LoginUser loginUser, String msgId){

		//获取推荐方案配置文件、方案模型文件、空间模型文件
		UnityDesignPlanRecommended unityDesignPlanRecommended = designPlanRecommendedService.getDesignPlanRecommendedInfoByMath(planRecommended);

		//设置方案资源拼花信息
		this.setSpellingFlowerProduct(unityDesignPlanRecommended);

		//获取匹配到产品
		List<UnityPlanProductRecommended> unityPlanProductRecommendedList = planRecommended.getUnityPlanProductRecommendedList();

		//获取匹配到的产品idList
		List<Integer> productIdList = unityPlanProductRecommendedList.stream().map(UnityPlanProductRecommended::getProductIdToInt).collect(Collectors.toList());

		//推荐方案产品
		designPlanRecommendedProductServiceV2.getUnityPlanProductRecommendedListByMatch(productIdList,unityPlanProductRecommendedList);

		//设置产品资源详情信息
		this.setUnityPlanProductRecommended(unityPlanProductRecommendedList);
		...
		unityDesignPlanRecommended.setUnityPlanProductRecommendedList(unityPlanProductRecommendedList);

		return new  ResponseEnvelope<UnityDesignPlanRecommended>(true,unityDesignPlanRecommended,"获取推荐方案资源成功",msgId);
	}
	 ...
	 */
	@Override
	public Object getDesignRecommendedInfo(String planId,String planRecommendedId,String designTempletId,LoginUser loginUser,String msgId){

		Integer designPlanRecommendeId = Integer.parseInt(planId);

		//获取推荐方案配置文件、方案模型文件、空间模型文件
		UnityDesignPlanRecommended unityDesignPlanRecommended = designPlanRecommendedService.getDesignPlanRecommendedInfo(designPlanRecommendeId);
		unityDesignPlanRecommended.setDesignTempletId(designTempletId);
		...
		//设置产品资源详情信息
		this.setUnityPlanProductRecommended(designPlanRecommendedProductList);
	 ...
	 * @param msgId
	 * @return
	 * @date 2018/12/14 0014 18:04
	 */
	@Override
	public Object getProductSourceInfo(List<Integer> idList,LoginUser loginUser,String msgId){

		//获取产品基础数据
		List<UnityPlanProductRecommended> unityPlanProductRecommendedList = designPlanRecommendedProductServiceV2.getUnityPlanProductRecommendedList(idList);
		if(null == unityPlanProductRecommendedList || 0 >= unityPlanProductRecommendedList.size())
			throw new RuntimeException("产品数据不存在");

		//设置产品资源详情信息
		this.setUnityPlanProductRecommended(unityPlanProductRecommendedList);

		return new ResponseEnvelope<UnityPlanProductRecommended>(true,"获取产品资源信息成功",msgId,null,unityPlanProductRecommendedList.size(),unityPlanProductRecommendedList);
	}


	/**
	 * 设置方案资源拼花信息
	 * @param unityDesignPlanRecommended 资源对象类
	 */
	private void setSpellingFlowerProduct(UnityDesignPlanRecommended unityDesignPlanRecommended){

		if(null == unityDesignPlanRecommended)
			return;
		...
		if(StringUtils.isNotBlank(unityDesignPlanRecommended.getSpellingFlowerProduct())){
			...
			List<BaseProduct> productList = baseProductServiceV2.getBatchData(productIDList);
	...
	/**
	 * 设置产品资源详情信息
	 * @param unityPlanProductRecommendedList 产品基础信息集合
	 */
	private void setUnityPlanProductRecommended(List<UnityPlanProductRecommended> unityPlanProductRecommendedList){

		if(null == unityPlanProductRecommendedList || 0 >= unityPlanProductRecommendedList.size())
			return;

		for (UnityPlanProductRecommended unityPlanProductRecommended : unityPlanProductRecommendedList) {
	...
	public List<WaterjetInfoReturnDTO> getWaterjetInfoReturnDTOListByJson(String waterjetInfo) {
		...
		if(StringUtils.isEmpty(waterjetInfo)) {
			logger.error("StringUtils.isEmpty(waterjetInfo) = true");
			return null;
		}
		// 参数验证 ->end
	...
	private List<WaterjetInfoReturnDTO> waterJetInfoDTOToWaterjetInfoReturnDTO(
		...
		if(Lists.isEmpty(waterJetInfoDTOList)) {
			logger.error("Lists.isEmpty(waterJetInfoDTOList) = true");
			return null;
		}
		// 参数验证 ->end
	...
	private List<SplitTextureDTO> waterJetTextureInfoDTOToSplitTextureDTO(
		...
		if(Lists.isEmpty(waterJetTextureInfoList)) {
			logger.error("Lists.isEmpty(waterJetTextureInfoList) = true");
			return null;
		}
		// 参数验证 ->end
	...
	private SplitTextureDTO waterJetTextureInfoDTOToSplitTextureDTO(WaterJetTextureInfoDTO waterJetTextureInfoDTO) {
		...
		if(waterJetTextureInfoDTO == null) {
			logger.error("waterJetTextureInfoDTO = null");
			return null;
		}
		// 参数验证 ->end
