...
import com.nork.onekeydesign.cache.DesignTempletCacher;
import com.nork.onekeydesign.dao.DesignTempletMapper;
import com.nork.onekeydesign.model.DesignSpaceResult;
import com.nork.onekeydesign.model.DesignTemplet;
import com.nork.onekeydesign.model.DesignTempletPutawayState;
import com.nork.onekeydesign.model.DesignTempletResult;
...
public class DesignTempletServiceImpl implements DesignTempletService {
	...
	public void backfill(DesignTemplet designTemplet) {
		...
		if (designTemplet.getPicId() != null && designTemplet.getPicId() > 0) {
			String picKey = "onekeydesign.designTemplet.pic";
		...
		if (StringUtils.isNotBlank(effectPicIdStr)) {
			...
			for (int i = 0; i < effectPicIds.length; i++) {
				if (StringUtils.isNotBlank(effectPicIds[i])) {
					String picKey = "onekeydesign.designTemplet.piclist";
		...
		if (designTemplet.getModel3dId() != null
			...
			String modelKey = "onekeydesign.designTemplet.3dmodel";
		...
		if (designTemplet.getConfigFileId() != null
			...
			String fileKey = "onekeydesign.designTemplet.configFile";
		...
		if (StringUtils.isNotBlank(effectPlanIds)) {
			...
			for (int i = 0; i < effectPlanIdList.length; i++) {
				if (StringUtils.isNotBlank(effectPlanIdList[i])) {
					String picKey = "onekeydesign.designTemplet.effectPlan";
					resPicService.backFillResPic(designId,
...
//			}
	...
	@Override
	public DesignTemplet getForOneKey(Integer id) {
		if(id == null) {
			return null;
		}
		return designTempletMapper.getForOneKey(id);
	}
