...
public class MobileSearchDesignPlanProductServiceImpl implements MobileSearchDesignPlanProductService {
	 ...
	 Logger.getLogger(MobileSearchDesignPlanProductServiceImpl.class);
	...
	public Object getDesignPlanProductList(MobileRenderingModel model) {
		...
		if ("onekeydesign".equals(msgId)) {
			...
			if (designPlanRenderScene != null) {
				userId = designPlanRenderScene.getUserId();
			}
		}
		if ("oneKey".equals(msgId)) {
			// DesignPlan DesignPlan = optimizePlanMapper.selectByPrimaryKey(planId);
			DesignPlan designPlan = optimizePlanMapper.selectByPrimaryKeyPlan(planId);
		...
		if (userId.intValue() != 0) {
			...
			/*
			 * List<ProductsCostType> list = panoramaService.getProductsCost(planId, userId,
			 * userType);
			 */
