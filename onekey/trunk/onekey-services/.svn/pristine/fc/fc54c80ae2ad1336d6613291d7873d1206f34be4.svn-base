...
import com.nork.onekeydesign.dao.DesignPlanProductDao;
...
import com.nork.onekeydesign.dao.OptimizePlanMapper;
...
import com.nork.onekeydesign.service.DesignPlanRecommendedProductServiceV2;
...
public class DesignPlanProductServiceImpl implements DesignPlanProductService {
	...
	private static final String LOGPREFIX = "[设计方案产品模块]:";
...
//			String fileContext = FileUploadUtils.getFileContext(filePath);
			...
			for (JsonProduct jsonProduct : productCodes) {
				...
				String planPosName = jsonProduct.getPlanPosName();
				String initPosName = jsonProduct.getInitPosName();
				//获取产品对象
				if( StringUtils.isNotEmpty(jsonProduct.getItemCode()) ){
					baseProduct = baseProductService.findOneByCode(jsonProduct.getItemCode());
					//如果查不到，是否能查到该产品已被逻辑删除
					if (baseProduct == null || baseProduct.getId() == null) {
						baseProduct = baseProductService.findContainDeleteProductByCode(jsonProduct.getItemCode());
					}
				}else{
					continue;
				}
				//获取初始化产品对象，方案需要存储初始化属性字段值
				if (StringUtils.isNotEmpty(jsonProduct.getInitProductCode())) {
					for (DesignProductResult templetProductResult : templetProductList) {
						if (jsonProduct.getInitProductCode().equals(templetProductResult.getProductCode())
								&& initPosName.equals(templetProductResult.getPosName())) {
					...
					for (DesignProductResult designProductResult : planProductList) {
						if (jsonProduct.getItemCode().equals(designProductResult.getProductCode())
								...
								&& planPosName.equals(designProductResult.getPosName())) {
				...
				if (baseProduct != null && baseProduct.getId() > 0) {
					...
					if( baseProduct.getProductCode().startsWith("baimo_") && initProduct !=null ){
						...
						//获取方案产品属性或样板房白膜产品属性
						for (DesignProductResult designProductResult : planProductList) {
							if (planPosName != null && planPosName.equals(designProductResult.getPosName())) {
						...
						if (StringUtils.isNotEmpty(jsonProduct.getStructCode())) {
							...
							if( list != null && list.size() > 0 ){
								...
								setPlanProductInfo(designProduct,initProduct == null? new DesignProductResult() : initProduct );
						...
						//获取方案planProduct多维材质信息
						if (planProduct != null && initProduct != null) {
				...
				if (Lists.isNotEmpty(bindPointDataExList) && initProduct != null) {
	...
	private void setPlanProductInfo(DesignPlanProduct designProduct, DesignProductResult initProduct){
		...
		designProduct.setGroupProductUniqueId(initProduct.getGroupProductUniqueId());
	}
...
//			autoUpdateStructureRelation(designTempletId);
	...
	public Integer createByPlanProductInfo(PlanProductInfo planProductInfo, Integer planId, String username, Integer opType,UnityDesignPlanRecommended unityDesignPlanRecommended) {
		...
		if(null == unityDesignPlanRecommended && (planId == null || planId < 0)) {
		...
		if(null != unityDesignPlanRecommended){
			...
			this.setUnityPlanProductRecommended(designPlanProduct,unityDesignPlanRecommended);
	...
	private void setUnityPlanProductRecommended(DesignPlanProduct  designPlanProduct,UnityDesignPlanRecommended unityDesignPlanRecommended){

		List<UnityPlanProductRecommended> unityPlanProductRecommendedList = unityDesignPlanRecommended.getUnityPlanProductRecommendedList();
		if(null == unityPlanProductRecommendedList){
			unityPlanProductRecommendedList = new ArrayList<>();
		}

		UnityPlanProductRecommended unityPlanProductRecommended = new UnityPlanProductRecommended();
		unityPlanProductRecommended.setProductId(designPlanProduct.getProductId() + "");				//产品id
		unityPlanProductRecommended.setInitProductId(designPlanProduct.getInitProductId() + "");		//产品白模id
		unityPlanProductRecommended.setModelProductId(designPlanProduct.getModelProductId());
		unityPlanProductRecommendedList.add(unityPlanProductRecommended);
...
//		List<BaseProduct> baseProductList = baseProductMapper.getByDesignPlanId(designPlanId);
    ...
    /* (non-Javadoc)    
     * @see com.nork.onekeydesign.service.DesignPlanProductService#getDesignPlanProductList(com.nork.onekeydesign.model.DesignPlanProduct)
     */
    @Override
			...
			for (SplitTextureInfoDTO productTexture : productSplitTextureList) {
				for (SplitTextureInfoDTO designgProductTexture : designProductSplitTextureList) {
					...
					if (productTexture.getKey().equals(designgProductTexture.getKey())) {
						if(productTexture.getTextureIds().indexOf(designgProductTexture.getDefaultId().toString()) != -1){
							return designTexturesInfo;
						}
					}
				}
...
//		}
	...
	public void copyFromDesignPlanTemplateProduct(Integer designPlanTemplateId, Integer designPlanId, Integer opType) throws IntelligenceDecorationException {
		String exceptionLogPrefix = "副本产品数据转换为设计方案产品数据失败";
		
		// 参数验证 ->start
		if(designPlanTemplateId == null) {
			String log = "params error: designPlanTemplateId = null";
		...
		// copy ->start
		List<DesignPlanTemplateProduct> designPlanTemplateProductList = designPlanTemplateProductService.getListByPlanId(designPlanTemplateId);
		for(DesignPlanTemplateProduct designPlanTemplateProduct : designPlanTemplateProductList) {
			DesignPlanProduct designPlanProduct = EntityCopyUtils.copyData(designPlanTemplateProduct, DesignPlanProduct.class);
			designPlanProduct.setId(null);
			designPlanProduct.setPlanId(designPlanId);
			this.add(designPlanProduct, opType);
		}
		// copy ->end
	}

	@Override
	public Integer add(DesignPlanProduct designPlanProduct, Integer opType) throws IntelligenceDecorationException {
		String exceptionLogPrefix = "添加设计方案产品失败";
		
		// 参数验证 ->start
		if(designPlanProduct == null) {
			String log = "params error: designPlanProduct = null";
			logger.error(LOGPREFIX + log);
			throw new IntelligenceDecorationException(exceptionLogPrefix + "(" + log + ")");
		}
		if(opType == null) {
		...
		if(DesignPlanConstants.USER_RENDER.intValue() == opType.intValue()) {
			// 通用版本一键装修
			return this.add(designPlanProduct);
		}else {
			// 渲染机一键装修
			return optimizePlanService.addPlanProduct(designPlanProduct);
		}
	}
