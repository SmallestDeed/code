...
import com.nork.onekeydesign.model.vo.UnityPlanProductresTexture;
...
import com.nork.onekeydesign.model.AutoCarryBaimoProducts;
import com.nork.onekeydesign.model.DesignPlan;
import com.nork.onekeydesign.model.DesignPlanProduct;
import com.nork.onekeydesign.model.DesignTempletPutawayState;
...
//		productResult.setProductOrdering(100);
		...
		try {
			...
			if (SplitTextureList != null && SplitTextureList.size() >= 1) {
				for (SplitTextureInfoDTO splitTextureInfoDTO : SplitTextureList) {
					...
					logger.error("------baseProduct的SplitTextureInfo信息格式错误,SplitTextureInfo:" + splitTexturesInfo);
				}
				return map;
			}
	...
	/**
	 * 通过产品code查找产品 包含逻辑删除的数据
	 * @author xiaoxc
	 * @param productCode
	 * @return
	 */
	@Override
	public BaseProduct findContainDeleteProductByCode (String productCode){
		BaseProductSearch baseProductSearch = new BaseProductSearch();
		baseProductSearch.setStart(new Integer(0));
		baseProductSearch.setLimit(new Integer(1));
		baseProductSearch.setSchProductCode_(productCode);
		List<BaseProduct> list = this.getPaginatedList(baseProductSearch);
		if (list != null && list.size() > 0) {
			return list.get(0);
		}
...
//				}
	...
	public BaseProductSearch getBaseProductSearchByPlanProductInfo(PlanProductInfo planProductInfo) {
		...
		// add by huangsongbo 2018.7.9 ->start
		if(StringUtils.isNotEmpty(planProductInfo.getProductCode())) {
			baseProductSearch.setProductCode(planProductInfo.getProductCode());
		}
		// add by huangsongbo 2018.7.9 ->end
	...
	@Override
	public List<BaseProduct> selectProductEasyFromDB(BaseProductSearch baseProductSearch) {
		// *参数验证 ->start
	...
	@Override
	public List<BaseProduct> selectProductEasy(ProductSearchForOneKeyDTO productSearchForOneKeyDTO) throws ElasticSearchException {
		// *参数验证 ->start
		...
		// update by huangsongbo 2018.8.9:只搜已发布的产品 ->start
		...
		// update by huangsongbo 2018.8.9:只搜已发布的产品 ->end
				
		productSearchForOneKeyDTO.setIsDeleted(0);
		// 添加搜索条件 ->end
		
		List<ProductIndexMappingData> productIndexMappingData = productSearchOpenService.searchProduct(productSearchForOneKeyDTO);
		return this.transformToBaseProductList(productIndexMappingData);
	}
	
	private List<BaseProduct> transformToBaseProductList(List<ProductIndexMappingData> productIndexMappingDataList) {
		// 参数验证 ->start
		if(Lists.isEmpty(productIndexMappingDataList)) {
			logger.info("Lists.isEmpty(productIndexMappingDataList) = true");
			return null;
		}
		// 参数验证 ->end
		...
		for(ProductIndexMappingData productIndexMappingData : productIndexMappingDataList) {
			BaseProduct baseProduct = new BaseProduct();
			baseProduct.setId(productIndexMappingData.getId());
			SysDictionary sysDictionary = sysDictionaryService.findOneByTypeAndValueAndValue("productType", productIndexMappingData.getProductTypeValue(), productIndexMappingData.getProductTypeSmallValue());
			baseProduct.setBigTypeValueKey(sysDictionary.getType());
			baseProduct.setSmallTypeValueKey(sysDictionary.getValuekey());
			baseProduct.setProductCode(productIndexMappingData.getProductCode());
			baseProduct.setProductLength(productIndexMappingData.getProductLength() + "");
			baseProduct.setProductWidth(productIndexMappingData.getProductWidth() + "");
			baseProduct.setProductHeight(productIndexMappingData.getProductHeight() + "");
			baseProduct.setFullPaveLength(productIndexMappingData.getProductFullPaveLength() + "");
			baseProduct.setMeasureCode(productIndexMappingData.getProductMeasurementCode());
			baseProduct.setSplitTexturesInfo(productIndexMappingData.getSplitTexturesInfo());
			baseProduct.setStyleId(productIndexMappingData.getStyleModelId());
			baseProductList.add(baseProduct);
		}
		return baseProductList;
	}
	...
	public Map<getSmallTypeValueListBySmallTypeValueKeyMapKeyEnum, Object> getSmallTypeValueListBySmallTypeValueKey(String smallTypeValueKey) {
		...
		Map<getSmallTypeValueListBySmallTypeValueKeyMapKeyEnum, Object> returnMap = new HashMap<BaseProductServiceImpl.getSmallTypeValueListBySmallTypeValueKeyMapKeyEnum, Object>();
	...
	@Override
	public ProductSearchForOneKeyDTO transformToProductSearchForOneKeyDTO(PlanProductInfo planProductInfo) {
		...
		if(StringUtils.isNotEmpty(bigTypeValueKey)) {
			...
			if(sysDictionary == null) {
				...
				productSearchForOneKeyDTO.setProductTypeValue(sysDictionary.getValue() + "");
			}
		}
		// 产品小类
		if(StringUtils.isNotEmpty(smallTypeValuekey)) {
			...
			if(sysDictionary == null) {
				...
				productSearchForOneKeyDTO.setProductSmallTypeValue(sysDictionary.getValue());
			}
		}
		// 产品小类排序
		if(StringUtils.isNotBlank(planProductInfo.getOrderSmallTypeValueKey())) {
			...
			if(sysDictionary == null) {
				...
				productSearchForOneKeyDTO.setOrderSmallTypeValue(sysDictionary.getValue());
			}
		}
		// 产品长度
		if(StringUtils.isNotEmpty(planProductInfo.getProductLength())) {
			productSearchForOneKeyDTO.setProductLength(planProductInfo.getProductLength());
		}
		// 产品宽度
		if(StringUtils.isNotEmpty(planProductInfo.getProductWidth())) {
			productSearchForOneKeyDTO.setProductWidth(planProductInfo.getProductWidth());
		}
		// 产品高度
		if(StringUtils.isNotEmpty(planProductInfo.getProductHeight())) {
			productSearchForOneKeyDTO.setProductHeight(planProductInfo.getProductHeight());
		}
		// 产品长度范围start
		if(planProductInfo.getProductLengthStart() != null) {
			try {
				productSearchForOneKeyDTO.setProductLengthStartInteger(planProductInfo.getProductLengthStart());
			}catch (Exception e) {
		...
		if(planProductInfo.getProductLengthEnd() != null) {
			try {
				productSearchForOneKeyDTO.setProductLengthEndInteger(planProductInfo.getProductLengthEnd());
			}catch (Exception e) {
		...
		// 尺寸代码
		if(StringUtils.isNotEmpty(planProductInfo.getMeasureCode())) {
			productSearchForOneKeyDTO.setMeasureCode(planProductInfo.getMeasureCode());
		}
		// 款式
		if(planProductInfo.getStyleId() != null) {
			productSearchForOneKeyDTO.setStyleId(planProductInfo.getStyleId());
		}
		// 款式排序
		if(planProductInfo.getOrderStyleId() != null) {
			productSearchForOneKeyDTO.setOrderStyleId(planProductInfo.getOrderStyleId());
		}
		// 白膜id(定制产品对应的白膜id)
		if(StringUtils.isNotEmpty(planProductInfo.getBmIds())) {
			productSearchForOneKeyDTO.setBmIds(planProductInfo.getBmIds());
		}
		if(StringUtils.isNotEmpty(planProductInfo.getWallType())) {
			productSearchForOneKeyDTO.setWallType(planProductInfo.getWallType());
		}
		// 布局标识
		/*if(planProductInfo.getProductSmallpoxIdentify() != null && 0 != planProductInfo.getProductSmallpoxIdentify().intValue()) {*/
		if(planProductInfo.getProductSmallpoxIdentify() != null && !StringUtils.equals("0", planProductInfo.getProductSmallpoxIdentify())) {
			productSearchForOneKeyDTO.setProductSmallpoxIdentify(planProductInfo.getProductSmallpoxIdentify());
		}
		// 布局标识list
		if(Lists.isNotEmpty(planProductInfo.getProductSmallpoxIdentifyList())) {
			productSearchForOneKeyDTO.setIdentifyList(planProductInfo.getProductSmallpoxIdentifyList());
		}
		// 产品过滤属性
		if(Lists.isNotEmpty(planProductInfo.getProductFilterPropList())) {
			/*productSearchForOneKeyDTO.setProductFilterPropList(planProductInfo.getProductFilterPropList());*/
			Map<String, String> productFilterAttributeMap = new HashMap<String, String>();
			planProductInfo.getProductFilterPropList().forEach(item -> productFilterAttributeMap.put(item.getKey(), item.getValue() + ""));
			productSearchForOneKeyDTO.setProductFilterAttributeMap(productFilterAttributeMap);
		}
		// 是否是背景墙
		if(planProductInfo.isBeijing()) {
			productSearchForOneKeyDTO.setBeijing(planProductInfo.isBeijing());
		}
		// 产品品牌排序
		if(planProductInfo.getOrderBrandId() == null) {
			productSearchForOneKeyDTO.setOrderBrandId(planProductInfo.getOrderBrandId());
		}
		// 产品型号排序
		if(StringUtils.isNotEmpty(planProductInfo.getOrderProductModelNumber())) {
			productSearchForOneKeyDTO.setOrderProductModelNumber(planProductInfo.getOrderProductModelNumber());
		}
		// 产品系列
		if(planProductInfo.getSeriesId() != null && 0 != planProductInfo.getSeriesId().intValue()) {
			productSearchForOneKeyDTO.setSeriesId(planProductInfo.getSeriesId());
		}else{
			productSearchForOneKeyDTO.setSeriesId(null);
		}
		// 产品小类list(排除)
		if(Lists.isNotEmpty(planProductInfo.getExcludeSmallTypeValueList())) {
			productSearchForOneKeyDTO.setExcludeSmallTypeValueList(planProductInfo.getExcludeSmallTypeValueList());
		}
		// 小类list
		if(Lists.isNotEmpty(planProductInfo.getSmallTypeValueList())) {
			productSearchForOneKeyDTO.setSmallTypeValueListForShowAll(planProductInfo.getSmallTypeValueList());
			productSearchForOneKeyDTO.setProductSmallTypeValue(null);
			productSearchForOneKeyDTO.setExcludeSmallTypeValueList(null);
		}
		// 布局标识排序
		if(StringUtils.isNotEmpty(planProductInfo.getOrderProductSmallpoxIdentify())) {
			productSearchForOneKeyDTO.setOrderProductSmallpoxIdentify(planProductInfo.getOrderProductSmallpoxIdentify());
		}
		// 产品长度差值绝对值排序
		if(planProductInfo.getOrderAbsProuductLength() != null) {
			productSearchForOneKeyDTO.setOrderAbsProuductLength(planProductInfo.getOrderAbsProuductLength());
		}
		// 产品高度下限
		if(planProductInfo.getProductHeightStart() != null) {
			productSearchForOneKeyDTO.setProductHeightStart(planProductInfo.getProductHeightStart());
		}
		// 产品高度上限
		if(planProductInfo.getProductHeightEnd() != null) {
			productSearchForOneKeyDTO.setProductHeightEnd(planProductInfo.getProductHeightEnd());
		}
