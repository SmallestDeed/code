...
@Api(tags = "SpecialGoodsController", description = "特卖商品管理")
@RestController
@Slf4j
@RequestMapping("/v1/special/goods")
public class SpecialGoodsController {
    ...
    public Object getMainPageGoodsList(Integer companyId){
        ...
        List<Object> list = baseGoodsBizService.getMainPageGoodsList(GoodsTypeEnum.SPECIAL_GOODS, companyId);
        return ReturnData.builder().code(SUCCESS).list(list);
    }

    @GetMapping("/getSpecialGoodsList")
    public Object getSpecialGoodsList(Integer companyId){
        if (companyId == null){
            return ReturnData.builder().code(ERROR).message("公司ID为空");
        }
        List<Object> list = baseGoodsBizService.getBizGoodsList(GoodsTypeEnum.SPECIAL_GOODS, companyId);
        return ReturnData.builder().code(SUCCESS).list(list);
    }
    ...
    public Object updateMainPageState(@PathVariable("spuId") Integer spuId, @PathVariable("state") Integer state){
        ...
        int success = baseGoodsBizService.updateMainPageState(GoodsTypeEnum.SPECIAL_GOODS, spuId, state);
        if (success > 0){
