...
public class FileUploadUtils {
	...
	public final static ResourceBundle res = ResourceBundle.getBundle("config/res");
	/** 上传通用常量 **/
	...
	public static boolean saveFile(Map map) {
		...
		if (file != null) {
			...
			try {
				...
				map.put("finalFileName", finalFlieName);
...
//		try {
//			FileUploadUtils.downloadFile("//onekeydesign/designTemplet/E01_0003_001/3dmodel/559463_20160523152754025.zip","f://");
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
...
//		}
	...
	public static void deleteFile(String path){
		FileEncrypt_old.deleteAllFile(path);
	}
...
//					int height = bufferedImage.getHeight();// 图片高度
	 ...
	 * @return
	 */
	public static boolean saveBase64File(Map map){

		boolean flag = false;
		// 业务对应的文件存储目录key值
		String uploadPathKey = (String) map.get(UPLOADPATHTKEY);
		String FileDate = (String)map.get(FILE);//文件数据
		String data = null;
		String dataPrefix = null;

		//校验图片数据是否合法
		if(org.apache.commons.lang.StringUtils.isNotBlank(FileDate)){
			String[] dataStrArr = FileDate.split("base64,");
			if(dataStrArr != null && dataStrArr.length == 2){
				dataPrefix = dataStrArr[0];
				data = dataStrArr[1];
			}else{
				throw new RuntimeException("图片数据不合法.上传失败!");
			}
		}else{
			return flag;
		}
		//解析数据
		String picSuffix = null;//后缀
		if(data != null){
			if ("data:image/jpeg;".equalsIgnoreCase(dataPrefix)) {//data:image/jpeg;base64,base64编码的jpeg图片数据
				picSuffix = ".jpg";
			} else if ("data:image/jpg;".equalsIgnoreCase(dataPrefix)) {//data:image/x-icon;base64,base64编码的jpg图片数据
				picSuffix = ".jpg";
				...
				picSuffix = ".png";
			} else if ("data:image/bmp;".equalsIgnoreCase(dataPrefix)) {//data:image/png;base64,base64编码的bmp图片数据
				picSuffix = ".bmp";
			} else {
		...
		try {
			imageByte = new BASE64Decoder().decodeBuffer(data);
		} catch (IOException e) {
			e.printStackTrace();
			logger.error("saveBase64File------------>解码文件时出现异常:",e);
		}
		...
		try {
			...
			//自动生成文件名
			String finalFlieName = Utils.generateRandomDigitString(6)
					+ "_" + Utils.getCurrentDateTime(Utils.DATETIMESSS)
					+ picSuffix;
			// 获取配置的业务文件目录
			String filePath = res.getString(uploadPathKey);
			...
			// 文件服务器的存储路径
			String realPath = Utils.dealWithPath(Utils.getAbsolutePath(dbFilePath, null), null);
			logger.info("saveBase64File--------->realPath:{}",realPath);
			File sFile = new File(realPath);
			...
			long fileSize = sFile.length();
			String suffix = fname.substring(fname.lastIndexOf("."));
			String filename = finalFlieName.substring(0, finalFlieName.lastIndexOf("."));
			if(!StringUtils.isEmpty(uploadPathKey)){
				map.put(FileModel.FILE_KEY,uploadPathKey.replace(".upload.path", ""));
			}
			if (!StringUtils.isEmpty(filename)) {
			...
			e.printStackTrace();
			logger.error("saveBase64File--------->上传文件时出现异常:",e);
			return false;
		}
		flag = true;
		return flag;
