...
import com.sandu.pay.model.PayOrder;
...
public class ProfessionalRenderController {
    ...
    public Object getRenderPayInfo(@ModelAttribute RenderPayInput payInput,HttpServletRequest request){
        ...
        try {
            ...
            logger.error("getRenderCost 出现异常:{}",e1);
            return new ResponseEnvelope<>(false,"出现异常!");
        }
    ...
    public Object saveHighDefinitionRenderOrder(HttpServletRequest request,@ModelAttribute RenderPayInput payInput) {
        ...
        if (ret) {
            return new ResponseEnvelope<>(true, "支付成功，后台将会为您努力渲染，请耐心等待哦！");
        } else {
     ...
     */
    @RequestMapping("/recordHDRendertaskFailInfo")
    @ResponseBody
    public ResponseEnvelope<AutoRenderTaskState> recordHDRendertaskFailInfo(Integer planId, Integer templateId, Integer businessId , Integer failType, String failReason, Integer taskId, HttpServletRequest request, HttpServletResponse response, String msgId) {
        ...
        //测试日志
        logger.error("渲染失败调用接口参数:[planId:{},templateId:{},businessId:{},failType:{},failReason:{},taskId:{}]",planId,templateId,businessId,failType,failReason,taskId);
        ...
        try {
            //记录失败原因并更改任务状态,以及发送消息和退款
            highDefinitionRenderService.recordHDRendertaskFailInfo(taskState);
        } catch (BizException e){
            logger.error("",e);
            return new ResponseEnvelope<>(false,e.getMessage(),msgId);
        }catch (Exception e1){
            logger.error("",e1);
            return new ResponseEnvelope<>(false,"出现异常，记录失败!",msgId);
        }

        return new ResponseEnvelope<>(true,"已记录",msgId);
    }
