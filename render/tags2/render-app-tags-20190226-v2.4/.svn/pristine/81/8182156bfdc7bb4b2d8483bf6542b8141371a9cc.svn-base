...
public class ShareController {
    ...
    public Object getWXConfig(HttpServletRequest request, HttpServletResponse response, String url){
        ...
        if( cacheObj != null ){
            ...
            System.out.println("=========access_token from redis:"+access_token);
            logger.info("=========access_token from redis:"+access_token);
        }else {
            ...
            if( StringUtils.isBlank(responseBody) ){
                System.out.println("请求 access_token 返回体为空！");
                logger.error("请求 access_token 返回体为空！");
                return null;
            }
            jsonObject = JSONObject.fromObject(responseBody);
            if (jsonObject != null && jsonObject.get("access_token") != null) {
                access_token = jsonObject.getString("access_token");
                System.out.println("=========access_token from httpclient:"+access_token);
                logger.error("=========access_token from httpclient:"+access_token);
                // 放到redis缓存中
                cacher.set("ACCESS_TOKEN", access_token, 7000);
            }else{
                System.out.println("获取access_token时出现错误:"+ jsonObject);
                logger.error("获取access_token时出现错误:" , jsonObject);
            }
        }
        ...
        if( StringUtils.isNotBlank(access_token) ){
            ...
            System.out.println("=========cacher.get(\"JSAPI_TICKET\"):" + cacheObj);
            logger.info("=========cacher.get(\"JSAPI_TICKET\"):" + cacheObj);
            if( cacheObj != null ){
                ...
                if( StringUtils.isBlank(responseBody) ){
                    logger.error("请求jsapi_ticket 返回体为空！");
                    return null;
                }
                jsonObject = JSONObject.fromObject(responseBody);
                jspia_ticket = jsonObject.getString("ticket");
                if(Utils.isBlank(jspia_ticket)){
                    System.out.println("获取jspia_ticket 出错:" + jsonObject);
                    logger.error("获取jspia_ticket 出错:",jsonObject);
                }else{
                    System.out.println("请求到的jspia_ticket:" + jspia_ticket);
                    logger.error("请求到的jspia_ticket:",jspia_ticket);
                    // 放到redis中
            ...
            System.out.println("获取到的access_token为空！");
            logger.error("获取到的access_token为空！");
        }
...
//        Object access_token_object = cacher.get("SAMLL_ACCESS_TOKEN");
    ...
    @RequestMapping("getPlanQrCodeUrlList")
    @ResponseBody
    public Object getPlanQrCodeUrlList(HttpServletRequest request, HttpServletResponse response,Integer planId){
        if(planId == null){
            return new ResponseEnvelope<>(false,"缺少参数planId");
        }
        //平台
        String platformType = request.getHeader(HeaderConstants.PLATFORM_CODE);
        if(Utils.isBlank(platformType)){
            return new ResponseEnvelope<>(false,"缺少平台标识");
        }
        List<SceneQrCodeResult> codeResultList = new ArrayList<>();
        List<QrCodeSceneDto>  codeSceneDtos = new ArrayList<>();
        codeSceneDtos = sceneQrCodeInfoService.getPlanShareCodeInfoByScenePlanId(planId);

        if(CustomerListUtils.isNotEmpty(codeSceneDtos)){
            SceneQrCodeResult codeResult = null;
            for (QrCodeSceneDto sceneDto : codeSceneDtos){
                codeResult = new SceneQrCodeResult();
                codeResult.setRenderingType(sceneDto.getRenderingType());
                codeResult.setRenderingTypeStr(sceneDto.getRenderingTypeStr());
                if(sceneDto.getRenderingType() == RenderTypeCode.COMMON_720_LEVEL || sceneDto.getRenderingType() == RenderTypeCode.ROAM_720_LEVEL){
                    String url =   SCENE_SHARE_H5_QRCODE_URL + "?renderId="+ sceneDto.getRenderId() +"&sceneType=" + sceneDto.getSceneType()
                            + "&planSourceType=" + sceneDto.getPlanSourceType() + "&hasChanged=" + sceneDto.getHasChanged() + "&qrCodeType="
                            + sceneDto.getQrCodeType()
                            //platformNewCode 写死为 pc2b,因为请求产品清单的接口中，其他端未兼容没有登录用户的分享场景
                            + "&platformNewCode=" + "pc2b";

                    codeResult.setQrUrl(url);
                }else{
                    codeResult.setQrUrl(sceneDto.getQrCodeContentUrl());

                }
                codeResultList.add(codeResult);
            }
        }
        long count = CustomerListUtils.isEmpty(codeResultList) ? 0 : codeResultList.size();
        return new ResponseEnvelope<SceneQrCodeResult>(count,codeResultList);
    }
