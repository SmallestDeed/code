...
public class DesignPlanAutoRenderServiceImpl implements DesignPlanAutoRenderService {
	...
	/** PC端高清渲染任务 **/
	public static final String REDIS_HD_RENDER_TASK_LIST = "highDefinitionRenderTaskList";
	/** PC端高清渲染置顶任务 **/
	public static final String REDIS_HD_RENDER_TASK_STICK = "renderHDTaskStick";
...
//			}
    ...
    public Integer handleAutoRenderTask(AutoRenderTask autoRenderTask, ResRenderPic resRenderPic, String orderNo) throws GeneratePanoramaException {
       ...
       designPlanAutoRenderMapper.createTask(autoRenderTask);
       int taskId = autoRenderTask.getId();
       if(taskId == 0){
						...
						if (renderSceneId != null && renderSceneId > 0) {
							...
							if (renderScene != null) {
								...
								designPlanProduct = designPlanProductService.getByPlanIdAndProductId
										(designPlanId, productReplaceTaskDetail.getSourceProductId());
...
//								}
								...
								url += "?" + "productId=" + productReplaceTaskDetail.getDestProductId() + "&" + "designPlanProductId=" + designPlanProduct.getId();
								...
								Double obj = (Double) result.getObj();
								practicableProductId  = obj.intValue();
...
/*
	...
	@Override
	public int addSubtaskList(List<AutoRenderTask> subtaskList) {
		return designPlanAutoRenderMapper.addSubtaskList(subtaskList);
	}

	@Override
	public synchronized Long addListToRedisLists(List<AutoRenderTask> subtaskList) {
		Long result = 0L;
		for (AutoRenderTask autoRenderTask : subtaskList) {
			String json = GSON.toJson(autoRenderTask);
			result = JedisUtils.listAdd(REDIS_TASK_LIST, json);
			logger.info("Redis list size ====>"+result);
		}
		return result;
	...
	@Override
	public int addBaseHousePicFullHousePlanRel(BaseHousePicFullHousePlanRel baseHousePicFullHousePlanRel) {
		return designPlanAutoRenderMapper.insertBaseHousePicFullHousePlanRel(baseHousePicFullHousePlanRel);
	}

	@Override
	public List<BaseHouseGuidePicInfo> getBaseHouseGuidePicInfo(Integer houseId,Integer designTempletId) {
		return designPlanAutoRenderMapper.selectBaseHouseGuidePicInfo(houseId,designTempletId);
	}

	@Override
	public BaseHousePicFullHousePlanRel getBaseHousePicFullHousePlanRelByHouseId(ResRenderPic resRenderPic) {
		List<BaseHousePicFullHousePlanRel> baseHousePicFullHousePlanRelList = designPlanAutoRenderMapper.selectBaseHousePicFullHousePlanRelList(resRenderPic.getHouseId(),resRenderPic.getTemplateId(),resRenderPic.getNewFullHousePlanId());
		if(Lists.isNotEmpty(baseHousePicFullHousePlanRelList)){
			return baseHousePicFullHousePlanRelList.get(0);
		}
	...
	@Override
	public int updateBaseHousePicFullHousePlanRel(BaseHousePicFullHousePlanRel newBaseHousePicFullHousePlanRel) {
		return designPlanAutoRenderMapper.updateBaseHousePicFullHousePlanRel(newBaseHousePicFullHousePlanRel);
	}

	@Override
	public List<BaseHouseGuidePicInfo> getBaseHouseGuidePicInfoList(List<AutoRenderTask> autoRenderTaskList) {
		return designPlanAutoRenderMapper.selectBaseHouseGuidePicInfoList(autoRenderTaskList,autoRenderTaskList.get(0).getHouseId());
	}

	@Override
	public int addBaseHousePicFullHousePlanRelList(List<BaseHousePicFullHousePlanRel> newBaseHousePicFullHousePlanRelList) {
		return designPlanAutoRenderMapper.insertBaseHousePicFullHousePlanRelList(newBaseHousePicFullHousePlanRelList);
	}

	@Override
	public List<AutoRenderTask>  getALLTaskByMainTaskId(Integer userId, Integer taskId) {
		return designPlanAutoRenderMapper.selectALLTaskByMainTaskId(userId,taskId);
	}

	@Override
	public List<BaseHousePicFullHousePlanRel> getBaseHousePicFullHousePlanRelListByFullHousePlanId(Integer fullHousePlanId) {
		return designPlanAutoRenderMapper.getBaseHousePicFullHousePlanRelListByFullHousePlanId(fullHousePlanId);
	}

	@Override
	public Integer selectHouseIdByFullHousePlanId(Integer fullHousePlanId) {
		return designPlanAutoRenderMapper.selectHouseIdByFullHousePlanId(fullHousePlanId);
	}

	@Override
	public Integer updateAutoRenderTaskStateByTaskId(Integer mainTaskId, Integer state, Integer taskType) {
		return designPlanAutoRenderMapper.updateAutoRenderTaskStateByTaskId(mainTaskId, state, taskType);
	}

	@Override
	public Integer updateSubTaskStateByBusinessId(Integer preRenderSceneId) {
		return designPlanAutoRenderMapper.updateSubTaskStateByBusinessId(preRenderSceneId);
	}

	@Override
	public int updateFullHouseDesignPlan(Integer fullHouseDesignPlanId) {
		return designPlanAutoRenderMapper.updateFullHouseDesignPlan(fullHouseDesignPlanId);
	}

	@Override
	public int addRedisHDRenderTaskList(AutoRenderTask autoRenderTask) {
		Gson gson = new Gson();
		String json = gson.toJson(autoRenderTask);
		Long result = JedisUtils.listAdd(REDIS_HD_RENDER_TASK_LIST, json);
		logger.info("Redis list size ====>"+result);
		return result.intValue();
	}
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public AutoRenderTask getHDRenderTaskByAuto(String redisKey,String renderMachineIp, Integer renderLevel, String renderProgramVersion) {
		/**
		 * 1.从redis里获取任务
		 * 2.判断状态
		 * 3.生成states 数据
		...
		String hostIp = addr.getHostAddress().toString();
		String hostName = addr.getHostName().toString();
		List<String> taskListStr = JedisUtils.getBrpopList(redisKey);
		if(CustomerListUtils.isEmpty(taskListStr) || taskListStr.size() < 1){
			return null;
		}
		String taskStr = taskListStr.get(1);
		AutoRenderTask renderTask = gson.fromJson(taskStr,AutoRenderTask.class);
		//2.判断是否已处理过
		AutoRenderTaskState taskState = this.getStateByTaskId(renderTask.getTaskId());
		if(taskState != null && taskState.getId() != null){
			return null;
		}
		//3.生成 渲染任务状态数据
		AutoRenderTask task = designPlanAutoRenderMapper.getRenderTaskById(renderTask.getId());
		if(task == null || task.getIsDeleted() == 1){
			//此任务可能已经被删除了，此时不再渲染
			return null;
		}

		taskState = new AutoRenderTaskState();
		try {
			/*BeanUtils.copyProperties(task,taskState);*/
			BeanUtils.copyProperties(taskState,task);
		} catch (IllegalAccessException e) {
			e.printStackTrace();
		} catch (InvocationTargetException e) {
			e.printStackTrace();
		}
		if(task.getRenderTypesStr() != null){
			if (task.getRenderTypesStr().equals(ProductType.PHOTO)) {
				taskState.setRenderPic(DesignPlanConstants.RENDERING);
				task.setRenderingType(ProductType.COMMON_PICTURE_LEVEL);
			} else if (task.getRenderTypesStr().equals(ProductType.ROAM720)) {
				taskState.setRender720(DesignPlanConstants.RENDERING);
				task.setRenderingType(ProductType.COMMON_720_LEVEL);
			} else if (task.getRenderTypesStr().equals(ProductType.VIDEO)) {
				taskState.setRenderN720(DesignPlanConstants.RENDERING);
				task.setRenderingType(ProductType.COMMON_VIDEO);
			} else if (task.getRenderTypesStr().equals(ProductType.ROAMN720)) {
				taskState.setRenderVideo(DesignPlanConstants.RENDERING);
				task.setRenderingType(ProductType.ROAM_720_LEVEL);
			}
		}

		taskState.setDesignPlanId(task.getDesignPlanId());
		taskState.setPlanId(task.getPlanId());
		...
		taskState.setRenderParamsStr(task.getRenderParamsStr());

		// 插入任务状态表
		addTaskStateToDB(taskState);
		//逻辑删除掉任务
		deleteRenderTaskByTaskId(task.getId());
		return task;
	}

	@Override
	public AutoRenderTask getRenderTaskById(Integer taskId) {
		return designPlanAutoRenderMapper.getRenderTaskById(taskId);
	}

	@Override
	public boolean deleteTaskAndStateByTaskId(Integer taskId, LoginUser loginUser) {
		//TODO：
		designPlanAutoRenderMapper.deleteRenderTaskByTaskId(taskId);
		AutoRenderTaskState taskState = new AutoRenderTaskState();
	...
	public int updateTaskStateByTaskId(AutoRenderTaskState autoRenderTaskState) {
		return designPlanAutoRenderMapper.updateTaskStateByTaskId(autoRenderTaskState);
	}
}
