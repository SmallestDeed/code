...
public class HighDefinitionRenderServiceImpl implements HighDefinitionRenderService {
    ...
    public boolean saveRenderOrderWithDuBi(RenderPayInput payInput, LoginUser loginUser) throws BizException {
       ...
       /* if(renderPriceMap == null){
        ...
        BasePlatform platform = basePlatformService.getByCode(payInput.getPlatformCode());
        if(platform == null){
            logger.error("支付专业渲染订单时未找到该平台信息,platform:{}" , payInput.getPlatformCode());
            throw new BizException("出现异常，扣费失败!");
        }
        payInput.setPlatformId(platform.getId());
        PayAccount userAccount = payAccountService.getUserPayAccountByUserId(loginUser.getId().longValue(),platform.getPlatformBussinessType());
        if(userAccount == null){
    ...
    public void sendMessageForCreateHDRenderTask(String intenalTradeNo) throws BizException {
        String messageContent = "专业渲染订单"+intenalTradeNo+"支付成功,后台将会为您努力渲染，请耐心等待哦！";
        this.sendMeaasgeForRenderTask(intenalTradeNo,messageContent);
    }

    private void sendMeaasgeForRenderTask(String intenalTradeNo,String messageContent) throws BizException {
       ...
       /* AutoRenderTask renderTask = designPlanAutoRenderService.getRenderTaskById(payOrder.getTaskId());*/
        ...
        baseMessage.setContent(messageContent);
        int id = baseMessageService.add(baseMessage);

        BaseMessageRecieve messageRecieve = new BaseMessageRecieve();
    ...
    @Override
    public void sendMessageForFailingHDRenderTask(String intenalTradeNo) throws BizException {
        String messageContent = "专业渲染订单"+intenalTradeNo+"渲染失败,稍后会将渲染费用退还至您的账户";
        this.sendMeaasgeForRenderTask(intenalTradeNo,messageContent);
    }

    @Override
    public void recordHDRendertaskFailInfo(AutoRenderTaskState taskState) throws BizException {

       /* AutoRenderTask renderTask = designPlanAutoRenderService.getRenderTaskById(taskState.getTaskId());*/
        //更改任务状态
        if (Objects.equals(ProductType.PHOTO,taskState.getRenderTypesStr())) {
            taskState.setRenderPic(DesignPlanConstants.RENDER_FAIL);
        } else if (Objects.equals(ProductType.ROAM720,taskState.getRenderTypesStr())) {
            taskState.setRender720(DesignPlanConstants.RENDER_FAIL);
        } else if (Objects.equals(ProductType.ROAMN720,taskState.getRenderTypesStr())) {
            taskState.setRenderN720(DesignPlanConstants.RENDER_FAIL);
        } else if (Objects.equals(ProductType.VIDEO,taskState.getRenderTypesStr())) {
            taskState.setRenderVideo(DesignPlanConstants.RENDER_FAIL);
        }
        taskState.setTaskType(DesignPlanConstants.TASKSTATE.FAILUE.getValue());
        designPlanAutoRenderService.updateTaskStateByTaskId(taskState);
        //发送退款消息
        this.sendMessageForFailingHDRenderTask(taskState.getOrderNumber());
        //TODO:退款
