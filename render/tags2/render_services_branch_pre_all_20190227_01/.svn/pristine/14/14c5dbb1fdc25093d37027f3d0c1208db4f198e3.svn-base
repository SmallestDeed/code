...
public abstract class AbstractAutoRenderProcessor {
    ...
    @Autowired
    protected DesignPlanAutoRenderService designPlanAutoRenderService;
    @Autowired
    ...
    protected AutoRenderTask getAutoRenderTask(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        autoRenderTask.setNewFullHousePlanId(resRenderPic.getFullHousePlanId());
        autoRenderTask.setMainTaskId(resRenderPic.getTaskId());
        autoRenderTask.setSpaceFunctionId(resRenderPic.getSpaceFunctionId());
        autoRenderTask.setSupplyDemandId(resRenderPic.getSupplyDemandId());
    ...
    protected void createSingleHouseTask(ResRenderPic resRenderPic, AutoRenderTask autoRenderTask) throws GeneratePanoramaException {
        ...
        if (operationSource.intValue() == AutoRenderTaskConstant.OPERATION_SOURCE_MY_DESIGN) {
            ...
            //来源是推荐方案
            //先查询方案信息，判断是否是组合方案，是组合方案的话就去获取最适合的子方案
            DesignPlanRecommended planRecommended = designPlanRecommendedService.get(planRecommendedId);
            if (null == planRecommended) {
            ...
            if (groupPrimaryId != null && groupPrimaryId != 0 && groupPrimaryId.equals(planRecommendedId) &&
                    AutoRenderTaskConstant.TASK_TYPE_AUTO_RENDER.equals(taskType)) {
...
//        }
     ...
     * @return
     */
    protected ResponseEnvelope handleFullHouseAutoRenderTask(ResRenderPic resRenderPic) throws GeneratePanoramaException{
        ...
        try {
            ...
            throw new GeneratePanoramaException(e.getMessage());
        }
        ...
        if (subtaskList == null || subtaskList.size() == 0) {
            ...
            throw new GeneratePanoramaException("获取子任务集合为空");
        }
        //设置主任务的子任务数量字段
        autoRenderTask.setSubtaskCount(subtaskList.size());
        //回填主任务的部分属性
        Integer res = designPlanAutoRenderService.updateAutoRenderTask(autoRenderTask);
        if (res <= 0) {
        ...
        //全屋方案子任务集合添加到db
        designPlanAutoRenderService.addSubtaskList(subtaskList);
        //全屋方案主任务添加到redis
        Integer result = designPlanAutoRenderService.addRedisLists(autoRenderTask);
        //全屋方案子任务集合添加到redis
        long taskListSize = designPlanAutoRenderService.addListToRedisLists(subtaskList);
        ...
        if (result > 0 && status && taskListSize > 0) {
            Long taskReplaceListSize = redisService.getSizeOfList(REDIS_TASK_REPLACE_LIST);
            logger.error("创建全屋一次性装进我家渲染任务，taskReplaceListSize="+taskReplaceListSize+",taskListSize="+taskListSize);
            flag = true;
            return new ResponseEnvelope<>(flag, taskReplaceListSize + taskListSize, "创建渲染任务成功！","");
        }
        logger.error("全屋方案装进我家创建渲染任务失败===>result:" + result + ",status:" + status + ",listSize:" + taskListSize);
     ...
     * @return
     */
    protected ResponseEnvelope handleFullHousePlanAutoRenderNewTask(ResRenderPic resRenderPic) throws GeneratePanoramaException{
        ...
        if (AutoRenderTaskConstant.FULL_HOUSE_PLAN_ACTION_CREATE.equals(fullHousePlanAction)) {
            ...
            if (null == bizType || 0 == bizType || 1 == bizType) {
                ...
                //全屋方案一次性装进我家先创建全屋方案的逻辑
                fullHouseRenderTaskVO = this.replaceFullHousePlanCreate(resRenderPic);
            }
        ...
        //逻辑废弃，先注释掉
        // } else if (AutoRenderTaskConstant.FULL_HOUSE_PLAN_ACTION_COPY.equals(fullHousePlanAction)) {
        //     // 处理复制过来的全屋方案渲染逻辑，有全屋方案没有主子任务
        //     fullHouseRenderTaskVO = this.fullhouseplanCopy(resRenderPic);
     ...
     * @return
     */
    public FullHouseRenderTaskVO fullhouseplanCopy(ResRenderPic resRenderPic) throws GeneratePanoramaException{
        // 如果没有任务id，表示复制完全屋方案之后第一次装修，就创建主任务
        if (null == resRenderPic.getTaskId() || 0 == resRenderPic.getTaskId()) {
            //公共的属性
            ...
            Integer mainTaskId = designPlanAutoRenderService.add(mainTask);
            mainTask.setId(mainTaskId);
            mainTask.setMainTaskId(mainTaskId);

            //回填主任务的部分属性
            Integer res = designPlanAutoRenderService.updateAutoRenderTask(mainTask);
            if (res <= 0) {
                throw new GeneratePanoramaException("回填主任务相关字段失败");
            }
            ...
            //添加子任务到db
            Integer subtaskId = designPlanAutoRenderService.add(subtask);
            if (subtaskId == null || 0 == subtaskId) {
                throw new GeneratePanoramaException("生成子任务失败");
            }
            subtask.setId(subtaskId);
            Integer newFullHousePlanId = resRenderPic.getNewFullHousePlanId();
            // 保存全屋户型文件数据
            addBaseHousePicFullHousePlanRel(resRenderPic, resRenderPic.getNewFullHousePlanId(), mainTaskId, subtaskId);
            //全屋方案主任务添加到redis
            Integer mainResult = designPlanAutoRenderService.addRedisLists(mainTask);
            //子任务添加到redis
            Integer subResult = designPlanAutoRenderService.addRedisLists(subtask);
            // 修改复制过来的全屋方案的is_update为1
            int result = designPlanAutoRenderService.updateFullHouseDesignPlan(newFullHousePlanId);
            if (result <= 0) {
                throw new GeneratePanoramaException("修改复制过来的全屋方案isUpdate失败，result="+result+
                        ",newFullHousePlanId="+newFullHousePlanId);
            }
            ...
            // 如果有任务id，表示不是第一次装修，就调用update全屋的接口
            return this.fullHousePlanUpdate(resRenderPic);
        }
    }


    protected FullHouseRenderTaskVO replaceFullHousePlanCreate(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        //调用core的接口生成空的全屋主方案
        ...
        Integer mainTaskId = designPlanAutoRenderService.add(mainAutoRenderTask);
        ...
        if (subtaskList == null || subtaskList.size() == 0) {
            throw new GeneratePanoramaException(false, "获取子任务集合为空");
        }
        //设置主任务的子任务数量字段
        mainAutoRenderTask.setSubtaskCount(subtaskList.size());
        //回填主任务的部分属性
        Integer res = designPlanAutoRenderService.updateAutoRenderTask(mainAutoRenderTask);
        if (res <= 0) {
            throw new GeneratePanoramaException(false, "回填主任务相关字段失败");
        }
        ...
        designPlanAutoRenderService.addSubtaskList(subtaskList);
        {
            ...
            //通过主任务查询所有子任务集合
            List<AutoRenderTask> autoRenderTaskList = designPlanAutoRenderService.getALLTaskByMainTaskId(resRenderPic.getUserId(), mainTaskId);
            ...
            if (null != newBaseHousePicFullHousePlanRelList && newBaseHousePicFullHousePlanRelList.size() > 0) {
                int count;
                try {
                    count = designPlanAutoRenderService.addBaseHousePicFullHousePlanRelList(newBaseHousePicFullHousePlanRelList);
                    if (0 == count) {
                        logger.error("保存全屋户型文件失败", GSON.toJson(newBaseHousePicFullHousePlanRelList));
                        throw new GeneratePanoramaException(false, "保存全屋户型文件失败");
                    }
                } catch (Exception e) {
                    logger.error("保存全屋户型文件数据异常", e);
                    throw new GeneratePanoramaException(false, "保存全屋户型文件数据异常");
                }
            }
            /*保存全屋户型文件数据end*/
        }
        //全屋方案主任务添加到redis
        Integer result = designPlanAutoRenderService.addRedisLists(mainAutoRenderTask);
        //全屋方案子任务集合添加到redis
        Long listSize = designPlanAutoRenderService.addListToRedisLists(subtaskList);
        ...
        if (result > 0 && listSize > 0) {
            ...
            fullHouseRenderTaskVO.setMainTaskId(mainTaskId);
            // 获取替换渲染redis中任务list的长度  +自动任务返回的长度一起返回
            Long taskReplaceListSize = redisService.getSizeOfList(REDIS_TASK_REPLACE_LIST);
            logger.error("创建单空间替换渲染任务，taskReplaceListSize="+taskReplaceListSize+",taskListSize="+listSize);
            fullHouseRenderTaskVO.setRedisTaskListSize(taskReplaceListSize+listSize);

            return fullHouseRenderTaskVO;
        }
        logger.error("全屋方案装进我家创建渲染任务失败===>result:" + result + ",listSize:" + listSize);
        throw new GeneratePanoramaException(flag, "创建渲染任务失败！");
    }
    ...
    protected List<BaseHouseGuidePicInfo> getBaseHouseGuidePicInfoList(List<AutoRenderTask> subTasks) throws GeneratePanoramaException {
        ...
        try {
            baseHouseGuidePicInfoList = designPlanAutoRenderService.getBaseHouseGuidePicInfoList(subTasks);
    ...
    private FullHouseRenderTaskVO fullHousePlanUpdateNew(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        if (mainTaskId > 0 && subtaskId > 0 && mainResult > 0 && subResult > 0) {
            ...
            if (null != baseHousePicFullHousePlanRel) {
                ...
                BaseHousePicFullHousePlanRel newBaseHousePicFullHousePlanRel = this.createBaseHousePicFullHousePlanRelUpdate(resRenderPic, baseHouseGuidePicInfo, subtaskId, baseHousePicFullHousePlanRel);
                int baseHousePicFullHousePlanRelId;
                try {
                    baseHousePicFullHousePlanRelId = designPlanAutoRenderService.updateBaseHousePicFullHousePlanRel(newBaseHousePicFullHousePlanRel);
    ...
    private FullHouseRenderTaskVO fullHousePlanUpdate(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        autoRenderTask.setNewFullHousePlanId(resRenderPic.getFullHousePlanId());
        //属性基本都从上面已经复制完成，保存到数据库
        Integer taskId = designPlanAutoRenderService.add(autoRenderTask);
        //修改主任务渲染状态
        int res = designPlanAutoRenderService.updateAutoRenderTaskStateByTaskId(autoRenderTask.getMainTaskId(), 2,autoRenderTask.getTaskType());
        logger.error("添加渲染任务============> 修改主任务状态改动条数："+res);
        //Update the render state to rending(2),
        //修改全屋方案渲染状态
        FullHouseDesignPlan currentFullHouseDesignPlan = new FullHouseDesignPlan();
        currentFullHouseDesignPlan.setId(resRenderPic.getFullHousePlanId());
        ...
        // 如果没传前一个效果图方案ID，说明是新增子方案，新增数据
        if (resRenderPic.getPreRenderSceneId() == null || resRenderPic.getPreRenderSceneId() == 0) {
            ...
            // 删除旧任务状态
            designPlanAutoRenderService.updateSubTaskStateByBusinessId(resRenderPic.getPreRenderSceneId());
        }
        //属性基本都从上面已经复制完成，保存到缓存
        Integer result = designPlanAutoRenderService.addRedisLists(autoRenderTask);
        ...
        if (taskId > 0 && result > 0) {
            ...
            fullHouseRenderTaskVO.setCurTaskId(taskId);
            // 获取替换渲染redis中任务list的长度  +自动任务返回的长度一起返回
            Long taskReplaceListSize = redisService.getSizeOfList(REDIS_TASK_REPLACE_LIST);
            logger.error("创建单空间替换渲染任务，taskReplaceListSize="+taskReplaceListSize+",taskListSize="+result);
            fullHouseRenderTaskVO.setRedisTaskListSize(taskReplaceListSize+result);
    ...
    private void updateBaseHousePicFullHousePlanRel(ResRenderPic resRenderPic, int taskId)
        ...
        BaseHousePicFullHousePlanRel baseHousePicFullHousePlanRel = this.getBaseHousePicFullHousePlanRel(resRenderPic);
        if (null != baseHousePicFullHousePlanRel) {
            BaseHousePicFullHousePlanRel newBaseHousePicFullHousePlanRel = this.createBaseHousePicFullHousePlanRelUpdate(resRenderPic, baseHouseGuidePicInfo, taskId, baseHousePicFullHousePlanRel);
            int baseHousePicFullHousePlanRelId;
            try {
                baseHousePicFullHousePlanRelId = designPlanAutoRenderService.updateBaseHousePicFullHousePlanRel(newBaseHousePicFullHousePlanRel);
                if (0 == baseHousePicFullHousePlanRelId) {
    ...
    private void addBaseHousePicFullHousePlanRel(ResRenderPic resRenderPic, int fullHousePlanId, int mainTaskId, int subtaskId)
        ...
        BaseHousePicFullHousePlanRel baseHousePicFullHousePlanRel = this.getBaseHousePicFullHousePlanRel(resRenderPic);
        if (null == baseHousePicFullHousePlanRel) {
            BaseHousePicFullHousePlanRel newBaseHousePicFullHousePlanRel = this.createBaseHousePicFullHousePlanRelAdd(resRenderPic, fullHousePlanId, baseHouseGuidePicInfo, mainTaskId, subtaskId);
            int baseHousePicFullHousePlanRelId;
            try {
                baseHousePicFullHousePlanRelId = designPlanAutoRenderService.addBaseHousePicFullHousePlanRel(newBaseHousePicFullHousePlanRel);
                if (0 == baseHousePicFullHousePlanRelId) {
    ...
    private void copyBaseHousePicFullHousePlanRelToNewPlan(ResRenderPic resRenderPic, Integer newFullHousePlanId, Integer mainTaskId,Integer subTaskId)
            throws GeneratePanoramaException {
        logger.error("-------- 拷贝全屋渲染任务状态参数：resRenderPic：{},newFullHousePlanId:{},mainTaskId:{}", resRenderPic, newFullHousePlanId, mainTaskId);
        /*保存全屋户型文件数据start*/
        Integer houseId = resRenderPic.getHouseId();
        Integer templetId = resRenderPic.getTemplateId();
        ...
        if (baseHousePicFullHousePlanRelList != null && baseHousePicFullHousePlanRelList.size() > 0) {
            ...
            for (BaseHousePicFullHousePlanRel baseHousePicFullHousePlanRel : baseHousePicFullHousePlanRelList) {
                ...
                if (baseHousePicFullHousePlanRel.getHouseId().equals(houseId) && baseHousePicFullHousePlanRel.getDesignTempletId().equals(templetId)) {
                    baseHousePicFullHousePlanRel.setState(2L);
                    baseHousePicFullHousePlanRel.setTaskId(subTaskId.longValue());
                }
            }
            int num = designPlanAutoRenderService.addBaseHousePicFullHousePlanRelList(baseHousePicFullHousePlanRelList);
            if (num <= 0) {
    ...
    private List<BaseHousePicFullHousePlanRel> getBaseHousePicFullHousePlanRelListByFullHousePlanId(Integer fullHousePlanId) {
        return designPlanAutoRenderService.getBaseHousePicFullHousePlanRelListByFullHousePlanId(fullHousePlanId);
    }

    protected BaseHousePicFullHousePlanRel createBaseHousePicFullHousePlanRelUpdate(ResRenderPic resRenderPic, BaseHouseGuidePicInfo baseHouseGuidePicInfo, Integer taskId, BaseHousePicFullHousePlanRel baseHousePicFullHousePlanRel) {
        Date date = new Date();
    ...
    private FullHouseRenderTaskVO fullHousePlanCreate(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        if (mainTaskId > 0 && subtaskId > 0 && mainResult > 0 && subResult > 0) {
            ...
            fullHouseRenderTaskVO.setFullHousePlanId(fullHousePlanId);
            fullHouseRenderTaskVO.setMainTaskId(mainTaskId);
            fullHouseRenderTaskVO.setCurTaskId(subtaskId);
            // 获取替换渲染redis中任务list的长度  +自动任务返回的长度一起返回
            Long taskReplaceListSize = redisService.getSizeOfList(REDIS_TASK_REPLACE_LIST);
            logger.error("创建单空间替换渲染任务，taskReplaceListSize="+taskReplaceListSize+",taskListSize="+subResult);
            fullHouseRenderTaskVO.setRedisTaskListSize(taskReplaceListSize+subResult);
    ...
    private BaseHousePicFullHousePlanRel getBaseHousePicFullHousePlanRel(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        try {
            baseHousePicFullHousePlanRel = designPlanAutoRenderService.getBaseHousePicFullHousePlanRelByHouseId(resRenderPic);
    ...
    private BaseHouseGuidePicInfo getBaseHouseGuidePicInfo(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        try {
            baseHouseGuidePicInfos = designPlanAutoRenderService.getBaseHouseGuidePicInfo(resRenderPic.getHouseId(), resRenderPic.getTemplateId());
    ...
    protected BaseHousePicFullHousePlanRel createBaseHousePicFullHousePlanRelAdd(ResRenderPic resRenderPic, Integer fullHousePlanId, BaseHouseGuidePicInfo baseHouseGuidePicInfo
    ...
    private Integer createTempFullHousePlan(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        if (resRenderPic.getFullHousePlanId() != null) {
            url = coreUrl + "/v1/core/fullHouse/addTempFullHouseDesignPlan?userId=" + userId +
                    "&oldFullHouseDesignPlanId=" + resRenderPic.getFullHousePlanId() +
                    "&houseId=" + resRenderPic.getHouseId();
        } else {
            url = coreUrl + "/v1/core/fullHouse/addTempFullHouseDesignPlan?userId=" + userId +
                    "&houseId=" + resRenderPic.getHouseId();
        }
    ...
    private FullHouseRenderTaskVO fullHousePlanCreateNew(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        subtask.setId(subtaskId);
        // 保存全屋户型文件数据
        copyBaseHousePicFullHousePlanRelToNewPlan(resRenderPic, newFullHousePlanId, mainTaskId, subtaskId);
        //全屋方案主任务添加到redis
        Integer mainResult = designPlanAutoRenderService.addRedisLists(mainTask);
        //子任务添加到redis
        Integer subResult = designPlanAutoRenderService.addRedisLists(subtask);
     ...
     * @return
     */
    protected ResponseEnvelope handleFullHouseAutoReplaceTask(ResRenderPic resRenderPic) throws GeneratePanoramaException{
        int action = resRenderPic.getFullHousePlanAction();
        // 更新全屋方案坐标信息的时候用,从推荐方案列表进的时候不用
        // 根据之前的效果图方案id查它里面的templateId
        if (resRenderPic.getHouseId() == null || resRenderPic.getHouseId() == 0 ||
                null == resRenderPic.getTemplateId() || 0 == resRenderPic.getTemplateId()){
            Integer preRenderSceneId = resRenderPic.getPreRenderSceneId();
            if (preRenderSceneId == null || preRenderSceneId == 0) {
                return new ResponseEnvelope(false, "替换之前的效果图id为空");
            }
            DesignPlanRenderScene preRenderScene = designPlanRenderSceneService.get(preRenderSceneId);
            if (preRenderScene == null) {
                return new ResponseEnvelope(false, "替换之前的效果图id有误");
            }
            resRenderPic.setTemplateId(preRenderScene.getDesignTemplateId());
        }

        if (AutoRenderTaskConstant.FULL_HOUSE_PLAN_ACTION_UPDATE.equals(action)) {
            // 替换旧方案，更新全屋方案，有全屋方案，有主任务
            try {
        ...
        // 逻辑有缺陷，不走这里，注释掉
        // else if (AutoRenderTaskConstant.FULL_HOUSE_PLAN_ACTION_COPY.equals(action)) {
        //     // 处理复制过来的全屋方案，只有全屋方案，没有主任务的情况
        //     return this.fullHouseAutoReplaceCopy(resRenderPic);
        //
        // }
     ...
     * @return
     */
    private ResponseEnvelope fullHouseAutoReplaceCopy(ResRenderPic resRenderPic) throws GeneratePanoramaException{
        // 如果没有taskId表示复制完全屋方案第一次做替换渲染，生成主任务
        if (null == resRenderPic.getTaskId() || 0 == resRenderPic.getTaskId()) {
            Boolean flag = false;
            //先创建主任务
            AutoRenderTask mainTask = getAutoRenderTask(resRenderPic);
            //单空间方案
            ...
            // 新720要用到templateId
            subtask.setTemplateId(resRenderPic.getTemplateId()==null?-1:resRenderPic.getTemplateId());
            // 处理保存自动渲染任务数据
            Integer subtaskId = designPlanAutoRenderService.handleAutoRenderTask(subtask, resRenderPic, resRenderPic.getOrder());
            if (subtaskId == null) {
                return new ResponseEnvelope(false, "生成子任务失败");
            }
            Integer newFullHousePlanId = resRenderPic.getNewFullHousePlanId();
            // 保存全屋户型文件数据
            if (resRenderPic.getHouseId() != null || resRenderPic.getHouseId() != 0){
            ...
            Integer mainResult = designPlanAutoRenderService.addRedisReplaceList(mainTask);
            Integer subResult = designPlanAutoRenderService.addRedisReplaceList(subtask);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    private ResponseEnvelope fullHouseAutoReplaceUpdate(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        ...
        AutoRenderTask mainTask;
        // 判断是不是复制方案的第一次替换渲染,则生成主任务
        if (null == resRenderPic.getTaskId() || 0 == resRenderPic.getTaskId()) {
            Integer fullHousePlanId = resRenderPic.getFullHousePlanId();
            FullHouseDesignPlan fullHouseDesignPlan = fullHousePlanService.get(fullHousePlanId);
            if (null != fullHouseDesignPlan) {
                if (DesignPlanConstants.FULL_HOUSE_DESIGN_PLAN_SOURCE_COPY == fullHouseDesignPlan.getSourceType() &&
                        DesignPlanConstants.ISUPDATE.NO.getValue() == fullHouseDesignPlan.getIsUpdate()) {
                    //先创建主任务
                    ...
                    // 主任务添加到缓存
                    designPlanAutoRenderService.addRedisReplaceList(mainTask);
                    resRenderPic.setTaskId(mainTaskId);

                    // 修改复制的全屋方案的is_update字段
                    int result = designPlanAutoRenderService.updateFullHouseDesignPlan(fullHousePlanId);
                    if (result <= 0) {
                        throw new GeneratePanoramaException("修改复制过来的全屋方案isUpdate失败，result="+result);
                    }
                }
        ...
        // 新720要用到templateId
        autoRenderTask.setTemplateId(resRenderPic.getTemplateId()==null?-1:resRenderPic.getTemplateId());
        // 处理保存自动渲染任务数据
        Integer subtaskId = designPlanAutoRenderService.handleAutoRenderTask(autoRenderTask, resRenderPic, resRenderPic.getOrder());
        //修改主任务渲染状态
        designPlanAutoRenderService.updateAutoRenderTaskStateByTaskId(autoRenderTask.getMainTaskId(), 2, autoRenderTask.getTaskType());
        //修改全屋方案渲染状态
        FullHouseDesignPlan currentFullHouseDesignPlan = new FullHouseDesignPlan();
        currentFullHouseDesignPlan.setId(resRenderPic.getFullHousePlanId());
        currentFullHouseDesignPlan.setRenderState(Integer.valueOf(2));
        ...
        // 自动渲染任务添加至Redis缓存
        Integer subResult = designPlanAutoRenderService.addRedisReplaceList(autoRenderTask);
        ...
        if (subResult > 0 && status) {
            ...
            fullHouseRenderTaskVO.setMainTaskId(resRenderPic.getTaskId());
            fullHouseRenderTaskVO.setCurTaskId(subtaskId);
            // 获取自动渲染redis中任务list的长度  +替换返回的长度一起返回
            Long taskListSize = redisService.getSizeOfList(REDIS_TASK_LIST);
            logger.error("创建单空间替换渲染任务，taskReplaceListSize="+subResult+",taskListSize="+taskListSize);
            fullHouseRenderTaskVO.setRedisTaskListSize(taskListSize+subResult);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    private ResponseEnvelope fullHouseAutoReplaceAdd(ResRenderPic resRenderPic) throws GeneratePanoramaException {
        Boolean flag = false;
        ...
        try {
            ...
            mainTask.setNewFullHousePlanId(newFullHousePlanId);
            mainTask.setFullHousePlanId(resRenderPic.getFullHousePlanId());
        } catch (GeneratePanoramaException e) {
            return new ResponseEnvelope(e.isFlag(), e.getMessage());
        }
        //主任务添加到db
        ...
        if (mainResult > 0 && subResult > 0 && status) {
            ...
            fullHouseRenderTaskVO.setMainTaskId(mainTaskId);
            fullHouseRenderTaskVO.setCurTaskId(subtaskId);
            // 获取自动渲染redis中任务list的长度  +替换任务返回的长度一起返回
            Long taskListSize = redisService.getSizeOfList(REDIS_TASK_LIST);
            logger.error("创建单空间替换渲染任务，taskReplaceListSize="+subResult+",taskListSize="+taskListSize);
            fullHouseRenderTaskVO.setRedisTaskListSize(taskListSize+subResult);
    ...
    public AutoRenderTask createHDRenderTask(RenderPayInput payInput, String orderNo, LoginUser loginUser) throws BizException {
        Integer planId = payInput.getDesignPlanId();
        if(planId == null){
            throw  new BizException("设计方案id为空");
        }
        DesignPlan plan = designPlanService.get(planId);
        if(plan == null){
           throw  new BizException("找不到这个设计方案"+planId);
        }
        DesignTemplet designTemplet = designTempletService.get(plan.getDesignTemplateId());
        SpaceCommon spaceCommon = spaceCommonService.get(plan.getSpaceCommonId());
        BaseHouse baseHouse = baseHouseService.getHouseInfoBySpaceCommonId(plan.getSpaceCommonId());
        /**
         * 1.拼装数据
         * 2.保存
         * 3.存入缓存
        ...
        renderTask.setPlanId(payInput.getDesignPlanId());
        renderTask.setTemplateId(plan.getDesignTemplateId());
        renderTask.setDesignPlanId(payInput.getDesignPlanId());
        renderTask.setOrderNumber(orderNo);
        renderTask.setIsDeleted(0);
        renderTask.setCreator(loginUser.getLoginName());
        renderTask.setGmtCreate(new Date());
        renderTask.setModifier(loginUser.getLoginName());
        renderTask.setGmtModified(new Date());
        renderTask.setOperationUserId(loginUser.getId());
        renderTask.setOperationUserName(loginUser.getLoginName());
        renderTask.setTaskType(AutoRenderTaskConstant.TASK_TYPE_DIY_HD_RENDER);
        renderTask.setOperationSource(AutoRenderTaskConstant.OPERATION_SOURCE_DIY_DESIGN);
        renderTask.setPlatformId(payInput.getPlatformId());
        renderTask.setPlanHouseType(AutoRenderTaskConstant.PLAN_SINGLE_HOUSE_TYPE);
        renderTask.setTaskSource(AutoRenderTaskConstant.TASK_SOURCE_PC);
        renderTask.setHouseId(baseHouse != null ? baseHouse.getId() : null);
        renderTask.setSpaceFunctionId(spaceCommon.getSpaceFunctionId());
        renderTask.setSpaceCode(spaceCommon.getSpaceCode());
        renderTask.setTemplateCode(designTemplet.getDesignCode());
        renderTask.setDesignName(plan.getPlanName());
        renderTask.setPriority(99);
        if (Objects.equals(ProductType.COMMON_PICTURE_LEVEL , payInput.getRenderingType()) ){
            // 普通照片级
            renderTask.setRenderTypesStr(ProductType.PHOTO);
            renderTask.setRenderPic(AutoRenderTaskConstant.NO_RENDER_TASK);
        } else if (Objects.equals(ProductType.COMMON_720_LEVEL , payInput.getRenderingType())) {
            // 720全景
            renderTask.setRenderTypesStr(ProductType.ROAM720);
            renderTask.setRender720(AutoRenderTaskConstant.NO_RENDER_TASK);
        } else if (Objects.equals(ProductType.COMMON_VIDEO, payInput.getRenderingType())) {
            // 漫游视频
            renderTask.setRenderTypesStr(ProductType.VIDEO);
            renderTask.setRenderVideo(AutoRenderTaskConstant.NO_RENDER_TASK);
        } else if (Objects.equals(ProductType.ROAM_720_LEVEL , payInput.getRenderingType())) {
            // 720多点
            renderTask.setRenderTypesStr(ProductType.ROAMN720);
            renderTask.setRenderN720(AutoRenderTaskConstant.NO_RENDER_TASK);
        } else {
            throw new BizException("未知渲染类型");
        }
        renderTask.setRenderParamsStr(payInput.getRenderParamsStr());
        //增加渲染任务
        int taskId = designPlanAutoRenderService.add(renderTask);
        //填入缓存
        designPlanAutoRenderService.addRedisHDRenderTaskList(renderTask);
        return renderTask;
    }
}
