...
public class BaseHouseServiceImpl implements BaseHouseService {
	private static Logger logger = Logger
			.getLogger(BaseHouseServiceImpl.class);
	...
	public Object newHouseSearchWeb(String provinceCode, String cityCode,
		...
		try {
			...
			list = (List<BaseHouseResult>) Utils.paging2(list,
						...
						if (areaCode.contains(".")) {
							...
							for (int i = 0; i < area.length; i++) {
								...
								while (i == area.length - 1) {
									if (CustomerListUtils.isNotEmpty(areaList)) {
										houseResult.setDistrictName(areaList
												.get(0).getAreaName());
									}
									break;
								}
	...
	public Object newHouseList(String style, String livingId, String msgId,
		...
		try {
			...
			baseHouseSearch.setIsDeleted(new Integer(0));
			if (Utils.enableRedisCache()) {
				total = BaseHouseCacher.getBaseHouseCount(baseHouseSearch);
			} else {
				total = this.getCount(baseHouseSearch);
			}

			if (total > 0) {
				/** list = baseHouseService.getPaginatedList(baseHouseSearch); */
				if (loginUser.getId() > 0) {
					logger.info("------小区搜索接口->登录用户未识别");
				} else {
					if (loginUser.getUserType() != null) {
						baseHouseSearch.setUserType(loginUser.getUserType());
					}
				}
				if (Utils.enableRedisCache()) {
					list = BaseHouseCacher.getPaginatedList(baseHouseSearch);
				} else {
					list = this.getPaginatedListFilter(baseHouseSearch);
				}
				for (BaseHouse baseHouse : list) {
					if (baseHouse.getPicRes1Id() != null
							&& baseHouse.getPicRes1Id() != 0) {
						ResHousePic resHousePic = resHousePicService
								.get(baseHouse.getPicRes1Id());
						/*logger.error("户型缩略图ID：" + baseHouse.getPicRes1Id());*/
						if (null != resHousePic) {
							/*logger.error("户型缩略图路径："
									+ resHousePic.getThumbnailPath());*/
							baseHouse.setThumbnailPath(resHousePic
									.getThumbnailPath());
							baseHouse.setLargeThumbnailPath(resHousePic
									.getLargeThumbnailPath());
						}
					}
				}
			}
	...
	@Override
	public BaseHouse getHouseInfoBySpaceCommonId(Integer spaceCommonId) {
		return baseHouseMapper.selectHouseInfoBySpaceCommonId(spaceCommonId);
	}
}
