...
public class SysUserGroupServiceImpl implements SysUserGroupService {
	...
	private final String SERVERURL = Utils.getValue("app.render.server.url","");
	@Autowired
    ...
    public SysUserGroup getPlanGroupByPrimaryKey(Integer id) {
        return sysUserGroupMapper.selectPlanGroupByPrimaryKey(id);
    }

    @Override
    public boolean addShareTimes(SysUserGroup group) {
        return sysUserGroupMapper.addShareTimes(group);
    }
}
