...
public class DesignPlanProductServiceImpl implements DesignPlanProductService {
	...
	@Autowired
	private DesignTempletProductService templetProductService;
	@Autowired
...
//				usedConfigPath = usedConfigPath.replace("[code]",dpp.getPlanCode());
	 ...
	 * @param loginUser
	 * @param designPlanProduct
	 * @param baseBrandLs 同城联盟中同组员选择品牌列表
	 */
	@Override
	public List<ProductsCostType> costList(LoginUser loginUser, DesignPlanProduct designPlanProduct, costListEnum type, List<BaseBrand> baseBrandLs){
		if(type == null) {
			type = costListEnum.designPlan;
		}
		...
		if(null != companyId && companyId > 0) {
			...
			if(null != baseCompany) {
				if(3 == sysUser.getUserType().intValue()) {//经销商  basecompany brandids
					brandIdsStr = baseCompany.getBrandIds();
				}else if(2 == sysUser.getUserType()){//厂商 basebrand 
...
//				 brandIdsStr = baseCompany.getBrandIds();
		    ...
		    if(!flag){
				...
				if(null != baseBrandList && baseBrandList.size() > 0) {
					for(BaseBrand brand : baseBrandList) {
						...
						baseProduct.setStatusShowWu(brand.getStatusShowWu());//是否显示无品牌产品
						baseProduct.setBrandId(brand.getId());
						baseProductList.add(baseProduct);
					}
				}
		    }
		    //在同城联盟分享的方案中，要显示同组组员选择的品牌产品
	       if(baseBrandLs != null && baseBrandLs.size() > 0) {
	           for (BaseBrand brand : baseBrandLs) {
	             BaseProduct baseProduct= new BaseProduct();
	             baseProduct.setBrandId(brand.getId());
	             baseProductList.add(baseProduct);
	           }
		...
		if( total > 0 ){
			...
			for(ProductsCostType costType : costTypeList){
				...
				/*得到结算汇总清单
				 * 设置查询条件(序列号)*/
				costType.setProductPutawayState(designPlanProduct.getProductPutawayStateList());
...
//					cost.setAuthorizedConfigList(list);
					...
					if("1".equals(cost.getCostTypeValue())||"2".equals(cost.getCostTypeValue())||"3".equals(cost.getCostTypeValue())){
						...
						if (totalPrice != null) {
							BigDecimal new_totalPrice  = totalPrice.subtract(salePrice==null?new BigDecimal(0):salePrice);
					...
					cost.setProductPutawayState(designPlanProduct.getProductPutawayStateList());
					...
					if(costDetails!=null&&costDetails.size()>0){
						for (ProductCostDetail productCostDetail : costDetails) {
							...
							if (2 == sysUser.getUserType() && Objects.equals(productCostDetail.getCompanyId(),companyId)) {
...
//			autoUpdateStructureRelation(designTempletId);
		...
		try {
			...
			for (int index = 0; index < jsonArray.size(); index++) {
				...
				if(DesignPlanConstants.AUTO_RENDER != opType) {
					...
					// 自动渲染
					optimizeDesingPlanService.updatePlanProduct(designPlanProduct);
				}
			}
		}catch (Exception e) {
	...
	public Integer createByPlanProductInfo(PlanProductInfo planProductInfo, Integer planId, String username, Integer opType) {
		...
		if(DesignPlanConstants.AUTO_RENDER != opType) {
			...
			optimizeDesingPlanService.addPlanProduct(designPlanProduct);
		}
...
//		}
	...
	@Override
	public DesignPlanProduct getByPlanIdAndProductId(Integer planId, Integer productId) {
		if (planId == null || planId == 0 || productId == null || productId == 0) {
			return null;
		}
		return designPlanProductMapper.getByPlanIdAndProductId(planId, productId);
	}
