...
//	}
	...
	public ResponseEnvelope<DesignTempletProductResult> findDesignTempletProductList(Integer designTempletId,
		...
		List<DesignTempletProductResult> list = templetProductService.getTempletIdProductList(designTempletproduct);
		int total = Utils.getListTotal(list);
...
//		List<String> codesArrys = Arrays.asList(codesArry);
		...
		List<DesignTempletProduct> templetStructureProducts = templetProductService.getStructureProductByDesignId(designTempletId);
...
//			statusList.add(StructureProductStatusCode.TESTING);
	...
	public ResponseEnvelope<MatchGroupProductResult> getGroupProductData(Integer templetId, Integer designTempletId,
		...
		List<DesignTempletProduct> templetList = templetProductService.getByTempletIdMainProduct(designTempletProduct);
...
//				designTempletProduct.setCenter(product.getCenter());
				...
				int i = templetProductService.updateSameProductTypeIndex(designTempletProduct);
				if (i == 0) {
	...
	public List<SearchStructureProductResult> matchingStructureDataV2(
		...
		List<DesignTempletProduct> templetStructureProducts = templetProductService.getStructureProductByDesignId(designTemplet.getId());
...
//								againMatchMap.put(templetProduct,planProduct);
	...
	private List<Map<deletedBeijingProductInfoMapKeyEnum, String>> getDeletedBeijingProductInfo(Integer recommendedPlanId) throws IntelligenceDecorationException {
		...
		// 获取样板房中所有背景墙信息
		List<DesignTempletProduct> beijingDesignTempletProductList = templetProductService.getBeijingProductInfo(designTempletId);
	...
	public List<MatchGroupProductResult> getGroupProductDataV2(DesignPlanRecommended designPlanRecommended,
		...
		// 查询样板房白膜组合的主产品列表
		List<DesignTempletProduct> designTempletProductList = templetProductService.getMainProductListByTempletId(designTemplet.getId());
	...
	public Object updatePlanConfig(Integer planId,String context, Integer planProductId,String bDirtyConfig, Integer opType,LoginUser loginUser){
		...
		if (opType == DesignPlanConstants.USER_RENDER.intValue() ) {
			try {
				...
				//copy
				DesignPlanRes designPlanRes = optimizeDesingPlanService.getDesignPlanRes(planId);
				Long resultId = optimizeDesingPlanService.saveAsRenderOnekey(designPlanRes);
			...
			try {
				 ...
				 * 先更新数据库，再更新配置文件
				 **/
				boolean flag = optimizeDesingPlanService.updatePlanProductByConfig(context, planId);
				if (!flag) {
					resultJSON.put("success",false);
					resultJSON.put("message","配置文件更新异常,");
					return resultJSON;
