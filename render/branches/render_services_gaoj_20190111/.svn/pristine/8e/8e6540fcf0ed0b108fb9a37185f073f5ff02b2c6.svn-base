...
import com.nork.product.dao.BaseCompanyMiniProgramConfigMapper;
...
public class BaseCompanyServiceImpl implements BaseCompanyService {
	...
	@Autowired
	private BaseCompanyMiniProgramConfigMapper baseCompanyMiniProgramConfigMapper;
	@Autowired
	...
	@Override
	public BaseCompany getHeadMessageOfCompany(Integer companyId) {
		if (companyId == null || companyId == 0) {
			return null;
		}
		BaseCompany baseCompany = baseCompanyMapper.getHeadMessageOfCompany(companyId);

		Integer pid = baseCompany.getPid();
		if (pid != null && pid != 0) {
			baseCompany = baseCompanyMapper.getHeadMessageOfCompany(pid);
		}
		return baseCompany;
	}

	@Override
	public CompanyShop getHeadMessageOfShop(Integer shopId) {
		if (shopId == null || shopId == 0) {
			return null;
		}
		CompanyShop companyShop = baseCompanyMapper.getHeadMessageOfShop(shopId);

		return companyShop;
	}

	@Override
	public List<BaseCompanyMiniProgramConfig> getBaseCompanyMiniProgramConfigList(BaseCompanyMiniProgramConfig config) {
		return baseCompanyMiniProgramConfigMapper.selectList(config);
	}

	@Override
	public BaseCompanyMiniProgramConfig getBaseCompanyMiniProgramConfigByUserId(Integer userId) throws BizException {
		if(userId == null){
			throw new BizException("未获取到用户信息！");
		}
		SysUser sysUser = sysUserService.get(userId);
		if(sysUser == null || (sysUser.getBusinessAdministrationId() == null || sysUser.getCompanyId() == null)){
			return null;
		}
		...
		BaseCompany company = this.get(companyId);
		if(company == null){
			return null;
		}
		if(company.getBusinessType() == null){
			SysDictionary dictionary = sysDictionaryService.findOneByTypeAndValue(SysDictionaryConstant.SYS_DICTIONARY_COMPANY_BUSINESS_TYPE
					,company.getBusinessType());
			if(dictionary == null){
				return null;
			}
			if(SysDictionaryConstant.COMPANY_BUSINESS_TYPE_C_DESIGNER.equals(dictionary.getValuekey())||
					SysDictionaryConstant.COMPANY_BUSINESS_TYPE_DESIGN_COMPANY.equals(dictionary.getValuekey())
					){
				//设计公司和设计师的装企小程序入口先屏蔽掉
				return null;
			}
		}

		BaseCompanyMiniProgramConfig config = this.getMiniProgramByCompanyId(companyId);
		if(config != null){
			return config;
		}else if(Objects.equals(UserTypeConstant.USER_TYPE_DEALERS,sysUser.getUserType())){
			companyId = sysUser.getCompanyId();
			BaseCompanyMiniProgramConfig pConfig = this.getMiniProgramByCompanyId(companyId);
			if(pConfig != null){
				return pConfig;
			}
		}
	...
	@Override
	public BaseCompanyMiniProgramConfig getMiniProgramByCompanyId(Integer companyId) {
		if(companyId == null){
			return null;
		}
		BaseCompanyMiniProgramConfig config = null;
		BaseCompanyMiniProgramConfig configSearch = new BaseCompanyMiniProgramConfig();
		configSearch.setCompanyId(companyId);
		configSearch.setIsDeleted(0);
		List<BaseCompanyMiniProgramConfig> list = this.getBaseCompanyMiniProgramConfigList(configSearch);
