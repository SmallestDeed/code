...
//		LoginUser loginUser = SystemCommonUtil.getLoginUserInfoByAuthData(request);
    ...
    public Object payDesignPlanCopyRight(HttpServletRequest request, @RequestBody PayDesignPlanFree payDesignPlanFree) {
        ...
        if (!userDesignIsExist) {
            try {
                ...
                logger.error("支付信息: =>{}"+payResult.toString());
    ...
    @ResponseBody
    public Object checkDesignCopyRight(Integer recommendedPlanId,Long userId) {
    ...
    @ResponseBody
    public Object replaceDesignPlanPay(Integer recommendedPlanId,Integer designPlanRenderSceneId,
        ...
        try {
            if (recommendedPlanId != null){
                resultMap = userDesignPlanPurchaseRecordService.checkReplaceDesignPlanPay(userId.intValue(),recommendedPlanId,platformCode);
                if (!(boolean)resultMap.get("flag")){
                    //获取方案信息
                    return new ResponseEnvelope<>(true,resultMap);
                }
            }else{
                Integer designPlanId  = userDesignPlanPurchaseRecordService.checkReplaceByDesignPlanRenderSceneId(designPlanRenderSceneId);
                resultMap = userDesignPlanPurchaseRecordService.checkReplaceDesignPlanPay(userId.intValue(),designPlanId,platformCode);
                if (!(boolean)resultMap.get("flag")){
                    //获取方案信息
                    return new ResponseEnvelope<>(true,resultMap);
                }
