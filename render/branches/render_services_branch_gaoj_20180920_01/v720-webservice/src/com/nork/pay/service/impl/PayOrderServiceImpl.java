...
public class PayOrderServiceImpl implements PayOrderService {
    ...
    @Override
    public ResponseEnvelope updateRenderPayState(String orderNo, boolean flag) {
        ResponseEnvelope message = new ResponseEnvelope();
        PayOrder payOrder = payOrderMapper.findOneByOrderNo(orderNo);
        if (payOrder == null) {
            message.setMessage("找不到该订单信息.");
            return message;
        }

        SysUser user= sysUserService.get(payOrder.getUserId());
        if(null == user){
            message.setMessage("找不到用户信息.");
            return message;
        }
        ...
        if (flag) {
            ...
            if (i > 0) {
                ...
                if(!this.updateUserFinance(orderNo)){
                    ...
                    newPayOrder = new PayOrder();
                    newPayOrder.setId(payOrder.getId());
                    newPayOrder.setCurrentIntegral(user.getBalanceAmount().intValue()+payOrder.getTotalFee());
    ...
    @Override
    public void updateOrderByOrderNo(String orderNo, int taskId) throws GeneratePanoramaException {
	    int n = payOrderMapper.updateOrderByOrderNo(orderNo, taskId);
	    logger.error("updateOrderByOrderNo  n =>"+n);
//        if(n != 1){
//            throw new GeneratePanoramaException(false, "更新订单失败，请联系管理员");
//        }
    }
     ...
     * @return
     */
    private boolean updateUserFinance(String orderNo) {
        PayOrder order = findOneByOrderNo(orderNo);
        if (order == null) {
            return false;
        }
        ...
        if (order.getPayState() != null && order.getBizType() != null) {
            if(order.getFinanceType().equalsIgnoreCase(FinanceType.OUT)){
                if (order.getPayState().equalsIgnoreCase(PayState.SUCCESS)
                    ...
                    return sysUserService.updateFinance(user);
                }
                if (order.getPayState().equalsIgnoreCase(PayState.SUCCESS)
            ...
            if(order.getFinanceType().equalsIgnoreCase(FinanceType.IN)){
                if (order.getPayState().equalsIgnoreCase(PayState.SUCCESS)
                        && order.getBizType().equalsIgnoreCase(BizType.REFUND)) {
                    user.setBalanceAmount(Double.parseDouble(order.getTotalFee().toString()));
                    user.setConsumAmount(0-Double.parseDouble(order.getTotalFee().toString()));
                    return sysUserService.updateFinance(user);
                }
            }
        }
