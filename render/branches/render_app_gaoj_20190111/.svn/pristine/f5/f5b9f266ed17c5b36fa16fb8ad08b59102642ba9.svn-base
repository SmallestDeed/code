...
public class ShareController {
    ...
    // 720 H5场景分享访问的前端地址
    private static final String SCENE_SHARE_H5_QRCODE_URL = Utils.getPropertyName("config/share","scene.share.H5.qrcode.url","");
    //720 PC端浏览场景访问的前端地址
    private static final String SCENE_SHARE_PC_QRCODE_URL = Utils.getPropertyName("config/share","scene.share.PC.qrcode.url","");
            ...
            if(Objects.equals(SceneDataSearch.QRCodeType.QRCODE_TYPE_BROWSE,qrCodeType)){
                ...
                if(Objects.equals(SceneDataSearch.QRCodeType.QRCODE_TYPE_SELECTDECORATION,qrCodeType)){
                    ...
                    try {
                        ...
                        return new ResponseEnvelope(false, "获取access_token出现异常，生成二维吗失败!", msgId);
                    }
                }
    ...
    public Object getQRCodeTypeCount(HttpServletRequest request, HttpServletResponse response,String msgId){
        ...
        //二维码的数量
        int count = 2;
        Integer companyId = loginUser.getBusinessAdministrationId();
        if(!Objects.equals(SANDU_COMPANY_ID,companyId)){
            BaseCompanyMiniProgramConfig configSearch = new BaseCompanyMiniProgramConfig();
            configSearch.setCompanyId(companyId.longValue());
            configSearch.setIsDeleted(0);
            List<BaseCompanyMiniProgramConfig> list = baseCompanyService.getBaseCompanyMiniProgramConfigList(configSearch);
...
//        Object access_token_object = cacher.get("SAMLL_ACCESS_TOKEN");
    ...
    private String getaccess_token(Integer userId) throws  BizException{
        ...
        if(user.getBusinessAdministrationId() == null){
            throw new BizException("当前用户缺失企业信息!");
        }
        Integer companyId = user.getBusinessAdministrationId();
        BaseCompanyMiniProgramConfig configSearch = new BaseCompanyMiniProgramConfig();
        configSearch.setCompanyId(companyId.longValue());
        configSearch.setIsDeleted(0);
        List<BaseCompanyMiniProgramConfig> miniProgramConfigs = baseCompanyService.getBaseCompanyMiniProgramConfigList(configSearch);
        if(CustomerListUtils.isEmpty(miniProgramConfigs)){
            throw new BizException("该企业没有企业小程序!");
        }
        if(miniProgramConfigs.size() > 1 && Objects.equals(SANDU_COMPANY_ID,companyId) ){
            //如果是三度云享家 ，则不显示企业小程序
            throw new BizException("该企业没有企业小程序!");
        }
        BaseCompanyMiniProgramConfig config = miniProgramConfigs.get(0);
