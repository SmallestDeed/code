...
//		LoginUser loginUser = SystemCommonUtil.getLoginUserInfoByAuthData(request);
    	...
    	if (resRenderPic.getTaskSource().intValue() == 1) { 
			...
			logger.error("运营网站getOrderNoWeb====userId="+userId+"&renderTaskType="+renderTaskType+"&planId="+planId+"&houseId="+houseId+"&token"+token+"&platformCode"+platformCode+"&companyDomainName="+companyDomainName);
