...
public class AddRenderTaskController {
    ...
    public final static Integer PLAN_FULL_HOUSE_TYPE = 2;//全屋方案一次性创建渲染逻辑
    public final static Integer PLAN_FULL_HOUSE_NEW_TYPE = 3;//全屋方案新逻辑
...
//		LoginUser loginUser = SystemCommonUtil.getLoginUserInfoByAuthData(request);
        ...
        if (null == resRenderPic.getHouseId()) {
            resRenderPic.setHouseId(0);
        }
        Integer houseId = null==resRenderPic.getHouseId()?0:resRenderPic.getHouseId();
        //区分推荐方案ID 或副本方案ID
        Integer planId = null;
        //订单号
        String orderNo = null;
        ...
        logger.error("调用时间1："+ (System.currentTimeMillis()-startTime));
        ...
        } else if (PLAN_FULL_HOUSE_NEW_TYPE.equals(resRenderPic.getPlanHouseType())) {
            responseEnvelope = autoRenderTaskApi.createFullHouseNewTask(resRenderPic);
        } else {
            responseEnvelope = autoRenderTaskApi.createAutoRenderTask(resRenderPic);
        }

        logger.error("调用时间2："+ (System.currentTimeMillis()-startTime));
        startTime = System.currentTimeMillis();

        // 调用春节活动相关接口
        // try {
        //     Long activityId = userHouseRecordService.getSpringActivityId(userId.longValue());
        //     HttpClient client = HttpClient.getInstance();
        //     if (activityId != null) {
        //         Map<String, String> stateMap = userHouseRecordService.getUserTaskState(userId.longValue(), activityId);
        //         if (resRenderPic.getTaskType() != null) {
        //             if ((resRenderPic.getTaskType() == 0 && "1".equals(stateMap.get("2"))) ||
        //                     (resRenderPic.getTaskType() == 1 && "1".equals(stateMap.get("3")))) {
        //                 String doneTask = Utils.getPropertyName("app", "spring.activity.done.task", "https://core.sanduspace.com/v1/core/filmTicketActivity/doneTask");
        //                 logger.error(CLASS_LOG_PREFIX + "调用春节活动接口doneTaskUrl="+doneTask);
        //                 String response = client.sendHttpPost(doneTask, request.getHeader("Authorization"), "activityId=" + activityId.longValue());
        //                 logger.error(CLASS_LOG_PREFIX + "调用春节活动接口/doneTask返回值="+response);
        //             }
        //         }
        //         String increase = Utils.getPropertyName("app", "spring.activity.increase.lottery.rate", "https://core.sanduspace.com/v1/core/filmTicketActivity/increaseLotteryRate");
        //         logger.error(CLASS_LOG_PREFIX + "调用春节活动接口increaseUrl="+increase);
        //         String response = client.sendHttpPost(increase, request.getHeader("Authorization"), "activityId=" + activityId.longValue());
        //         logger.error(CLASS_LOG_PREFIX + "调用春节活动接口/increaseLotteryRate返回值="+response);
        //     }
        // }catch (Exception e) {
        //     logger.error(CLASS_LOG_PREFIX + "调用春节活动接口异常", e);
        // }

        logger.error("渲染完成，返回值："+responseEnvelope);
    ...
    @RequestMapping(value = "/payDesignPlanCopyRight")
    @ResponseBody
    public Object payDesignPlanCopyRight(HttpServletRequest request,@RequestBody PayDesignPlanFree payDesignPlanFree) {

        if (Objects.isNull(payDesignPlanFree.getPlanRecommendedId())){
            return new ResponseEnvelope<>(false,"parameter planRecommendedId is null");
        }

        if (org.apache.commons.lang3.StringUtils.isEmpty(request.getHeader("Platform-Code"))){
            return new ResponseEnvelope<>(false,"parameter Platform-Code is null");
        }
        ...
        Boolean userDesignIsExist = companyDesignPlanIncomeService.isExitsBuyDesignPlanCopyRight(userId, payDesignPlanFree.getPlanRecommendedId().intValue(),payDesignPlanFree.getPlanType());

        logger.info("current DesignPlan is pay? userDesignIsExist =>{}" + userDesignIsExist);
        Map<String, Object> resultMap = new HashMap<>();
        if (!userDesignIsExist) {
            try {
                resultMap =  payAccountService.handlerUserPayDubi(payDesignPlanFree);
    ...
    @RequestMapping(value = "/checkDesignCopyRight")
    @ResponseBody
    public Object checkDesignCopyRight(Integer recommendedPlanId,Integer planType) {

        Map map = LoginContext.getTokenData();
        Long userId = (Long) map.get("uid");

        if (Objects.isNull(recommendedPlanId)) {
        ...
        try {
            Boolean isExits = companyDesignPlanIncomeService.isExitsBuyDesignPlanCopyRight(userId, recommendedPlanId, planType);
            if (isExits) {
                return new ResponseEnvelope<>(true, isExits);
            }
        } catch (Exception e) {
            return new ResponseEnvelope<>(false, "系统错误");
        }
        return new ResponseEnvelope<>(false, Boolean.FALSE);
    }
    ...
    public void callBack(HttpServletRequest request) {
        ...
        try {
            ...
            //插入企业设计方案推荐消费记录
            userDesignPlanPurchaseRecordService.addCompanyDesignPlanIncome(userDesignPlanPurchaseRecord, userDesignPlanPurchaseRecord.getUseType());
    ...
    @ResponseBody
    public Object replaceDesignPlanPay(Integer recommendedPlanId, Integer designPlanRenderSceneId,Integer planType,
                                       String uuid,
        ...
        try {
            if (Objects.nonNull(recommendedPlanId)) {
                resultMap = companyDesignPlanIncomeService.checkReplaceDesignPlanPay(userId, recommendedPlanId, platformCode,planType);
                if (!(boolean) resultMap.get("flag")) {
                    //获取方案信息
                    return new ResponseEnvelope<>(true, resultMap);
                }
            } else if (Objects.nonNull(designPlanRenderSceneId)){
                /*Integer designPlanId = companyDesignPlanIncomeService.planSceneIdTransformRecommendedPlanId(designPlanRenderSceneId);
                resultMap = companyDesignPlanIncomeService.checkReplaceDesignPlanPay(userId, designPlanId, platformCode,planType);
                if (!(boolean) resultMap.get("flag")) {
                    //获取方案信息
                    return new ResponseEnvelope<>(true, resultMap);
                }*/
                resultMap.put("flag",Boolean.TRUE);
                resultMap.put("designPlanRecommended",new DesignPlanRecommended());
                return new ResponseEnvelope<>(true,resultMap);
            }else if (org.apache.commons.lang3.StringUtils.isNotEmpty(uuid)){
                //全屋方案 =>{} 获取全屋方案的id
                Integer fullHouseId = companyDesignPlanIncomeService.getFullHouseDesignPlanId(uuid);
                resultMap = companyDesignPlanIncomeService.checkReplaceDesignPlanPay(userId, fullHouseId, platformCode,planType);
                if (!(boolean) resultMap.get("flag")) {
                    //获取方案信息
                    return new ResponseEnvelope<>(true, resultMap);
                }
