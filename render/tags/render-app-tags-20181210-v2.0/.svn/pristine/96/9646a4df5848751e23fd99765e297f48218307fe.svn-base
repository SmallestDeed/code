...
public class WebDesignPlanAutoRenderController {
	...
	public ResponseEnvelope<AutoRenderTask> getRenderTaskList(HttpServletRequest request, HttpServletResponse response,Integer maxSize,String msgId
		...
		if (preTaskId != null && preTaskId > 0) {
			AutoRenderTaskState autoRenderTaskState = designPlanAutoRenderService.getStateByTaskId(preTaskId);
			if (autoRenderTaskState != null) {
				if (autoRenderTaskState.getState().intValue() == 2 ) {
					...
					designPlanAutoRenderService.updateAutoRenderTaskState(taskState,msgId,token,platformCode);
				}
			}
		}
		...
		// 置顶任务 最高优先级
		AutoRenderTask stickRenderTask = designPlanAutoRenderService.getRedisStickTaskList(maxSize, loginUser,renderMachineIp,renderLevel,renderProgramVersion);
		...
		if (stickRenderTask != null && stickRenderTask.getId() > 0) {
			...
			/*replaceRenderTask = designPlanAutoRenderService.getRedisReplaceTaskList(maxSize, loginUser);
			if (replaceRenderTask != null && replaceRenderTask.getId() > 0) {
				taskList.add(replaceRenderTask);
			} else {
				autoRenderTask = designPlanAutoRenderService.getRedisTaskList(maxSize, loginUser);
			...
			do {
				 ...
				 **/
				replaceRenderTask = designPlanAutoRenderService.getRedisReplaceTaskList(maxSize, loginUser,renderMachineIp,renderLevel,renderProgramVersion);
			...
			} while ((JedisUtils2.getSizeOfList(SystemCommonConstant.REDIS_TASK_REPLACE_LIST) > 0
					&& taskList.size() == 0));

			if (taskList.size() == 0) {
				do {
					// 自动渲染任务 第三优先级
					autoRenderTask = designPlanAutoRenderService.getRedisTaskList(maxSize, loginUser,renderMachineIp,renderLevel,renderProgramVersion);
					// 查看渲染状态是否有存在 如果存在取下一条
					if (autoRenderTask != null) {
						logger.error("getRedisTaskList get replace taskId=>" + autoRenderTask.getId());
						taskList.add(autoRenderTask);
					}
				} while ((JedisUtils2.getSizeOfList(SystemCommonConstant.REDIS_TASK_LIST) > 0 && taskList.size() == 0));
			}
...
//		LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
		...
		return designPlanAutoRenderService.updateAutoRenderTaskState(taskState, msgId,token,platformCode);
	}
	...
	public void addRedisRenderStick(HttpServletRequest request,HttpServletResponse response,Integer taskId){
    	...
    	designPlanAutoRenderService.getRedisStickList(taskId);
	}
}
