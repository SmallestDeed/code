...
//			if (!request.getRequestURI().endsWith(mobileLoginUrl) && request.getAttribute("AuthorizationData") != null ) {
//				LoginUser currentUser = SystemCommonUtil.getCurrentLoginUserInfo(request);
//	            if(currentUser == null) {
//	               outputMSg.put("success", false);
//	               outputMSg.put("message", "您的登陆信息已过期，请重新登录！");
...
//			}
		...
		for (String s : str) {
			...
			if (StringUtils.isNotEmpty(ss) && currentPath.indexOf(ss) != -1) {
				...
				//logger.error("except url==>" + "msgId=" + currentMsgId + ";requestUrl=" + currentURL);
				if (StringUtils.isNotBlank(authorization)) {
					loginUserInfo = (LoginUser) new JsonDataServiceImpl<LoginUser>().getJsonToBean(authorization,LoginUser.class);
		...
		if(StringUtils.isNotBlank(IS_DEBUG) && "true".equals(IS_DEBUG)){
			...
			net.sf.json.JSONObject jsonObject = net.sf.json.JSONObject.fromObject(authorizationStr);
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("uname", jsonObject.getString("loginName"));
			map.put("uid", Long.valueOf(jsonObject.getString("id")));
			map.put("mtype", 3L);
			map.put("utype", 1L);
			map.put("ukey", "aaa");
			map.put("appkey", jsonObject.getString("appkey"));
			request.setAttribute("AuthorizationData", map);
			// test ->end
		...
		if(StringUtils.isNotBlank(authorization) && currentURL.lastIndexOf(".htm") != -1) {
			loginUserInfo = (LoginUser) new JsonDataServiceImpl<LoginUser>().getJsonToBean(authorization,
					LoginUser.class);
    ...
    private boolean checkOneLogin(FilterChain arg2, HttpServletRequest request,
        ...
        boolean checkResult = true;
          ...
          user = (LoginUser) CacheManager.getInstance().getCacher().getObject(loginFlag + appkey);
          if(user == null ) {
             logger.error("user为空");
          ...
          loginUser = (LoginUser) CacheManager.getInstance().getCacher().getObject(loginFlag + user.getId().toString());
          ...
          if(!appkey.equals(loginUser.getAppKey())) {
                ...
                //移除之前登录用户的缓存
                CacheManager.getInstance().getCacher().remove((loginFlag+appkey));
                outputMSg.put("success", false);
                outputMSg.put("CheckOneLogin", true);
                outputMSg.put("message", "当前账号已在其他设备登录，请重新登录");
                output(outputMSg.toJSONString(), response);
