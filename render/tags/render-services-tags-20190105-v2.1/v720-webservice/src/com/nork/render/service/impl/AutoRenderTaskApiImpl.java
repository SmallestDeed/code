...
public class AutoRenderTaskApiImpl implements IAutoRenderTaskApi{
    ...
    public ResponseEnvelope createAutoRenderTask(ResRenderPic resRenderPic) {
        ...
        //参数校验
        mess = this.checkParam(resRenderPic);
        if (mess != null) {
            return mess;
        }
    ...
    @Override
    public ResponseEnvelope createFullHouseAutoRenderTask(ResRenderPic resRenderPic) {
        ResponseEnvelope mess = null;
        //参数校验
        mess = this.checkParam(resRenderPic);
        if (mess != null) {
        ...
        if( resRenderPic.getTaskType().intValue() == AutoRenderTaskConstant.TASK_TYPE_AUTO_RENDER){
            // 装进我家接口服务
            try {
                mess = autoRenderService.handleFullHouseAutoRenderProcessor(resRenderPic);
            } catch (GeneratePanoramaException e) {
                mess = new ResponseEnvelope(false,e.getMessage());
            } catch (Exception e) {
                mess = new ResponseEnvelope(false,e.getMessage());
            }
        }else if( resRenderPic.getTaskType().intValue() == AutoRenderTaskConstant.TASK_TYPE_REPLACE){
            // 替换渲染服务
            try {
                mess = autoRenderService.handleFullHouseAutoReplaceProcessor(resRenderPic);
            } catch (GeneratePanoramaException e) {
                mess = new ResponseEnvelope(false,e.getMessage());
            } catch (Exception e) {
                mess = new ResponseEnvelope(false,e.getMessage());
    ...
    @Override
    public ResponseEnvelope createFullHouseNewTask(ResRenderPic resRenderPic) {
        ResponseEnvelope mess = null;
        //参数校验
        mess = this.checkParam(resRenderPic);
        if (mess != null) {
        ...
        if( resRenderPic.getTaskType().intValue() == AutoRenderTaskConstant.TASK_TYPE_AUTO_RENDER){
            // 装进我家接口服务
            try {
                mess = autoRenderService.handleFullHouseAutoRenderNewProcessor(resRenderPic);
            } catch (GeneratePanoramaException e) {
                mess = new ResponseEnvelope(false,e.getMessage());
            } catch (Exception e) {
                mess = new ResponseEnvelope(false,e.getMessage());
    ...
    @Override
    public boolean addRenderTaskStateList(List<Integer> taskIdList) {
    ...
    //参数校验
    private ResponseEnvelope checkParam(ResRenderPic resRenderPic) {
