...
public class StructureProductServiceImpl implements StructureProductService {
	...
	private static Logger logger = Logger.getLogger(ResDesignService.class);
...
//				//System.out.println(cameraLook.length());
				...
				if(flag){
					...
					List<DesignTempletProduct> designTempletProductList=templetProductService.getPaginatedList(designTempletProductSearch);
			...
			if(flag){
				for(DesignTempletProduct designTempletProduct : designTempletProductListFLag){
					templetProductService.update(designTempletProduct);
				}
			}
		}
		//读取配置文件信息,保存结构明细->end
