...
//		LoginUser loginUser = SystemCommonUtil.getLoginUserInfoByAuthData(request);
        ...
        // 调用春节活动相关接口
        try {
            Long activityId = userHouseRecordService.getSpringActivityId(userId);
            if (activityId != null) {
                HttpClient client = HttpClient.getInstance();
                String doneTask = Utils.getPropertyName("app", "spring.activity.done.task", "");
                String increase = Utils.getPropertyName("app", "spring.activity.increase.lottery.rate", "");
