package com.nork.pay.model.search;

import java.io.Serializable;
import java.util.Date;

import com.nork.pay.model.PayRefund;

/**
 * @Title: PayRefundSearch.java
 * @Package com.nork.pay.model
 * @Description:支付-支付退款查询对象
 * @createAuthor pandajun
 * @CreateDate 2016-09-22 14:41:47
 * @version V1.0
 */
public class PayRefundSearch extends PayRefund implements Serializable {
	private static final long serialVersionUID = 1L;
	private String beginDate;
	private String endDate;
	/** 参考号-模糊查询 **/
	private String sch_RefId_;
	/** 参考号-左模糊查询 **/
	private String sch_RefId;
	/** 参考号-右模糊查询 **/
	private String schRefId_;
	/** 参考号-区间查询-开始字符串 **/
	private String refIdStart;
	/** 参考号-区间查询-结束字符串 **/
	private String refIdEnd;
	/** 退款号-模糊查询 **/
	private String sch_RefundNo_;
	/** 退款号-左模糊查询 **/
	private String sch_RefundNo;
	/** 退款号-右模糊查询 **/
	private String schRefundNo_;
	/** 退款号-区间查询-开始字符串 **/
	private String refundNoStart;
	/** 退款号-区间查询-结束字符串 **/
	private String refundNoEnd;
	/** 退款原因说明-模糊查询 **/
	private String sch_RefundReason_;
	/** 退款原因说明-左模糊查询 **/
	private String sch_RefundReason;
	/** 退款原因说明-右模糊查询 **/
	private String schRefundReason_;
	/** 退款原因说明-区间查询-开始字符串 **/
	private String refundReasonStart;
	/** 退款原因说明-区间查询-结束字符串 **/
	private String refundReasonEnd;
	/** 退款状态-模糊查询 **/
	private String sch_RefundState_;
	/** 退款状态-左模糊查询 **/
	private String sch_RefundState;
	/** 退款状态-右模糊查询 **/
	private String schRefundState_;
	/** 退款状态-区间查询-开始字符串 **/
	private String refundStateStart;
	/** 退款状态-区间查询-结束字符串 **/
	private String refundStateEnd;
	/** 审核意见-模糊查询 **/
	private String sch_AuditOpinion_;
	/** 审核意见-左模糊查询 **/
	private String sch_AuditOpinion;
	/** 审核意见-右模糊查询 **/
	private String schAuditOpinion_;
	/** 审核意见-区间查询-开始字符串 **/
	private String auditOpinionStart;
	/** 审核意见-区间查询-结束字符串 **/
	private String auditOpinionEnd;
	/** 结果详情-模糊查询 **/
	private String sch_ResultDetails_;
	/** 结果详情-左模糊查询 **/
	private String sch_ResultDetails;
	/** 结果详情-右模糊查询 **/
	private String schResultDetails_;
	/** 结果详情-区间查询-开始字符串 **/
	private String resultDetailsStart;
	/** 结果详情-区间查询-结束字符串 **/
	private String resultDetailsEnd;
	/** 退款时间-区间查询-开始时间 **/
	private Date refundDateStart;
	/** 退款时间-区间查询-结束时间 **/
	private Date refundDateEnd;
	/** 系统编码-模糊查询 **/
	private String sch_SysCode_;
	/** 系统编码-左模糊查询 **/
	private String sch_SysCode;
	/** 系统编码-右模糊查询 **/
	private String schSysCode_;
	/** 系统编码-区间查询-开始字符串 **/
	private String sysCodeStart;
	/** 系统编码-区间查询-结束字符串 **/
	private String sysCodeEnd;
	/** 创建者-模糊查询 **/
	private String sch_Creator_;
	/** 创建者-左模糊查询 **/
	private String sch_Creator;
	/** 创建者-右模糊查询 **/
	private String schCreator_;
	/** 创建者-区间查询-开始字符串 **/
	private String creatorStart;
	/** 创建者-区间查询-结束字符串 **/
	private String creatorEnd;
	/** 创建时间-区间查询-开始时间 **/
	private Date gmtCreateStart;
	/** 创建时间-区间查询-结束时间 **/
	private Date gmtCreateEnd;
	/** 修改人-模糊查询 **/
	private String sch_Modifier_;
	/** 修改人-左模糊查询 **/
	private String sch_Modifier;
	/** 修改人-右模糊查询 **/
	private String schModifier_;
	/** 修改人-区间查询-开始字符串 **/
	private String modifierStart;
	/** 修改人-区间查询-结束字符串 **/
	private String modifierEnd;
	/** 修改时间-区间查询-开始时间 **/
	private Date gmtModifiedStart;
	/** 修改时间-区间查询-结束时间 **/
	private Date gmtModifiedEnd;
	/** 字符备用1-模糊查询 **/
	private String sch_Att1_;
	/** 字符备用1-左模糊查询 **/
	private String sch_Att1;
	/** 字符备用1-右模糊查询 **/
	private String schAtt1_;
	/** 字符备用1-区间查询-开始字符串 **/
	private String att1Start;
	/** 字符备用1-区间查询-结束字符串 **/
	private String att1End;
	/** 字符备用2-模糊查询 **/
	private String sch_Att2_;
	/** 字符备用2-左模糊查询 **/
	private String sch_Att2;
	/** 字符备用2-右模糊查询 **/
	private String schAtt2_;
	/** 字符备用2-区间查询-开始字符串 **/
	private String att2Start;
	/** 字符备用2-区间查询-结束字符串 **/
	private String att2End;
	/** 备注-模糊查询 **/
	private String sch_Remark_;
	/** 备注-左模糊查询 **/
	private String sch_Remark;
	/** 备注-右模糊查询 **/
	private String schRemark_;
	/** 备注-区间查询-开始字符串 **/
	private String remarkStart;
	/** 备注-区间查询-结束字符串 **/
	private String remarkEnd;

	public String getSch_RefId_() {
		return sch_RefId_;
	}

	public void setSch_RefId_(String sch_RefId_) {
		this.sch_RefId_ = sch_RefId_;
	}

	public String getSch_RefId() {
		return sch_RefId;
	}

	public void setSch_RefId(String sch_RefId) {
		this.sch_RefId = sch_RefId;
	}

	public String getSchRefId_() {
		return schRefId_;
	}

	public void setSchRefId_(String schRefId_) {
		this.schRefId_ = schRefId_;
	}

	public String getRefIdStart() {
		return refIdStart;
	}

	public void setRefIdStart(String refIdStart) {
		this.refIdStart = refIdStart;
	}

	public String getRefIdEnd() {
		return refIdEnd;
	}

	public void setRefIdEnd(String refIdEnd) {
		this.refIdEnd = refIdEnd;
	}

	public String getSch_RefundNo_() {
		return sch_RefundNo_;
	}

	public void setSch_RefundNo_(String sch_RefundNo_) {
		this.sch_RefundNo_ = sch_RefundNo_;
	}

	public String getSch_RefundNo() {
		return sch_RefundNo;
	}

	public void setSch_RefundNo(String sch_RefundNo) {
		this.sch_RefundNo = sch_RefundNo;
	}

	public String getSchRefundNo_() {
		return schRefundNo_;
	}

	public void setSchRefundNo_(String schRefundNo_) {
		this.schRefundNo_ = schRefundNo_;
	}

	public String getRefundNoStart() {
		return refundNoStart;
	}

	public void setRefundNoStart(String refundNoStart) {
		this.refundNoStart = refundNoStart;
	}

	public String getRefundNoEnd() {
		return refundNoEnd;
	}

	public void setRefundNoEnd(String refundNoEnd) {
		this.refundNoEnd = refundNoEnd;
	}

	public String getSch_RefundReason_() {
		return sch_RefundReason_;
	}

	public void setSch_RefundReason_(String sch_RefundReason_) {
		this.sch_RefundReason_ = sch_RefundReason_;
	}

	public String getSch_RefundReason() {
		return sch_RefundReason;
	}

	public void setSch_RefundReason(String sch_RefundReason) {
		this.sch_RefundReason = sch_RefundReason;
	}

	public String getSchRefundReason_() {
		return schRefundReason_;
	}

	public void setSchRefundReason_(String schRefundReason_) {
		this.schRefundReason_ = schRefundReason_;
	}

	public String getRefundReasonStart() {
		return refundReasonStart;
	}

	public void setRefundReasonStart(String refundReasonStart) {
		this.refundReasonStart = refundReasonStart;
	}

	public String getRefundReasonEnd() {
		return refundReasonEnd;
	}

	public void setRefundReasonEnd(String refundReasonEnd) {
		this.refundReasonEnd = refundReasonEnd;
	}

	public String getSch_RefundState_() {
		return sch_RefundState_;
	}

	public void setSch_RefundState_(String sch_RefundState_) {
		this.sch_RefundState_ = sch_RefundState_;
	}

	public String getSch_RefundState() {
		return sch_RefundState;
	}

	public void setSch_RefundState(String sch_RefundState) {
		this.sch_RefundState = sch_RefundState;
	}

	public String getSchRefundState_() {
		return schRefundState_;
	}

	public void setSchRefundState_(String schRefundState_) {
		this.schRefundState_ = schRefundState_;
	}

	public String getRefundStateStart() {
		return refundStateStart;
	}

	public void setRefundStateStart(String refundStateStart) {
		this.refundStateStart = refundStateStart;
	}

	public String getRefundStateEnd() {
		return refundStateEnd;
	}

	public void setRefundStateEnd(String refundStateEnd) {
		this.refundStateEnd = refundStateEnd;
	}

	public String getSch_AuditOpinion_() {
		return sch_AuditOpinion_;
	}

	public void setSch_AuditOpinion_(String sch_AuditOpinion_) {
		this.sch_AuditOpinion_ = sch_AuditOpinion_;
	}

	public String getSch_AuditOpinion() {
		return sch_AuditOpinion;
	}

	public void setSch_AuditOpinion(String sch_AuditOpinion) {
		this.sch_AuditOpinion = sch_AuditOpinion;
	}

	public String getSchAuditOpinion_() {
		return schAuditOpinion_;
	}

	public void setSchAuditOpinion_(String schAuditOpinion_) {
		this.schAuditOpinion_ = schAuditOpinion_;
	}

	public String getAuditOpinionStart() {
		return auditOpinionStart;
	}

	public void setAuditOpinionStart(String auditOpinionStart) {
		this.auditOpinionStart = auditOpinionStart;
	}

	public String getAuditOpinionEnd() {
		return auditOpinionEnd;
	}

	public void setAuditOpinionEnd(String auditOpinionEnd) {
		this.auditOpinionEnd = auditOpinionEnd;
	}

	public String getSch_ResultDetails_() {
		return sch_ResultDetails_;
	}

	public void setSch_ResultDetails_(String sch_ResultDetails_) {
		this.sch_ResultDetails_ = sch_ResultDetails_;
	}

	public String getSch_ResultDetails() {
		return sch_ResultDetails;
	}

	public void setSch_ResultDetails(String sch_ResultDetails) {
		this.sch_ResultDetails = sch_ResultDetails;
	}

	public String getSchResultDetails_() {
		return schResultDetails_;
	}

	public void setSchResultDetails_(String schResultDetails_) {
		this.schResultDetails_ = schResultDetails_;
	}

	public String getResultDetailsStart() {
		return resultDetailsStart;
	}

	public void setResultDetailsStart(String resultDetailsStart) {
		this.resultDetailsStart = resultDetailsStart;
	}

	public String getResultDetailsEnd() {
		return resultDetailsEnd;
	}

	public void setResultDetailsEnd(String resultDetailsEnd) {
		this.resultDetailsEnd = resultDetailsEnd;
	}

	public Date getRefundDateStart() {
		return refundDateStart;
	}

	public void setRefundDateStart(Date refundDateStart) {
		this.refundDateStart = refundDateStart;
	}

	public Date getRefundDateEnd() {
		return refundDateEnd;
	}

	public void setRefundDateEnd(Date refundDateEnd) {
		this.refundDateEnd = refundDateEnd;
	}

	public String getSch_SysCode_() {
		return sch_SysCode_;
	}

	public void setSch_SysCode_(String sch_SysCode_) {
		this.sch_SysCode_ = sch_SysCode_;
	}

	public String getSch_SysCode() {
		return sch_SysCode;
	}

	public void setSch_SysCode(String sch_SysCode) {
		this.sch_SysCode = sch_SysCode;
	}

	public String getSchSysCode_() {
		return schSysCode_;
	}

	public void setSchSysCode_(String schSysCode_) {
		this.schSysCode_ = schSysCode_;
	}

	public String getSysCodeStart() {
		return sysCodeStart;
	}

	public void setSysCodeStart(String sysCodeStart) {
		this.sysCodeStart = sysCodeStart;
	}

	public String getSysCodeEnd() {
		return sysCodeEnd;
	}

	public void setSysCodeEnd(String sysCodeEnd) {
		this.sysCodeEnd = sysCodeEnd;
	}

	public String getSch_Creator_() {
		return sch_Creator_;
	}

	public void setSch_Creator_(String sch_Creator_) {
		this.sch_Creator_ = sch_Creator_;
	}

	public String getSch_Creator() {
		return sch_Creator;
	}

	public void setSch_Creator(String sch_Creator) {
		this.sch_Creator = sch_Creator;
	}

	public String getSchCreator_() {
		return schCreator_;
	}

	public void setSchCreator_(String schCreator_) {
		this.schCreator_ = schCreator_;
	}

	public String getCreatorStart() {
		return creatorStart;
	}

	public void setCreatorStart(String creatorStart) {
		this.creatorStart = creatorStart;
	}

	public String getCreatorEnd() {
		return creatorEnd;
	}

	public void setCreatorEnd(String creatorEnd) {
		this.creatorEnd = creatorEnd;
	}

	public Date getGmtCreateStart() {
		return gmtCreateStart;
	}

	public void setGmtCreateStart(Date gmtCreateStart) {
		this.gmtCreateStart = gmtCreateStart;
	}

	public Date getGmtCreateEnd() {
		return gmtCreateEnd;
	}

	public void setGmtCreateEnd(Date gmtCreateEnd) {
		this.gmtCreateEnd = gmtCreateEnd;
	}

	public String getSch_Modifier_() {
		return sch_Modifier_;
	}

	public void setSch_Modifier_(String sch_Modifier_) {
		this.sch_Modifier_ = sch_Modifier_;
	}

	public String getSch_Modifier() {
		return sch_Modifier;
	}

	public void setSch_Modifier(String sch_Modifier) {
		this.sch_Modifier = sch_Modifier;
	}

	public String getSchModifier_() {
		return schModifier_;
	}

	public void setSchModifier_(String schModifier_) {
		this.schModifier_ = schModifier_;
	}

	public String getModifierStart() {
		return modifierStart;
	}

	public void setModifierStart(String modifierStart) {
		this.modifierStart = modifierStart;
	}

	public String getModifierEnd() {
		return modifierEnd;
	}

	public void setModifierEnd(String modifierEnd) {
		this.modifierEnd = modifierEnd;
	}

	public Date getGmtModifiedStart() {
		return gmtModifiedStart;
	}

	public void setGmtModifiedStart(Date gmtModifiedStart) {
		this.gmtModifiedStart = gmtModifiedStart;
	}

	public Date getGmtModifiedEnd() {
		return gmtModifiedEnd;
	}

	public void setGmtModifiedEnd(Date gmtModifiedEnd) {
		this.gmtModifiedEnd = gmtModifiedEnd;
	}

	public String getSch_Att1_() {
		return sch_Att1_;
	}

	public void setSch_Att1_(String sch_Att1_) {
		this.sch_Att1_ = sch_Att1_;
	}

	public String getSch_Att1() {
		return sch_Att1;
	}

	public void setSch_Att1(String sch_Att1) {
		this.sch_Att1 = sch_Att1;
	}

	public String getSchAtt1_() {
		return schAtt1_;
	}

	public void setSchAtt1_(String schAtt1_) {
		this.schAtt1_ = schAtt1_;
	}

	public String getAtt1Start() {
		return att1Start;
	}

	public void setAtt1Start(String att1Start) {
		this.att1Start = att1Start;
	}

	public String getAtt1End() {
		return att1End;
	}

	public void setAtt1End(String att1End) {
		this.att1End = att1End;
	}

	public String getSch_Att2_() {
		return sch_Att2_;
	}

	public void setSch_Att2_(String sch_Att2_) {
		this.sch_Att2_ = sch_Att2_;
	}

	public String getSch_Att2() {
		return sch_Att2;
	}

	public void setSch_Att2(String sch_Att2) {
		this.sch_Att2 = sch_Att2;
	}

	public String getSchAtt2_() {
		return schAtt2_;
	}

	public void setSchAtt2_(String schAtt2_) {
		this.schAtt2_ = schAtt2_;
	}

	public String getAtt2Start() {
		return att2Start;
	}

	public void setAtt2Start(String att2Start) {
		this.att2Start = att2Start;
	}

	public String getAtt2End() {
		return att2End;
	}

	public void setAtt2End(String att2End) {
		this.att2End = att2End;
	}

	public String getSch_Remark_() {
		return sch_Remark_;
	}

	public void setSch_Remark_(String sch_Remark_) {
		this.sch_Remark_ = sch_Remark_;
	}

	public String getSch_Remark() {
		return sch_Remark;
	}

	public void setSch_Remark(String sch_Remark) {
		this.sch_Remark = sch_Remark;
	}

	public String getSchRemark_() {
		return schRemark_;
	}

	public void setSchRemark_(String schRemark_) {
		this.schRemark_ = schRemark_;
	}

	public String getRemarkStart() {
		return remarkStart;
	}

	public void setRemarkStart(String remarkStart) {
		this.remarkStart = remarkStart;
	}

	public String getRemarkEnd() {
		return remarkEnd;
	}

	public void setRemarkEnd(String remarkEnd) {
		this.remarkEnd = remarkEnd;
	}

	public String getBeginDate() {
		return beginDate;
	}

	public void setBeginDate(String beginDate) {
		this.beginDate = beginDate;
	}

	public String getEndDate() {
		return endDate;
	}

	public void setEndDate(String endDate) {
		this.endDate = endDate;
	}

}
