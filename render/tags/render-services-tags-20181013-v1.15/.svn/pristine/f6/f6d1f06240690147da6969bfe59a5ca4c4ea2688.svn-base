...
public class MobileAutoRenderAndOneKeyCopyServiceImpl implements MobileAutoRenderAndOneKeyCopyService {
	...
	public Integer transformAndCopyPlan(Integer planId, LoginUser loginUser, Integer platformId) {
		//获取登录用户信息
		DesignPlanRes autoandonekey = optimizeDesingPlanService.getAutoRenderDesignPlanRes(planId);
		Integer designPlanSceneId = optimizeDesingPlanService.saveAsRenderBakScene(autoandonekey, loginUser,platformId);//复制到效果图的方案ID
