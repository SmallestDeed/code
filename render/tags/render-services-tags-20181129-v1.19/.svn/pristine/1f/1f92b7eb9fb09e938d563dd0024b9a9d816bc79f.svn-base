...
//		}
	...
	//临时读取远程文件
	private static void downloadPic(String webFilePath, String localFilePath) {
		try {
			URL httpurl = new URL(webFilePath);
	     	/*	File dirfile = new File(localFilePath.substring(0, localFilePath.lastIndexOf("/")));*/
			File dirfile = new File(localFilePath.substring(0, localFilePath.lastIndexOf("\\")));
			if (!dirfile.exists()) {
				dirfile.mkdirs();
			}
			org.apache.commons.io.FileUtils.copyURLToFile(httpurl, new File(localFilePath));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
