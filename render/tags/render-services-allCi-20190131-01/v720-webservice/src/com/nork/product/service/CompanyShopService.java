package com.nork.product.service;

import java.util.List;

import com.nork.product.model.CompanyShop;
import com.nork.system.model.SysUser;
import org.springframework.stereotype.Component;

/**   
 * @Title: CompanyShopService.java 
 * @Package com.nork.product.service
 * @Description:企业商铺-企业店铺Service
 * @createAuthor pandajun 
 * @CreateDate 2018-05-22 20:08:57
 * @version V1.0   
 */
@Component
public interface CompanyShopService {
	/**
	 * 新增数据
	 *
	 * @param companyShop
	 * @return  int 
	 */
	public int add(CompanyShop companyShop);

	/**
	 *    更新数据
	 *
	 * @param companyShop
	 * @return  int 
	 */
	public int update(CompanyShop companyShop);

	/**
	 *    删除数据
	 *
	 * @param id
	 * @return  int 
	 */
	public int delete(Integer id);

	/**
	 *    获取数据详情
	 *
	 * @param id
	 * @return  CompanyShop 
	 */
	public CompanyShop get(Integer id);

	/**
	 * 所有数据
	 * 
	 * @param  companyShop
	 * @return   List<CompanyShop>
	 */
	public List<CompanyShop> getList(CompanyShop companyShop);

	/**
	 * 根据企业Id更新店铺分类
	 *
	 * @param companyShop
	 * @return
	 */
	int updateCategoryByCompanyId(CompanyShop companyShop);

	/**
	 * 通过企业ID查询店铺主键列表
	 * @param companyId
	 * @return
	 */
	List<Integer> findIdListByCompanyId(Integer companyId);

	/**
	 * 通过用户id查询用户所属店铺列表
	 * @param userId 用户Id
	 * @return list
	 */
	List<CompanyShop> getShopListByUserId(Long userId);

	/**
	 * 通过用户Id删除属于用户的店铺
	 * @param userId 用户Id
	 * @return
	 */
	Integer deleteShopByUserId(Long userId, String loginUserName);


	/**
	 * 通过企业Id查询企业店铺
	 * @param companyId 企业Id
	 * @return list
	 */
	List<CompanyShop> getShopListByCompanyId(Long companyId);

	/**
	 * 通过企业Id删除企业店铺
	 * @param companyId 企业Id
	 * @return int
	 */
	int deleteShopByCompanyId(Long companyId, String loginUserName);

	/**
	 * 根据用户信息查找关联的店铺
	 * 要求：
	 * 如果用户类型是设计师，则查找个人店铺
	 * 不是则查找用户所属企业店铺
	 * 有多个店铺，则取创建时间最早的一个
	 * @param user
	 * @return
	 */
	CompanyShop getCompanyShopByUserInfo(SysUser user);

	/**
	 * 根据公司id获取品牌馆
	 * 有多个则获取最早的
	 * @param companyId
	 * @return
	 */
	CompanyShop getCompanyShopByCompanyId(Integer companyId);

	/**
	 * 查找用户对应关联店铺
	 * 需求:
	 * 如果是厂商账号，则取品牌馆信息,如果有多个，则取最新的;
	 * 非厂商账号，取该账号发布到随选网的店铺信息,
	 * 如果有多个店铺，则取最早创建的店铺
	 * @param userId
	 * @return
	 */
	CompanyShop getUserCompanyShopForNewScene(Integer userId);
}
