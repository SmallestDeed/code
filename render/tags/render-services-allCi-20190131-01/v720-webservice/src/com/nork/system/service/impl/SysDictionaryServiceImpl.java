...
//		}
	...
	@Override
	public SysDictionary findOneByTypeAndValueKeyInCache(String type, String valueKey) {
		List<SysDictionary> dictionaryList = new ArrayList<>();
		/*if(Utils.enableRedisCache()){
			dictionaryList = SysDictionaryCacher.getAllList();
		}
		if(CustomerListUtils.isEmpty(dictionaryList)){*/
			SysDictionary search = new SysDictionary();
			search.setType(type);
			search.setValuekey(valueKey);
			search.setIsDeleted(0);
			dictionaryList = this.getList(search);
		/*}
		...
		/*for(SysDictionary dictionary:dictionaryList){
			if(Objects.equals(type,dictionary.getType()) && Objects.equals(valueKey,dictionary.getValue())){
				return dictionary;
			}
		}*/
		return dictionaryList.get(0);
