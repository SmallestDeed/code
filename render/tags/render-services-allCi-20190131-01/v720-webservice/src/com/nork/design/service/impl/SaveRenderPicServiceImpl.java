...
public class SaveRenderPicServiceImpl implements SaveRenderPicService {
	...
	public boolean saveRenderPicOf720(RenderPicVO renderPic) {
		...
		if (renderPic.getOpType().intValue() == 0) {
			resPicId = designPlanAutoRenderResouceService.saveRenderPicOf720(designPlan, fileMap, viewPoint, scene,
					isTurnOn, renderingType, loginUser, taskId, panoLevel, roamJson, sourcePlanId, templateId);
		} else {
			resPicId = designPlanService.saveRenderPicOf720(designPlan, fileMap, viewPoint, scene, isTurnOn,
					renderingType, loginUser, taskId, panoLevel, roamJson, sourcePlanId, templateId);
		}
		// Integer resPicId =
	...
	private boolean updateTaskStateByTaskId(RenderPicVO renderPic) {
		...
		Integer platformId = arts.getPlatformId();
		...
		if (arts.getTaskType() == 0) {
			designPlanSceneId = mobileAutoRenderAndOneKeyCopyService.transformAndCopyPlan(planId, loginUser,platformId);
		...
		//获取图片
		ResRenderPic resRenderPic = resRenderPicService.getBytaskIdAndPicType(picType, taskId);
		...
		}else if(AutoRenderTaskConstant.TASK_TYPE_DIY_HD_RENDER == taskState.getTaskType().intValue()){
			sysUserMessage.setContent("高清渲染| "+taskState.getDesignName());
		}
	...
	public void saveTaskBussinessId(AutoRenderTaskState taskState) {
		...
		sysUserMessage.setUserId(taskState.getOperationUserId());
		sysUserMessage.setPlatformId(taskState.getPlatformId());
		logger.info("生成我的消息结束 sysUserMessage -----------------------"+sysUserMessage);
		//生成一条渲染成功的消息记录--end
		//插入消息记录到我的消息表
		int id = sysUserMessageService.add(sysUserMessage);
		logger.info("insert a success taskMessage into sys_user_message---->id："+id);
	}
