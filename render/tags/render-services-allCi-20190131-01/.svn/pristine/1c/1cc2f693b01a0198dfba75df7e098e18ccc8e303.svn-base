...
public interface UnionDesignPlanStoreReleaseMapper {
	...
	List<UnionDesignPlanStoreRelease> selectByPopularityValueAndDateBefore(
			@Param("popularityValue") Integer popularityValue, 
			@Param("dateBefore") Date dateBefore
			);

	void updatePopularityValue(UnionDesignPlanStoreRelease unionDesignPlanStoreRelease);
