...
Page({
  ...
  monitorMessage() {
    ...
    getApp().mySocket.on('im_chat_msg_event', content => {
      console.log('接受到的消息内容，', content.msgBody)
  ...
  toThreeD(e) {
    ...
    let mainTaskId;
    API.copyMsgFullHouseExist({
        historyMsgId: e.currentTarget.dataset.item.id
      })
      .then(res => {
        if (res.obj.copyFlag == 1) {
          API.copyPlan({
              fullHouseDesignPlanId: item.fullHouseId ? item.fullHouseId : item.planRecommendedId,
              mainTaskId: item.mainTaskId,
              supplyDemandId: this.data.id,
              msgId: e.currentTarget.dataset.item.id
            })
            .then(res => {
