...
let myForEach = getApp().myForEach, mySplitUrl = getApp().mySplitUrl,
  myCompoundUrl = getApp().myCompoundUrl, $App = getApp(), API = getApp().API, ttt = 3
...
Page({
  ...
  getPlan() {
    ...
    API.getSuperiorPlan({
      ...
      console.log('33',this.data.showList)
  ...
  getRecommendCaseList(o) { // 获取方案列表
    console.log("44",this.data.showList)
    let obj = {
    ...
    API.getNewRecommendCaseList(parm).then(res=>{
      if (res.success) {
        ...
        if(res.datalist){
          ...
          if (obj.designPlanStyleId || obj.spaceArea || obj.decoratePriceType || obj.decoratePriceRange){
            console.log('abc')
            this.setData({
            ...
            console.log('abcd')
            let list = this.data.SuperiorList;
            console.log('123444',list)
            list = list.concat(res.datalist)
            this.setData({
              showList: list,
              totalCount: res.totalCount
            })
            console.log("55",this.data.showList)
          }
        }else{
          this.setData({
          ...
          if (this.data.showList.length < 1) {
            ...
            this.setData({ recommendCaseList: [], totalCount: 1 })
          }else{
            console.log('8989')
            this.emptyTemplateShow('hide')
          }
