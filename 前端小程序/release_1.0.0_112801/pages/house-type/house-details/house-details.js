...
Page({
  ...
  getIsBindingMobile() {
    ...
    API.getIsBindingMobile().then(res => {
      this.setData({ issBindingMobile: res.success?res.obj:false })
    })
  },
  showOperatingInstruction(options) { // 是否显示操作指南
    if (options.type === 'search' || options.type === 'myPlan') {
      if (wx.getStorageSync('isLoginBefore') === 0 && wx.getStorageSync('operatingInstructionShow') !== 1) {
        this.setData({ operatingInstructionShow: true })
        wx.setStorageSync('operatingInstructionShow', 1)
      }
    }
  },
  userKnowoperating() { // 用户清楚操作
    if (this.data.operatingInstructionActive < 3) {
      ...
      this.setData({ operatingInstructionShow: false })
    }
  },
  ...
  async putInHouse(obj) {
      ...
      console.log('yyyyy',planInfo);
      if (this.data.planInfo.copyRightPermission == 1 && !this.data.planInfo.havePurchased){
          this.setData({
              showShade: true,
              showSelectAreaConfirm:false,
          })
          ...
          if (!objs.templateId) {
              ...
              console.log(item.taskStateList, 'item.taskStateList')
  ...
  replacePlanOrMakeNewPlan(e) {
    ...
    if (this.data.pageParams.mainTaskId && this.data.pageParams.matchState == 5) {
      ...
      let selectAreaInfo = wx.getStorageSync('matchData')['houseGuidePicInfo'];
      console.log(selectAreaInfo, 'selectAreaInfo')      
      selectAreaInfo.guideInfoList.forEach((item, index) => {
        Object.assign(item, { active: !index });
      })
      setTimeout(() => {
