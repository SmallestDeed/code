...
Page({
  ...
  onLoad: function (options) {
    if (options.scene) {
      ...
      this.data.isGrass = 0
      this.setData({ webUrl: options.item ? myCompoundUrl(JSON.parse(options.item)) : getApp().data.webUrl + '&lp=' + previousPath})
    }
  ...
  sendRenderIdParams(e) {
    ...
    console.log(this.data.shareText, this.data.coverPath, 'sqwqwq')
