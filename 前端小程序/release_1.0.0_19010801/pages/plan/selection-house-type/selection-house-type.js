...
Page({
      ...
      .then(res => {
        if (res.obj) {
          ...
          this.setData({
            ...
            totalCount: 0,
            emptyPageObj: {
              imgUrl: $App.staticImageUrl + 'huxing_bg_no.png',
              title: '您还没有收藏户型哦',
              btnText: '去逛一逛',
  ...
  confirmBtn() {
    ...
    prevPage.setData({
      addHouseId: this.data.useList[this.data.isCheck].id,
      addHouseImg: this.data.useList[this.data.isCheck].thumbnailPath,
      addHouseTitle: this.data.useList[this.data.isCheck].houseCommonCode ? this.data.useList[this.data.isCheck].houseCommonCode : this.data.useList[this.data.isCheck].houseName,
    })
    wx.navigateBack({
      delta: 1
    })
