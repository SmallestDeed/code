...
Page({
  ...
  onShareAppMessage: function (res) {
    let that = this;
    let item = res.target.dataset.item, _res = res;
    this.data.i++;
    if (res.from === 'menu') {
      return $App.shareAppMessageObj
    }
    if (res.from === 'button') {
      // rzd 181226元旦红包活动，记录用户参见活动 start
      let activityType = item.taskType == 0 ? '720' : 'yuandanActivity';
      if (item.taskType == 0) {
        return this.shareFun(_res, activityType)
      } else {
        // 用户参与元旦红包活动接口，需传任务id
        // 随机文案
        if (this.data.i >= 5) {
          ...
          this.setData({
            randomTitle: this.data.titleArr[index].val,
            i: 0
          })
        }
        console.log(this.data.randomTitle, '分享文案------------------------------------');
        return this.shareFun(_res, activityType, this.data.regid);
      }
      // rzd 181226元旦红包活动，记录用户参见活动 end

    }
  },
  // rzd 181226元旦红包活动,封装分享方法 start
  shareFun(res, activityType, yuandanRegId) {
    ...
    return {
      title: activityType == "720" ? item.designName : this.data.randomTitle,
      // 判断分享是一键装修（720）还是产品替换，产品替换需传任务id
      path: activityType == '720' ? '/pages/home/home?shareType=' + activityType + '&url=' + webUrl : '/pages/home/home?shareType=' + activityType + '&url=' + webUrl + '&yuandanRegId=' + yuandanRegId,
      imageUrl: imgurl,
      ...
      .then(res => {
        ...
        if (res.datalist && res.datalist.length > 0) {
          ...
          this.setData({
            ...
            totalCount: 0,
            emptyPageObj: {
              imgUrl: '/static/image/huxing_bg_no.png',
              title: '您还没有用过我们的产品呢',
              btnText: '去体验',
  ...
  noTo(e){
    ...
    wx.showModal({
      title: '提示',
      content: item.isSuccess == 1 ? "方案正在渲染，请稍后~" :"方案渲染失败~",
      showCancel: false,
    })
  },
})