...
Page({
  ...
  selectSpace(e) {
    ...
    if(currentSpace==2){
      let creatTime = currentData.taskCreateTime;
      let nowTime = new Date();
      let time = nowTime.getTime() - new Date(creatTime).getTime(); 
      time =time/1000
      let txt = Math.round(time / 60 * 100)
      ...
      this.drawProgressbg();
      this.drawCircle(time/30);
      this.countInterval(index);
    }
    this.setData({
  ...
  async queryRecommandPlan() {
    ...
    let { datalist } = await API.getRecommendCaseList(parms);
  ...
  countInterval: function (e) {
    ...
    this.countTimer = setInterval(() => {
      console.log(this.countTimer,'timer')
    ...
    setTimeout(function () { that.getCurrentSpaceState(e); }, 60000)
  },
  ...
  drawCircle: function (step) {
    ...
    context.arc(50, 50, 30, -Math.PI / 2, step * Math.PI - Math.PI / 2, false); 
    context.stroke();
    context.draw()
  },
  scroll() {
  ...
  getCurrentSpaceState(e){
    var that=this
    // API.queryWholeHousePic({ houseId: this.data.pageParams.houseId, fullHousePlanId: this.data.pageParams.fullHousePlanId || '' }).then(res => {
    //   if (res.obj.guideInfoList[e].renderStatus != 2 ){
    //     this.setData({
    //       currentSpaceIndex:e,
    ...
    //   }
    // })
    this.queryWholeHousePic();
    if(this.data.wholeHousePicData.guideInfoList[e].renderStatus!=2){
      console.log("渲染完成")
    }else{
      setTimeout(function () { that.getCurrentSpaceState(e); }, 5000)
    }
  }
})