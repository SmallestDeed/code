...
App(Object.assign({
  ...
  onHide(){
    // 销毁我的消息的红点状态的缓存
    wx.setStorageSync('renderNewsObj.isRead', null)
    wx.setStorageSync('comment.isRead', null)
    wx.setStorageSync('userInformation[0].unreadMsg', null)
    wx.setStorageSync('systemNewObj.isRead', null)
  },
  onError(err) {
    console.log(err, 'err')
  }
