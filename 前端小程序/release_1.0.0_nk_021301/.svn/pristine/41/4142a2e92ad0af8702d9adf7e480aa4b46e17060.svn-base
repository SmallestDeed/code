...
Page({
  ...
  bindChange: function(e) {
    ...
    let datas = this.data.cityData[val[0]].baseAreaVos[val[1]].baseAreaVos;
    this.setData({
  ...
  valChange: function(e) {
    ...
    this.setData({
      livingName: e.detail.value.trim()
    })
  },
  open: function(e) {
    this.setData({
  ...
  ishow: function(e) {
    ...
    if (type == 2) {
      ...
      if (!this.data.spaceList.length) {
        fetch('/designplan/designplanconditionmetadata', 'get')
          .then((res) => {
            if (res.success) {
              this.setData({
                spaceList: res.obj,
      ...
      if (!this.data.shapeList.length) {
        fetch('/designplan/getspaceshape', 'get')
          .then((res) => {
            if (res.success) {
              this.setData({
                shapeList: res.obj
  ...
  onLoad: function() {
    ...
    wx.getStorage({
      ...
      success: function (res) {
        that.setData({
          cityData: $App.cityDataFilter(res.data)
        })
      }
    })
  },
  ...
  search: function() {
    let url = "/base/basearea/listall?pid=0&levelId=1"
    fetch(url, 'get')
      .then((res) => {
  ...
  spacesearch: function(message) {
    var that = this
    var url = "/home/spacecommon/spacesearch"
    var data = {
  ...
  toHouseDetails(e) { // 空间入口
    ...
    let item = e.currentTarget.dataset.item
    wx.navigateTo({
      url: '../house-details/house-details?type=houseSpace&item=' + JSON.stringify(item)
    })
  }
})