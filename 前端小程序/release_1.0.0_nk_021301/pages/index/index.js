...
Page({
  ...
  onGotUserInfo(e) {
    if(e.detail.userInfo){
      ...
      if (!wx.getStorageSync('bindingPhone')) {
        if (!this.data.click && !this.data.thisFlag) {
          this.setData({ getPhone: true })
        }
      }
    }else{
      this.setData({wxInfo:true})
      if (!wx.getStorageSync('bindingPhone')) {
        if (!this.data.click && !this.data.thisFlag) {
          this.setData({ getPhone: true })
        }
  ...
  closeBtn() {
    ...
    getConfig() {
        ...
        fetch(url, 'get', '', 'core').then((res) => {
            ...
            if (res.obj) {
              ...
              if (res.obj.configMap.moreGoods){
                that.setData({
                  moreGoodsInfo: res.obj.configMap.moreGoods
                })
              }
    ...
    onShow: function () {
      this.setData({ planNum: 1 })
      $App.demo = this.accredit;
      $App.userLoginStatus.then(() => { this.onShowInit()}) 
      // let userInfo = wx.getStorageSync('userInfo')
      // getApp().accreditStatus = this.accredit
      // console.log(getApp().accreditStatus)
      console.log(getApp().globalData.indexLoginStatus)
      if (getApp().globalData.indexLoginStatus) {
        ...
        getApp().accreditStatusCallBack = () => {
          if (getApp().globalData.indexLoginStatus != false) {
            this.setData({
              wxInfo: true,
            })
          }
    ...
    accredit(data){
      ...
      let isLogin = data.obj.headPic != "";
      console.log(getApp().globalData.indexLoginStatus)

      if (getApp().indexLoginStatus) {
        this.setData({
          wxInfo: true
        })
        if (!wx.getStorageSync('bindingPhone')) {
