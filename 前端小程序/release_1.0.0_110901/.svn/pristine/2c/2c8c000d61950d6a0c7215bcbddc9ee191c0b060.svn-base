...
Page({
  ...
  getBannerList() {
    ...
    API.getIndexBanner({
      ...
      // console.log('123123',res.datalist)
      obj = res.datalist
      // console.log(obj)
      that.setData({
        imageArray: obj
    ...
    let timer = setInterval(() => {
      if (getApp().webSocketStatus) {
        getApp().mySocket.on('im_unread_msg_event', content => {
          console.log(content, '消息1');
          this.setData({ RenderNewsIsRead: 1 })
        }) // 消息提醒
        getApp().mySocket.on('im_push_msg_event', content => {
          console.log(content, '消息2');
          this.setData({ RenderNewsIsRead: 1 })
        }) // 渲染消息
        clearInterval(timer)
      }
  ...
  getPopular() {
    API.getShopPopularityList({
      ...
      console.log(this.data.popularList1)
    })
  },
  ...
  navigat(e) {
    // console.log(this.data.systemNewObj.isRead);
    // console.log(this.data.RenderNewsIsRead)
    // console.log(this.data.commentIsRead)
    // console.log(this.data.UserChatIsRead)
    let url = e.currentTarget.dataset.url,
        type=e.currentTarget.dataset.type,
