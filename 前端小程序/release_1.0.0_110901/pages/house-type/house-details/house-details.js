...
Page({
  ...
  // 全屋按钮事件
  selectWholeHouse(e) {
    console.log(this.data.wholeHouseSelectIcon.active)
    console.log(this.data.wholeHouseSelectIcon.icons)
  ...
  async queryRecommandPlan() {
    ...
    let recommendationPlanSearchVo={
      ...
      spaceAreas: isFullHouse ? '' : selectedSpace && selectedSpace.spaceArea,
  ...
  async queryCollectPlan() {
    ...
    let [currentStyle] = this.data.currentSpaceInStyleList.filter(item => item.active);
    let isFullHouse = this.data.wholeHousePicData.guideInfoList.every(item => item.active);
    let { datalist } = await API.getDesignplanfavorite({
      start: 0,
      limit: this.data.pageSize,
      houseType: isFullHouse ? 13 : selectedSpace.spaceType,
      designStyleId: currentStyle && currentStyle.styleCode,
      areaValue: isFullHouse ? '' : selectedSpace && selectedSpace.spaceArea 
    });
  ...
  replacePlanOrMakeNewPlan(e) {
    ...
    this.putInHouse(this.data.pageParams.templateId, planInfo.spaceFunctionId, type);
  },
  keepDecorate() {
    wx.redirectTo({
