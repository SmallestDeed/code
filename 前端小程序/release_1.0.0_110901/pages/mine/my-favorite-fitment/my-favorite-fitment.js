 ...
 Page({
   ...
   onShareAppMessage: function(res) {
     ...
     if (res.from === 'button') {
       ...
       let item = res.target.dataset.item,
         routerQueryType = res.target.dataset.type,
         webUrl = null,
         sevenObj = null
       let imgurl;
   ...
   toLanding(e) {
     API.getUserInfo().then(res => {
       let url = '/pages/landing/landing?id=' + e.currentTarget.dataset.id + '&type=' + e.currentTarget.dataset.type + '&userId=' + res.obj.userId + '&iid=' + e.currentTarget.dataset.iid;
       console.log(url);
       wx.navigateTo({
         url: url,
       })
   ...
   delete(e) { //删除装修
     ...
     //planHouseType=1 单空间我的方案   2,3我的全屋方案
     item.planHouseType == 1 ? taskId = item.taskId : taskId = item.mainTaskId 
     { {
