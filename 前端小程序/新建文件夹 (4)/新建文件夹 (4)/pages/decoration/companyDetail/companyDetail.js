...
Page({
  ...
  getRecommendCaseList(obj) { // 获取方案列表
    ...
    API.getCompanyPlan(Object.assign(data, { displayType: 'decorate' }))
      .then(res => {
        if (res.datalist) {
          this.setData({ decorateList: res.datalist.slice(0, 1), decorateListNum: res.totalCount })
          console.log(this.data.decorateList)
        } else {
          this.setData({ decorateList: [] })
        }
      })
    API.getCompanyPlan(Object.assign(data, { displayType: 'public' }))
      .then(res => {
        if (res.datalist) {
          this.setData({ publicList: res.datalist.slice(0, 1) })
        } else {
  ...
  getDetail: function () {
    API.getCompanyDetails({ id: this.data.targetId })
      .then(res => {
        if (res.code == 200) {
          if (!res.data) return;
  ...
  getActivity: function () {
    API.getCompanyDesignerList({ companyId: this.data.companyId })
      .then(res => {
        if (res.code == 200) {
          let arr = [];
          if (res.data.length > 3) {
            ...
            this.setData({ teamListShow: arr, teamList: res.data })
          } else {
            this.setData({ teamListShow: res.data, teamList: res.data })
          }
  ...
  toPlanList(e) {
    ...
    wx.navigateTo({
      url: '/pages/decoration/planList/planList?type=' + type + '&companyId=' + companyId + '&shopId=' + this.data.shopId,
    })
  },
  toCaseList() {
    wx.navigateTo({
  ...
  toTeam() {
    wx.navigateTo({
      url: '/pages/decoration/teamList/teamList?companyId=' + this.data.companyId,
    })
  },
  openMap() {
