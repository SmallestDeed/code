...
Page({
  ...
  onPageScroll(e) {
    if (this.data.showCityView || this.data.sortFlag2 || this.data.sortFlag || this.data.sortFlag3 || this.data.sortFlag4) { this.setData({ praiseRecommendedFixed: true }); return}
    this.setData({ praiseRecommendedFixed: (this.data.praiseRecommended != 0 && e.scrollTop >= this.data.praiseRecommended) ? true : false })
  },
  goToTop() { // 回到顶部
  ...
  getPopularList() {
    API.getShopPopularityList({
      ...
      .then(res => {
        ...
        console.log(this.data.popularList)
      })
  },
  getCityData() {
    let cityData = $App.cityDataFilter(wx.getStorageSync('cityData'))
    cityData[0].baseAreaVos.push([{
      areaCode: '',
      areaName: '全部',
      baseAreaVos: null,
      id: null,
    ...
    let that = this, code = this.data.shareOptions.code;
    ...
    console.log(this.data.provinceObj, 'wqwq')
    ...
    this.setData({
      ...
      cityObj: this.data.provinceObj[this.data.choosex].objList[this.data.choosey].baseAreaVos
    })
    console.log(that.data.provinceData, 'wq1')
    that.data.cityData = cityData
    console.log(new Date().getTime(), '02')
  },
  ...
  submitFun: function (e) {
    let obj = {},
      minValue = Math.min(Math.floor(this.data.minValue), Math.floor(this.data.maxValue)),
      maxValue = Math.max(Math.floor(this.data.minValue), Math.floor(this.data.maxValue))
    if (maxValue <= 2000 && maxValue!='' && minValue!='' && minValue>=0 && maxValue>0 && minValue!=maxValue) {
      if (this.data.priceType == 3 || this.data.priceType == 4) {
        obj.designFeeStaring = minValue 
