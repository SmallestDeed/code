...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.source === 'webH5') {
      this.sellingPoint({ name: 'decorate-house'}, 'webH5')
    }
  },
  // 关闭户型引导提示
  closeBoot() {
  ...
  toHouseType() {
    this.sellingPoint({ name: 'ppg_search' })
    wx.navigateTo({
  ...
  match(e) {
    let item = e.currentTarget.dataset.item,
      ...
      caseItem;
    this.sellingPoint({ name: 'house-one-key' })
      ...
      API.matchPlanHouse({ houseId: item.id, recommendedPlanId: caseItem.designPlanRecommendId || caseItem.planRecommendedId })
        .then(res => {
          if (res.obj.status == 1) {
            ...
            wx.setStorageSync('matchData', res.obj)
            wx.navigateTo({
              url: '/pages/house-type/house-details/house-details?type=' + this.data.type + '&matchState=' + res.obj.status + '&houseId=' + item.id + '&houseName=' + encodeURIComponent(item.houseName)
            })
          }
        })
    }
