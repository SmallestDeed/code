...
Page({
    ...
    init(options) { // 初始化
        ...
        $App.userLoginStatus.then(res => { this.sellingPoint() })
        ...
        if (options.scene) {
            API.getJsonData({
              ...
              let shareTimer = setInterval(() => {
                ...
                console.log(res, 'swq')     
                  getApp().data.webUrl = myCompoundUrl(Object.assign(JSON.parse(res.obj.jsonData), { token: wx.getStorageSync('token') }))
    ...
    isSevenShare(options) {
        if (options.item) { // 720
          ...
          let shareTimer = setInterval(() => {
            if (wx.getStorageSync('token')) { // 获取token后再进行跳转       
              getApp().data.webUrl = myCompoundUrl(Object.assign(item, { token: wx.getStorageSync('token') }))
              wx.navigateTo({ url: '/pages/web-720/web-720' })
              clearInterval(shareTimer)
            }
          }, 100) 
        } else if (options.url) { // 视频
            wx.navigateTo({ url: '/pages/video/video?url=' + options.url })
    ...
    getDesignplanconditionmetadata() {
      $App.userLoginStatus.then(res => { this.getRecommendCaseList(this.setObj()) })
    },
    onShow: function() {
    ...
    getConditionMetadata() { // 获取方案筛选条件
      API.getConditionMetadata()
        .then(res => {
          if (res.status) {
            let obj = res.obj.map((value) => { value.designPlanStyleVoList.unshift({ styleName: '全部', styleCode: '' }); return value })
            this.setData({
              spaceList: obj,
    ...
    styleTypeFun: function (e) {
        let dataset = e.currentTarget.dataset, styleList = this.data.spaceList[dataset.index].designPlanStyleVoList;
        this.setData({
