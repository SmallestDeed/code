...
Page({
  ...
  getBannerList() {
    API.getIndexBanner({
      ...
      this.data.imageArray.map(res => { //有春节活动才显示弹窗
        if (res.skipPath.indexOf('festivalActivity') > -1) {
          this.setData({ homePopup: true })
        }
      })
    })
  },
  ...
  kfViewBut() {
    $App.sellingPoint(API, 'homekfViewBut', this.data.nowPath, this.data.previousPath, '首页');
  },
  onLoad: function (options) {
    ...
    $App.sellingPoint(API, '', this.data.nowPath, this.data.previousPath, '首页');



    //消息通知跳转 2018.12.28zb
    if (options.page === 'livingHouse') {
      wx.navigateTo({
        url: '/pages/house-type/house-list/house-list?id=' + options.livingId + '&name=' + options.livingName,
      })
    } else if (options.page === 'searchHouse') {
      wx.navigateTo({
    ...
    this.setData({ homePopup: isLoginBefore == '0' })
    console.log(this.data.homePopup)  //true
    // 小程序缓存判断第一次登陆开始
    // wx.getStorage({
    //   key: 'isFirstLogin',
  ...
  init(options) { // 初始化
    console.log(options, 'swq')
    var that = this
    // this.webSocketInit() // 初始化webSocket中的事件
    if (options.activityName && options.activityName === 'yuandan') {
      console.log('options.activityName' + options.activityName)
      return wx.navigateTo({ url: '/pages/yuandan-activity/yuandan-activity?activityname=yuandan' })
    }
    options.navToUrl && wx.navigateTo({ url: decodeURIComponent(options.navToUrl) })
    options.actId && this.toCutprice(options)
    this.posterSharePanorama(options) // 获取海报分享到砍价活动
    $App.userLoginStatus.then(() => { $App.sellingPoint(API, '', this.data.nowPath, this.data.previousPath, '首页'); }) // 埋点
    if (options.shareType == '720') {
      ...
      $App.userLoginStatus.then(() => {
        ...
        $App.myNavigateBack('pages/web-720/web-720')
        API.recordInvitation({ regId: options.yuandanRegId }).then(res => { console.log('记录邀请信息接口调用', res) });
      })
      //  rzd 181226元旦红包活动,记录邀请信息(好友通过分享链接进入时) end
    } else if (options.activityName === 'yuandan') {
      wx.navigateTo({ url: '/pages/yuandan-activity/yuandan-activity' })
    }
  ...
  posterSharePanorama(options) {
    if (options.scene) {
      API.getJsonData({ id: options.scene }).then(res => {
        $App.userLoginStatus.then(() => {
          getApp().data.cutpriceUrl = myCompoundUrl(Object.assign(JSON.parse(res.obj.jsonData), { token: wx.getStorageSync('token') }))
          console.log(getApp().data.cutpriceUrl, 'getApp().data.cutpriceUrl ')
          wx.navigateTo({ url: '/pages/cutprice/cutprice' })
        })
      })
    }
  },
  ...
  onShowInit() {
    this.getCityData();
    $App.userLoginStatus.then(res => { this.getPlanList() })
  ...
  getPlanList() {
    API.getSuperiorPlan().then(res => { this.setData({ caseObj: res.status ? res.obj : [] }) })
  },
  /**
   * 用户点击右上角分享
   */
    ...
    if (this.data.formIndex >= 4) {
      setTimeout(() => {
        let url = e.detail.target.dataset.url,
          ...
          formId = [];
        console.log(url, 'wqwqwqwq')
        if (e.detail.target.dataset.type == "信息栏") {
          ...
          if (e.detail.target.dataset.url == 'bHIndex') {
            wx.switchTab({
              url: '/pages/brandHall/bHIndex/bHIndex'
            })
            return;
