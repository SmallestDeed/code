...
Page({
   ...
   */
  onReady: function () {
    // console.log(this.data.wholeHousePicData,'data')
    // console.log(this.data.currentSpaceIndex)
    // console.log(this.data.wholeHousePicData.guideInfoList[this.data.currentSpaceIndex].planPic)
  },
  ...
  async queryWholeHousePic() {
    ...
    console.log(this.data.currentSpace,'currentSpace')
    if (this.data.currentSpace == 2) {
      let currentData = this.data.wholeHousePicData.guideInfoList[this.data.currentSpaceIndex];
      let creatTime = currentData.taskCreateTime.replace(/\-/g, '/');
      let nowTime;
      API.getSystemTime({}).then(res=>{
        nowTime = res.message.replace(/\-/g,'/')
        let time = new Date(nowTime).getTime() - new Date(creatTime).getTime();
        let a = new Date(creatTime)
        console.log(nowTime,'nowTime')
        console.log(a, 'creatTime')
        console.log(new Date(nowTime).getTime(), 'getNow')
        console.log(new Date(creatTime).getTime(), 'getCre')
        time=time/1000
        let txt;
        if (time > 58) {
          txt = 95
        } else {
          txt = Math.round(time / 60 * 100)
  ...
  async putInHouse(obj) {
    ...
    console.log('yyyyy', planInfo, this.data.pageParams, 'swq');
  ...
  isExperience() {
    let self = this;
    console.log(decodeURIComponent(self.data.pageParams.houseName), self.data.pageParams, 'sss')
    return
    wx.getStorage({
      key: 'actId',
  ...
  replacePlanOrMakeNewPlan(e) {
    console.log(this.data.pageParams, 'this.data.pageParams')
    ...
    if (this.data.pageParams.mainTaskId && this.data.pageParams.matchState == 5) {
      ...
      setTimeout(() => {
        ...
        console.log(this.data.showSelectAreaConfirm, 'showSelectAreaConfirm')
      }, 250);
      return;
    }
    let objjj = {
  ...
  countInterval: function (e) {
    ...
    this.countTimer = setInterval(() => {
      if (this.data.count <57) {
        ...
        */
        ...
        this.data.count++;
        let num = Math.round(this.data.realnum+1.6)
        this.setData({
          progress_txt: num,
          realnum: this.data.realnum+1.6
        })
      } else {
        this.setData({
          progress_txt:95
    ...
    setTimeout(function () {; that.getCurrentSpaceState(); }, 58000)//
  },
  ...
  drawCircle: function (step) {
    ...
    var context = wx.createCanvasContext('canvasProgress');
    // 设置渐变
    var gradient = context.createLinearGradient(235*rpx, 0*rpx, 455*rpx, 220*rpx);
    gradient.addColorStop("0.5", "#1adcae");
    gradient.addColorStop("0.5", "#29cccc");
    // gradient.addColorStop("1.0", "#5956CC");

    context.setLineWidth(10*rpx);
    context.setStrokeStyle(gradient);
    context.setLineCap('round')
    context.beginPath();
    // 参数step 为绘制的圆环周长，从0到2为一周 。 -Math.PI / 2 将起始角设在12点钟位置 ，结束角 通过改变 step 的值确定
    context.arc(170*rpx, 55*rpx, 40*rpx, -Math.PI / 2, step * Math.PI - Math.PI / 2, false); 
    context.stroke();
    context.draw()
  },
  scroll() {
  ...
  getCurrentSpaceState(){
    ...
    API.queryWholeHousePic({ houseId: this.data.pageParams.houseId, fullHousePlanId: this.data.pageParams.fullHousePlanId || '' }, 'noLoading').then(res=>{
      ...
      if (flag) {
        setTimeout(function () { that.getCurrentSpaceState(); }, 10000)
      }else{
        this.setData({
