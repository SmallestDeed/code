...
import appid from './utils/appid.js'
const io = require('./lib/socket.io.slim.js')
console.log(appid, '11111')
App(Object.assign({
  ...
  wxUrl: 'https://servicewechat.com/wx6e369e0fac7d3273/devtools/page-frame.html',
  sevenUrl,
  webUrl: '',
  mySocket: null,
  ...
  regMobile: /(^1[3|4|5|7|8|9]\d{9}$)|(^09\d{8}$)/,
  regEmoji: /\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/g,
  onLaunch: function () {
    this.userLogin()
    this.forcedUpdating()
  ...
  userLogin() { // 用户登录
    ...
    wx.login({
      success: res => {
        ...
        API.getUserOpenId({ code: res.code, appid: 'wx6e369e0fac7d3273' })
          .then(res => {
            if (res.success) {
              wx.setStorageSync('openId', res.obj)
              // 登录
              API.setUserLogin({
                openid: res.obj, appid: 'wx6e369e0fac7d3273' })
                .then(res => {
                  if (res.success) {
                    console.log(res)
                    wx.setStorageSync('token', res.obj.token)
                    wx.setStorageSync('id', res.obj.id)
                    wx.setStorageSync('loginStatus', res.obj)
