...
Page({
  ...
  closeBoot() {
    ...
    wx.setStorageSync('isFirstShow', true)
  },
  ...
  toHouseType() {
    wx.navigateTo({
      url: '/pages/seekHouse/seekHouse?type=' + this.data.type,
    })
  },
  ...
  //   wx.navigateTo({
  //     url: '/pages/OnekeyFinish/OnekeyFinish?type=plan&houseId=' + houseItem.id
  //   })
  // },
  scroll(e) {
    this.setData({
  ...
  match(e) {
    ...
    if (caseItem.spaceType == 13 || caseItem.spaceFunctionId == 13) {
      ...
      wx.navigateTo({
        url: '/pages/seekHouse/seekHouse?type=' + this.data.type + '&matchState=3&houseId=' + item.id
      })
      return;
    }
    if (this.data.type == "search") {
      wx.navigateTo({
        url: '/pages/seekHouse/seekHouse?type=' + this.data.type + '&houseId=' + item.id
      })
      return;
    }
    if (eType == 'plan') {
      API.matchPlanHouse({ houseId: item.houseId, fullHousePlanId: item.newFullHousePlanId, recommendedPlanId: caseItem.designPlanRecommendId || caseItem.planRecommendedId })
        .then(res => {
          ...
          if (res.obj.status == 1) {
            ...
            wx.navigateTo({
              url: '/pages/seekHouse/seekHouse?type=' + this.data.type + '&matchState=' + res.obj.status + '&houseId=' + item.houseId + '&templateId=' + res.obj.templateId + '&fullHousePlanId=' + item.newFullHousePlanId + '&mainTaskId=' + item.mainTaskId + '&preRenderSceneId=' + (res.obj.status == 4 ? res.obj.taskStateList[0]['businessId'] : '')
            })
          }
        })
    } else {
      API.matchPlanHouse({ houseId: item.id, recommendedPlanId: caseItem.designPlanRecommendId || caseItem.planRecommendedId })
        .then(res => {
          if (res.obj.status == 1) {
            ...
            wx.setStorageSync('matchData', res.obj)
            wx.navigateTo({
              url: '/pages/OnekeyFinish/OnekeyFinish?type=' + this.data.type + '&matchState=' + res.obj.status + '&houseId=' + item.id + '&houseName=' + encodeURIComponent(item.houseName)
            })
          }
        })
    }
  ...
  planImproper(e) {
    ...
    if (options == 1) {
      ...
      wx.navigateTo({
        url: '/pages/OnekeyFinish/OnekeyFinish?houseId=' + houseId + '&type=search',
      })
      this.setData({
        status: ''
      })
