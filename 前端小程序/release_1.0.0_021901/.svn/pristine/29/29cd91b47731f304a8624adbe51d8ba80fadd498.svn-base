...
Page({
  ...
  onLoad: function (options) {
    ...
    if (options.groupItem) {
      // let str = JSON.parse(options.groupItem)
      // str.houseName = str.houseName.replace(/\*/g, "?");
      // console.log(options.groupItem)
      this.setData({
    ...
    if (options.type != 'houseSpace') {
      ...
      console.log(this.data.spaceInedxType, '000000000000')
      this.gethousetypelist()
  ...
  gethousetypelist: function () {
    var that = this, data = {};
    API.gethouseDetailsTypeList(data).then((res) => {
      if (res.obj) {
        that.setData({
  ...
  getspace: function () {
    ...
    API.examineHouseIsMatching(data).then((res) => {
      if (res.status) {
        if (res.obj[0].spaceFunctionId != '13') {
          ...
          setTimeout(function () {
            ...
            console.log(wx.getStorageSync('caseItem'), 'caseItemqq')
            console.log(that.data.caseItem, 'caseItem2')
            that.getinformation(that.data.spaceAreas) // 获取方案
          }, 200)
      ...
      .catch((res) => {
        ...
        console.log(this.data.caseItem, 'caseItem3')
      })
  },
  //获取推荐方案列表
  getinformation: function (spaceAreas) {
    ...
    API.getRecommendCaseList(data).then((res) => {
      if (res.success) {
        that.setData({
  ...
  toFunc() {
    wx.navigateTo({
      url: '/pages/goAddMoney/goAddMoney',
    })
  },
  putInMyhousess(obj) {
