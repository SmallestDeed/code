...
Page({
  ...
  getHouseCount() { // 购买户型参数
    let urlOne = `/home/basehouse/getexpandhousedict`
  ...
  routerToBuyPoints() {
    wx.navigateTo({
      url: '/pages/goAddMoney/goAddMoney?score=' + this.data.userMessage.balanceIntegral
    })
  },
  radioChange(e) {
  ...
  confirmPurchaseHouse() { // 确认购买户型
    let index = myFindIndex(this.data.purchaseList, (value) => {
      return value.flag
    })
    let value = this.data.purchaseList[index].value
    // let url = `/web/pay/payOrder/expandhouse`
    API.buyHOuseType({})
      .then((res) => {
        let $self = this
        if (res.status) {
