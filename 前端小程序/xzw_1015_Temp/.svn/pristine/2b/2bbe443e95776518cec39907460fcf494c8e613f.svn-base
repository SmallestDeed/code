...
Page({
    data: {
        ...
        imgUp: '../../static/image/page_icon_up.png',
        imgDown: '../../static/image/page_icon_down.png',
        provinceList: [],
        cityList: [],
        threeLevelValue: [0, 0],
    ...
    myRules(type, param) {
        ...
        switch (type) {
            ...
            case 'mobile': flag = /^1[3|4|5|8][0-9]\d{8}$/.test(param); title = '请正确输入手机号'; break;
    ...
    enterpriseEntry() {
        ...
        if (flag) {
            API.enterpriseEntry({
                ...
                if (res.success) {  this.setData({ showTost: true })
                } else {
    ...
    provincialLinkageHide(e) { // 隐藏三级
        ...
        if (e.currentTarget.dataset.type === 'confirm') {
            this.data.cityVal = this.data.cityData[val[0]].areaName + this.data.cityData[val[0]].baseAreaVos[val[1]].areaName
            this.setData({
