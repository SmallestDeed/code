...
Page({
  ...
  monitorMessage() {
    getApp().mySocket.emit('im_loc_msg_event', { userSessionId: wx.getStorageSync('uuid'), loc: 3, appId: 16, }) // 上报位置
    getApp().mySocket.on('im_unread_msg_event', content => { this.getUserChatList() })
  },
  ...
  onHide: function () {
    ...
    getSystemNewsList() { // 获取系统消息轮询
        API.getSystemRenderList({
          ...
          this.setData({
            systemNewObj: res.success && res.obj ? { isRead: res.obj[0].isRead, title: res.obj[0].title } : { isRead: 0, title: '暂无系统消息' }
          })
