...
public class RabbitMqHandler {
    ...
    @Autowired
    public RabbitMqHandler(ProductMessageHandler productMessageHandler,
                           RecommendationMessageHandler recommendationMessageHandler,
                           GoodsMessageHandler goodsMessageHandler,
                           GroupProductMessageHandler groupProductMessageHandler) {
    ...
    public void consumerMessage(String message) {
        ...
        switch (moduleType) {
            ...
            case QueueMessage.SHOPPLAN_MODULE:
                ...
                break;
            //组合产品模块
            case QueueMessage.GROUPPRODUCT_MODULE:
                //操作类型
                if (QueueMessage.UPDATE_ACTION == actionType || queueMessage.ADD_ACTION == actionType) {
                    //更新组合产品数据
                    messageConsumerStatus = groupProductMessageHandler.addAndUpdate(JsonUtil.fromJson(queueMessage.getObject().toString(), new TypeToken<List<GroupProductIndexMappingData>>() {
                    }.getType()));
                } else if (QueueMessage.DELETE_ACTION == actionType) {
                    messageConsumerStatus = groupProductMessageHandler.delete(JsonUtil.fromJson(queueMessage.getObject().toString(), new TypeToken<List<GroupProductIndexMappingData>>() {
                    }.getType()));
                }
                break;
            default:
