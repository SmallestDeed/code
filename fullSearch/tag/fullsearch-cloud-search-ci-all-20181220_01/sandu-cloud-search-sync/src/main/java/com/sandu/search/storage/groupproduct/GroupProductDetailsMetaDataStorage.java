...
import com.sandu.search.entity.elasticsearch.po.metadate.GroupProductDetailPo;
...
public class GroupProductDetailsMetaDataStorage {
    ...
    public void updateData() {
        ...
        try {
            ...
            groupProductDetailPoList = metaDataService.queryProductGroupDetail();
        ...
        if (null == groupProductDetailPoList || groupProductDetailPoList.size() <= 0) {
            log.error(CLASS_LOG_PREFIX + "获取产品组合详细信息元数据为空，请检查数据。。。");
            return;
        }
        log.info(CLASS_LOG_PREFIX + "获取产品组合详细信息元数据，总条数：{}", groupProductDetailPoList.size());

        Map<Integer, List<GroupProductDetailPo>> tempProductGroupDetailPoMap = new HashMap<>();

        groupProductDetailPoList.forEach(productGroupDetailPo -> {
            ...
            if (null != productGroupId && 0 < productGroupId) {
                if (tempProductGroupDetailPoMap.containsKey(productGroupId)) {
                    List<GroupProductDetailPo> groupDetailPoList = tempProductGroupDetailPoMap.get(productGroupId);
                    List<GroupProductDetailPo> groupDetailPos = new ArrayList<>();
     ...
     * @return
     */
    public List<GroupProductDetailPo> getGroupProductDetailsList(Integer groupId) {
        ...
        return JsonUtil.fromJson(json, new TypeToken<List<GroupProductDetailPo>>() {
        }.getType());
    }
}
