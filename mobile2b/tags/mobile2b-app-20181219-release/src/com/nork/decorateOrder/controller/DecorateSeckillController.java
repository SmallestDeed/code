...
import com.nork.decorateOrder.model.dto.DecorateSeckillUpdateStatusDTO;
...
public class DecorateSeckillController {
	...
	@ResponseBody
	public ResponseEnvelope<String> updateStatus(
			/*Long id, Integer status*/
			@RequestBody DecorateSeckillUpdateStatusDTO paramsDTO
			) {
		...
		try {
			decorateSeckillService.updateStatus(paramsDTO.getId(), paramsDTO.getStatus(), Utils.getLong(loginUser == null ? null : loginUser.getId()));
		} catch (BizException e) {
			return new ResponseEnvelope<String>(false, e.getMessage());
		}
		
		return new ResponseEnvelope<String>(true, "success");
	}
