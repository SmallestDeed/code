...
public class DecorateOrderController {
            ...
            if (result){
                ...
                if (null!=update.getOrderStatus()&&update.getOrderStatus().equals(DecorateOrderConstants.DECORATEORDER_ORDERSTATUS_HAVE_HAS_SIGNED)){
                    ...
                    if (null!=detail.getAppointUserId()){
                        ...
                        /**退度币开始**/
                        logger.error(LOGPREFIX + "(修改客户订单状态（已签约）成功---自动退款--start----装修业主Id)"
                                +detail.getId().longValue()+"(订单Id：)"+detail.getOrderId().longValue());
                        ResponseEnvelope refund = this.refund(detail.getId().longValue(),detail.getOrderId().longValue());
                        if (!refund.isSuccess()){
                            return refund;
                        }
                        logger.error(LOGPREFIX + "(修改客户订单状态（已签约）成功---自动退款--end)");
                        return new ResponseEnvelope<>(true ,"修改状态完成，退款完成");
                        /**退度币结束**/
                    }
     ...
     * @return
     */
    private ResponseEnvelope refund(Long customerId,Long orderId){
        ...
        if (null!=orderInfo&&orderInfo.size()>0){
            ...
            for (NotBeenSelectedOrderVO info:orderInfo) {
                //不是签约订单且订单支付
                if (!info.getOrderId().equals(orderId)&&info.getOrderStatus()>1){
                    //校验退款用户Id
                    ...
                    //退款成功，写入记录
                    int totalFee = Integer.parseInt(new DecimalFormat("0").format(info.getPrice() * 10 * 10));
                    PayOrder payOrder=buildPayOrder(info.getUserId().intValue(),totalFee);
    ...
    private boolean updateRefundStatus(Long orderId){
        ...
        decorateOrder.setContractFee(new BigDecimal(0));
     ...
     * @return
     */
    private PayOrder buildPayOrder(Integer UserId,Integer totalFee) {
        Date now = new Date();
