...
//	@TriggersRemove(cacheName="baseCache",removeAll=true)
	...
	public List<SysDictionary> findAllByType(String type) {
		...
		List<SysDictionary> list = getPaginatedList(sysDictionarySearch);
		List<SysDictionary> newList = new ArrayList<>();
		if(list != null && list.size() >0){
			for(SysDictionary sysDictionary : list){
				if(sysDictionary.getValue() != 9 && sysDictionary.getValue() != 10
						&& sysDictionary.getValue() !=11 && sysDictionary.getValue() !=12){
					newList.add(sysDictionary);
				}
			}
		}
	...
	@Override
	public List<SysDictionaryGetListDTO> getDTOList(String type) {
		...
		List<SysDictionaryGetListDTO> returnList = new ArrayList<SysDictionaryGetListDTO>();
		
		List<SysDictionary> sysDictionarieList = this.getSomeInfoList(type);
		
		if(Lists.isNotEmpty(sysDictionarieList)) {
			sysDictionarieList.forEach(item -> {
				SysDictionaryGetListDTO sysDictionaryGetListDTO = new SysDictionaryGetListDTO();
				sysDictionaryGetListDTO.setValue(item.getValue());
				sysDictionaryGetListDTO.setName(item.getName());
				returnList.add(sysDictionaryGetListDTO);
			});
		}
	...
	@Override
	public List<SysDictionary> getSomeInfoList(String type) {
		if(StringUtils.isEmpty(type)) {
			logger.error(LOGPREFIX + "StringUtils.isEmpty(type) = true");
			return null;
		}
		
		return sysDictionaryMapper.selectSomeInfoByType(type);
	}
}
