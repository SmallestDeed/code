    ...
    public Boolean call() throws Exception {
        ...
        if (success) {
            ...
            if (renderPicVO.getIsAuto().intValue() == 1) {
                ...
                try {
                    logger.error("saveRenderPicJob====>businessId=" + businessId);
                    designPlanRenderScene = saveRenderPicService.getDesignPlanRenderScene(businessId);
                } catch (Exception e) {
                    e.printStackTrace();
                    logger.error("saveRenderPicJob====>!!!!!!!!!!!!!exception:" + e);
                }
                logger.error("saveRenderPicJob====>designPlanRenderScene=" + designPlanRenderScene);
                ...
                //处理渲染成功的逻辑
                try {
                    saveRenderPicService.handleSuccessfulRender(taskId);
                } catch (Exception e) {
                    logger.error("处理渲染成功的逻辑 ====> exception：" + e);
                }
