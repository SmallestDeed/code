...
public class DesignPlanAutoRenderServiceImpl implements DesignPlanAutoRenderService {
    private static Logger logger = LoggerFactory.getLogger(DesignPlanAutoRenderServiceImpl.class);
    ...
    private AutoRenderTaskState handleFullHouseAutoRenderTask(AutoRenderTaskState taskState) {
        ...
        if (null != subTaskList && 0 < subTaskList.size()) {
            Integer subtaskCount = subTaskList.size();
            ...
            if (subtaskCount == null) {
                ...
                mainTaskState.setState(successed);
                if (null != mainTaskState.getSubtaskCount() && 0 < mainTaskState.getSubtaskCount()) {
    ...
    private void updateFullHouseTaskState(AutoRenderTaskState taskState) {
        ...
        autoRenderTaskState.setState(DesignPlanConstants.TASKSTATE.FAILUE.getValue());
        autoRenderTaskState.setRender720(DesignPlanConstants.RENDER_FAIL);
        autoRenderTaskState.setIsValid(0);
    ...
    private int createFailMessage(AutoRenderTaskState taskState) {
        ...
        logger.error("DB插入渲染失败的消息成功===>id="+add);
        //发送渲染完成消息
        try {
            SocketIOUtil.adviceRenderResultToUser(sysUserMessage);
        } catch (Exception e) {
            logger.error("发送渲染完成的消息异常====>{}", e);
        }
        return add;
    }
    ...
    @Override
    public synchronized AutoRenderTask getRedisTaskList(Integer maxSize, LoginUser loginUser, String renderMachineIp,
        ...
        if (task != null) {
            ...
            taskId = task.getId();
            logger.error("getRedisTaskList ===> taskId="+taskId);
            task2 = designPlanAutoRenderMapper.getRenderTaskById(taskId);
            if (null == task2) {
                Long res = JedisUtils.listAdd(REDIS_TASK_LIST, jsonStr);
                logger.error("reAdd task To taskList  >>>>>>>>>>>> "+res);
            }
            if(task2.getIsDeleted() == 1){
                //被逻辑删除的任务，不处理
                return null;
            }
            if (task2 != null) {
    ...
    public AutoRenderTask getRedisReplaceTaskList(Integer maxSize, LoginUser loginUser, String renderMachineIp,
        ...
        if (task != null) {
            taskId = task.getId();
            logger.error("getRedisReplaceTaskList =====> taskId = "+taskId);
            task2 = designPlanAutoRenderMapper.getRenderTaskById(taskId);
            if (null == task2) {
                long res = JedisUtils.listAdd(REDIS_TASK_REPLACE_LIST, jsonStr);
                logger.error("reAdd replaceTask To taskReplaceList  >>>>>>>>>>>> "+res);
            }
            if (task2 != null) {
            ...
            //如果主任务id和任务id相同，则表示方案为主任务，不进入渲染机，删除缓存中的任务,但是得复制到state表
            if(task2.getId().equals(task2.getMainTaskId())) {
                return null;
            }
        }
        return task2;
    ...
    @Override
    public List<ThumbData> getUserRenderPlanAndTaskList(MobileRenderingSearch search) {
        List<ThumbData> thumbDataList = new ArrayList<>();

        List<MobileRenderingBo> renderingBos = designPlanAutoRenderMapper.selectUserRenderList(search);
        if(CustomerListUtils.isEmpty(renderingBos)){
            return null;
        }
        //未渲染的渲染任务id集合
        List<Integer> taskIdList = renderingBos.stream()
                .filter(e -> (e.getType() == 3))
                .map(mobileRenderingBo -> mobileRenderingBo.getPrimaryId())
                .collect(Collectors.toList());
        //渲染任务状态的id集合(移动端渲染记录)
        List<Integer> taskStateIdList = renderingBos.stream()
                .filter(e -> (e.getType() == 1))
                .map(mobileRenderingBo -> mobileRenderingBo.getPrimaryId())
                .collect(Collectors.toList());
        //PC端渲染的效果图方案id集合
        List<Integer> scenePlanIdList = renderingBos.stream()
                .filter(e -> (e.getType() == 2))
                .map(mobileRenderingBo -> mobileRenderingBo.getPrimaryId())
                .collect(Collectors.toList());
        //分别查询数据
        Map<Integer,ThumbData>  taskStateDataMap = new HashMap<>();
        Map<Integer,ThumbData>  renderPlanDataMap = new HashMap<>();
        Map<Integer,ThumbData>  taskDataMap = new HashMap<>();
        if(search.getPlanSourceType() == null || search.getPlanSourceType() == 1){
            if(CustomerListUtils.isNotEmpty(taskStateIdList)){
                taskStateDataMap = this.getTaskStateRenderData(taskStateIdList,search);
            }
            if(CustomerListUtils.isNotEmpty(taskIdList)){
                taskDataMap = this.getTaskRenderData(taskIdList,search);
            }
        }
        if(search.getPlanSourceType() == null || search.getPlanSourceType() == 2){
            if(CustomerListUtils.isNotEmpty(scenePlanIdList)){
                renderPlanDataMap = this.getRenderScenePlanData(scenePlanIdList,search);
            }
        }

        //拼装数据
        for (MobileRenderingBo renderingBo : renderingBos){
            if(renderingBo.getType() == 1){
                if(taskStateDataMap != null && taskStateDataMap.size() > 0){
                    ThumbData data = taskStateDataMap.get(renderingBo.getPrimaryId());
                    if(data != null){
                        thumbDataList.add(data);
                    }

                }
            }else if(renderingBo.getType() == 2){
                if(renderPlanDataMap != null && renderPlanDataMap.size() > 0){
                    ThumbData data = renderPlanDataMap.get(renderingBo.getPrimaryId());
                    if(data != null){
                        thumbDataList.add(data);
                    }
                }
            }else if(renderingBo.getType() == 3){
                if(taskDataMap != null && taskDataMap.size() > 0){
                    ThumbData data = taskDataMap.get(renderingBo.getPrimaryId());
                    if(data != null){
                        thumbDataList.add(data);
                    }
                }
     ...
     * @param taskIdList
     * @param search
     * @return: java.util.Map<java.lang.Integer,com.nork.design.model.ThumbData>
     */
    private Map<Integer,ThumbData> getTaskRenderData(List<Integer> taskIdList,MobileRenderingSearch search){
        if(CustomerListUtils.isEmpty(taskIdList)){
            return null;
        }
        Map<Integer,ThumbData> thumbDataMap = new HashMap<>(taskIdList.size());

        List<AutoRenderTask> renderTaskList = designPlanAutoRenderMapper.getRenderTaskListByIdList(taskIdList);

        for (AutoRenderTask task : renderTaskList){
            ThumbData data = new ThumbData();
            data.setRenderState(task.getState());
            data.setPlanSourceType(1);
            data.setTaskId(task.getId());
            data.setTaskType(task.getTaskType());

            //渲染中或渲染失败的方案
            data.setPlanHouseType(0);
            data.setCpId(task.getId());
            data.setCtime(Utils.formatDate(task.getGmtCreate(),Utils.DATE_TIME));
            data.setName(task.getDesignName());
            //不可删除别人的方案
            if(Objects.equals(task.getOperationUserId(),search.getUserId())){
                data.setPermitDeleted(true);
                data.setPermitUpdateName(true);
            }
            if(Objects.equals(0,task.getTaskType())){
                //装进我家的任务,planId记录的是推荐方案id
                //查询空间类型及面积
                this.getSpaceInfoByPlanId(task.getPlanId(),0,data);
            }else{
                //替换渲染的任务,planId记录的是效果图方案id
                //查询空间类型及面积
                this.getSpaceInfoByPlanId(task.getPlanId(),1,data);
            }
            //过滤空间类型
            if(search.getSpaceFunctionId() != null){
                //因为有些渲染任务没有记录 spaceFunctionId 字段数据，所以要再次过滤
                if(data.getSpaceFunctionId() != null){
                    if(Objects.equals(search.getSpaceFunctionId(),data.getSpaceFunctionId())){
                        continue;
                    }
                }
            }

            thumbDataMap.put(task.getId(),data);
     ...
     * @return: java.util.Map<java.lang.Integer,com.nork.design.model.ThumbData>
     */
    private Map<Integer,ThumbData> getTaskStateRenderData(List<Integer> taskIdList,MobileRenderingSearch search){
        if(CustomerListUtils.isEmpty(taskIdList)){
            return null;
        }
        Map<Integer,ThumbData> thumbDataMap = new HashMap<>(taskIdList.size());

        List<AutoRenderTaskState> renderTaskStateList = designPlanAutoRenderMapper.getTaskStateListByIdList(taskIdList);
         for (AutoRenderTaskState taskState : renderTaskStateList){
             ThumbData data = new ThumbData();
             data.setRenderState(taskState.getState());
             data.setPlanSourceType(1);
             data.setTaskId(taskState.getTaskId());
             data.setTaskType(taskState.getTaskType());
             if(taskState.getState() == 1){
                //渲染成功的方案
                 if(taskState.getPlanHouseType() == 2){
                     //去除了全屋方案逻辑，所以这块以后再维护
                     //全屋方案
                     FullHouseDesignPlan fullHouseDesignPlan = fullHouseDesignPlanMapper.getFullHousePlanById(taskState.getNewFullHousePlanId());
                     if(fullHouseDesignPlan == null){
                         logger.error("未找到该全屋方案信息,taskStateId:{},fullHousePlanId:{}",taskState.getId(),taskState.getNewFullHousePlanId());
                         continue;
                     }
                     if(fullHouseDesignPlan.getIsDeleted() == 1){
                         continue;
                     }
                     data.setName(fullHouseDesignPlan.getPlanName());
                     data.setCtime(fullHouseDesignPlan.getGmtCreate().toString());
                     data.setCpId(fullHouseDesignPlan.getId());
                     data.setPlanHouseType(AutoRenderTaskConstant.PLAN_FULL_HOUSE_TYPE);
                     //不可删除别人的方案
                     if(Objects.equals(fullHouseDesignPlan.getUserId(),search.getUserId())){
                         data.setPermitDeleted(true);
                         data.setPermitUpdateName(true);
                     }
                 }else{
                     //单空间方案
                    DesignPlanRenderScene planRenderScene = designPlanRenderSceneMapper.get(taskState.getBusinessId());
                    if(planRenderScene == null){
                        continue;
                    }
                    if(planRenderScene.getIsDeleted() == 1){
                        continue;
                    }
                    data.setPlanHouseType(AutoRenderTaskConstant.PLAN_SINGLE_HOUSE_TYPE);
                    data.setName(planRenderScene.getPlanName());
                    data.setCtime(planRenderScene.getGmtCreate().toString());
                    data.setCpId(planRenderScene.getId());
                    data.setSceneId(planRenderScene.getId());
                    //处理封面
                    this.coverPicHandling(planRenderScene,data);
                    //控制按钮显示
                    this.planRenderSceneButtonHandling(planRenderScene,data);
                    //不可删除别人的方案
                    if(Objects.equals(planRenderScene.getUserId(),search.getUserId())){
                        data.setPermitDeleted(true);
                        data.setPermitUpdateName(true);
                    }
                     //查询空间类型及面积
                     this.getSpaceInfoByPlanId(planRenderScene.getId(),1,data);
                 }

             }else{
                 //渲染中或渲染失败的方案
                 data.setPlanHouseType(0);
                 data.setCpId(taskState.getTaskId());
                 data.setCtime(Utils.formatDate(taskState.getGmtCreate(),Utils.DATE_TIME));
                 data.setName(taskState.getDesignName());
                 //不可删除别人的方案
                 if(Objects.equals(taskState.getOperationUserId(),search.getUserId())){
                     data.setPermitDeleted(true);
                     data.setPermitUpdateName(true);
                 }
                 if(Objects.equals(0,taskState.getTaskType())){
                     //装进我家的任务,planId记录的是推荐方案id
                     //查询空间类型及面积
                     this.getSpaceInfoByPlanId(taskState.getPlanId(),0,data);
                 }else{
                     //替换渲染的任务,planId记录的是效果图方案id
                     //查询空间类型及面积
                     this.getSpaceInfoByPlanId(taskState.getPlanId(),1,data);
                 }
             }
             //过滤空间类型
             if(search.getSpaceFunctionId() != null){
             ...
             thumbDataMap.put(taskState.getId(),data);
     ...
     * @author: chenm
     * @date: 2019/1/31 17:16
     * @param scenePlanIdList
     * @param search
     * @return: java.util.Map<java.lang.Integer,com.nork.design.model.ThumbData>
     */
    private Map<Integer,ThumbData> getRenderScenePlanData(List<Integer> scenePlanIdList,MobileRenderingSearch search){
        if(CustomerListUtils.isEmpty(scenePlanIdList)){
            return null;
        }
        Map<Integer,ThumbData> dataMap = new HashMap<>(scenePlanIdList.size());
        List<DesignPlanRenderScene> planRenderSceneList = designPlanRenderSceneMapper.getPlanListByIdList(scenePlanIdList);
        for (DesignPlanRenderScene planRenderScene:planRenderSceneList){
            ThumbData data = new ThumbData();
            data.setCpId(planRenderScene.getId());
            data.setName(planRenderScene.getPlanName());
            data.setCtime(Utils.formatDate(planRenderScene.getGmtCreate(),Utils.DATE_TIME));
            data.setSceneId(planRenderScene.getId());
            data.setPlanSourceType(2);
            data.setPlanHouseType(AutoRenderTaskConstant.PLAN_SINGLE_HOUSE_TYPE);
            //处理封面
            this.coverPicHandling(planRenderScene,data);
            //控制按钮显示
            ...
            //查询空间类型及面积
            this.getSpaceInfoByPlanId(planRenderScene.getId(),1,data);
            dataMap.put(planRenderScene.getId(),data);
        }
    ...
    @Override
    public int selectUserRenderCount(MobileRenderingSearch search) {
        return designPlanAutoRenderMapper.selectUserRenderCount(search);
    }

    @Override
    public Object deleteTaskAndTaskStateByTaskId(Integer taskId, LoginUser loginUser) {

        AutoRenderTask task = new AutoRenderTask();
        task.setId(taskId);
        task.setIsDeleted(1);
        task.setGmtModified(new Date());
        task.setModifier(loginUser.getLoginName());
        int ret = designPlanAutoRenderMapper.updateTaskById(task);
        AutoRenderTaskState taskState = new AutoRenderTaskState();
        taskState.setTaskId(taskId);
        taskState.setModifier(loginUser.getLoginName());
        taskState.setGmtModified(new Date());
    ...
    @Override
    public int updateAutoRenderTask(AutoRenderTask autoRenderTask) {
        return designPlanAutoRenderMapper.updateAutoRenderTask(autoRenderTask);
    }



    /**
     * 查询效果图方案有哪些渲染图，控制按钮显示
     * @author: chenm
     * @date: 2019/1/28 20:11
     * @param scene 效果图方案
     * @param thumbData 返回对象
     * @return: void
     */
    public void planRenderSceneButtonHandling(DesignPlanRenderScene scene, ThumbData thumbData) {
        if (scene == null || thumbData == null) {
            return;
        }
        List<ResRenderPic> picList = new ArrayList<>(); // 查询该设计方案的全部渲染缩略图列表
     ...
     * @param planId 效果图方案/推荐方案 id
     * @param planType 方案类型 0:推荐方案;1:效果图方案
     * @param thumbData
     * @return: com.nork.design.model.ThumbData
     */
    public ThumbData getSpaceInfoByPlanId(Integer planId,Integer planType,ThumbData thumbData){
        if(planId == null || planType == null){
            return thumbData;
        }
        SpaceCommon spaceCommon = null;
        if(Objects.equals(planType,0)){
            DesignPlanRecommended planRecommended = designPlanRecommendedServiceV2.get(planId);
            if(planRecommended != null && planRecommended.getSpaceCommonId() != null){
                spaceCommon = spaceCommonMapper.selectByPrimaryKey(planRecommended.getSpaceCommonId());
            }

        }else if(Objects.equals(planType,1)){
            DesignPlanRenderScene planRenderScene = designPlanRenderSceneMapper.get(planId);
            if(planRenderScene != null && planRenderScene.getSpaceCommonId() != null){
                spaceCommon = spaceCommonMapper.selectByPrimaryKey(planRenderScene.getSpaceCommonId());
            }
        }
        if(spaceCommon != null){
            thumbData.setSpaceCommonId(spaceCommon.getId());
            thumbData.setSpaceArea(spaceCommon.getSpaceAreas());
            if(spaceCommon.getSpaceFunctionId() != null){
                thumbData.setSpaceFunctionId(spaceCommon.getSpaceFunctionId());
                SysDictionary dictionary = sysDictionaryService.findOneByTypeAndValue(SysDictionaryConstant.SYS_DICTIONARY_HOUSE_TYPE,spaceCommon.getSpaceFunctionId());
                thumbData.setSpaceType(dictionary != null ? dictionary.getName() : null);
            }
        }
