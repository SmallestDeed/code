...
//		LoginUser loginUser = (LoginUser) JedisUtils2.getObject(cacheKey);
	...
	public ResponseEnvelope addDesingPlanCollect(@RequestBody DesignPlanRecommendedModel model, HttpServletRequest request) {
        ...
        Integer planType = model.getPlanHouseType();
        //获取登录用户
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        envelope.setMsgId(msgId);

        if (StringUtils.isEmpty(favoriate_businessId)) {
            envelope.setSuccess(false);
            envelope.setMessage("params error");
            return envelope;
        }
        ...
        //add by xiaoxc20181119
        if (DesignPlanType.RECOMMENDED_PLAN_TYPE == planType) {
            favoriteRef.setRecommendId(recommendId.intValue());
            favoriteRef.setDesignPlanType(DesignPlanType.COLLECT_RECOMMENDED_PLAN_TYPE);
        } else if (DesignPlanType.FULL_HOUSE_PLAN_TYPE == planType) {
            favoriteRef.setFullHouseDesignPlanId(recommendId);
            favoriteRef.setDesignPlanType(DesignPlanType.COLLECT_FULL_HOUSE_PLAN_TYPE);
        } else {
            favoriteRef.setRecommendId(recommendId.intValue());
            favoriteRef.setDesignPlanType(DesignPlanType.COLLECT_RECOMMENDED_PLAN_TYPE);
        }
