...
public class DesignPlanAutoRenderResouceServiceImpl implements DesignPlanAutoRenderResouceService {
	...
	public Integer saveRenderPicOf720(DesignPlan designPlan, Map<String, MultipartFile> fileMap, Integer viewPoint,
			...
			Integer panoLevel, String roamJson, Integer sourcePlanId, Integer templateId, Integer needShear) {
		logger.error("-----------------------------------进入");
		/* 应用根节点目录 */
		/* 渲染图存放相对路径 */
...
//						 resScreenPicId = resRenderPicService.add(resPic);
						...
						logger.error("=====从装进我家进来的保存截图=======resScreenPicId："+resScreenPicId);
...
//						resRenderPicService.update(rrp);
					...
					logger.error("-----------------------------------进入 保存截图结束");
				} else {// 保存曲线图
					logger.error("-----------------------------------进入 保存漫游");
					// 如果为720漫游
					if (RenderTypeCode.ROAM_720_LEVEL == renderingType) {
						resPicId = saveRoam720(resRenderPic, storePath, storeRealPath, mf, bigFileName, viewPoint,
								scene, renderingType, loginUser, designPlan, panoLevel, resScreenPicId, date,sourcePlanId, needShear);
						...
						logger.error("=---------------renderFilePath="+renderFilePath);
                        logger.error("=---------------renderFileDir="+renderFileDir);
                        File renderPic = new File(renderFilePath);
                        logger.error("=---------------renderPic="+renderPic);
						if(SystemCommonConstant.COMPRESSION_IMAGE.equalsIgnoreCase("Y")) {
							FileUploadUtils.saveCompression720Image(mf.getInputStream(), renderPic,panoLevel);
						}else{
							FileUtils.copyInputStreamToFile(mf.getInputStream(), renderPic);
						}
                        logger.error("=---------------copyInputStreamToFile 处理完成");
						/* 保存渲染图片 */
						...
						if (resScreenPicId != null && resScreenPicId > 0) {
							...
							logger.error("=====从装进我家进来的保存不是漫游曲线图，截图id不为空=======resPicId："+resPicId+" ,resScreenPicId:"+resScreenPicId);
						} else {
							resPicId = this.add(resRenderPic);// 保存曲线图
							logger.error("=====从装进我家进来的保存不是漫游曲线图，截图id为空=======resPicId："+resPicId);
						}
					}
					logger.error("-----------------------------------进入 保存漫游结束");
				}
...
//					}
					...
					if (resScreenPicId != null && resScreenPicId > 0) {
						...
						logger.error("=====从装进我家进来的保存缩略图，截图id不为空=======smallPicId："+smallPicId+" ,resScreenPicId:"+resScreenPicId);
					} else {
						// smallPicId =
						// resRenderPicService.add(smallRenderResPic);
						smallPicId = this.add(smallRenderResPic);
						logger.error("=====从装进我家进来的保存缩略图，截图id为空=======smallPicId："+smallPicId);
					}
...
//		ResPic resPic = new ResPic();
	...
	public int saveRoam720(ResRenderPic resRenderPic, String storePath, String storeRealPath, MultipartFile mf,
			...
			DesignPlan designPlan, Integer panoLevel, Integer resScreenPicId, Date date,Integer sourcePlanId,Integer needShear) {
		logger.error("==============================saveRoam720");
		int resPicId = 0;
		/* 保存渲染图片 */
		...
		if (resScreenPicId != null && resScreenPicId > 0) {
			resRenderPic.setSysTaskPicId(resScreenPicId);// 关联截图信息
			resPicId = this.add(resRenderPic);// 保存曲线图
			logger.error("=====从装进我家进来的保存漫游曲线图，截图id不为空=======resPicId："+resPicId+" ,resScreenPicId:"+resScreenPicId);
		} else {
			// resPicId = resRenderPicService.add(resRenderPic);// 保存曲线图
			resPicId = this.add(resRenderPic);// 保存曲线图
			logger.error("=====从装进我家进来的保存漫游曲线图，截图id为空=======resPicId："+resPicId);

		}
		logger.error("==============================saveRoam720 end");
		return resPicId;
	}
