...
public class OptimizePlanServiceImpl implements OptimizePlanService {
	...
	@Override
	public Integer saveAsRenderBakScene(DesignPlanRes designPlanRes, LoginUser loginUser, Integer platformId, RenderPicVO renderPicVO) {
		...
		designPlanEctype.setSpellingFlowerFileId(designPlan.getSpellingFlowerFileId());
		designPlanEctype.setSpellingFlowerProduct(designPlan.getSpellingFlowerProduct());
		logger.info("optimizePlanServiceImpl--->saveAsRenderBakScene---->designPlanEctype-:"+designPlanEctype);
		Integer desiginPlanSceneId = designPlanRenderSceneService.add(designPlanEctype);// 效果图方案
		logger.error("自动渲染111111 +++++++++++++++++++++++++ saveAsRenderBakScene designPlanSceneId add====>" + desiginPlanSceneId);
		...
		newDesignPlan.setPlatformId(platformId);

		Integer sketchId = designPlanService.add(newDesignPlan);// 草图方案
        logger.error("自动渲染11111111 +++++++++++++++++++++++++++++++++++++++++++++++>草图方案id" + sketchId);
		newDesignPlan.setId(sketchId);
		renderPicVO.setDesignPlan(newDesignPlan);
		// 设计方案存副本 ->end
...
//				}
				...
				if (resPic.getRenderingType() == RenderTypeCode.COMMON_720_LEVEL) {
					try {
					    resPic.setSysTaskPicId(null);
						logger.error("OptimizePlanServiceImpl====>update:"+resPic);
						resRenderPicService.update(resPic);
					} catch (Exception e) {
						e.printStackTrace();
						logger.error("OptimizePlanServiceImpl====>exception:"+e);
					}
					//记录下新的多点的渲染截图的id
					if(resPic.getFileKey().equals(ResProperties.DESIGNPLAN_RENDER_FILEKEY)) {
						//将所有720单点的图的sys_task_pic_id修改为新的截图的id
						try {
							logger.error("OptimizePlanServiceImpl====>businessId="+sketchId+", picId="+picId);
							resRenderPicService.updateAllByBusinessId(sketchId,picId);
						} catch (Exception e) {
							e.printStackTrace();
							logger.error("OptimizePlanServiceImpl====>exception:"+e);
						}
				...
				if (resPic.getRenderingType().intValue() == 4
						|| resPic.getRenderingType().intValue() == 1) {
					// 获取自动渲染的720的缩略图id
...
//		Integer resFileId = designPlanService.saveRenderRoamConfig(null,roamJsonArray.toString(),renderRoamId);
	 ...
	 * @return
	 */
	public Integer getPlatformIdBytaskId(Integer taskId) {
		return optimizePlanMapper.getPlatformIdBytaskId(taskId);
	}
