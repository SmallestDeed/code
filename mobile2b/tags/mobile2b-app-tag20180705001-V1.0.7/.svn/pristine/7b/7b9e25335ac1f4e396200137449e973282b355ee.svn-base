		...
		try {
			...
			if (Utils.enableRedisCache()) {
				if (userType != null && userType.intValue() == 1) {
					total = DesignTempletCacher.getTotalWithParameter2(designTempletSearch, userType,
							loginUser.getId());
				} else {
			...
			if (total > 0) {
				if (Utils.enableRedisCache()) {
					if (userType != null && userType.intValue() == 1) {
						list = DesignTempletCacher.getPageWithParameter2(designTempletSearch, userType,
								loginUser.getId());
					} else {
