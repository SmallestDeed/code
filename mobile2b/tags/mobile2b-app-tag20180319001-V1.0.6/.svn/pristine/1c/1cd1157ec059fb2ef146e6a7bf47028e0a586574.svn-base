...
//		productResult.getProductTypeCode(),productResult.getProductSmallTypeCode(),spaceCommonId,designTempletId,new DesignRules(),map,request);
	...
	public BaseProductSearch getProductInfoFilter(BaseProductSearch searchProduct,DesignProductResult baimoProduct) {
		...
		String curtainLength = Utils.getValue("app.filter.3mAbove.stretch.length", "20");
		String bjType = Utils.getValue("app.smallProductType.stretch", "");
		boolean stretch = Utils.isMateProductType(baimoProduct.getSmallTypeKey(),bjType);
		if (stretch) {
...
//				}
				...
				if( StringUtils.isNotBlank(fullPaveLength) && Utils.getIntValue(fullPaveLength) > 0
					...
					//窗帘3米以上过滤40cm
					if( "16".equals(baimoProduct.getBigTypeValue()) && Utils.getIntValue(fullPaveLength) > 300){
						searchProduct.setStartLength((Utils.getIntValue(fullPaveLength) - Utils.getIntValue(curtainLength))+1);
						searchProduct.setEndLength(Utils.getIntValue(fullPaveLength) + Utils.getIntValue(curtainLength));
					}else{
						searchProduct.setStartLength((Utils.getIntValue(fullPaveLength) - Utils.getIntValue(filterLength))+1);
						searchProduct.setEndLength(Utils.getIntValue(fullPaveLength) + Utils.getIntValue(filterLength));
					}
					searchProduct.setStretchHeight(productHeight);
				}else{
...
//			this.setBasicModelMap(categoryProductResult, autoCarryMap, map, designPlan, designPlanProduct, mediaType);
		...
		ResponseEnvelope<CategoryProductResult> envelope = new ResponseEnvelope<>();
		if(Lists.isNotEmpty(datalist)) {
			envelope.setDatalist(datalist);
			envelope.setTotalCount(idList.size());
		}
