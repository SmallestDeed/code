...
//		storeRealPath=storeRealPath.replace("[code]", plan.getPlanCode());
	...
	public void getSingleTexture(BaseProduct product,List<SplitTextureDTO> splitTextureDTOList) {
		...
		if(resTexture!=null && resTexture.getPicId() !=null && resTexture.getPicId().intValue() > 0) {
...
//					flag = FileUploadUtils.saveFile(map);
					...
					if (flag) {
						...
						logger.error("=====从替换渲染进来的保存截图=======resScreenPicId："+resScreenPicId);
...
//								new File(storeRealPath, bigFileName));
						if(SystemCommonConstant.COMPRESSION_IMAGE.equalsIgnoreCase("Y")) {
							FileUploadUtils.saveCompression720Image(mf.getInputStream(), new File(storeRealPath, bigFileName),panoLevel);
						}else{
						...
						if (resScreenPicId != null && resScreenPicId > 0) {
							...
							logger.error("=====从替换渲染进来的保存曲线图，截图id不为空=======resPicId："+resPicId+" ,resScreenPicId:"+resScreenPicId);
						} else {
							resPicId = resRenderPicService.add(resRenderPic);// 保存曲线图
							logger.error("=====从替换渲染进来的保存曲线图，截图id为空=======resPicId："+resPicId);
...
//					}
					...
					logger.error("Begin call ThumbnailUtil==> + serverFilePath" + serverFilePath);
...
//							ThumbnailUtil.PIC_WIDTH,ThumbnailUtil.PIC_HIGHT);
					...
					ThumbnailUtil.createThumbnail2(mf,("linux".equals(FileUploadUtils.SYSTEM_FORMAT))
									? targetSmallFilePath : targetSmallFilePath.replaceAll("/", "\\\\"),
							ThumbnailUtil.PIC_WIDTH,ThumbnailUtil.PIC_HIGHT);
					/* 保存缩略图 */
					File targetSmallFile = new File(targetSmallFilePath);
					...
					if (resScreenPicId != null && resScreenPicId > 0) {
						...
						logger.error("=====从替换渲染进来的保存缩略图，截图id不为空=======smallPicId："+smallPicId+" ,resScreenPicId:"+resScreenPicId);

					} else {
						smallPicId = resRenderPicService.add(smallRenderResPic);
						logger.error("=====从替换渲染进来的保存缩略图，截图id为空=======smallPicId："+smallPicId);
...
//			response.setContentType("text/html;charset=utf-8");
		...
		renderSuccess = resPicService.savePlanRenderPicOfPhot(designPlan.getId(), list, level,renderingType,sourcePlanId,templateId,taskId);
...
//					new File(storeRealPath, bigFileName));
		...
		if (resScreenPicId != null && resScreenPicId > 0) {
			resRenderPic.setSysTaskPicId(resScreenPicId);// 关联截图信息
			resPicId = resRenderPicService.add(resRenderPic);// 保存曲线图
			logger.error("=====从替换渲染进来的保存漫游曲线图，截图id不为空=======resPicId："+resPicId+" ,resScreenPicId:"+resScreenPicId);
		} else {
			resPicId = resRenderPicService.add(resRenderPic);// 保存曲线图
			logger.error("=====从替换渲染进来的保存漫游曲线图,截图id为空=======resPicId："+resPicId);
		}
		return resPicId;
	}
	...
	public ResponseEnvelope<DesignPlan> updatePlanConfig(LoginUser loginUser,Integer planId,Integer opType,String context,String bDirtyConfig,String msgId){
		...
		try {
			logger.error("updatePlanConfig  =====> planId =  " + planId);
