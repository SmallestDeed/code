...
public class DesignPlanRecommendFavoriteServiceImpl implements DesignPlanRecommendFavoriteService {
    ...
    public ResponseEnvelope<DesignPlanRecommendedResult> favoritePlanRecommendedList(FavoriteRecommendedModel model,
        ...
        Integer franchiserId = model.getFranchiserId();//经销商主账号ID
...
//		brandIds.add(-1);//无品牌
        ...
        if (userType.intValue() == 3) {        //经销商获取父公司
...
//			BaseCompany baseCompany = baseCompanyService.get(companyId);
//			if(null != baseCompany) {
////				String brandIdsStr = baseCompany.getBrandIds();
//				String brandIdsStr = "";
...
//					brandIds = Utils.getIntegerListFromStringList(brandIdsStr);
//				}else {
...
//		}
        List<Favorite> franchiserFavoriteList = new ArrayList<>();
        if (null != franchiserId) {
            Favorite favorite = new Favorite();
            favorite.setUserId(franchiserId);
            franchiserFavoriteList = sysBusinessFavoriteMapper.listFavorites(favorite);
        }
        DesignPlanRecommended designPlanRecommended = new DesignPlanRecommended();

        //将无品牌的推荐方案和收藏方案显示给所有用户
//		brandIds.add(-1);
//		designPlanRecommended.setBrandIds(brandIds);
        designPlanRecommended.setCompanyId(companyId);
        designPlanRecommended.setFranchiserId(franchiserId);
        designPlanRecommended.setFranchiserFavoriteList(franchiserFavoriteList);
        ...
        if (StringUtils.isNotEmpty(start)) {
            Integer page = Integer.parseInt(start);
            designPlanRecommended.setStart(page * Integer.parseInt(limit));
        }
        designPlanRecommended.setUserId(userId);
        if (type.intValue() == 1) {
            total = designPlanRecommendedServiceV2.getAllFavoritePlanRecommendedCount(designPlanRecommended);
        } else {
            total = designPlanRecommendedServiceV2.getFavoritePlanRecommendedCount(designPlanRecommended);
        }
        if (total > 0) {
            if (type.intValue() == 1) {
                list = designPlanRecommendedServiceV2.getAllFavoritePlanRecommendedList(designPlanRecommended);
            } else {
                list = designPlanRecommendedServiceV2.getFavoritePlanRecommendedList(designPlanRecommended);
            }
            if (list != null && list.size() > 0) {
                /**
                 * add by qtq 增加显现后方案收费
                 */
                List<Integer> designPlanIds = new ArrayList<>();
                designPlanIds = designPlanRecommendedServiceV2.selectDesignPlanIdsFromRecord(userId);
                Map<Integer, DesignPlanRecommended> designPlanRecommendedLists = this.getDesignPlanRecommendedLists(list);
                for (DesignPlanRecommendedResult result : list) {
                    ...
                    //处理方案收费逻辑
                    this.handleDesignPlanFee(designPlanRecommendedLists, result, designPlanIds, sysUser.getCompanyId());
                }
            }
        }
    ...
    private void handleDesignPlanFee(Map<Integer, DesignPlanRecommended> designPlanRecommendedLists, DesignPlanRecommendedResult result, List<Integer> designPlanIds, Integer companyId) {
        DesignPlanRecommended remcommended = designPlanRecommendedLists.get(result.getPlanRecommendedId());
        result.setChargeType(remcommended.getChargeType());
        result.setPlanPrice(remcommended.getPlanPrice());
        if (Objects.equals(result.getChargeType(), 1) && !designPlanIds.contains(result.getPlanRecommendedId())) {
            //判断是否是同一公司的方案
            if (Objects.equals(remcommended.getCompanyId(), companyId)) {
    ...
    private Map<Integer, DesignPlanRecommended> getDesignPlanRecommendedLists(List<DesignPlanRecommendedResult> list) {
        Set<Integer> planRecommendedIds = list.stream()
                .map(DesignPlanRecommendedResult::getPlanRecommendedId)
                .collect(Collectors.toSet());

        List<DesignPlanRecommended> lists = designPlanRecommendedServiceV2.bacchQueryByIds(planRecommendedIds);

        Map<Integer, DesignPlanRecommended> map = null;
        if (lists != null && !lists.isEmpty()) {
            map = lists.stream().collect(Collectors.toMap(DesignPlanRecommended::getId, k -> k, (k1, k2) -> k1));
        }
        return map;
    }
    ...
    public String getHasCollected(DesignPlanRecommendFavoriteRef ref) {
        ...
        String bid = sysBusinessFavoriteMapper.getHasCollected(ref);
        if (bid == null) {
            return "0";//0表示没有被收藏
        }
        return bid;
