...
import com.nork.product.controller.BaseCommonController;
...
public class WebBaseProductControllerV2 extends BaseCommonController {
	...
	public Object productDetailsWeb(@PathVariable String style, @ModelAttribute("baseProduct") BaseProduct baseProduct,
			Integer designPlanId, Integer spaceCommonId, String planProductId, HttpServletRequest request,
		...
		baseProduct = (Utils.enableRedisCache()? BaseProductCacher.get(id)
				: baseProductService.get(id));
		if (baseProduct == null) {
			return new ResponseEnvelope<BaseProduct>(false, "产品不存在!", msgId);
		}
		// 获取该用户绑定的序列号品牌
		baseProduct = baseProductService.getAuthorized(baseProduct, loginUser);
		// 产品材质处理
		baseProductService.productMaterial(baseProduct, id, planProductId, designPlanId, spaceCommonId,
				loginUser, request);

		responseEnvelope = new ResponseEnvelope<BaseProduct>(baseProduct, msgId, true);
		if (Utils.enableRedisCache()) {
			CommonCacher.addAll(ModuleType.BaseProduct, "productDetailsWeb", paramsMap, responseEnvelope);
		}
		return responseEnvelope;
	}
	...
	@ResponseBody
	public ResponseEnvelope<CategoryProductResult> list(@PathVariable String style, @ModelAttribute("baseProductSearch") BaseProduct baseProduct,
			HttpServletRequest request, HttpServletResponse response) {
		LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
		int total = 0;
		List<CategoryProductResult> list = baseProductService.list(baseProduct, loginUser, total);
	...
	@ResponseBody
	public ResponseEnvelope<BaseProduct> findSameTypeProductList(Integer productId,Integer designPlanId, Integer planProductId,Integer spaceCommonId,
			String msgId, HttpServletRequest request, HttpServletResponse response) {
		if( StringUtils.isBlank(msgId) || productId == null){
			return new ResponseEnvelope<BaseProduct>(false, "产品或msgId参数为空!", msgId);
		}
		LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
		...
		baseProductNew = Utils.enableRedisCache() ? BaseProductCacher.get(productId) : baseProductService.get(productId);	
		...
		bp = Utils.enableRedisCache() ? BaseProductCacher.get(id) : baseProductService.get(id);	
		if( bp == null ){
			return new ResponseEnvelope<BaseProduct>(total, list, msgId);
		}
		FindSameTypeProductListDTO result = baseProductService.findSameTypeProductList(baseProductNew,loginUser,bp,productId);
	...
	public Object categoryProductList(@ModelAttribute("baseProduct") BaseProduct baseProduct,
		...
		BaseProduct product = baseProductService.categoryProductList(baseProduct);
