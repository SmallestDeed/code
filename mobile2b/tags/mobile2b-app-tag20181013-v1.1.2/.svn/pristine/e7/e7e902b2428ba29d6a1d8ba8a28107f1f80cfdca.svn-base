...
public class WebDesignPlanAutoRenderController {
	...
	public ResponseEnvelope<AutoRenderTask> getRenderTaskList(HttpServletRequest request, HttpServletResponse response,Integer maxSize,String msgId
		...
		// 置顶任务 最高优先级
        AutoRenderTask stickRenderTask = null;
        if(StringUtils.isNotEmpty(renderMachineIp) && renderMachineIp.startsWith("10.10.20.6")) {
             stickRenderTask = designPlanAutoRenderServiceImpl.getRedisStickTaskList(maxSize, loginUser,renderMachineIp,renderLevel,renderProgramVersion);
        }
...
//		AutoRenderTaskState autoRenderTaskState = null;
	...
	@RequestMapping("/retryTaskById")
	@ResponseBody
	public ResponseEnvelope<AutoRenderTaskState> reTryTask(HttpServletRequest request, HttpServletResponse response,Integer taskId) throws UnknownHostException{
		String token = request.getHeader("Authorization");
		AutoRenderTaskState autoRenderTaskState = designPlanAutoRenderServiceImpl.getStateByTaskId(taskId);
		LoginUser loginUser = SystemCommonUtil.checkLoginUserIsValidByToken(token);
		if(loginUser == null) {
			 return new ResponseEnvelope<AutoRenderTaskState>(false, "清登录", "err");
		}
		return designPlanAutoRenderServiceImpl.handleTimeOut(autoRenderTaskState, loginUser);
	}
