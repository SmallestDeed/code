...
public class DesignPlanProductServiceImpl implements DesignPlanProductService
{
    ...
    private List<ProductsCostType> newCostTypeList(List<ProductsCost> productsCosts, costListEnum type)
    {
        // 参数验证/处理 ->start
        if (productsCosts == null || productsCosts.size() <= 0)
        {
            return null;
        }
        if (type == null)
        ...
        if (type == costListEnum.designPlan)
        {
            return designPlanProductMapper.newCostTypeList(productsCosts);
        } else if (type == costListEnum.designPlanRenderScene)
        {
            return designPlanProductRenderSceneService.newCostTypeList(productsCosts);
        } else if (type == costListEnum.designPlanRecommended)
        {
            return designPlanRecommendedProductServiceV2.newCostTypeList(productsCosts);
        } else if (type == costListEnum.oneKeyDesignPlan)
        {
            return optimizePlanMapper.newCostTypeList(productsCosts);
        } else
        {
...
//				usedConfigPath = usedConfigPath.replace("[code]",dpp.getPlanCode());
    ...
    public List<ProductsCostType> costList(LoginUser loginUser, DesignPlanProduct designPlanProduct, costListEnum type)
    {
        ...
        if (null != companyId && companyId > 0)
        {
            ...
            if (3 == sysUser.getUserType().intValue())
            {//经销商
                brandIdsStr = baseCompany.getBrandIds();
            } else if (2 == sysUser.getUserType())
            {//厂商
                brandIdsStr = baseBrandService.getBrandIdsStrByCompanyId(companyId);
            } else
            {//其他的都没有品牌
            ...
            if (null != baseCompany)
            {
                ...
                if (null != brandIdsStr && !"".equals(brandIdsStr))
                {
                    slist = Utils.getListFromStr(brandIdsStr);
                    brandIdsList = Utils.getIntegerListFromStringList(brandIdsStr);
...
//						//大小类
//						baseProduct.setProductTypeValue("" + bo.getBigTypeValue());
//						baseProduct.setProductSmallTypeValue(bo.getSmallTypeValue());
...
//				costType.setBaseProduct(baseProductList);
                ...
                for (ProductsCost cost : costList)
                {
					...
					List<ProductCostDetail> productCostDetails = new ArrayList<>();
					if(costDetails!=null&&costDetails.size()>0){
						for (ProductCostDetail productCostDetail : costDetails) {
							...
							if(null != baseProduct) {
								if(!brandIdsList.contains(baseProduct.getBrandId())) {
							...
							if (2 == sysUser.getUserType() && productCostDetail.getCompanyId() == companyId) {
								productCostDetails.add(productCostDetail);
							} else {
								if (productCostDetail!=null && productCostDetail.getSecrecyFlag()!=null &&
										productCostDetail.getSecrecyFlag().intValue() == 0) {
									productCostDetails.add(productCostDetail);
								}
							}
						}
					}
					cost.setDetailList(productCostDetails);
				}
    ...
    private List<ProductsCost> newCostList(List<ProductCostDetail> productCostDetails, costListEnum type)
    {
        // 参数验证/参数处理 ->start
        if (productCostDetails == null || productCostDetails.size() <= 0)
        {
            return null;
        }
        if (type == null)
        ...
        if (type == costListEnum.designPlan)
        {
            return designPlanProductMapper.newCostList(productCostDetails);
        } else if (type == costListEnum.designPlanRenderScene)
        {
            return designPlanProductRenderSceneService.newCostList(productCostDetails);
        } else if (type == costListEnum.designPlanRecommended)
        {
            return designPlanRecommendedProductServiceV2.newCostList(productCostDetails);
        } else if (type == costListEnum.oneKeyDesignPlan)
        {
            return optimizePlanMapper.newCostList(productCostDetails);
        } else
        {
    ...
    private List<ProductCostDetail> newCostDetail(DesignPlanProduct designPlanProduct, costListEnum type)
    {
        // 参数验证/参数处理 ->start
        if (designPlanProduct == null)
        {
        ...
        if (type == costListEnum.designPlan)
        {
            List<ProductCostDetail> productCostDetailsPartOne = designPlanProductMapper.costDetailPartOne(designPlanProduct);
            if (productCostDetailsPartOne == null || productCostDetailsPartOne.size() == 0)
                return null;
            List<ProductCostDetail> productCostDetailsPartTwo = designPlanProductMapper.costDetailPartTwo(productCostDetailsPartOne);

            for (ProductCostDetail partOne : productCostDetailsPartOne)
            {
                for (ProductCostDetail partTwo : productCostDetailsPartTwo)
                {
                    if (partTwo.getProductId() == partOne.getProductId())
                    {
                        partOne.setBrandName(partTwo.getBrandName());
                        partOne.setProductPicPath(partTwo.getProductPicPath());
                        partOne.setProCategoryLongCode(partTwo.getProCategoryLongCode());
                        partOne.setProductOriginalPicPath(partTwo.getProductOriginalPicPath());
                        partOne.setProductUnit(partTwo.getProductUnit());
                        partOne.setProCategoryId(partTwo.getProCategoryId());
                    }
                }
            }
            return productCostDetailsPartOne;
        } else if (type == costListEnum.designPlanRenderScene)
        {
            return designPlanProductRenderSceneService.newCostDetail(designPlanProduct);
        } else if (type == costListEnum.designPlanRecommended)
        {
            return designPlanRecommendedProductServiceV2.newCostDetail(designPlanProduct);
        } else if (type == costListEnum.oneKeyDesignPlan)
        {
            List<ProductCostDetail> productCostDetailsPartOne = optimizePlanMapper.costDetailPartOne(designPlanProduct);
            if (productCostDetailsPartOne == null || productCostDetailsPartOne.size() == 0)
                return null;
            List<ProductCostDetail> productCostDetailsPartTwo = optimizePlanMapper.costDetailPartTwo(productCostDetailsPartOne);
...
//			autoUpdateStructureRelation(designTempletId);
    ...
    public List<DesignPlanProduct> getListByPlanId(Integer planId)
    {
        ...
        return designPlanProductMapper.getListByPlanIdAndIsDeleted(planId, 0);
    }
...
//						SysDictionary sysDictionary = new SysDictionary();
                                ...
                                && StringUtils.isNotBlank(productTypeValue))
                        {
                            ...
                            if (ProductTypeConstant.PRODUCT_BIG_TYPE_QIANGM.equals(sysDictionary.getType()))
                            {//墙面
...
//		List<BaseProduct> baseProductList = baseProductMapper.getByDesignPlanId(designPlanId);
     ...
     */
    @Override
    public boolean updatePlanProducts(String planProductJson, Integer planId, LoginUser loginUser)
    {
...
//		}
        Gson gson2 = new Gson();
        List<PlanProductJsonModel> modelList = gson2.fromJson(planProductJson, new TypeToken<List<PlanProductJsonModel>>()
        {
        }.getType());
     ...
     */
    public List<ProductsCostType> newCostList(LoginUser loginUser, DesignPlanProduct designPlanProduct, costListEnum type)
    {
        ...
        //去掉序列号的逻辑改从用户关联的企业取品牌信息
        // 获取企业关联的品牌id集合
        List<Integer> brandIdsList = getBrandIdsList(sysUser);

		/*add品牌,大类,小类,产品查询条件*/
        List<BaseProduct> baseProductList = new ArrayList<>();

        List<BaseBrand> baseBrands = baseBrandService.getListByIds(brandIdsList);
        boolean flag = false;
        for (Integer brandId : brandIdsList)
        {
            for (BaseBrand baseBrand : baseBrands)
            {
                if (baseBrand.getId() == brandId && Utils.isSanduUser(baseBrand.getBrandReferred()))
                {
                    flag = true;
                    break;
                }
        ...
        // update by huangsongbo 2017.12.8
        if (Utils.isExternalUser(sysUser.getUserType()))
        {
            //用户序列号如果绑定三度则产品清单不做过滤
            if (!flag)
            {
                List<BaseBrand> baseBrandList = new ArrayList<>();
                for (Integer brandId : brandIdsList)
                {
                    for (BaseBrand baseBrand : baseBrands)
                    {
                        if (baseBrand.getId() == brandId)
                        {
                            baseBrandList.add(baseBrand);
                            break;
                        }
                    }
                }
        ...
        designPlanProduct.setStart(-1);
        designPlanProduct.setLimit(-1);
        List<ProductCostDetail> newCostDetails = this.newCostDetail(designPlanProduct, type);

        if (newCostDetails != null && newCostDetails.size() > 0)
        {
            List<ProductsCost> newCostList = this.newCostList(newCostDetails, type);

            if (newCostList != null && newCostList.size() > 0)
            {
                List<ProductsCostType> newCostTypeList = this.newCostTypeList(newCostList, type);

                for (ProductCostDetail detail : newCostDetails)
                {
                    for (ProductsCost c : newCostList)
                    {
                        if (c == null) continue;
                        if (detail.getAtt2().equals(c.getCostTypeCode()))
                        {
                            if (c.getDetailList() == null)
                            {
                                c.setDetailList(new ArrayList<>());
                            }
                            //detail.setTotalPrice(detail.getUnitPrice().multiply(BigDecimal.valueOf(detail.getCount())));
                            c.getDetailList().add(detail);
                            if (null == c.getProductCount())
                                c.setProductCount(0);
                            c.setProductCount(c.getProductCount() + detail.getCount());
                            if (null == c.getTotalPrice())
                            {
                                c.setTotalPrice(new BigDecimal(0));
                            }
                            if (null == c.getProductIds())
                            {
                                c.setProductIds("" + detail.getProductId());
                            } else
                            {
                                c.setProductIds(c.getProductIds() + "," + detail.getProductId());
                            }
                            c.setTotalPrice(c.getTotalPrice().add(detail.getTotalPrice()));
                            break;
                        }
                    }
                }

                for (ProductsCostType costType : newCostTypeList)
                {
                    List<ProductsCost> pcList = new ArrayList<>();
                    Integer productCount = 0;
                    costType.setPlanId(designPlanProduct.getPlanId());
                    costType.setUserId(sysUser.getId());
                    for (ProductsCost pc : newCostList)
                    {
                        if (pc == null) continue;
                        String[] costCodes = costType.getCostCodes().split(",");
                        for (String costCode : costCodes)
                        {
                            if (costCode.equals(pc.getCostTypeCode()))
                            {
                                pcList.add(pc);
                                productCount += pc.getProductCount();
                                pc.setPlanId(designPlanProduct.getPlanId());
                                pc.setUserId(sysUser.getId());

                                if (dictionary != null)
                                {
                                    pc.setSalePriceValueName(dictionary.getName());
                                }
                                if ("1".equals(pc.getCostTypeValue()) || "2".equals(pc.getCostTypeValue()) || "3".equals(pc.getCostTypeValue()))
                                {
                                    pc.setTotalPrice(null);
                                } else
                                {
                                    if (null == costType.getTotalPrice())
                                        costType.setTotalPrice(new BigDecimal(0));
                                    costType.setTotalPrice(costType.getTotalPrice().add(pc.getTotalPrice()));
                                }
                                break;
                            }
                        }
                    }
                    costType.setDetailList(pcList);
                    costType.setProductCount(productCount);
                    if (dictionary != null)
                    {
                        costType.setSalePriceValueName(dictionary.getName());
                ...
                {
                    if (c == null) continue;
                    if (c.getDetailList() != null && c.getDetailList().size() > 0)
                    {
                        for (ProductCostDetail productCostDetail : c.getDetailList())
                        {
                            //添加一个字段，前端用来判断背景墙是否可以删除，add by yangz
                            if ("qiangm".equals(productCostDetail.getCategoryCode()))
                            {//墙面
                                productCostDetail.setBgWall(Utils.getIsBgWall(productCostDetail.getValuekey()));
                            } else
                            {
                            ...
                            if (null != productCostDetail.getBrandId())
                            {
                                if (!brandIdsList.contains(productCostDetail.getBrandId()))
                                {
                                    productCostDetail.setTotalPrice(null);
                                    productCostDetail.setUnitPrice(null);
                                }
                            ...
                            //讲 地面  墙面 天花的 价格 从总价中  去除
                            if ("1".equals(productCostDetail.getProductTypeValue()) || "2".equals(productCostDetail.getProductTypeValue()) || "3".equals(productCostDetail.getProductTypeValue()))
                            {
                                if (productCostDetail.getProductTypeValue() != null)
                                {
                                    productCostDetail.setTotalPrice(null);
                                    productCostDetail.setUnitPrice(null);
                                }
                ...
                return newCostTypeList;
            }
        }

		/*add品牌,大类,小类,产品查询条件->end*/
		/*设置查询条件(序列号)*/
		/*System.out.println("typeList_start:"+System.currentTimeMillis());
        List<ProductsCostType> costTypeList = this.costTypeList(designPlanProduct, type);
        System.out.println("typeList_end:"+System.currentTimeMillis());
        if (null != costTypeList && costTypeList.size() > 0)
        {
			...
			*//*查询软硬装下面包含的产品小类等信息(带入序列号查询条件)*//*
			...
			// costList
			ProductsCostType productsCostType = new ProductsCostType();
			productsCostType.setPlanId(designPlanProduct.getPlanId());
			productsCostType.setUserId(sysUser.getId());
            System.out.println("costList_start:"+System.currentTimeMillis());
            List<ProductsCost> costList = this.costList(productsCostType, type);
            System.out.println("costList_end:"+System.currentTimeMillis());
            List<ProductsCost> cl = new ArrayList<>();
            for (ProductsCostType costType : costTypeList)
            {
                List<ProductsCost> pcList = new ArrayList<>();
                Integer productCount = 0;
                ...
                for (ProductsCost pc : costList)
                {
                    ...
                    for (String costCode : costCodes)
                    {
                        if (costCode.equals(pc.getCostTypeCode()))
                        {
                            ...
                            if ("1".equals(pc.getCostTypeValue()) || "2".equals(pc.getCostTypeValue()) || "3".equals(pc.getCostTypeValue()))
                            {
                                BigDecimal salePrice = pc.getTotalPrice();
                                BigDecimal totalPrice = costType.getTotalPrice();
                                BigDecimal new_totalPrice = totalPrice.subtract(salePrice);
                                costType.setTotalPrice(new_totalPrice);
                                pc.setTotalPrice(null);
            ...
            cost.setStart(-1);
            cost.setLimit(-1);
            System.out.println("detailList_start:"+System.currentTimeMillis());
            List<ProductCostDetail> costDetails = this.costDetail(cost, type);
            System.out.println("detailList_end:"+System.currentTimeMillis());
            for (ProductCostDetail detail : costDetails)
            {
                for (ProductsCost c : cl)
                {
                    if (detail.getAtt2().equals(c.getCostTypeCode()))
                    {
                        if (c.getDetailList() == null)
    ...
    private List<Integer> getBrandIdsList(SysUser sysUser)
    {
        String brandIdsStr = "";
        Integer companyId = sysUser.getBusinessAdministrationId();
        if (null != companyId && companyId > 0)
        {
            ...
            if (null != brandIdsStr && !"".equals(brandIdsStr))
            {
                return Utils.getIntegerListFromStringList(brandIdsStr);
            }
        }
