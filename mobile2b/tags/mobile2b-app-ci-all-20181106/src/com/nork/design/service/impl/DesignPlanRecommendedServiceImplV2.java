...
public class DesignPlanRecommendedServiceImplV2 implements DesignPlanRecommendedServiceV2 {
    ...
    private static Logger logger = Logger.getLogger(DesignPlanRecommendedServiceImplV2.class);
    @Autowired
    ...
    public Integer getAllFavoritePlanRecommendedCount(DesignPlanRecommended designPlanRecommended) {
        ...
        return designPlanRecommendedMapperV2.getAllFavoritePlanRecommendedCount(designPlanRecommended);
    }
    ...
    @Override
    public List<DesignPlanRecommendedResult> getAllFavoritePlanRecommendedList(DesignPlanRecommended designPlanRecommended) {
        List<DesignPlanRecommendedResult> resList = new ArrayList<DesignPlanRecommendedResult>();
        if (designPlanRecommended == null) {
            return resList;
        }

        resList = designPlanRecommendedMapperV2.getAllFavoritePlanRecommendedList(designPlanRecommended);
        return resList;
    }
    ...
    public ResponseEnvelope<DesignPlanRecommendedResult> getPlanRecommendedList(PlanRecommendedListModel model) {
        ...
        if(sysUser.getBusinessAdministrationId() == null || sysUser.getBusinessAdministrationId() == 0){
            return  new ResponseEnvelope<>(false,"该用户没有关联企业");
        }

        BaseCompany baseCompany = baseCompanyService.get(sysUser.getBusinessAdministrationId());
        if(baseCompany == null ){
            return  new ResponseEnvelope<>(false,"该用户关联的企业不存在");
        }

        //获取三度云享家的公司详情
        BaseCompany sanduCompany = baseCompanyService.getCompanyByCompanyCode("C0000071");
        ...
        if ("decorate".equals(displayType)) {
            ...
            //如果公司是厂商,是正式账号,,查询自己公司的一键方案 或公司是厂商,是试用账号,未拥有查看一键方案权限,查询自己公司的一键方案
            if(baseCompany.getBusinessType() != null && baseCompany.getBusinessType() ==1 && sysUser.getUseType() != null &&
                    (sysUser.getUseType()==1 || (sysUser.getShowSanduPlan() != null && sysUser.getUseType()==0 && sysUser.getShowSanduPlan()==0))){
                companyIds.add(baseCompany.getId());
                designPlanRecommended.setCompanyId(baseCompany.getId());
            }


            //如果公司是厂商,是试用账号,拥有查看一键方案权限,查询自己公司和三度云享家的一键方案
            if(baseCompany.getBusinessType() != null && baseCompany.getBusinessType() ==1 && sysUser.getUseType() != null
                    && sysUser.getShowSanduPlan() != null && sysUser.getUseType()==0 && sysUser.getShowSanduPlan()==1){
                companyIds.add(baseCompany.getId());
                designPlanRecommended.setCompanyId(baseCompany.getId());
                if(sanduCompany != null ){
                    companyIds.add(sanduCompany.getId());
                }
                companyIds.add(baseCompany.getId());
            }


            //如果是经销商,是正式账号,查询经销商的厂商ID的一键方案或如果是经销商,是试用账号,未拥有查看一键方案权限,查询经销商的厂商ID的一键方案
            if(baseCompany.getBusinessType() != null && baseCompany.getBusinessType() ==2 && sysUser.getUseType() != null
                    && (sysUser.getUseType()==1 || (sysUser.getShowSanduPlan() != null && sysUser.getUseType()==0
                    && sysUser.getShowSanduPlan()==0))){
                ...
                if (StringUtils.isNotBlank(brandIdStr)) {
                    ...
                    for (String idStr : split) {
                        brandIds.add(Integer.parseInt(idStr));
                    }
            ...
            //如果是经销商,是试用账号,拥有查看一键方案权限,查询经销商的厂商+三度云享家的一键方案
            if(baseCompany.getBusinessType() != null && baseCompany.getBusinessType() ==2 && sysUser.getUseType() != null
                    && sysUser.getShowSanduPlan() != null &&  sysUser.getUseType()==0 && sysUser.getShowSanduPlan()==1 ){
                List<Integer> brandIds = new ArrayList<>();
                if(sanduCompany != null ){
                ...
                companyIds.add(baseCompany.getPid());
                List<Integer> ids = baseCompanyService.getbrandIdByCompanyId(sanduCompany.getId());
                ...
                designPlanRecommended.setDealerOneKeyRecommended(1);
                designPlanRecommended.setCompanyId(baseCompany.getPid());
                //经销商推荐方案为公司+品牌过滤
                String brandIdStr = baseCompany.getBrandIds();
                if (StringUtils.isNotBlank(brandIdStr)) {
            ...
            //如果是其他角色,查询自己公司+三度云享家的一键方案
            if(baseCompany.getBusinessType() != null && baseCompany.getBusinessType() != 0
                    && baseCompany.getBusinessType() != 1 && baseCompany.getBusinessType() != 2){
                if(sanduCompany != null ){
                    companyIds.add(sanduCompany.getId());
                }
                companyIds.add(baseCompany.getId());
            ...
            //如果公司是厂商,查询自己公司的一键方案
            if(baseCompany.getBusinessType() != null && baseCompany.getBusinessType() !=2 ){
                companyIds.add(baseCompany.getId());
                designPlanRecommended.setCompanyId(baseCompany.getId());
            }
        ...
        Integer total = 0;
        List<DesignPlanRecommendedResult> list = new ArrayList<>();
        List<Integer> designPlanIds = new ArrayList<>();
        total = this.getPlanRecommendedCount(designPlanRecommended);
        int totalCountOffullHouseDesignPlan = 0;
        int totalCountOfSingleDesignPlan = 0;

        if (SpaceCommonConstant.FULL_HOUSE_SPACE_FUNCTION_ID.equals(houseType)) {
            //如果是全屋方案
            total = designPlanRecommendedMapperV2.getFullHousePlanCount(designPlanRecommended);

        } else if ( (designPlanRecommended.getSpaceFunctionId() == null || designPlanRecommended.getSpaceFunctionId()<= 0) &&
                "decorate".equals(designPlanRecommended.getDisplayType())) {
            //如果是全部空间类型，并且是一键方案的时候，查询所有全屋和推荐方案
             totalCountOffullHouseDesignPlan = designPlanRecommendedMapperV2.getFullHousePlanCount(designPlanRecommended);
             totalCountOfSingleDesignPlan = this.getPlanRecommendedCount(designPlanRecommended);

             total = totalCountOffullHouseDesignPlan + totalCountOfSingleDesignPlan;

        } else {
            //不是全部空间类型或者不是一键方案，查询指定空间的推荐方案
            total = this.getPlanRecommendedCount(designPlanRecommended);
        }
        ...
        if (total != null && total.intValue() > 0) {
            if (SpaceCommonConstant.FULL_HOUSE_SPACE_FUNCTION_ID.equals(houseType)) {
                //如果是全屋方案
                list = designPlanRecommendedMapperV2.getFullHousePlanList(designPlanRecommended);

            } else if (designPlanRecommended.getSpaceFunctionId() == null || designPlanRecommended.getSpaceFunctionId()<= 0) {
                //如果是全部空间类型，查询所有全屋和推荐方案
                List<DesignPlanRecommendedResult> fullHousePlanList =
                        designPlanRecommendedMapperV2.getFullHousePlanList(designPlanRecommended);
                List<DesignPlanRecommendedResult> planRecommendedList = new ArrayList<>();
                //全屋方案如果不够分页的数据，再去查推荐方案
                if (fullHousePlanList == null || fullHousePlanList.size() < limit) {
                    int size = 0;
                    if (fullHousePlanList == null) {

                    } else if (fullHousePlanList.size() < limit) {
                        size = fullHousePlanList.size();
                    }
                    start = start - totalCountOffullHouseDesignPlan;
                    if (start <= 0) {
                        start = 0;
                    }
                    designPlanRecommended.setStart(start);
                    designPlanRecommended.setLimit(limit-size);
                    planRecommendedList = this.getPlanRecommendedList(designPlanRecommended);
                }

                list.addAll(fullHousePlanList);
                list.addAll(planRecommendedList);

            } else {
                //不是全部空间类型，查询指定空间的推荐方案
                list = this.getPlanRecommendedList(designPlanRecommended);
            }
            if (list != null && list.size() > 0) {
                //获取用户购买方案版权记录
                designPlanIds =  designPlanRecommendedMapperV2.selectDesignPlanIdsFromRecord(loginUser.getId());
                ...
                for (DesignPlanRecommendedResult result : list) {
                    if (Objects.equals(result.getChargeType(),1) && !designPlanIds.contains(result.getPlanRecommendedId())){
                        //判断是否是同一公司的方案
                        if (Objects.equals(result.getCompanyId(),sysUser.getCompanyId())){
                           if ("share".equals(result.getPlanSource())){
                    ...
                    if (AutoRenderTaskConstant.PLAN_FULL_HOUSE_TYPE != result.getPlanHouseType()) {
                        //单空间方案获取封面图
                        ...
                        if (renderPic != null) {
                            result.setCoverPath(renderPic.getPicPath());
                        }
                        //单空间方案获取装修报价信息
                        List<PlanDecoratePriceBO> planDecoratePriceBOList =
                                designPlanRecommendedMapperV2.getPlanDecoratePrice(result.getPlanRecommendedId());
                    ...
                    //全屋方案获取装修报价信息
//                    List<PlanDecoratePriceBO> planDecoratePriceBOList =
//                    designPlanRecommendedMapperV2.getPlanDecoratePriceByFullHouseId(result.getPlanRecommendedId());
//                    result.setPlanDecoratePriceList(planDecoratePriceBOList);
                }
            }
        }
    ...
    @Override
    public List<DesignPlanRecommended> bacchQueryByIds(Set<Integer> planRecommendedIds) {
        return designPlanRecommendedMapperV2.bacchQueryByIds(planRecommendedIds);
    }

    @Override
    public List<Integer> selectDesignPlanIdsFromRecord(Integer userId) {
        return designPlanRecommendedMapperV2.selectDesignPlanIdsFromRecord(userId);
    }
}
