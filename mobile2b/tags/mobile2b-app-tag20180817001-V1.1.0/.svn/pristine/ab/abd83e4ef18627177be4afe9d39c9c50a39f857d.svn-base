...
//			.getBundle("config/webSocket");
	...
	//多个服务化的地址
	private final String SERVITIZATION = app.getString("app.servitization.urls");
	

	/**
	 * 移动通过账号密码查询余额的接口
	 * 供续费和开通移动端的页面显示用户当前积分使用
	 * 前端需将返回的金额转成积分显示
	 * @param sysUser
	 * @return
	 */
	...
	@ResponseBody
	public ResponseEnvelope getUserBalance(@RequestBody SysUser sysUser) {
		...
		if (user == null) {
			...
			return new ResponseEnvelope<>(balance);
		}
	...
	public ResponseEnvelope login(@RequestBody SysUser sysUser, HttpServletRequest request) {
		String mediaType = request.getHeader("MediaType").toString();
...
//		SysUser user = mobileUserService.findByMobileAndPwd(account, pwd);
		...
		LoginUser loginUser = this.getLoginUser(user,sessionId,mediaType);
		
		//用户是否开通移动端
		if(user.getExistsMobile() != SystemCommonConstant.HAS_EXISTS_MOBILE) {
			return new ResponseEnvelope<SysUser>(false, "您尚未开通移动版功能，请联系客服开通!",user.getId().toString());
		}else {
			Date mobileClosedDate = user.getMobileClosedDate();
			Date date = new Date();
			if(mobileClosedDate.before(date)) {
				return new ResponseEnvelope<SysUser>(true, "移动版已到期，请续费开通！",user.getId().toString());
			}
		}
		...
		loginUser.setServitization(SERVITIZATION);
		
		return new ResponseEnvelope<LoginUser>(true, loginUser);
	}
	
	
	private LoginUser getLoginUser(SysUser user, String sessionId,String mediaType) {
...
//		String token = UUID.randomUUID().toString().replace("-", "");
		...
		payload.put("ext",date.getTime()+1000*60*60*24*3);//过期时间1小时 改为三天
		payload.put("signflat", SystemCommonConstant.LOGIN_FROM_MOBILE);
		payload.put("mediaType", mediaType);
		payload.put("mtype", mediaType);
		
		String token=Jwt.createToken(payload);
		
		//移动端返回一个文件加密的key，app端用来解密
		user.setToken(token);
		user = sysUserService.EencryptKey(user); //TODO Check this code
		loginUser.setCryptKey(user.getCryptKey());
		
		loginUser.setAppKey(appKey);
		loginUser.setUserKey(appKey);
		...
		String loginFlagPc = SystemCommonConstant.LOGIN_FROM_PC;//pc端登录标识
		String key = loginFlag + appKey;
		String PCkey = loginFlagPc + appKey;
		
		int expireInSeconds = 60 * 60 * 24 * 3;// 改为三天
		
		//查找是否有用户的登录信息缓存
		if (Utils.enableRedisCache()) {

//			LoginUser onlineUser = new LoginUser();
//			onlineUser = (LoginUser) JedisUtils2.getObject((loginFlag + user.getId().toString()));
			LoginUser onlineUser = LoginContext.getLoginUser(LoginUser.class);
			if (onlineUser != null && onlineUser.getAppKey() != null) {
				logger.info("移除用户缓存onlineUser:" + onlineUser + ",appkey:" + onlineUser.getAppKey());
				// 移除之前登录用户的缓存
//				JedisUtils2.remove((loginFlag + user.getId().toString()));
//				JedisUtils2.remove(onlineUser.getAppKey());
				LoginContext.removeUserCache(loginFlag + user.getId().toString());
				LoginContext.removeUserCache(onlineUser.getAppKey());
			}
		} else {
			logger.error("getLoginUser redis未开启");
		}
		
		JedisUserUtils.setObject((loginFlag + user.getId().toString()), loginUser, expireInSeconds);
		JedisUserUtils.setObject(key, loginUser, expireInSeconds);
		logger.info("getLoginUser 存储loginUser:"+loginUser+",appkey:"+loginUser.getAppKey());
		//将移动端登录时的用户信息用PC端登录标识保存一份，供unity使用
		JedisUserUtils.setObject((loginFlagPc + user.getId().toString()), loginUser, expireInSeconds);
		JedisUserUtils.setObject(PCkey, loginUser, expireInSeconds);
	...
	public Object sms(@RequestBody SysUser sysUser2,
		...
		if ("register".equals(type)) {
			if(null != checkUserExist) {
                return new ResponseEnvelope<SysUser>(false, "该用户已被注册，请重新输入手机号！");
			}
			...
			if(checkUserExist.getId() != loginUser.getId()) {
				return new ResponseEnvelope<SysUser>(false, "输入的号码跟账号不一致，不能获取验证码");
			}
		}
	...
	public Object findPassword(/*String mobile, String newPassword, String code,String msgId,*/
		...
		if ("1".equals(cacheEnable)) {
			...
			try {
				...
				e.printStackTrace();
				return new ResponseEnvelope<SysUser>(false, "验证码不对，请重新输入！", msgId);
			}
		}
...
//		return new ResponseEnvelope<PayOrder>(total, list, model.getMsgId());
	 ...
	 * @param
	 */
	@RequestMapping(value = "/getUserIdByOrderNo")
	@ResponseBody
	public Object getUserIdByOrderNo(/*@RequestBody MobileRenderingModel model*/
			@RequestParam(value = "orderNo", required = false) String orderNo,
			@RequestParam(value = "httpUrl", required = false) String httpUrl,
			HttpServletRequest request) {
		//获取登录用户信息
		@SuppressWarnings("unchecked")
		HashMap<String, String> userMap = (HashMap<String, String>) request.getAttribute("AuthorizationData");
//		String appKey = userMap.get("appKey");
		...
		//发送http请求
		String envelope = httpClient.sendHttpPost(httpUrl,map);
		//将返回结果转成map
		Map<?, ?> envelopeMap;
		try {
			envelopeMap = Utils.parserToMap(envelope);
		} catch (Exception e) {
			return "envelope不是正确的格式 ，envelope = " + envelope;
		}
		Integer userId = (Integer) envelopeMap.get("userId");
		System.out.println("getUserIdByOrderNo---------userId == " + userId);
		if(userId != null && userId.intValue() > 0) {
			sysUserService.updateUserMobileInfo(userId);
