...
public class DesignPlanRenderServiceImpl implements DesignPlanRenderService {
    ...
    public long saveAsRenderBakScene(DesignPlanRes designPlanRes) {
        ...
        AutoRenderTaskState taskState = null;
        try {
            logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>taskId="+taskId);
            taskState = designPlanAutoRenderServiceImpl.getStateByTaskId(taskId);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>exception:"+e);
        }
        logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>taskState="+taskState);
        // 设计方案存副本 ->start
        ...
        if (AutoRenderTaskConstant.PLAN_FULL_HOUSE_TYPE.equals(taskState.getPlanHouseType())) {
            designPlanEctype.setIsFullHousePlan(1);
        } else {
            designPlanEctype.setIsFullHousePlan(0);
        }

        long id = 0;
        try {
            logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>designPlanEctype="+designPlanEctype);
            id = designPlanRenderSceneService.add(designPlanEctype);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>exception:"+e);
        }
        logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>id="+id);
        // 设计方案存副本 ->end

        // 配置文件copy一份,存副本 ->start
        long resDesignEctypeId = 0;
        try {
            logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>resDesign:"+designPlanRes.getResDesign()+" , id="+id);
            resDesignEctypeId = resDesignRenderSceneService.copyFromResDesign(designPlanRes.getResDesign(), id);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>exception:"+e);
        }
        logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>resDesignEctypeId="+resDesignEctypeId);
...
//        }
        ...
        // 设计方案产品列表存副本 ->start
        try {
            logger.error("DesignPlanRenderServiceImpl===>saveAsRenderBakScene===>designPlanProductList="+designPlanRes.getDesignPlanProductList().size()+", id="+id);
    ...
    public void associatedRenderResWithBakScene(long resId, long bakSceneId, String loginName) {
        ...
        try {
            logger.error("DesignPlanRenderServiceImpl===>associatedRenderResWithBakScene===>renderPic="+renderPic);
            int i = resRenderPicService.update(renderPic);
            logger.error("DesignPlanRenderServiceImpl===>associatedRenderResWithBakScene===>i="+i);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("DesignPlanRenderServiceImpl===>associatedRenderResWithBakScene===>!!!!!!!!!!!exception:"+e);
        }
        ...
        ResRenderPic smallPic = null;
        try {
            logger.error("DesignPlanRenderServiceImpl===>associatedRenderResWithBakScene===>(int) resId="+(int) resId);
            smallPic = resRenderPicService.get((int) resId);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("DesignPlanRenderServiceImpl===>associatedRenderResWithBakScene===>!!!!!!!!!!!!e:"+e);
        }
        logger.error("DesignPlanRenderServiceImpl===>associatedRenderResWithBakScene===>smallPic="+smallPic);
        if(smallPic != null){
            ...
            if(pid != null && pid > 0){
                ...
                try {
                    logger.error("DesignPlanRenderServiceImpl===>resRenderPicService.update(resRenderPic)"+resRenderPic);
                    resRenderPicService.update(resRenderPic);
                } catch (Exception e) {
                    e.printStackTrace();
                    logger.error("DesignPlanRenderServiceImpl===>!!!!!!!!!!!!!!!exception:"+e);
                }
            }
        }
        try {
            logger.error("DesignPlanRenderServiceImpl===>render="+render);
        ...
        int taskId = 0;
        try {
            logger.error("DesignPlanRenderServiceImpl===>(int) resId="+(int) resId);
            taskId = resRenderPicService.getTaskIdBySmallId((int) resId);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("DesignPlanRenderServiceImpl===>!!!!!!!!!!!!exception:"+e);
        }
        logger.error("DesignPlanRenderServiceImpl===>taskId="+taskId);
        if(taskId > 0){
            try {
                logger.error("DesignPlanRenderServiceImpl===>(int) bakSceneId="+(int) bakSceneId+", taskId="+taskId);
...
//        renderPic.setFileKey(ResProperties.DESIGNPLAN_RENDER_PIC_SMALL_FILEKEY);
        ...
        int picId = 0;
        try {
            logger.error("========= 渲染保存图片 ====> 获取最后一张封面 =======fileKeyList:"+fileKeyList+" ,(int)designId："+(int)designId);
            picId = resRenderPicService.getLatestSmallPic(renderPic);
        } catch (Exception e) {
            logger.error("------- 渲染保存图片 -----> 获取最后一张封面 exception："+e);
        }
        logger.error("======= 渲染保存图片 ====> 获取最后一张封面=======picId："+picId);
        return picId;
    }
    /* (non-Javadoc)    
