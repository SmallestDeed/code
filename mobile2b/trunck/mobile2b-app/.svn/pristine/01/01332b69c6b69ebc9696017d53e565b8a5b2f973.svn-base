...
public class MobileHouseSearchServiceImpl implements MobileHouseSearchService {
	...
	private final Logger logger = LoggerFactory.getLogger(MobileHouseSearchServiceImpl.class);
	@Autowired
	 ...
	 */
	@Override
	public Object searchProvinceCodeAndCityCode(String pid,String areaPid) {
		BaseArea baseArea = new BaseArea();
		baseArea.setIsDeleted(0);
		if(pid != null && !pid.equals("")) {
			baseArea.setPid(pid);//根据省级的编码查询市区
		}
		else if (areaPid!=null && !areaPid.equals("")){//根据市级的编码查区级
			baseArea.setPid(areaPid);
			baseArea.setLevelId(new Integer(3));
		}
		else if(pid == null && areaPid == null){
	 ...
	 * @param provinceCode
	 * @param cityCode
	 * @param areaCode 区编码 add by WangHaiLin
	 * @param livingName
	 * @param msgId
	 * @param limit
	 * @param start
	 ...
	 */
	@Override
	public Object newHouseSearchWeb(String provinceCode, String cityCode,String areaCode,
									String livingName, String msgId, String limit, String start,
									LoginUser loginUser) {
		// 调用APP端 原有的serviceImpl里的方法
		return baseHouseService.newHouseSearchWeb(provinceCode, cityCode,areaCode,
	...
	public Object newHouseSpaceList(String houseId, String msgId, String limit,
		...
		logger.error("通过户型过滤空间布局图 --------- spaceFunctionId:",spaceFunctionId);
		// 调用APP端 原有的serviceImpl里的方法
...
//				start, loginUser);
		...
		if (StringUtils.isEmpty(spaceFunctionId) ||
