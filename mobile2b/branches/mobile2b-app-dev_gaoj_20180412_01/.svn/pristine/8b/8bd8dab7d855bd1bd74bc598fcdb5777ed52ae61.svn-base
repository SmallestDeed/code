...
public class WebDesignPlanControllerV2 {
	...
	public Object planSave(@PathVariable String style, @RequestParam(value = "templateId") Integer templateId,
		...
		// == null || request.getSession().getAttribute("mediaType") == null)? "2" :
		...
		if (isNewPlan) {
			newFlag = 1;
			Integer id = designPlanService.CopyDesignTemplet(mediaType, designPlan, designTemplet, loginUser,
					request);/* 三：从样板房中拷贝数据到设计方案中 创建新的设计方案信息 */
			if (id == null || id <= 0) {
				return new ResponseEnvelope<DesignPlan>(false, "数据异常", msgId);
			}
			Boolean flag = designPlanService.CopyDesignTempletProduct(id, designTemplet,
					request);/* 四：拷贝一份 样板房的产品到 到设计方案产品表中 */
			if (!flag) {
				return new ResponseEnvelope<DesignPlan>(false, "数据异常", msgId);
			}
		...
		Tools.fixExecutorService.execute(new Runnable() {
			...
			public void run() {
				...
				designPlanOperationLog.setSysCode(
	...
	public Object getDesignPlan(@RequestParam(value = "houseId", required = false) String houseId,
		...
		ResponseEnvelope responseEnvelopeInfo = (ResponseEnvelope) designPlanService.getDesignPlanInfo(designPlanId,
				newFlag, houseId, livingId, residentialUnitsName, isRelease, loginUser, mediaType, null);
	...
	@ResponseBody
	public Object updateDesignPlanCache(@RequestParam(value = "houseId", required = false) String houseId,
			@RequestParam(value = "livingId", required = false) String livingId,
			@RequestParam(value = "designPlanId", required = false) Integer designPlanId,
			@RequestParam(value = "residentialUnitsName", required = false) String residentialUnitsName,
			Integer newFlag, @RequestParam(value = "msgId", required = false) String msgId, HttpServletRequest request)
		...
		UpdateDesignPlanCacheParameter parameter = new UpdateDesignPlanCacheParameter(designPlanId, houseId, livingId,
				residentialUnitsName, newFlag, request);
	...
	public Object transformAndCopyPlan(@RequestParam(value = "houseId", required = false) String houseId,
		...
		try {
			...
			if ("2".equals(type)) {
				...
				// designPlanRecommendedServiceV2.planFormalIsReleaseCheck(DesignPlanConstants.FUNCTION_TYPE_TRANSFORM,plan,
				// null);
				// if ("false".equals(resMap.get("success")) ||
				// !"true".equals(resMap.get("success"))) {
				// return new ResponseEnvelope<DesignPlanRecommendedResult>(false,
				// resMap.get("data"), msgId);
				// }
				/* end */
			...
			if (plan.getConfigFileId() != null) {
				Integer resFileId = this.planCopyFile(plan.getConfigFileId().toString(), "file",
	...
	public Integer planCopyFile(String resId, String resType, String fileKey, Integer bussniess,
		...
		if (!StringUtils.isEmpty(resId)) {
			...
			if (!StringUtils.isEmpty(resFilePath)) {
				...
				/*
				 * if ("linux".equals(FileUploadUtils.SYSTEM_FORMAT)) { srcPath =
				 * local_uploadRoot + resFilePath; } else { srcPath = local_uploadRoot +
				 * resFilePath.replace("/", "\\"); }
				 */
				 ...
				 * if ("linux".equals(FileUploadUtils.SYSTEM_FORMAT)) { local_targetPath =
				 * local_uploadRoot + targetName; }
				 */
	 ...
	 * @return
	 */
	public Integer planCopyFileFromResDesignScene(String resId, String fileKey, Integer bussniess,
			HttpServletRequest request, String code) {
		String resFilePath = "";
		Integer newResId = -1;
		ResDesign resDesign = new ResDesign();
	...
	public ResponseEnvelope delTempDesignPaln(HttpServletRequest request, HttpServletResponse response) {
		...
		// designPalnRenderService.deleteTempDesignPaln4RenderBakScene(
		// Long.valueOf(tempDesignPalnId),userId);
		return envelope;
	}
	...
	public Object transformAndCopyPlan(@RequestParam(value = "houseId", required = false) String houseId,
			...
			@RequestParam(value = "operationSource", required = false) Integer operationSource) throws Exception {
		LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
		...
		if (operationSource == OP_SOURCE_MYDESIGN) {
			...
			DesignPlanRenderScene designPlanRenderScene = designPlanRenderSceneService.get(planRecommendedId);// this id
																												// is
																												// designPlanRenderScene
																												// id
			if (designPlanRenderScene == null) {
				return new ResponseEnvelope<DesignPlan>(false, "方案推荐不存在，或已被下架", msgId);
			}
			return transformByDesignPlanRenderScene(designPlanRenderScene, houseId, livingId, residentialUnitsName,
					newFlag, msgId, loginUser.getId(), request, taskId);
		} else { // 方案推荐改造
			...
			return transformBydesigPlanRecommended(designPlanRecommended, houseId, livingId, residentialUnitsName,
					newFlag, msgId, loginUser.getId(), request, taskId);
		}
	}
				...
				if (replaceProductList != null) {
					for (ProductReplaceTaskDetail replaceProduct : replaceProductList) {
						if (replaceProduct.getSourceProductId().intValue() == dprProduct.getProductId().intValue()) {
							...
							if (replaceProduct.getDestTexturesChooseInfo() != null
									&& replaceProduct.getDestTexturesChooseInfo() != "") {
	 ...
	 * @throws Exception
	 */
	public Object transformByDesignPlanRenderScene(DesignPlanRenderScene designPlanRenderScene, String houseId,
			String livingId, String residentialUnitsName, Integer newFlag, String msgId, Integer userId,
			HttpServletRequest request, Integer taskId) throws Exception {
		if (userId == null || userId.intValue() == 1 || userId.intValue() == 0) {
			return new ResponseEnvelope<DesignPlan>(false, "请重新登录", msgId);
		...
		if (designPlanRenderScene.getPlanCode() != null) {
			designPlan.setPlanCode(
					designPlanRenderScene.getPlanCode().substring(0, designPlanRenderScene.getPlanCode().length() - 6)
							+ Utils.generateRandomDigitString(6));
		...
		designPlan.setIsOpen(0);
		designPlan.setSysCode(
				designPlanRenderScene.getSysCode().substring(0, designPlanRenderScene.getSysCode().length() - 6)
						+ Utils.generateRandomDigitString(6));
		sysSave(designPlan, request);
		designPlan.setCoverPicId(null);
		if (designPlanRenderScene.getConfigFileId() != null) {
			Integer resFileId = this.planCopyFileFromResDesignScene(designPlanRenderScene.getConfigFileId().toString(),
					"design.designPlan.u3dconfig", null, request, designPlanRenderScene.getPlanCode());
			designPlan.setConfigFileId(resFileId);/* 设计方案配置文件 */
		} else {
			designPlan.setConfigFileId(-1);/* 设计方案配置文件 */
		}
		...
		if (newPlanId != null) {
			...
			List<DesignPlanProductRenderScene> planProductList = designPlanProductRenderSceneService
					.getListByPlanId(designPlanRenderScene.getId());
			for (DesignPlanProductRenderScene dprProduct : planProductList) {
				...
				if (replaceProductList != null && replaceProductList.size() > 0) {
					for (ProductReplaceTaskDetail replaceProduct : replaceProductList) {
						if (replaceProduct.getSourceProductId().intValue() == dprProduct.getProductId().intValue()) {
							logger.error("transformByDesignPlanRenderScene replaceProduct.getDestProductId() =>"
									+ replaceProduct.getDestProductId());
							planProduct.setProductId(replaceProduct.getDestProductId());
							if (replaceProduct.getDestTexturesChooseInfo() != null
									&& replaceProduct.getDestTexturesChooseInfo() != "") {
								planProduct.setSplitTexturesChooseInfo(replaceProduct.getDestTexturesChooseInfo());
