...
//			user = LoginContext.getLoginUser(SysUser.class);
			if (user == null) {
...
//			 loginUser = (LoginUser) CacheManager.getInstance().getCacher().getObject(loginFlagkey);
			 LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
			 if(loginUser != null && loginUser.getUserKey() != null) {
			   //检测用户之前是否登录，移除登录用户缓存
//			   CacheManager.getInstance().getCacher().remove(loginFlagkey);
//			   CacheManager.getInstance().getCacher().remove(getUserKeyByUserkey(loginUser.getUserKey()));
			 	LoginContext.removeUserCache(loginFlagkey);
			 	LoginContext.removeUserCache(getUserKeyByUserkey(loginUser.getUserKey()));
			 }
			  
			 CacheManager.getInstance().getCacher().setObject(loginFlagkey, user, exSeconds);
		 }
