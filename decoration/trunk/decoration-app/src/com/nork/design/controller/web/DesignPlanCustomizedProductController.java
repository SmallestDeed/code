	...
	public Object save(@ModelAttribute("productModel") DesignPlanCustomizedProductModel productModel,
		...
		if (request instanceof MultipartHttpServletRequest) {
			...
			for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
				...
				if (flag) {
					modelPath = map.get(FileUploadUtils.DB_FILE_PATH) == null ? null : map.get(FileUploadUtils.DB_FILE_PATH).toString();
//					customizedProduct.setModelUrl(map.get(FileUploadUtils.DB_FILE_PATH) + "");
		...
		try {
			int id = 0 ;
			String oldModelUrl = null;
			if (null == customizedProduct.getId()) {
				//新增定制柜必须传模型
				if (StringUtils.isEmpty(modelPath)) {
					return new ResponseEnvelope<DesignPlanCustomizedProduct>(false, "模型文件为空！", msgId);
				}
				customizedProduct.setModelUrl(modelPath);
				id = designPlanCustomizedProductService.add(customizedProduct);
			} else {
				//查询旧模型地址
				DesignPlanCustomizedProduct customizedProductModel = designPlanCustomizedProductService.get(customizedProduct.getId());
				if (null != customizedProductModel) {
					oldModelUrl = customizedProduct.getModelUrl();
				}
				//更新定制产品信息
				customizedProduct.setModelUrl(modelPath);
			...
			if (id == 0) {
				...
				//更新模型，删除旧模型文件
				if (Objects.equals("update", type) && !StringUtils.isEmpty(customizedProduct.getModelUrl())) {
					if (!StringUtils.isEmpty(oldModelUrl)) {
						FileUploadUtils.deleteFile(oldModelUrl);
					}
				}
			}
		} catch (Exception e) {
	...
	private MessageUtil paramVerify(DesignPlanCustomizedProductModel productAdd) {
		...
		if (StringUtils.isEmpty(productAdd.getExteriorProductId())) {
