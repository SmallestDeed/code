...
public class WebDesignPlanControllerV2 {
	...
	@Autowired
	private PlanSaveService planSaveService;
	@Autowired
	...
	public Object planSave(@PathVariable String style, @RequestParam(value = "templateId") Integer templateId,@RequestParam(value = "planSource",required=false) String planSource,
			...
			@RequestParam(value = "modelType",required=false) String modelType,HttpServletRequest request, HttpServletResponse response,
						   @RequestParam(value = "supplyDemandId",required=false) Integer supplyDemandId,
						   @RequestParam(value = "historyMsgId",required=false) String historyMsgId
	)throws Exception {
		...
		if(supplyDemandId !=null && supplyDemandId >0){
			designPlan.setSupplyDemandId(supplyDemandId);
		}

		if (StringUtils.isNotEmpty(historyMsgId)){
			designPlan.setHistoryMsgId(historyMsgId);
		}
		...
		boolean isNewPlan = planSaveService.isNewPlan(loginUser, map);

		Integer newFlag = 0;
		if (isNewPlan){
			newFlag = 1;
			/*三：从样板房中拷贝数据到设计方案中  创建新的设计方案信息*/
			Integer id = designPlanService.CopyDesignTemplet(mediaType,designPlan,designTemplet,loginUser,request,modelType);
			if (id == null || id <= 0) {
				return new ResponseEnvelope<DesignPlan>(false, "数据异常", msgId);
			}
			/*四：拷贝一份 样板房的产品到  到设计方案产品表中*/
			Boolean flag = designPlanService.CopyDesignTempletProduct(id,designTemplet,request);
			if(!flag){
				return new ResponseEnvelope<DesignPlan>(false, "数据异常", msgId);
			}
		}
		...
		//logger.error("复制样板房消耗时间："+(endTime - startTime));
		return getDesignPlan(houseId,livingId,designPlan.getId(),unitName, newFlag, msgId, request,true,modelType);
	}
	...
	@ResponseBody
		public Object getDesignPlan(@RequestParam(value = "houseId",required = false) String houseId,
			...
			@RequestParam(value = "designPlanId", required = false) Integer designPlanId,@RequestParam(value = "residentialUnitsName",required = false) String residentialUnitsName, 
				Integer newFlag, @RequestParam(value = "msgId", required = false) String msgId, HttpServletRequest request,Boolean isRelease,@RequestParam(value = "modelType", required = false) String modelType)
			throws Exception {
		Long startTime = System.currentTimeMillis();
...
//		LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
		if(null==loginUser){
			...
			return new ResponseEnvelope<DesignPlan>(false, "请重新!", msgId);
		}
		String mediaType = loginUser.getMediaType();
		String msg = "";
		if (StringUtils.isBlank(msgId)) {
		...
		return planSaveService.getDesignPlan(houseId,livingId,designPlanId,residentialUnitsName,newFlag,msgId,loginUser,isRelease,mediaType,startTime,modelType);
	}
	...
	public Object transformAndCopyPlan	(@RequestParam(value = "houseId",required = false) String houseId,
			...
			@RequestParam(value = "msgId", required = false) String msgId,@RequestParam(value = "modelType", required = false) String modelType, HttpServletRequest request) throws Exception{
...
//				}
		...
		return getDesignPlan(houseId,livingId,newPlanId,residentialUnitsName,newFlag,msgId,request,false,modelType);
	}
...
//        designPalnRenderService.deleteTempDesignPaln4RenderBakScene( Long.valueOf(tempDesignPalnId),userId);
	...
	public Object transformAndCopyPlan	(@RequestParam(value = "houseId",required = false) String houseId,
		...
		if (operationSource == OP_SOURCE_MYDESIGN) {  
			...
			return transformByDesignPlanRenderScene(designPlanRenderScene,houseId,livingId,residentialUnitsName,newFlag,msgId,loginUser.getId(),request,taskId,modelType);
		} else {  //方案推荐改造
			...
			return transformBydesigPlanRecommended(designPlanRecommended,houseId,livingId,residentialUnitsName,newFlag,msgId,loginUser.getId(),request,taskId,modelType);
		}
	}
	...
	public Object transformBydesigPlanRecommended(DesignPlanRecommended designPlanRecommended, String houseId,
			...
			HttpServletRequest request, Integer taskId,@RequestParam(value = "modelType", required = false) String modelType) throws Exception {
		LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
				...
				if (replaceProductList != null) {
					for (ProductReplaceTaskDetail replaceProduct : replaceProductList) {
						if (replaceProduct.getSourceProductId().intValue() == dprProduct.getProductId().intValue()) {
							...
							// update by gaoj, resolve texture is source product texture
							planProduct.setSplitTexturesChooseInfo(null);
	...
	public Object transformByDesignPlanRenderScene(DesignPlanRenderScene designPlanRenderScene,String houseId,String livingId,String residentialUnitsName,
			Integer newFlag,String msgId,Integer userId,HttpServletRequest request, Integer taskId,@RequestParam(value = "modelType", required = false) String modelType) throws Exception{
		LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
		if(userId ==null || userId.intValue() == 1 || userId.intValue() == 0){
			return new ResponseEnvelope<DesignPlan>(false,"请重新登录",msgId);
		}
		...
		if (designPlanRenderScene.getSpellingFlowerFileId() != null) {
			...
			String platformId = designPlanRenderScene.getPlatformId();

			if(StringUtils.isNotBlank(platformId) && "2".equals(platformId)){
				resFileId = designPlanService.planCopyFileFromResDesignSceneByPC(designPlanRenderScene.getSpellingFlowerFileId().toString(),
						"design.designPlan.spellingFlowerFile","/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/design/designPlan/spellingFlowerFile/",
						null, designPlan.getPlanCode(),loginUser);
			}else{
				resFileId = designPlanService.planCopyFileFromResDesignScene(designPlanRenderScene.getSpellingFlowerFileId().toString(),
						"design.designPlan.spellingFlowerFile","/AA/d_userdesign/[yyyy]/[MM]/[dd]/[HH]/design/designPlan/spellingFlowerFile/",
						null, designPlan.getPlanCode(),loginUser);
			}
					...
					if (replaceProductList != null && replaceProductList.size() > 0) {
		                   for(ProductReplaceTaskDetail replaceProduct : replaceProductList) {
		                     if(replaceProduct.getSourceProductId().intValue() == dprProduct.getProductId().intValue()) {
		                       ...
		                       planProduct.setProductId(replaceProduct.getDestProductId());
                                 //update by steve, add the texture replace
                                 logger.info("产品替换是否替换材质信息参数："+replaceProduct.getReplaceSplitTexturesParam());
                                 logger.error("产品替换目标产品默认材质信息："+replaceProduct.getDestTexturesChooseInfo());
                                 if(StringUtils.isNotBlank(replaceProduct.getReplaceSplitTexturesParam())) {
                                     String newTexturesInfo = mobilePayRenderService.getNewSplitTexturesChooseInfoByProductId(replaceProduct.getDestProductId(),replaceProduct.getReplaceSplitTexturesParam());
                                     planProduct.setSplitTexturesChooseInfo(newTexturesInfo);
                                     planProduct.setIsReplaceTexture(taskId);
                                 } else if (StringUtils.isNotBlank(replaceProduct.getDestTexturesChooseInfo())){
                                     logger.info("产品替换材质替换为目标产品材质：taskId="+taskId);
									 planProduct.setSplitTexturesChooseInfo(replaceProduct.getDestTexturesChooseInfo());
									 planProduct.setIsReplaceTexture(taskId);
								 }
		                     }
		                   }
	...
	public Object designPlanCopy(DesignPlan plan ,String houseId,String livingId,String residentialUnitsName,
			Integer newFlag,String msgId,Integer userId,HttpServletRequest request,@RequestParam(value = "modelType", required = false) String modelType){
		Integer newPlanId = null;
		try{
			if(plan == null){
	...
	public Object spellingFlower(HttpServletRequest request,HttpServletResponse response) {
		...
		String isRenderServer = request.getHeader("isRenderServer");
		String opType = request.getParameter("taskType");//0:一键装修、1：改造
		...
		logger.error("保存拼花信息isRenderServer="+isRenderServer);

		return designPlanService.spellingFlower(model,isRenderServer,opType);
	}
	 ...
	 * @return
	 */
	@RequestMapping(value="/spellingFlowerFileTo")
	@ResponseBody
	public Object spellingFlowerFileTo(HttpServletRequest request,HttpServletResponse response) {


		String result = designPlanService.spellingFlowerFileTo();
