...
//        tpm.init(); 
	...
	public void doFilter(ServletRequest arg0, ServletResponse arg1, FilterChain arg2)
		...
		if( StringUtils.isNotBlank(authorization) ){
			resultMap = Jwt.validToken(authorization);
		...
		if(StringUtils.isNotBlank(authorization) && currentURL.lastIndexOf(".htm") != -1) {
			...
			request.setAttribute("mediaType", resultMap.get("mtype"));
			...
			JSONObject dataObj = (JSONObject)resultMap.get("data"); //token信息
			if(dataObj.containsKey("rsource")) {
			  String source = (String) dataObj.get("rsource");
			  //自动渲染请求的接口，放行
			  if(source != null && "auto".equals(source)) {
				logger.info("IS_AUTO url==>requestUrl=" + currentURL);
				arg2.doFilter(request, response);
				return;
			  }
			}
			if(currentPath.indexOf("/sysUserQuit") != -1) {
			  //退出登录接口，放行
			  arg2.doFilter(request, response);
			  return;
			}
			...
			try {
				if (Utils.enableRedisCache() && Utils.checkOneLoginEnable()) {
					/** pc端  同一用户同一时刻只允许一个用户登录校验 begin  **/
					logger.debug("MyFilter.dataObj="+ JSON.toJSONString(dataObj));
				  	String appKey =  String.valueOf(dataObj.get("appKey")); //当前请求用户userKey
					String signflat = String.valueOf(dataObj.get("signflat")); //当前登录用户的入口类型
				  	String loginKey = signflat + appKey ; //当前登录用户缓存key
//				  	LoginUser user = new LoginUser();
//				  	user=(LoginUser) JedisUserUtils.getObject(loginKey);
					LoginUser user = LoginContext.getLoginUser(LoginUser.class);

					logger.debug("MyFilter.loginKey="+ JSON.toJSONString(loginKey)+"===user="+JSON.toJSONString(user));
				  	if(user == null || user.getUserKey() == null) {
						logger.error("token缺失用户信息");
						dealWithErrorForApp(request, response, currentMsgId, "请重新登录", null);
						return;
				    }
				  	if(!user.getUserKey().equals(appKey)) {
						logger.error("appkey不一致，强制退出====>loginUserAppkey:"+appKey+",onlineUserAppkey:"+user.getAppKey());
						dealWithErrorForApp(request, response, currentMsgId, "当前账号已在其他设备登录，请重新登录", null);
						return;
				    }
				}
				/** pc端  同一用户同一时刻只允许一个用户登录校验 end **/
			} catch (Exception e) {
				e.printStackTrace();
		  	}
		}
    ...
    private boolean checkOneLogin(FilterChain arg2, HttpServletRequest request,
        HttpServletResponse response, JSONObject outputMSg, Map<String, Object> resultMap)
        throws IOException, ServletException {
        ...
        boolean checkResult = true;
          ...
          LoginUser user = new LoginUser();
          user = (LoginUser)JedisUserUtils.getObject(loginUserKey); /*CacheManager.getInstance().getCacher().getObject(loginUserKey)*/;
          if(user == null ) {
          ...
          loginUser = (LoginUser)JedisUserUtils.getObject(onlineUserKey);
          //loginUser = (LoginUser) CacheManager.getInstance().getCacher().getObject(onlineUserKey);
          ...
          if(!appkey.equals(loginUser.getAppKey())) {
                ...
                //移除该请求用户的缓存
                JedisUserUtils.del(loginUserKey);
                /*CacheManager.getInstance().getCacher().remove(loginUserKey);*/
                outputMSg.put("message", "当前账号已在其他设备登录，请重新登录");
                output(outputMSg.toJSONString(), response);
                checkResult = false;
                return checkResult;
