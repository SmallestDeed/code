...
//		List<BaseProduct> baseProductList = baseProductMapper.getByDesignPlanId(designPlanId);
	 ...
	 * 更新方案结构产品
	 * @author xiaoxc
	 * @param structureProductJson
	 * [{"structureId":18274,"needDelStructureId":18275,"productId":287913,"initProductId":11111,"posIndexPath":"\"1/22/\"","posName":"basic_dim0","regionMark":10,"center":""}]
	 * @param designPlanId 方案ID
	 * @param loginUser
	 * @return
	 */
	@Override
	public List<PlanProductResult> updatePlanStructureProduct(Integer designPlanId, String structureProductJson, LoginUser loginUser) {

		List<PlanProductResult> list = new ArrayList<>();
		PlanProductResult result = null;

		Gson gson2 = new Gson();
		List<PlanStructureProductJsonModel> modelList = gson2.fromJson(structureProductJson, new TypeToken<List<PlanStructureProductJsonModel>>() {}.getType());
		if (null == modelList || 0 == modelList.size() ) {
			return null;
		}
		for (PlanStructureProductJsonModel jsonModel : modelList) {
			result = new PlanProductResult();
			Integer needDelStructureId = jsonModel.getNeedDelStructureId();
			if (!ParamCommonVerification.checkTheParamIsValid(jsonModel.getPosName(),
					jsonModel.getPosIndexPath(), jsonModel.getCenter(), jsonModel.getRegionMark())) {
				 return null;
			}
			if (!ParamCommonVerification.checkTheParamIsValid(jsonModel.getStructureId(),
					jsonModel.getInitProductId(), needDelStructureId)) {
				return null;
			}
			if (needDelStructureId != null && needDelStructureId.intValue() > 0) {
				int delNumber = designPlanProductMapper.delPlanProductByStructureId(designPlanId,needDelStructureId);
				if (delNumber > 0) {
					DesignPlanProduct designPlanProduct = new DesignPlanProduct();
					designPlanProduct.setPlanId(designPlanId);
					if (jsonModel.getProductId() == null || jsonModel.getProductId() == 0) {
						designPlanProduct.setProductId(jsonModel.getInitProductId());
					}else {
						designPlanProduct.setProductId(jsonModel.getProductId());
					}
					designPlanProduct.setInitProductId(jsonModel.getInitProductId());
					designPlanProduct.setRegionMark(jsonModel.getRegionMark());
					designPlanProduct.setProductGroupId(jsonModel.getStructureId());
					designPlanProduct.setGroupType(StructureProductConstant.STRUCTURE_TYPE_SIGN);
					designPlanProduct.setPlanGroupId(jsonModel.getStructureId()+"_"+Utils.getCurrentDateTime(Utils.DATETIMESSS));
					designPlanProduct.setPosName(jsonModel.getPosName());
					designPlanProduct.setPosIndexPath(jsonModel.getPosIndexPath());
					designPlanProduct.setCenter(jsonModel.getCenter());
					//TODO 通过挂节点来生成排序
					String productSequence = com.nork.common.util.StringUtils.replaceString(jsonModel.getPosIndexPath(), "/");
					designPlanProduct.setProductSequence(productSequence);
					designPlanProduct.setIsHide(0);
					//TODO 设置系统信息
					sysSave(designPlanProduct,loginUser);
					int insertNuber = designPlanProductMapper.insertSelective(designPlanProduct);
					if (insertNuber == 0) {
						return null;
					}
					//TODO 返回给客户端存储到配置文件用
					result.setPosName(jsonModel.getPosName());
					result.setPlanProductId(designPlanProduct.getId());
					list.add(result);
				}
			}
		}
		...
		designPlan.setIsDecorated(DesignPlanConstants.DECORATED_STATE_ONE);
