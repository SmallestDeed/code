...
public class MobilePayRenderController {
	...
	@RequestMapping(value = "/deductionOfPoints")
	@ResponseBody
	public Object deductionOfPoints(@RequestBody MobileRenderRecord model) {

		return mobilePayRenderService.deductionOfPoints(model);
	}
	...
	/**
	 * 我的消息列表 add by yangzhun
	 * @param model
	 * @return
	 */
	@RequestMapping(value = "/getMyMessageList")
	@ResponseBody
	public Object getMyMessageList(@RequestBody AutoRenderTask model) {

		model.setIsDeleted(new Integer(0));
		
		return mobilePayRenderService.getMyMessageList(model);
	}
	
	/**
	 * 移动端删除我的任务和设计 add by yangzhun
	 * @param model
	 * @return
	 */
