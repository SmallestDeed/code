...
import com.nork.product.model.ProductPlatform;
...
public interface GroupProductMapper {
	 ...
	 * @return
	 */
	ProductPlatform getGroupProductSalePrice(@Param("groupId") Integer groupId, @Param("platformType") String platformType);
...
//	List<GroupProduct> getGroupListByStructureId(GroupProductSearch groupSearch);
	...
	Integer getMainProductIdByGroupId(@Param("groupId") Integer groupId);

	List<GroupProductDetails> findBuildingHomeGroupList(BaseBrandSearch ResBrand);

	int findBuildingHomeGroupCount(BaseBrandSearch ResBrand);

	String getGroupConfigByGroupId(@Param("groupId") Integer groupId);

	/**
	 * 通过组合id集合查询数量（判断是否有删除的）
	 * @param groupIdList
	 * @return
	 */
	int getCountByGroupIdList(@Param("groupIdList") List<Integer> groupIdList);
	
	/**
	 * 获得组合最大id
	 * @return
	 */
    int getMaxGroupProductId();
}
