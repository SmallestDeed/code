...
import com.nork.product.model.GroupProductTypeConstant;
...
//            sysDictionary.setType("spaceType");
          ...
          //region 新增加参数校验 begin  add by chenm 20180605
//        if(groupType.intValue() == GroupProductTypeConstant.TYPE_INTELLIGENCE) {
          //校验用户类型，是否为内部用户或设计师
          if(loginUser.getUserType() == null) {
                return new ResponseEnvelope<BaseProduct>(false,"用户类型为空!",msgId);
          }
          if(!loginUser.getUserType().equals(UserTypeConstant.USER_TYPE_INTERNAL) && !loginUser.getUserType().equals(UserTypeConstant.USER_TYPE_DESIGNER)) {
                return new ResponseEnvelope<BaseProduct>(false,"抱歉，您无权限制作组合!",msgId);
          }
...
//        }
           //endregion 新增加的参数校验 end
          //endregion 参数校验完成 end
          ...
          if(designPlan == null) {
            throw new BaseProductException("没有该设计方案!designPlanId :" + designPlanId);
          }
          ...
          //编码会自动生成，去除编码重复校验,groupCode 的意义由用户自定义的编码变为组合名称
...
//          }else{
              ...
              //产品要求组合空间面积不赋值  update by chenm 20180626
//              try {
//                if(spaceCommon.getSpaceAreas() != null) {
//                  groupProduct.setSpaceAreaValue(Integer.parseInt(spaceCommon.getSpaceAreas()));
//                }
//              } catch (Exception e) {
//                e.printStackTrace();
//                return new ResponseEnvelope<BaseProduct>(false,"空间面积转换发生了错误！",msgId);
//              }
...
//                    }
                  ...
                  if(!isMainFlag) {
                    throw new BaseProductException("未找到主产品，保存组合明细数据失败!");
                  }
              /**保存结构组合**/
              }else if(isBmGroup == 2 ){
                //region 保存结构组合的逻辑 不知道有没有用
...
//                      }
                  ...
                  if(!isMainFlag) {
                      return new ResponseEnvelope<BaseProduct>(false, "缺失主产品，保存组合明细数据失败!", msgId);
                  }
                  //endregion 保存结构组合的逻辑
              /**保存普通组合**/ 
              }else{
...
//                        }
                  ...
                  if(!isMainFlag) {
                      throw new BaseProductException("缺失主产品，保存组合明细数据失败!");
                  }
              }
//          }
