...
public class UserSpellingflowerColletServiceImpl implements UserSpellingflowerColletService {
	 ...
	 */
	@Override
	public ResponseEnvelope getPaginatedList(UserSpellingflowerColletSearch colletSearch, LoginUser loginUser) {
		...
		if(null!=colletList&&colletList.size()>0){
			try {
				...
				for(int i = 0;i<colletList.size();i++){
					...
					//5.获取收藏拼花所对应的产品map集合
					Map<String,Object> spellingFlowerProductMap = desginPlanSpellingflowerRecordService.spellingFlowerData(productIds,loginUser);
					//6.设值到当前对象
