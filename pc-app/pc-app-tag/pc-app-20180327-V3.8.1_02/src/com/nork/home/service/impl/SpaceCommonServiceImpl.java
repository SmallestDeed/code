...
public class SpaceCommonServiceImpl implements SpaceCommonService {
  ...
  /*      float valNum = Float.valueOf(val);
	...
	public void setPicParams(SpaceCommon spaceCommon) {
		...
		List<DesignTemplet> list = designTempletMapper.selectPaginatedList(designTempletSearch);
		...
		if (StringUtils.isNotBlank(effectPlanIdsStr)) {
			if (effectPlanIdsStr.indexOf(",") != -1) {
				effectPlanIdsStr = effectPlanIdsStr.substring(0, effectPlanIdsStr.indexOf(","));
			}
			...
			if (resPic != null) {
				...
				if (effectPlanSmallId != null) {
					ResPic resPicSmall = resPicMapper.selectByPrimaryKey(effectPlanSmallId);
					if (resPicSmall != null) {
						spaceCommon.setViewPlanSmallPath(resPicSmall.getPicPath());
					}
				}
			}
		}
	...
	@Override
	public Integer getDrawSpaceCount(Integer houseId) {
		if (houseId == null || houseId <= 0) {
			return -1;
		}
		return spaceCommonMapper.getDrawSpaceCount(houseId);
	}
}
