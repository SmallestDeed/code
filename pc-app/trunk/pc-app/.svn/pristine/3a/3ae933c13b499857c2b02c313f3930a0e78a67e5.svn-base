...
public class WebDesignPlanControllerV2 {
	...
	String logPrefixClass = "function:WebDesignPlanControllerV2.";
	...
	@ResponseBody
	public Object getDesignRecommendedInfo(String designPlanRecommendeId,String msgId,HttpServletRequest request,HttpServletResponse response){
...
//		loginUser.setLoginName("chenqiang");
		...
		if(StringUtils.isBlank(designPlanRecommendeId))
			return new ResponseEnvelope<DesignPlan>(false, "推荐方案id不能为空", msgId);


		try {

			return (ResponseEnvelope)designPlanService.getDesignRecommendedInfo(designPlanRecommendeId,loginUser,msgId);
...
//        designPalnRenderService.deleteTempDesignPaln4RenderBakScene( Long.valueOf(tempDesignPalnId),userId);
	...
	public Object transformBydesigPlanRecommended(DesignPlanRecommended designPlanRecommended, String houseId,
		...
		String logPrefixFunction = logPrefixClass + "transformBydesigPlanRecommended -> ";
		...
		/*logger.error(logPrefixFunction + "designPlan.getSpaceCommonId() = " + designPlan.getSpaceCommonId());*/
	...
	public ResponseEnvelope enterRenderScene(HttpServletRequest request,HttpServletResponse response ){
        ...
        String msgId = request.getParameter("msgId");
		Integer demandId = 0;
        if (!StringUtils.isEmpty(request.getParameter("supplyDemandId"))) {
			demandId = Integer.parseInt(request.getParameter("supplyDemandId"));
		}

		String historyMsgId = "";
		if (!StringUtils.isEmpty(request.getParameter("historyMsgId"))) {
			historyMsgId = request.getParameter("historyMsgId");
		}
        ...
        if(demandId > 0){
			designPlanRes.setSupplyDemandId(demandId);
		}

		if(StringUtils.isNotEmpty(historyMsgId)){
			designPlanRes.setHistoryMsgId(historyMsgId);
		}
