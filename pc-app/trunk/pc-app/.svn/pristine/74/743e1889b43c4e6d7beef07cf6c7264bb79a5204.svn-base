...
public class ProductsCost  extends Mapper implements Serializable {
    ...
    private String status;//如果品牌是三度或者是用户是内部用户则产品清单不做过滤
    
	/*过滤条件:序列号里的品牌,大类,小类,产品ids(序列号条件设置到BaseProduct类中)*/
	private List<BaseProduct> baseProduct;
    /** 产品的状态集合 **/
    private List<Integer> productPutawayState;

    public List<Integer> getProductPutawayState() {
        return productPutawayState;
    }

    public void setProductPutawayState(List<Integer> productPutawayState) {
	...
	public void setBrandsStr(String brandsStr) {
		this.brandsStr = brandsStr;
	}
}
