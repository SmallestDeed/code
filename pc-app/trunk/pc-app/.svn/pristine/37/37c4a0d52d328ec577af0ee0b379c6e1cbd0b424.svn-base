...
//		SysUser user=(SysUser) CacheManager.getInstance().getUserCacher().getObject(getUserKeyByToken(token));
		SysUser sysUser = LoginContext.getLoginUser(SysUser.class);
		return sysUser;
	}
...
//		userCacher.setObject(getUserKeyByUserkey(user.getUserKey()), user, exSeconds);
	 ...
	 * 用于自动渲染用户登录存储信息
	 * @param user
	 * @param exSeconds 超时时间
	 */
	public static void cacheTheAutoLoginUserInfo(LoginUser user, int exSeconds) {
      CacheManager.getInstance().getUserCacher().setObject(getUserKeyByUserkey(user.getUserKey()), user, exSeconds);
    }
	
	public static LoginUser getCacheLoginUserByUserkey(String userKey) {
//		LoginUser user = (LoginUser) CacheManager.getInstance().getUserCacher().getObject(getUserKeyByUserkey(userKey));
//		LoginUser user = (LoginUser) CacheManager.getInstance().getUserCacher().getObject(userKey);
...
//		CacheManager.getInstance().getUserCacher().removeObject(getUserKeyByUserkey(userKey));
		LoginContext.removeUserCache(getUserKeyByUserkey(userKey));
	}
}
