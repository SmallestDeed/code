...
public class UnionGroupDetailsServiceImpl implements UnionGroupDetailsService {
		...
		try {
			...
			for (UnionGroupDetailsVO groupDetailsVO : groupDetailsList) {
				...
				if (groupDetailsVO.getGroupDetailsId() == null || groupDetailsVO.getGroupDetailsId() == 0) {
					...
					//处理企业Id
					this.dealDetailsCompanyId(groupDetails,loginUser);
	...
	private UnionGroupDetails dealDetailsCompanyId(UnionGroupDetails unionGroupDetails, LoginUser loginUser){
		if(unionGroupDetails.getCompanyId() != null && unionGroupDetails.getCompanyId() != 0){
			return unionGroupDetails;
		}
		//如果保存时没有传递公司Id，则查询改用户绑定的公司Id.如果没有,则根据品牌Id查询公司Id进行填充
		Integer companyId = null;
		BaseCompany baseCompany = null;
		if(loginUser != null && loginUser.getBusinessAdministrationId() != null && loginUser.getUserType() != null){
		...
		if(baseCompany == null || baseCompany.getId() == null){
			BaseBrand baseBrand = baseBrandService.get(unionGroupDetails.getBrandId());
			if(baseBrand != null && baseBrand.getCompanyId() != null){
				baseCompany = baseCompanyService.get( baseBrand.getCompanyId());
			}
		}
		if(baseCompany != null && baseCompany.getId() != null){
