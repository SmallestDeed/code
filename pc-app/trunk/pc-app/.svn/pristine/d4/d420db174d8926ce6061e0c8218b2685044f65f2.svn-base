...
//    private static Logger logger = Logger.getLogger(ResRenderPicServiceImpl.class);
	...
	public int add(ResRenderPic resRenderPic) {
	    ...
	    //判断草图方案是否修改:只有未被修改的草图方案（临时方案），才需要关联效果图方案
		if(1 == designPlan.getIsChange().intValue())
			resRenderPic.setDesignSceneId(null);
		else
...
//        }
	...
	@Override
	public int addDesignRender(ResRenderPic resRenderPic) {
		...
		DesignPlanRenderScene  designPlanRenderScene = designPlanRenderSceneService.get(resRenderPic.getDesignPlanSceneId());
	...
	public int update(ResRenderPic resRenderPic) {
		return resRenderPicMapper.updateByPrimaryKeySelective(resRenderPic);
	}
	 ...
	 */
	public int updateSysTaskId(Integer sysTaskId,List<Integer> idList){
		return resRenderPicMapper.updateSysTaskId(sysTaskId,idList);
	}
...
//	@Scheduled(fixedDelay = 1000*6)
	 ...
	 */
	public List<ResRenderPic> getListByIdList(List<Integer> idList){
		return resRenderPicMapper.selectListByIdList(idList);
	}
	 ...
	 */
	public int delResRenderPicScne(List<Integer> idList){

		return 0;
	}

	@Override
	public ResRenderPic get720SmallPicInfoByPlanId(Integer planId) {
		return resRenderPicMapper.find720SmallPicInfoByPlanId(planId);
	}
}
