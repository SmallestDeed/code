...
import com.nork.product.model.dto.ProductCeilingDataDTO;
...
import com.nork.product.service.impl.GroupProductServiceImpl;
...
public class WebBaseProductController extends BaseController {
	...
	//默认截面天花款式不处理
	private final static String DEFAULT_STYLE_CODE2 ="TA00002";
	private final static String DEFAULT_STYLE_CODE3 ="TA00003";
	@Autowired
	...
	@ResponseBody 
	public Object productDetailsWeb(@ModelAttribute("baseProduct") BaseProduct baseProduct,
			Integer designPlanId, Integer spaceCommonId, Integer sceneProductId, Integer initProductId, HttpServletRequest request, HttpServletResponse response) {
		...
		if (jsonStr != null && jsonStr.trim().length() > 0) {
			baseProduct = JsonUtil.getJsonToBean(jsonStr, BaseProduct.class);
			if (baseProduct == null) {
		...
		try {
			...
			if(null != baseProduct.getBedsideDimensions())
				baseProduct.setBedsideDimensions(baseProduct.getBedsideDimensions() / 100);
			...
			// add by huangsongbo 2018.9.4:去除null的元素 ->start
			if(Lists.isNotEmpty(baseBrand)) {
				baseBrand = baseBrand.stream().filter(item -> item != null).collect(Collectors.toList());
			}
			// add by huangsongbo 2018.9.4:去除null的元素 ->end
			...
			/**详情逻辑*/
			productDetailContent(baseProduct, sceneProductId, initProductId, loginUser);
...
//		}
	...
	public Object list(
		...
		try {
			//授权码绑定的品牌
			//String brandIds = authorizedConfigService.getAuthorizedBrandIdsByUserId(loginUser.getId());//2018-3-15 wutehua
			
			//BaseBrand baseBrand = baseBrandService.findBrandIdByUserId(loginUser.getId());
			...
			if(null != baseBrand){
				if(StringUtils.isNotBlank(baseBrand.getBrandStr())){
					brandIds = baseBrand.getBrandStr();
				}
			}
			
			//如果是厂商，则只能查询这个厂商品牌下的产品及行业之外的产品
			if (UserTypeCode.USER_TYPE_OUTER_B2B.equals(loginUser.getUserType())){
				//设置品牌行业值，用户过滤同行业品牌产品
			...
			for(CategoryProductResult cpr : list){
				/**品牌是否被绑定状态*/
				String productBrandId = cpr.getBrandId().toString();
			...
			//保密产品，只对所属企业用户可见
//			List<CategoryProductResult> listTemp = new ArrayList<CategoryProductResult>();
//			for (CategoryProductResult categoryProductResult : list) {
//				if(categoryProductResult.getSecrecyFlag() == 1){	//保密
//					if(baseBrand.toString().contains(categoryProductResult.getBrandId()+"")){	//员工所属企业品牌是否包含产品品牌
//						listTemp.add(categoryProductResult);
//					}
//				}else{
//					listTemp.add(categoryProductResult);
//				}
...
//			list = listTemp;
	...
	public void productDetailContent(BaseProduct baseProduct, Integer sceneProductId, Integer initProductId, LoginUser loginUser) throws Exception {
		...
		String modelId = baseProductService.getU3dModelId(loginUser.getMediaType(), baseProduct);
		if (!StringUtils.isEmpty(modelId)) {
			...
			if(resModel != null){
				...
				baseProduct.setModelCode(resModel.getModelCode());
			}
		}
		...
		if(StringUtils.isNotBlank(splitTexturesInfo)){
			//设值产品多维标识
			baseProduct.setIsMultidimensional(1);
		...
		if(StringUtils.isNotBlank(materialIds)){
			...
			if (Lists.isNotEmpty(materialPathList)) {
				...
				if (null != sceneProductId && sceneProductId > 0){
					BaseProduct product = baseProductService.get(sceneProductId);
					boolean isHard = false;
					if (product != null) {
						isHard = baseProductService.isHard(product);
					}
					if (isHard && null != initProductId) {
						BaseProduct baimoProduct = baseProductService.get(initProductId);
						//获取不同媒介u3d模型
						String u3dModelId = baseProductService.getU3dModelId(loginUser.getMediaType(), baimoProduct);
	...
	public Object updateBmGroupLocation(String groupCode, String config, Integer isBmGroup, String msgId,
		...
		try {
			...
			if (Lists.isNotEmpty(groupProducts)) {
				...
				/** 保存白模组合产品 **/
				if (isBmGroup == 1) {
					Integer fileId = baseProductService.locationTxt(config, loginUser, groupProduct);
					/* 讲文件ID保存至组合位置字段 */
	 ...
	 * 
	 * @param isBmGroup save白膜组合类型：1，save结构组合类型：2  ，0为产品组合类型
	 * @param groupType 组合类型: 0,普通组合;1,一键装修组合
	 * @spaceCommonType 所属房型  对应groupProduct 的type
	 * 
	 * @param isMainProductId 主产品Id 
	 * @param compositeTypeValue 组合类型value
	 * @param coverPicFile 封面图
	 * @param authorization 因为传不了请求头中的token ，所以就这样接收
	 * @return
	 */
...
//			String designPlanCode,
			String spaceCommonTypeCode,
			String designTempletCode,
			Integer groupType,
			Integer isMainProductId,
			Integer compositeTypeValue,
			@RequestParam(value ="coverPicFile",required = false) MultipartFile  coverPicFile
			,String authorization
			){
		try {
		  return baseProductService.updateProductGroup(groupCode, designPlanId, spaceCommonType, config, isBmGroup,
              msgId, request, structureCode, spaceCommonTypeCode, designTempletCode, groupType,isMainProductId,compositeTypeValue,coverPicFile,authorization);
        } catch (BaseProductException e){
			return new ResponseEnvelope<BaseProduct>(false, e.getMessage(),msgId);
		} catch (Exception e) {
           return new ResponseEnvelope<BaseProduct>(false, "出现异常！",msgId);
        }
	}
		...
		try{
			...
			if(!Lists.isNotEmpty(groupProducts)){
				if((structureCode==null||"".equals(structureCode))&&(designTempletCode!=null&&!"".equals(designTempletCode))){
					...
					Integer fileId= baseProductService.locationTxt(config,loginUser,groupProduct);/*讲文件ID保存至组合位置字段*/
					groupProduct.setDesignTempletId(designTemplet.getId());
					groupProduct.setLocation(fileId+"");
	...
	public Object findSameTypeProductList(Integer productId,Integer designPlanId, Integer planProductId,Integer spaceCommonId,
		...
		if(bp!=null){
			if(StringUtils.isNotBlank(baseProductNew.getSplitTexturesInfo())){
				Map<String,Object> map=baseProductService.dealWithSplitTextureInfo(baseProductNew.getId(), baseProductNew.getSplitTexturesInfo(),"info");
				isSplit=(Integer) map.get("isSplit");
...
/*	@RequestMapping("/testUnZip")
	...
	@ResponseBody
	public ResponseEnvelope productSelectGroupReplaceV2(HttpServletRequest request , Integer styleId){
		...
		if (null == loginUser) {
			return new ResponseEnvelope<>(false,"请登录！", msgId);
		}
		String mediaType = StringUtils.isEmpty(loginUser.getMediaType())?"3":loginUser.getMediaType();
	 ...
	 * @author huangsongbo
	 * @param productCodes
	 * @param reMessage 前端给的信息,最后再返给前端(不作处理,只传递)
	 * @param msgId
	 * @return
	 */
	...
	public ResponseEnvelope<CategoryProductResult> getInfoByProductCode(
		...
		// 添加reMessage ->start
		for(CategoryProductResult categoryProductResult : results) {
			categoryProductResult.setReMessage(reMessage);
		}
		// 添加reMessage ->start
	...
	public Object getDetailByIds(@RequestParam(required = false) String idArrs, Integer limit
		...
		return baseProductService.getDetailByIds(idIntegerList, limit, start, userId, msgId,
				isStandard, regionMark, styleId, measureCode);
	}
	...
	/**
	 * 通过款式Code,面积,筛选截面天花数据
	 *
	 * @return
	 */
	@RequestMapping(value = "/getProductCeilingData",method = RequestMethod.POST)
	@ResponseBody
	public Object getDetailByIds( ProductCeilingDataDTO productCeilingDataDTO) {
		if(productCeilingDataDTO==null ||productCeilingDataDTO.getKeyName() ==null ){
			return new ResponseEnvelope<>(false,"缺失参数");
		}
		ResponseEnvelope res = new ResponseEnvelope();

		Type type = new TypeToken<List<CeilingCrossDataDTO>>(){}.getType();
		List<CeilingCrossDataDTO> ceilingCrossDataDTOList=null;
		String ceilingCrossSectionDataStr = productCeilingDataDTO.getKeyName();
		if(StringUtils.isNotBlank(ceilingCrossSectionDataStr)){
			ceilingCrossDataDTOList =gson.fromJson(ceilingCrossSectionDataStr,type);
		}
		if( ceilingCrossDataDTOList == null || ceilingCrossDataDTOList.size() == 0 ){
			return new ResponseEnvelope<>(false,"缺少款式参数");
		}
		List<String> styles = new ArrayList<>();
		for(CeilingCrossDataDTO ceilingCrossDataDTO : ceilingCrossDataDTOList){
			String styleCode = ceilingCrossDataDTO.getStyleCode();
			if(StringUtils.isNotBlank(styleCode)){
				styles.add(styleCode);
			}
		}
		logger.info("截面天花筛选款式ID:"+gson.toJson(styles));
		List<CeilingCrossDataResult> ceilingList =baseProductService.getCeilingCrossDataByStyleId(styles);





		if(ceilingList == null || ceilingList.size() ==0){
			return new ResponseEnvelope<>(false,"未查询到天花截面数据");
		}
		...
		//通过面积筛选并封装数据
		List<CeilingCrossDataVO> ceilingCrossDataVOList = new ArrayList<>();

		for(CeilingCrossDataResult ceilingData : ceilingList){

			for(CeilingCrossDataDTO ceilingCrossDataDTO : ceilingCrossDataDTOList){

				if(ceilingData.getStyleCode().equals(ceilingCrossDataDTO.getStyleCode())){
					CeilingCrossDataVO crossDataVO = new CeilingCrossDataVO();
					crossDataVO.setAreaSign(ceilingCrossDataDTO.getAreaSign());

					//解析天花截面json串获取 截面坐标和灯带坐标
					Type type2 = new TypeToken<List<CeilingCrossSectionData>>(){}.getType();
					List<CeilingCrossSectionData> ceilingCrossSectionDataList = gson.fromJson(ceilingData.getCeilingCrossData(),type2);

					//去除空的截面天花数据
					List<CeilingCrossSectionData> emptyList = new ArrayList<>();
					//去除空的截面天花数据
					for(CeilingCrossSectionData ceilingCrossSectionData : ceilingCrossSectionDataList){
						if(StringUtils.isBlank(ceilingCrossSectionData.getCeilingInfo())){
							emptyList.add(ceilingCrossSectionData);
						}
					}
					ceilingCrossSectionDataList.removeAll(emptyList);


					for(CeilingCrossSectionData ceilingCrossSectionData : ceilingCrossSectionDataList){
						Integer applyArea = ceilingCrossDataDTO.getApplyArea();
						String[] areaStr = ceilingCrossSectionData.getApplyArea().split(",");
						for(String area :areaStr){
							if(StringUtils.isNotBlank(area)) {
								if (area.startsWith("70")) {
									if (applyArea > 70) {
										crossDataVO.setCeilingInfo(ceilingCrossSectionData.getCeilingInfo());
										crossDataVO.setLightInfo(ceilingCrossSectionData.getLightInfo());
										if (!DEFAULT_STYLE_CODE2.equals(ceilingData.getStyleCode()) && !DEFAULT_STYLE_CODE2.equals(ceilingData.getStyleCode())) {
											crossDataVO.setStyleCode(ceilingData.getStyleCode() + "_70D");
										} else {
											crossDataVO.setStyleCode(ceilingData.getStyleCode());
										}
										break;
									}
								} else {
									String[] areas = area.split("~");
									if (Integer.parseInt(areas[0]) < applyArea && applyArea <= Integer.parseInt(areas[1])) {
										crossDataVO.setCeilingInfo(ceilingCrossSectionData.getCeilingInfo());
										crossDataVO.setLightInfo(ceilingCrossSectionData.getLightInfo());
										if (!DEFAULT_STYLE_CODE2.equals(ceilingData.getStyleCode()) && !DEFAULT_STYLE_CODE2.equals(ceilingData.getStyleCode())) {
											crossDataVO.setStyleCode(ceilingData.getStyleCode() + "_" + areas[1]);
										} else {
											crossDataVO.setStyleCode(ceilingData.getStyleCode());
										}
										break;
					...
					ceilingCrossDataVOList.add(crossDataVO);
				}
		...
		logger.info("获取截面天花数据集合长度:"+ceilingCrossDataVOList==null?null:ceilingCrossDataVOList.size());
		res.setSuccess(true);
		res.setMsgId(productCeilingDataDTO.getMsgId());
		res.setDatalist(ceilingCrossDataVOList);
		return res;
	}
