	...
	public Object updatePassword(String id, String oldPassword,
		...
		if (sysUser.getPassword().equals(oldPassword)) {
			...
			if(null != userInfo && userInfo.size()>1){
				...
				List<Integer> idList = userInfo.stream().map(SysUser::getId).collect(Collectors.toList());
				sysUser.setUserId(idList);
	...
	public Object findPassword(String mobile, String newPassword, String code,
		...
		if (num < verifyCount) {
			if (!code.equals(yzm)) {
				...
				//success 设为true,配合前端设计（使提示由弹框提示变为文字提示）
				return new ResponseEnvelope<SysUser>(true , "验证码错误！", msgId);
			}
		} else {
		  	//success 设为true,配合前端设计（使提示由弹框提示变为文字提示）
			return new ResponseEnvelope<SysUser>(true , "验证码已失效，请重新获取！", msgId,false);
		...
		//获取手机号码B端所有用户
		List<SysUser> userList = sysUserService.selectByMobile(mobile);

		if (CustomerListUtils.isEmpty(userList)) {
			...
			//参数
			List<Integer> idList  = userList.stream().map(SysUser::getId).collect(Collectors.toList());
			SysUser user = new SysUser();
			user.setUserId(idList);
