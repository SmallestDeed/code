...
public class BaseProductServiceImplV2 implements BaseProductServiceV2 {
	...
	public List<BaseProduct> getBatchData(List<Integer> list,List<Integer> putawayStateList){
		return baseProductMapper.getBatchData(list,putawayStateList);
	}
	...
	public void productMaterial(BaseProduct baseProduct, Integer id, String planProductId, Integer designPlanId,
			Integer spaceCommonId, LoginUser loginUser, HttpServletRequest request)
			throws Exception {
		DesignTemplet designTemplet = new DesignTemplet();
		DesignPlan designPlan = null;
		if (designPlanId != null) {
			designPlan = (Utils.enableRedisCache()
					? DesignCacher.get(designPlanId) : designPlanService.get(designPlanId));
			designTemplet = (Utils.enableRedisCache()
					? DesignCacher.getTemplet(designPlan.getDesignTemplateId())
					: designTempletService.get(designPlan.getDesignTemplateId()));
		}
		// 详情逻辑
		...
		Map<String, CategoryProductResult> baimoRule = setbaimoRuleMap(spaceCommonId, request, loginUser.getId(),
				baseProduct.getProductSmallTypeCode(), designPlan, map);
		baseProduct.setBasicModelMap(baimoRule);
	}

	// 详情逻辑
	public void productDetailContent(BaseProduct baseProduct, String planProductId, DesignPlan designPlan,
		...
		if (StringUtils.isNotBlank(materialIds)) {
			...
			if (Lists.isNotEmpty(materialPathList)) {
				baseProduct
						.setMaterialPicPaths((String[]) materialPathList.toArray(new String[materialPathList.size()]));
	...
	public Map<String, CategoryProductResult> setbaimoRuleMap(Integer spaceCommonId, HttpServletRequest request,
					...
					for (AutoCarryBaimoProducts baimoProductConfig : baimoProductsConfigs) {
						...
						if (tempList != null && tempList.size() > 0) {
							...
							CategoryProductResult baimoProducts = this.assemblyUnityPanProduct(baimoProductId,
									spaceCommonId, designPlan, userId, request);
	...
	public CategoryProductResult assemblyUnityPanProduct(Integer productId, Integer spaceCommonId,
			DesignPlan designPlan, Integer userId, HttpServletRequest request) {
		String mediaType = com.nork.common.constant.util.SystemCommonUtil.getMediaType(request);
		...
		CategoryProductResult productResult = productCategoryRelService
	...
	public boolean isHard(BaseProduct baseProduct) {
		...
		if (StringUtils.isNotBlank(typeValue)) {
			...
			if ("qiangm,dim".indexOf(dlDic.getValuekey()) > -1) {
				...
				if (("chuangt".equals(valueKey) || "cant".equals(valueKey) || "shaf".equals(valueKey)
						|| "dians".equals(valueKey) || "xingx".equals(valueKey))
	...
	public List<CategoryProductResult>  list(BaseProduct baseProduct,LoginUser loginUser,int total) {
		if (loginUser.getUserType() == SystemCommonConstant.STATE) {
		...
		List<AuthorizedConfig> authorizedList = new ArrayList<>();
		authorizedList = StringUtils.equals(SystemCommonConstant.CLOSE_REDIS_CACHE_ENABLE, cacheEnable)
				? authorizedList = AuthorizedConfigCacher.getList(authorizedConfig) : authorizedConfigService.getList(authorizedConfig);
		String brandIds = "";
		for (AuthorizedConfig ac : authorizedList) {
			if (StringUtils.isNotBlank(ac.getBrandIds())) {
				brandIds += ac.getBrandIds() + ",";
		...
		if (3 == loginUser.getUserType()) {
			if (StringUtils.isNotBlank(brandIds)) {
				...
				baseProduct.setBrandIds(Arrays.asList(arrayBrandId));
			}
		}
		total = StringUtils.equals(SystemCommonConstant.CLOSE_REDIS_CACHE_ENABLE, cacheEnable)
				? BaseProductCacher.getBrandProductCount(baseProduct) : getBrandProductsCount(baseProduct);
		// 如果是厂商，则只能查询这个厂商品牌下的产品
		if (3 == loginUser.getUserType()) {
			// 如果没有则显示其它品牌产品
			if (total == 0) {
				baseProduct.setBrandIds(null);
			}
			total = StringUtils.equals(SystemCommonConstant.CLOSE_REDIS_CACHE_ENABLE, cacheEnable)
					? BaseProductCacher.getBrandProductCount(baseProduct) : getBrandProductsCount(baseProduct);
		}
		if (total > 0) {
			list = StringUtils.equals(SystemCommonConstant.CLOSE_REDIS_CACHE_ENABLE, cacheEnable)
					? BaseProductCacher.getBrandProductList(baseProduct) : getBrandProductsList(baseProduct);
		}
		for (CategoryProductResult cpr : list) {
			...
			if (productId != null) {
				BaseProduct basePro = get(productId);
				...
				if (spvValue != null) {
					...
					cpr.setSalePriceValueName(dictionary == null ? "" : dictionary.getName());
				}
			}
		}
		return list;
	...
	public FindSameTypeProductListDTO findSameTypeProductList(BaseProduct baseProductNew,LoginUser loginUser,BaseProduct bp,Integer productId) {
		...
		List<SplitTextureDTO> splitTexturesInfo = new ArrayList<SplitTextureDTO>();
			if(StringUtils.isNotBlank(baseProductNew.getSplitTexturesInfo())){
				Map<String,Object> map = dealWithSplitTextureInfo(baseProductNew.getId(), baseProductNew.getSplitTexturesInfo(),"info");
				isSplit = (Integer) map.get("isSplit");
				...
				if(baseProductNew.getParentId() != null && !baseProductNew.getParentId().equals(new Integer(0))){
					...
					total = getSameTypeProductCount(products);
					if(total > 0){
							list = getSameTypeProductList(products);
							SplitTextureDTO splitTextureDTO = new SplitTextureDTO("1", "", null);
							List<ResTextureDTO> resTextureDTOList = new ArrayList<ResTextureDTO>();
	...
	public BaseProduct categoryProductList(BaseProduct baseProduct) {
		BaseProduct product = get(baseProduct.getId());
		if (product != null) {
			product.setBrandName(product.getBrandId() == null ? "": baseBrandService.get(product.getBrandId()) == null ? ""
																  : baseBrandService.get(product.getBrandId()).getBrandName());
			product.setProductColor(product.getColorId() == null ? "" : sysDictionaryService.getSysDictionaryByValue("productColor", product.getColorId()).getName());
			product.setProductType(StringUtils.isEmpty(product.getProductTypeValue()) ? "" : sysDictionaryService
							.getSysDictionaryByValue("productType", new Integer(product.getProductTypeValue())).getName());
			String smallPicIds = product.getPicIds();
			if (smallPicIds != null && !"".equals(smallPicIds)) {
				if (smallPicIds.contains(",")) {
					List<ResPic> smallPic = resPicService.getBatchGet(Utils.StringToIntegerLst(Utils.getListFromStr(smallPicIds)));
						for (ResPic resPic : smallPic) {
