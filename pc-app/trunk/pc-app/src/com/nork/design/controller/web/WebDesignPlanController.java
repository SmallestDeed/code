...
import com.nork.design.model.unity.DesignPlanResListJson;
import com.nork.design.service.*;
import com.nork.product.model.*;
import com.nork.product.model.dto.CeilingCrossDataDTO;
...
public class WebDesignPlanController {
	...
	private static Logger logger = LoggerFactory.getLogger(WebDesignPlanController.class);
	private static org.slf4j.Logger log  = LoggerFactory.getLogger(WebDesignPlanController.class);
	...
	@ResponseBody
	public Object unityUpdate(Integer designPlanId, String planProductJson,
			String materialPicId, String msgId, String splitTexturesChoose, /* 保存可拆分材质产品选择的材质 */
			HttpServletRequest request)
			throws Exception {
		if (designPlanId == null || StringUtils.isEmpty(planProductJson)) {
		...
		if (null == loginUser) {
			return new ResponseEnvelope<DesignPlan>(false, "请登录！", msgId);
		}
		...
		return designPlanService.unityUpdate(designPlan, designPlanId, planProductJson, materialPicId, splitTexturesChoose, loginUser,msgId);
	}
	 ...
	 * @param sourcePlanId
	 *            源方案ID
	 * @param needShear 渲染图是否需要切割（1：表示单张全景，需要后端切割；0：表示传给后端的是6张切片图的压缩包，不需要切割）
	 * @return object
	 */
	...
	public Object saveDesignRenderPic(String level, String planId, String msgId, Integer viewPoint, Integer scene,
			...
			Integer templateId, Integer sourcePlanId,Integer userRenderType,Integer isAuto, String cameraLocation, Integer needShear) {
		String msg = "";
		logger.info("接收渲染图接口....");
		if (StringUtils.isBlank(planId)) {
			msg = "参数planId不能为空";
...
//				authorizationMap.get("token"), authorizationMap.get("deviceId"), authorizationMap.get("mediaType"));
		...
		if (RenderTypeCode.COMMON_720_LEVEL == Utils.getIntValue(renderingType)
			...
			if( RenderTypeCode.COMMON_720_LEVEL == Utils.getIntValue(renderingType) && StringUtils.isBlank(cameraLocation) ){
				msg = "参数cameraLocation不能为空";
				return new ResponseEnvelope<ResFileSmall>(false, msg, msgId);
			}
		} else {
		...
		RenderPicVO vo = this.createRenderPicVO720(designPlan, fileMap, viewPoint, scene, isTurnOn,
				...
				sourcePlanId,userRenderType,isAuto, cameraLocation, needShear);
		String platformCode = "pc2b";
		BasePlatform basePlatform = basePlatformService.getBasePlatformByCode(platformCode);
        if (basePlatform == null) {
        	return new ResponseEnvelope<BasePlatform>(false,"平台数据错误！");
		}
        //平台Id
		vo.setPlatformId(basePlatform.getId());
	...
	private RenderPicVO createRenderPicVO720(DesignPlan designPlan, Map<String, MultipartFile> fileMap,
			...
			Integer sourcePlanId,Integer userRenderType,Integer isAuto, String cameraLocation, Integer needShear) {
		if (opType == null) {
			opType = DesignPlanConstants.USER_RENDER;
		}
		RenderPicVO vo = new RenderPicVO();
		...
		vo.setIsAuto(isAuto);
		vo.setCameraLocation(cameraLocation);
		if( needShear == null ){
			vo.setNeedShear(1);
		}else{
			vo.setNeedShear(needShear);
		}
		return vo;
	}
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					//删除草图渲染图 同时删除同盟设计方案素材库相关数据及发布方案素材库相关数据
					unionDesignPlanStoreService.delDesignPlanStoreRelatedData(planId,resPicId);

				} catch (Exception e) {
					logger.error("删除删除草图渲染图同盟数据异步出错",e);
					e.printStackTrace();
				}
	 ...
	 * @return String
	 */
	@RequestMapping(value = "/queryDesignPlanRenderPicList")
	@ResponseBody
	public Object queryDesignPlanRenderPicList(@PathVariable String style, @RequestParam("msgId") String msgId, @RequestParam("designPlanId") Integer designPlanId,
									  HttpServletRequest request) throws Exception {
		if (designPlanId == null || StringUtils.isBlank(msgId))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "params error!", msgId);

		DesignPlan designPlan = designPlanService.get(designPlanId);
		if(null == designPlan)
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案不存在!", msgId);

		DesignToRenderPicInfo designToRenderPicInfo = designPlanService.getDesignToRenderPicInfo(designPlan);

		return new ResponseEnvelope<DesignToRenderPicInfo>(true, designToRenderPicInfo, msgId);
	}
	 ...
	 * @return String
	 */
	@RequestMapping(value = "/getDesignStyleList")
	@ResponseBody
	public Object getDesignStyleList(@PathVariable String style, @RequestParam("msgId") String msgId, @RequestParam("designPlanId") Integer designPlanId,
											   HttpServletRequest request) throws Exception {
		if (designPlanId == null || StringUtils.isBlank(msgId))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "params error!", msgId);

		DesignPlan designPlan = designPlanService.get(designPlanId);
		if(null == designPlan.getSpaceCommonId())
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "该方案所属空间为空!", msgId);

		int totalCount = 0;
		List<BaseProductStyle> styleList = null;

		try {
			styleList = designPlanService.getBaseProductStyleList(designPlan.getSpaceCommonId());
		}catch (Exception e){
			return new ResponseEnvelope<BaseProductStyle>(msgId,false,e.getMessage(),totalCount,styleList);
		}
		totalCount = null != styleList ? styleList.size() : 0;

		return new ResponseEnvelope<BaseProductStyle>(msgId,true,"获取成功",totalCount,styleList);
	}
	 ...
	 */
	@RequestMapping(value = "/createDesignPlanRenderScene")
	@ResponseBody
	public Object createDesignPlanRenderScene(@PathVariable String style,@RequestParam("msgId") String msgId,
											  @RequestParam("designPlanId") String designPlanId,
											  @RequestParam("designPlanName") String designPlanName,
											  @RequestParam("planDesc") String planDesc,
											  @RequestParam("styleId") String styleId,
											  @RequestParam("renderType") String renderType,
											  @RequestParam("renderPics") String renderPics,
									 HttpServletRequest request) throws Exception {
		//登录用户获取
		LoginUser loginUser = SystemCommonUtil.getCurrentLoginUserInfo(request);
		...
		//参数校验
		if(StringUtils.isBlank(msgId))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "msgId为空", msgId);
		if(null == loginUser)
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "请登录", msgId);
		if(!isNumeric(designPlanId))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案id参数有有误", msgId);
		if(StringUtils.isBlank(designPlanName))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案名称不能为空", msgId);
		if(designPlanName.length() > 100)
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案名称支持100长度", msgId);
		if(!isNumeric(styleId))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案风格参数有有误", msgId);
		if(!isNumeric(renderType))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案类型参数有有误", msgId);
		if(!isStringSToNUm(renderPics,","))
			return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案渲染参数有有误", msgId);

		//copy效果图方案
		Integer designRenderId = null;
		try {
			designRenderId = designPlanRenderService.createDesignPlanRenderScene( designPlanId, designPlanName, planDesc, styleId, renderType, renderPics,loginUser);
		}catch (Exception e){
			log.error("copy草图方案到效果图方案出错Exception:{}",e);
			return new ResponseEnvelope<DesignPlanRenderScene>(false, e.getMessage(), msgId);
		}

		//判断
		if(null == designRenderId)
			return new ResponseEnvelope<DesignPlanRenderScene>(false,"输出效果图方案失败", msgId);

		return new ResponseEnvelope<DesignPlanRenderScene>(true,"输出效果图方案成功", msgId);
	}
	 ...
	 * @date 2018/8/13 0013 10:36
	 */
	@RequestMapping(value = "/autocreateDesignPlanRenderScene",method = RequestMethod.POST)
	@ResponseBody
	public Object autoCreateDesignPlanRenderScene(@RequestBody DesignPlanResListJson designPlanResList,
											  HttpServletRequest request) throws Exception {
		LoginUser loginUser = new LoginUser();
		if(designPlanResList.getUserId() != null && designPlanResList.getUserId()>0){
			SysUser sysUser = sysUserService.get(designPlanResList.getUserId());
		...
		List<DesignPlanRes> designPlanResLists = new ArrayList<>();
		if(StringUtils.isNotBlank(designPlanResList.getDesignPlanResList())){
			Type type = new TypeToken<List<DesignPlanRes>>(){}.getType();
			designPlanResLists =new Gson().fromJson(designPlanResList.getDesignPlanResList(),type);
		}
		List<Integer> list = designPlanRenderService.autocreateDesignPlanRenderScene(designPlanResLists,loginUser);
		if(list == null || list.size() == 0){
			log.error("自动输出效果图失败");
			return new ResponseEnvelope<DesignPlanRenderScene>(false,"自动输出效果图失败");
		}

		return new ResponseEnvelope<DesignPlanRenderScene>(true,list);
	}



	/**
	 * 根据字符串与切割符号，获取集合 验证 集合元素为数字
	 * @author chenqiang
	 * @param str		字符串
	 * @param symbol	切割符
	 * @return
	 * @date 2018/8/9 0009 17:27
	 */
	private boolean isStringSToNUm(String str,String symbol){

		boolean bool = false;
		if(StringUtils.isNotBlank(str)){

			List<String> strs = Arrays.asList(str.split(symbol));
		...
		if (DesignPlanType.DESIGN_PLAN == designPlanType) {// 原方案类型
			...
			try {
				if (planRenderScene != null) {
					...
					//判断是否为组合方案
					if(null != planRenderScene.getGroupPrimaryId() && planRenderScene.getGroupPrimaryId() != 0){
						//获取组合方案列表
						List<DesignRenderGroup> groupList = designPlanProductRenderSceneService.getDesignPlanRenderGroupList(planRenderScene.getGroupPrimaryId());
						if(null != groupList && groupList.size() > 0){
							for (DesignRenderGroup designRenderGroup : groupList) {
								//判断
								if(designRenderGroup.getId().equals(designPlanId)){
									designRenderGroup.setIsChecked("1");
									break;
								}
							}
							//赋值组合方案列表
							planRenderScene.setGroupList(groupList);
						}

					}

				} else {
					return new ResponseEnvelope<DesignPlanRenderScene>(false, "方案id=" + designPlanId + "不存在方案或被删除", msgId);
				}
			} catch (Exception e) {
				logger.error("数据异常",e);
				return new ResponseEnvelope<DesignPlanRenderScene>(false, "数据异常", msgId);
			}
			ResponseEnvelope responseEnvelope = new ResponseEnvelope<DesignPlanRenderScene>(planRenderScene, msgId, true);
			// if (Utils.enableRedisCache()) {
	...
	@ResponseBody
	public Object updateDesignPlanDetailWeb(
			String remark, 
			String plan_name, 
			String msgId, 
			Integer designPlanId,
			String designPlanType, 
			// add by huangsongbo 2018.6.14 ->start
			// 方案描述
			String planDesc
			// add by huangsongbo 2018.6.14 ->end
			) {
		...
		try {
			...
			if ("1".equals(designPlanType) && StringUtils.isNotBlank(plan_name)) {// 修改设计方案副本的名称信息
				...
				// add by huangsongbo 2018.6.14 ->start
				designPlanRenderScene.setPlanDesc(planDesc);
				// add by huangsongbo 2018.6.14 ->end
			...
			if (designPlan != null) {
				...
				// add by huangsongbo 2018.6.14 ->start
				newDesignPlan.setPlanDesc(planDesc);
				// add by huangsongbo 2018.6.14 ->end
...
//				UnityPlanProduct unityPlanProduct_p = unityPlanProduct.copy();
				...
				BeanUtils.copyProperties(unityPlanProduct, unityPlanProduct_p);
				// 产品类别信息r
				if (!StringUtils.isEmpty(productTypeValue)) {
					// SysDictionary sd =
...
//					UnityPlanProduct punityPlanProduct = munityPlanProduct.copy();
					...
					BeanUtils.copyProperties(munityPlanProduct, punityPlanProduct);
					punityPlanProduct.setIsLeaf(0);
					List<UnityPlanProduct> list = productMap.get(pproductTypeCode);
					if (list != null && list.size() > 0) {
						punityPlanProduct.setLeafNum(list.size());
	...
	public Object myDesignPlanWeb(@PathVariable String style, @ModelAttribute("designPlan") DesignPlan designPlan,
        ...
        if (platformCode == null ) {
			return new ResponseEnvelope<>(false,"获取平台编码失败！");
		}
		LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
		if (loginUser==null) {
		...
		BasePlatform basePlatform = basePlatformService.getBasePlatformByCode(platformCode);
		if (basePlatform == null ) {
			return new ResponseEnvelope<>(false,"获取平台数据失败！");
		}
		designPlan.setPlatformBussinessType(basePlatform.getPlatformBussinessType());
		designPlan.setPlatformId(basePlatform.getId());
		List<DesignPlan> list = null;
		int total = 0;
		try {
			total = designPlanService.getTotalFromMyDesignPlan(loginUser, designPlan);
			list = designPlanService.getMyDesignPlanList(loginUser, designPlan, total);
		} catch (Exception e) {
			logger.error("数据异常 exception:" + e.getMessage());
			return new ResponseEnvelope<DesignPlan>(false, "数据异常!", designPlan.getMsgId());
		}
...
//		logger.info(JSONObject.fromObject(new ResponseEnvelope<DesignPlan>(total, list, msgId)));
		return new ResponseEnvelope<DesignPlan>(total, list, msgId);
	}
	...
	public Object del(HttpServletRequest request, DesignPlan designPlan) {
		 ...
		 */
		//unionDesignPlanStoreService.deleteCityUnionPlanStoreByDesignPlanId(designPlan.getId());
		...
		try {
			...
			if (designPlanProducts != null && designPlanProducts.size() > 0) {
				...
				if (productMessage.length() > 0) {
					logger.info(productMessage.toString());
...
//				authorizationMap.get("token"), authorizationMap.get("deviceId"), authorizationMap.get("mediaType"));
	 ...
	 * @return
	 */
	@RequestMapping("/updateWaterjetInfo")
	@ResponseBody
	public Object updateWaterjetInfo(Integer designPlanId, String json, String msgId) {
		// 参数验证 ->start
		if(designPlanId == null) {
			logger.error(logPrefix + "designPlanId = null");
			return new ResponseEnvelope<>(false, "参数designPlanId不能为空", msgId);
		}
		if(StringUtils.isEmpty(json)) {
			/*logger.error(logPrefix + "StringUtils.isEmpty(json) = true");
			return new ResponseEnvelope<>(false, "参数json不能为空", msgId);*/
			json = "";
		}
		// 参数验证 ->end
		
		boolean success = designPlanService.updateWaterjetInfo(designPlanId, json);
		if(!success) {
			return new ResponseEnvelope<>(false, "更新水刀配置失败", msgId);
		}
