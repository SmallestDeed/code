...
import com.nork.common.pano.ShearRenderPicJob;
...
import com.nork.design.service.ReleaseDesignPlanService;
...
public class WebDesignPlanRecommendedControllerV2 {
	...
	private static String debugModel = Utils.getValueByFileKey(AppProperties.APP, "app.server.debug.model", "false");
	...
	@Autowired
	ReleaseDesignPlanService releaseDesignPlanService;
	...
	public Object planRecommendedList(HttpServletRequest request,Integer start, Integer limit, HttpServletResponse response){
		...
		if( baseCompany != null && baseCompany.getBusinessType() != null){
			model.setBusinessType(baseCompany.getBusinessType());
			if(baseCompany.getBusinessType().intValue() == 2)
	...
	public ResponseEnvelope<DesignPlanRecommendedResult> planRecommendedListForFranchiser(
		...
		if(loginUser.getBusinessAdministrationId() != null) {
			 ...
			 * 经销商对应企业方案需根据经销商公司关联品牌过滤
			 */
			List<Integer> barndIdList = new ArrayList<Integer>();//公司关联品牌集合
			if(loginUser.getUserType() != null && UserTypeConstant.USER_TYPE_DEALERS == loginUser.getUserType()) {
			    BaseBrand baseBrand = new BaseBrand();
			    baseBrand = baseBrandService.findBrandIdByUserIdAndUserType(loginUser);
			    if(baseBrand != null && !Utils.isEmpty(baseBrand.getBrandStr())) {
			        String idStr = baseBrand.getBrandStr();
			        barndIdList = Utils.getIntegerListFromStringList(idStr);
			        barndIdList.add(-1);
			    }
			}
			dto.setBrandIdList(barndIdList);
		}
	...
	@ResponseBody
	public ResponseEnvelope<DesignPlanRecommendedResult> planRelease2(HttpServletRequest request, HttpServletResponse response) {

		//参数获取
		String msgId = request.getParameter("msgId");
		String brandIds = request.getParameter("brandIds");					/* 品牌ids */
		String thumbId = request.getParameter("thumbId");						/* 副本缩略图id */
		String isRelease = request.getParameter("isRelease");					/* 发布状态 */
		String planNumber = request.getParameter("planNumber");				/* 方案编号 */
		String applySpaceAreas = request.getParameter("applySpaceAreas");		/* 适用面积 */
		String isExamine = request.getParameter("isExamine");					/* 方案是否审核 */
		LoginUser loginUser = null;
		if(StringUtils.equals("true", debugModel))
		...
		//参数校验
		if(StringUtils.isBlank(thumbId))
			return new ResponseEnvelope <>(false, "副本缩略图id不能为空", msgId);
		if(loginUser == null || loginUser.getId() == null)
			return new ResponseEnvelope <>(false, "登录超时,请重新登录", msgId);
		if(StringUtils.isBlank(isRelease))
			return new ResponseEnvelope <>(false, "发布状态不能为空", msgId);
		if("0".equals(isRelease))
			return new ResponseEnvelope <>(false, "发布状态参数有误", msgId);
		if(StringUtils.isBlank(isExamine))
			return new ResponseEnvelope <>(false, "方案是否审核不能为空", msgId);

		//校验效果图方案是否存在
		DesignPlanRenderScene designPlanRenderScene = designPlanRenderSceneService.getDesignPlanSceneByThumbId(Long.parseLong(thumbId));
		if(null == designPlanRenderScene)
			return new ResponseEnvelope <>(false, "效果图方案不存在", msgId);

		//判断：非组合、并且状态为测试、或者审核 校验适用面积
		if(null == designPlanRenderScene.getGroupPrimaryId() || designPlanRenderScene.getGroupPrimaryId() == 0){
			if(isRelease.equals(RecommendedDecorateState.IS_RELEASEING) || isRelease.equals(RecommendedDecorateState.WAITING_CHECK_RELEASE))
				if(StringUtils.isBlank(applySpaceAreas))
					return new ResponseEnvelope <>(false, "适用面积不能为空", msgId);
		}

		String hasPermissionButton = Utils.getValue("app.hasPermissionButton", "true");
		if (StringUtils.equals(hasPermissionButton, "true")) {
			boolean hasPermission = checkPermissionOfReleasePlan(designPlanRenderScene.getRenderType().toString(), loginUser);
			if (!hasPermission) {
				return new ResponseEnvelope<>(false, "no permission", msgId);
			}
		}

		ReleaseDesignPlanModel model = new ReleaseDesignPlanModel();
		List<Integer> recommendedTypeArray = new ArrayList<>();
		recommendedTypeArray.add(designPlanRenderScene.getRenderType());
		model.setStyleId(designPlanRenderScene.getDesignStyleId()+"");
		model.setIsRelease(isRelease);
		model.setThumbId(thumbId);
		model.setBrandIds(brandIds);
		model.setPlanNumber(planNumber);
		model.setDesignPlanRenderScene(designPlanRenderScene);
		model.setApplySpaceAreas(applySpaceAreas);
		model.setIsCheck(isExamine);
		model.setRecommendedTypeArray(recommendedTypeArray);
		return releaseDesignPlanService.releaseDesignPlan(model, loginUser, msgId);
	}

	private List <Integer> getRecommendTypes(String recommendedTypeStrParam) {
		String[] typeArr = recommendedTypeStrParam.split(",");
		List <Integer> recommendedTypes = null;
		if (typeArr != null && typeArr.length > 0) {
			recommendedTypes = new ArrayList <>();
			if (typeArr[0] != null) {
				recommendedTypes.add(Integer.valueOf(typeArr[0]));
			}
			if (typeArr.length > 1 && typeArr[1] != null) {
				recommendedTypes.add(Integer.valueOf(typeArr[1]));
			}
		}
		return recommendedTypes;
	}

	private Boolean checkPermissionOfReleasePlan(String recommendedTypeStrParam,LoginUser loginUser) {
		Boolean result = true;
		if(StringUtils.isEmpty(recommendedTypeStrParam)) {
			return false;
		}
		...
		if(typeArr != null && typeArr.length > 0) {
			if(typeArr[0] != null){
				result = checkPermission(roles, Integer.valueOf(typeArr[0]));
			}
			if(typeArr.length > 1 && typeArr[1] != null){
				result = checkPermission(roles, Integer.valueOf(typeArr[1]));
			}
		}
		return result;
	}

	private Boolean checkPermission(Set<String> roles, Integer recommendType) {
		if(DesignPlanRecommendedConstant.RECOMMENDEDTYPE_NOOPSYCHE.intValue() == recommendType.intValue()) {
		...
		if(DesignPlanRecommendedConstant.RECOMMENDEDTYPE_COMMON.intValue() == recommendType.intValue()) {
	...
	public  ResponseEnvelope<DesignPlanRecommended>  submitCheck(HttpServletRequest request, HttpServletResponse response){
		...
		//非组合、验证适用面积
		DesignPlanRecommended designPlanRecommended = designPlanRecommendedServiceV2.get(Integer.parseInt(planRecommendedId));
		if(null == designPlanRecommended.getGroupPrimaryId() || designPlanRecommended.getGroupPrimaryId() == 0){
		 ...
		 LoginUser loginUser = SystemCommonUtil.getLoginUserFromSession(request);
		 return designPlanRecommendedServiceV2.submitCheck(msgId, loginUser, planRecommendedId, spaceAreas, brandIds, styleId/*, recommendedType*/);
	}
	...
	public ResponseEnvelope<DesignPlanRecommendedResult> planFormalIsReleaseCheck (HttpServletRequest request){
		...
		List <Integer> recommendedTypes = getRecommendTypes(recommendedType);
		DesignPlanRenderScene designPlanRenderScene = designPlanRenderSceneService.getDesignPlanSceneByThumbId(Long.parseLong(thumbId));
		if(designPlanRenderScene == null){
			return new ResponseEnvelope<DesignPlanRecommendedResult>(false,"方案副本不存在,或者已经被删除",msgId);
		}
		DesignPlanRecommendedCheck check = releaseDesignPlanService.validateCommonInfo(designPlanRenderScene,recommendedTypes);
		if(check.isState()){
			return new ResponseEnvelope<DesignPlanRecommendedResult>(true, check, msgId);//应app 要求这里改成true
		}else{
			return new ResponseEnvelope<DesignPlanRecommendedResult>(false, check.getErrMsg(), msgId);//应app 要求这里改成true
		}
	}
	...
	public ResponseEnvelope<FranchiserPlanListVO> franchiserPlanList(
		...
		if( sysUser.getBusinessAdministrationId() != null ){
			...
			if(UserTypeConstant.USER_TYPE_DEALERS == loginUser.getUserType()) {
			    BaseBrand baseBrand = new BaseBrand();
			    baseBrand = baseBrandService.findBrandIdByUserIdAndUserType(loginUser);
			    if(baseBrand != null && !Utils.isEmpty(baseBrand.getBrandStr())) {
			        brandIdList = Utils.getIntegerListFromStringList(baseBrand.getBrandStr());
			        brandIdList.add(-1);//也显示无品牌的方案
			    }
			}
			dto.setBrandIdList(brandIdList);
		}
	 ...
	 * @return
	 */
	@RequestMapping("/repairV720HistoryData")
	@ResponseBody
	public ResponseEnvelope repairRecommendedPlanV720HistoryData(String recommendedPlanId) {

		return resRenderPicService.repairRecommendedPlanV720HistoryData(recommendedPlanId);
	}
	 ...
	 * @return
	 */
	@RequestMapping("/shearV720HistoryData")
	@ResponseBody
	public ResponseEnvelope shearV720HistoryData(String planIds, int start, int limit) {
		//实例化查询条件对象
		...
		query.setLimit(limit == 0 ? ResRenderPicQuery.QUERY_RESRENDERPIC_MAX_LIMIT : limit);

		List<ResRenderPicPo> list = null;
		try {
			list = resRenderPicService.findHistoryData(query);
		...
		int shearPicJob = 6;
		//分配任务数据
		int shearJobCount = list.size()/shearPicJob;
		// 余数大于0 线程数 + 1
		if(list.size() % shearPicJob > 0) {
			shearPicJob = shearPicJob + 1;
		}
		// 线程任务列表计算
		List<List<ResRenderPicPo>> shearPicList = SystemCommonUtil.getListByLimit(list, shearJobCount, shearPicJob);
		// 取得线程池实列
		ThreadPool threadPool = threadPoolManager.getThreadPool();
		for (int i = 0; i < shearPicJob; i ++) {
			List<ResRenderPicPo> shearPicJobList = new ArrayList<>();
			if (i < shearPicList.size()) {
				shearPicJobList = shearPicList.get(i);
			}
			//线程名称
			String jobName = "shearJob" + i;
			ShearRenderPicJob shearRenderPicJob = new ShearRenderPicJob(resRenderPicService, shearPicJobList, jobName);
			threadPool.submit(shearRenderPicJob);
		}

		return new ResponseEnvelope(true, "OK！");
	}

	@RequestMapping("/shearTest")
	@ResponseBody
	public Object shearTest(){
//		System.load("/usr/local/share/OpenCV/java/libopencv_java300.so");
		logger.error("==================================================="+System.getProperty("java.library.path"));
		String sourcePath = "/data001/resource/test/img720_18-07-10_04-57-55.jpg";
		String targetPath = "/data001/resource/test/img720_18-07-10_04-57-55/";
		OpenCVUtil.doShear(sourcePath, targetPath);
		return new ResponseEnvelope(true, "asd");
	}
}