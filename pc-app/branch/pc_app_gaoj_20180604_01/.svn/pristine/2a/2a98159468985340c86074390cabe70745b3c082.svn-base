...
import com.nork.common.util.comparator.DateUtil;
...
//        authorizedConfig.setCompanyType(sysDictionary.getValue());
    ...
    @Override
    public JSONObject checkAuthorizedValidateTime(LoginUser user) throws GeneratePanoramaException {
        logger.debug("Process in AuthorizedConfigServiceImpl.checkAuthorizedValidateTime method parameter user : {}"
                + user.toString());
        Date nowTime = DateUtil.getNow();
        JSONObject responseJson = new JSONObject();
        responseJson.put("flag", false);

        if(null == user){
            throw new GeneratePanoramaException("用户未登录");
        }
        // 封装需要通知的序列号对象数据
        JSONArray needNotify = getJsonArray(user.getId(), nowTime);

        if(!needNotify.isEmpty()){
            responseJson.put("flag", true);
        }
        responseJson.put("data", needNotify);
        return responseJson;
    }

    private JSONArray getJsonArray(Integer userId, Date nowTime) throws GeneratePanoramaException {
        // 封装所有有效的序列号的对象 时间为key
        Map<String, JSONArray> map = getEveryTimeMapData(nowTime, userId);

        // 需要通知的记录为空，不需要弹窗
        if(map.isEmpty()){
            throw new GeneratePanoramaException("不需要通知");
        }
        // 根据key进行升序排序，最小的优先处理
        String[] keys = sort1(map.keySet(), true);
        JSONArray arr = map.get(keys[0]);
        JSONObject obj = (JSONObject) arr.get(0);
        long diffNum = obj.getInt("day");

        // 缓存工具类
        Cacher cache =  CacheManager.getInstance().getCacher();
        if(null == cache){
            throw new GeneratePanoramaException("缓存连接异常");
        }
        // 缓存key前缀
        final String cacheKey = "notify_";
        JSONArray needNotify = new JSONArray();
        // 小于等于3天，不需缓存每次请求都需要通知； 大于3天小于等于30天需要只需要通知一次
        if(diffNum >= 0 && diffNum <= 3){
            needNotify = arr;
        }else if(diffNum > 3 && diffNum <= 30){
            for(int i=0; i<arr.size(); i++){
                JSONObject object = (JSONObject)arr.get(i);
                String code = object.getString("code");
                String isTrue =  cache.get(cacheKey + code);
                // 缓存不存在则表明第一次通知
                if(StringUtil.isEmptyOrNull(isTrue)){
                    cache.set(cacheKey + code, "false", 30 * 24 * 60 * 60);
                    needNotify.add(object);
                }
            }
        }
        return needNotify;
    ...
    private Map<String, JSONArray> getEveryTimeMapData(Date nowTime, Integer userId) {
	    final String FORMAT_STR = "yyyy年MM月dd日 HH:mm";
        List<AuthorizedConfig> list = authorizedConfigMapper.getListByUserId(userId);
        Map<String, JSONArray> map = new HashMap<String, JSONArray>();
        for(AuthorizedConfig auth : list){
            // 序列号过期时间
            Date validTime = auth.getValidTime();
            if(null == validTime){
                continue;
            }

            long diffNum = 0;
            if(validTime.after(nowTime)){
                diffNum = DateUtil.diffDays(validTime, nowTime);
                if(null != map.get(DateUtil.format(validTime,""))){
                    JSONObject json = new JSONObject();
                    json.put("code", auth.getAuthorizedCode());
                    json.put("day", diffNum);
                    json.put("time", DateUtil.format(validTime, FORMAT_STR));
                    map.get(DateUtil.format(validTime,"")).add(json);
                }else{
                    ...
                    json.put("time", DateUtil.format(validTime, FORMAT_STR));
                    jsonArray.add(json);
                    map.put(DateUtil.format(validTime,""), jsonArray);
                }
            }
        }
    ...
    public static final String[] sort1(Collection<String> s, boolean isAscending) {
    ...
    public static void main(String[] args) throws ParseException {
       ...
       map.put("2017-11-20 11:11:11","1");
       map.put("2017-11-20 11:11:44", "3");
       map.put("2017-11-20 11:11:12", "2");
