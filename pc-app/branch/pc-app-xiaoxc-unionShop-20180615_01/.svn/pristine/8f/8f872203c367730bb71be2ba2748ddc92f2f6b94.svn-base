...
public class GroupProductServiceV2Impl implements GroupProductServiceV2 {
	...
	public List<SearchProductGroupResult> getGroupListByProduct(GroupProductSearch search, Integer userID, Integer structureId,Integer usertype,String versionType) {
		logger.error("search getCompositeType== " + search.getCompositeType());
...
//		this.productGroupDao.getProductInfoInTheGroupByGroupIDAndModelIDType(map)
		...
		for (ProductInfoInGroupModel productInfoInGroupModel : allProductInfoList) {
			...
			// 组合明细中记录的已选多材质信息
			detail.setSplitTexturesChooseInfo(productInfoInGroupModel.getSplitTexturesChooseInfo());
			...
			if ( list != null && !list.isEmpty() ) {
				...
				for (SearchProductGroupDetail searchProductGroupDetail : list) {
					...
					if ( StringUtils.isNotBlank(splitTexturesInfo)) {
						...
						if(StringUtils.isNotEmpty(searchProductGroupDetail.getSplitTexturesChooseInfo())) {
							splitTexturesInfo = designPlanProductService.matchSplitTexturesInfo(splitTexturesInfo, productID, searchProductGroupDetail.getSplitTexturesChooseInfo());
						}
						// 根据组合明细表中的splitTexturesInfo和产品表中的splitTexturesInfo处理多材质信息 ->start
					...
					searchProductGroupDetail.setSplitTexturesChooseInfo(splitTexturesInfo);
					/*处理拆分材质产品返回默认材质->end*/
				}
