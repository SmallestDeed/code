...
public class BaseBrandCacher {
	 ...
	 * @return
	 */
	public static List<BaseBrand> getList(Integer id,Integer type){
		List<BaseBrand> lstConfig=Lists.newArrayList();
		...
		String key = KeyGenerator.getAllListKeyWithMap(ModuleType.BaseBrand, map);
		lstConfig = CacheManager.getInstance().getCacher().getList(BaseBrand.class, key);
		if(CustomerListUtils.isEmpty(lstConfig)){
			// lstConfig = baseBrandService.findBrandIdByUserIdList(id);
			 if(2==type){//企业用户
				 	lstConfig = baseBrandService.findBrandIdByUserIdListBase(id);
				}else if(3==type){//经销商
					lstConfig = baseBrandService.findBrandIdByUserIdListB(id);
				}
