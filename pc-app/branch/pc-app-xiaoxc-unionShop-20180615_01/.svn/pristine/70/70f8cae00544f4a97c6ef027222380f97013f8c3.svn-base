		...
		if(StringUtils.isNotBlank(authorization) && currentURL.lastIndexOf(".htm") != -1) {
			...
			try {
				 ...
				 if (Utils.enableRedisCache() && Utils.checkOneLoginEnable()) {
					  String userKey =  String.valueOf(dataObj.get("ukey")); //当前请求用户userKey
					 logger.debug("----------pc端过滤器---从请求token解析的userKey："+userKey+"-------------" );
					  
//					  String loginUserId = String.valueOf(dataObj.get("uid"));
//					  String loginKey = SystemCommonConstant.LOGIN_FROM_PC + userKey ; //当前登录用户缓存key
//					  LoginUser user = new LoginUser();
//					  user = (LoginUser) CacheManager.getInstance().getUserCacher().getObject(loginKey);//当前登录用户
					 LoginUser user = LoginContext.getLoginUser(LoginUser.class);
					 logger.debug("----------pc端过滤器----从缓存中获取的user："+user.toString()+"-------userKey为"+user.getUserKey()+"-------------" );
					  ...
					  if(!user.getUserKey().equals(userKey)) {
						logger.error("appkey不一致，强制退出====>loginUserAppkey:"+userKey+",onlineUserAppkey:"+user.getUserKey());
