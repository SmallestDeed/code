...
public class BaseCompanyServiceImpl implements BaseCompanyService {
	...
	Logger logger = Logger.getLogger(BaseCompanyServiceImpl.class);
	
	private String logPrefixClass = "function:BaseCompanyServiceImpl.";
	...
	@Override
	public Map<String, List<Integer>> getCategoryListV2(Integer companyId) {
			...
			if(baseCompany.getIndustry() != null && StringUtils.isNotBlank(baseCompany.getSmallType())){
				...
				for(Integer smallType : smallTypeList){
					...
					if(StringUtils.isNotBlank(sysDictionary.getAtt1())){
						...
						Integer smallTypeValue = null;
						List<SysDictionary> sysDictionarieList = sysDictionaryService.findAllByValueKeyList(smallTypeValuekeyList);
						if(Lists.isNotEmpty(sysDictionarieList)) {
							for (SysDictionary sysDictionarySmallType : sysDictionarieList) {
								if(sysDictionarySmallType != null){
									bigTypeValuekey = sysDictionarySmallType.getType();
									smallTypeValue = sysDictionarySmallType.getValue();
									if(returnMap.containsKey(bigTypeValuekey)) {
										List<Integer> smallTypeValueList = returnMap.get(bigTypeValuekey);
										if(smallTypeValueList == null) {
											smallTypeValueList = new ArrayList<Integer>();
										...
										List<Integer> smallTypeValueList = new ArrayList<Integer>();
										smallTypeValueList.add(smallTypeValue);
										returnMap.put(bigTypeValuekey, smallTypeValueList);
									}
								}
							}
						}
	...
	@Override
	public List<Integer> getCompanyIdListDifferentIndustry(Integer companyId) {
		String logPrefixFunction = logPrefixClass + "getCompanyIdListDifferentIndustry -> ";
		...
		BaseCompany baseCompany = this.get(companyId);
		if(baseCompany == null) {
			logger.error(logPrefixFunction + "baseCompany = null;companyId = " + companyId);
			return null;
		}
		String categoryIds = baseCompany.getCategoryIds();
		if(StringUtils.isEmpty(categoryIds)) {
			return null;
		}
		List<String> categoryIdList = Utils.getListFromStr(categoryIds);
		if(Lists.isEmpty(categoryIdList)) {
			return null;
		}
		
		return baseCompanyMapper.getCompanyIdListDifferentIndustry(categoryIdList);
	}
