...
public class ResModelServiceImpl implements ResModelService {
	...
	public int add(ResModel resModel) {
		/**
		 * 新增前先检查是否存在file_key和model_path相同的数据
 		 */
		if( resModel == null ){
			return 0;
		}
		ResModelSearch resModelSearch = new ResModelSearch();
		resModelSearch.setIsDeleted(0);
		resModelSearch.setFileKey(resModel.getFileKey());
		resModelSearch.setModelPath(resModel.getModelPath());
		int count = resModelMapper.selectCount(resModelSearch);
		if( count > 0 ){
			return 0;
		}
