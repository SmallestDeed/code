...
import com.nork.system.sms.httpclient.HttpClientUtil;
...
public class WebAuthorizedConfigController {
	...
	public Object updateAuthorizedConfigStatus(String msgId, String terminalImei, String authorizedCode,
		...
		if (loginUser != null) {
			...
			JedisUserUtils.del(UserConstant.RBAC_USER_ROLE_PREFIX+userId);
		}
		/* 得到userIdEnd */
		...
		if (status.equals(new Integer(0))) {
			...
			//http方式调用给绑定序列号的用户赠送包年包月套餐===start
			String authorization =  request.getHeader("Authorization") != null ? (String) request.getHeader("Authorization") : "";
			Map<String, Object> headMap = new HashMap<String, Object>();
			headMap.put("Authorization", authorization);
			headMap.put(PlatformConstants.PLATFORM_CODE_KEY,PlatformConstants.PC_2B);
			String url = app.getString("pay.server.url");
			logger.info("包年包月赠送接口域名是：" + url + ",Authorization是" + authorization);
			if (!com.mockrunner.util.common.StringUtil.isEmptyOrNull(url) && !com.mockrunner.util.common.StringUtil.isEmptyOrNull(authorization)) {
			    url = url + "/web/pay/payModelConfig/addGiveGroopRef";
                String result = HttpClientUtil.doPost(url,null,headMap);
                logger.info("http调用赠送包年包月的接口返回结果是：" + result);
            }
			//http方式调用给绑定序列号的用户赠送包年包月套餐===end
		}
		...
		try {
			...
			if (authorizedConfig.getState() != null && authorizedConfig.getState().intValue() == 0) {
				...
				if (priceStrategyStr != null && !"".equals(priceStrategyStr)) {
					...
					if (sysUser != null) {
						Double priceStrategy = (Double.parseDouble(priceStrategyStr) * 100);
                        logger.info("用户id：userId{}" + sysUser.getId() + ",绑定序列号时候赠送的积分是：priceStrategy:{}" + priceStrategy);
                        payAccountService.updatePayAccount(sysUser,PlatformConstants.PC_2B,priceStrategy);
					} else {
    ...
    public Object checkUserAccountValidateTime(HttpServletRequest request){
        logger.debug("Process in WebAuthorizedConfigController.checkUserAccountValidateTime method");

        LoginUser user = SystemCommonUtil.getCurrentLoginUserInfo(request);
        ResponseEnvelope response = null;
        try {
            JSONObject msg = authorizedConfigService.checkAuthorizedValidateTime(user);
            response = new ResponseEnvelope(true, msg);
        } catch (GeneratePanoramaException e) {
            logger.error("Process in WebAuthorizedConfigController.checkUserAccountValidateTime method hannped " +
                    "GeneratePanoramaException:", e);
            response = new ResponseEnvelope(false, "not need notify");
        }catch (Exception e){
            logger.error("Process in WebAuthorizedConfigController.checkUserAccountValidateTime method hannped " +
                    "GeneratePanoramaException:", e);
            response = new ResponseEnvelope(false, "not need notify");
