 ...
 */
    ...
    public Object mediationAuthorizeInfo(@RequestParam(value = "id", required = true) Integer id, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //查询用户所属企业
            String companyName = mediationAuthorizeService.getUserSourceCompany(loginUser.getId());
            mediationAuthorizeVo.setSourceCompany(companyName);
            log.info("查询用户所属企业名称 =>{}"+companyName);
            //处理照片地址
            String cardPicPath = "";
            try{
