...
public class UserInviteController {
    ...
    LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
       if (loginUser == null) {
        ...
        if( null != shareType && 0 != shareType && Pattern.compile(SHARE_TYPE_REGEX).matcher(shareType+"").find()){
             sceneStr = loginUser.getId() + "," + shareType+","+System.currentTimeMillis()+","+"union";
        }else {
             sceneStr = loginUser.getId()+","+DEFAULT_DESHARE_TYPE+","+System.currentTimeMillis()+","+"union";
        }
        log.info("生成微信分享参数:"+sceneStr.length());
    ...
    public ResponseEnvelope inviteCallBack(
        ...
        int keyId;
        try {
            keyId = userInviteService.saveUserInviteInfo(userInviteAdd);
            if ( keyId == 0) {
                log.warn("邀请失败:"+gson.toJson(userInviteAdd));
                return new ResponseEnvelope(false, "邀请失败");
            }
        }catch (Exception e){
            log.error("邀请数据异常"+e);
            return new ResponseEnvelope(false,"邀请数据异常");
        }

        //插入用户邀请信息跟进表数据
        int userInviteTrackId;
        try {
            userInviteTrackId = userInviteService.saveUserInviteTrackInfo(userInviteAdd,loginUser.getName());
            if(userInviteTrackId == 0 ){
                log.warn("新增用户邀请信息跟进表数据失败:"+gson.toJson(userInviteAdd));
                return new ResponseEnvelope(false, "新增用户邀请信息跟进表数据失败");
            }

        }catch (Exception e){
            log.error("新增用户邀请信息跟进表数据"+e);
            return new ResponseEnvelope(false,"新增用户邀请信息跟进表数据");

        }

/*
        //插入中介佣金表数据
        int intermediaryCommisionId;
        try {
            intermediaryCommisionId = userInviteService.saveIntermediaryCommisionInfo(userInviteAdd,loginUser.getName(),userInviteTrackId);
            if(intermediaryCommisionId == 0 ){
                log.warn("新增中介佣金表数据失败:"+gson.toJson(userInviteAdd));
                return new ResponseEnvelope(false, "新增中介佣金表数据败");
            }

        }catch (Exception e){
            log.warn("新增中介佣金表数据"+e);
            return new ResponseEnvelope(false,"新增中介佣金表数据");

        }*/

        return new ResponseEnvelope(true, "邀请成功", keyId+"----"+userInviteTrackId);
    ...
    @ApiOperation(value = "邀请人注册后回调", response = ResponseEnvelope.class)
    @ApiImplicitParams({
            @ApiImplicitParam(name = "shareType", value = "分享类型:'1:微信朋友圈,2:微信好友,3:QQ好友,4:QQ空间,5:微博,6:贴吧,7:豆瓣,8:知乎,9:微头条,10:简书'", required = false, paramType = "query", dataType = "int"),
            @ApiImplicitParam(name = "shareSign", value = "分享标识'", required = true, paramType = "query", dataType = "String"),
            @ApiImplicitParam(name = "userId", value = "分享者ID'", required = true, paramType = "query", dataType = "int"),
            @ApiImplicitParam(name = "shareTime", value = "分享时间'", required = true, paramType = "query", dataType = "Date"),
    })
    public ResponseEnvelope shareCallBack(
            @RequestBody MiniProgramShareRecordAdd miniProgramShareRecordAdd,
            HttpServletRequest request) {
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        if (loginUser == null) {
            return new ResponseEnvelope(false, "请先登录");
        ...
        //校验传入信息
        ValidateResult result = AnnotationValidator.validate(miniProgramShareRecordAdd);
        if (!result.isValid()) {
            log.warn("参数校验未通过:"+gson.toJson(result));
            return new ResponseEnvelope(false, result.getFieldName() + "------------" + result.getMessage());
        }
        log.info("传参信息校验通过:"+gson.toJson(miniProgramShareRecordAdd));

        //判断该用户是否被分享注册过
        MiniProgramShareRecord miniProgramShareRecord = userInviteService.getMiniProgramShareRecordByFid(loginUser.getId());
        if (miniProgramShareRecord != null) {
            return new ResponseEnvelope(false, "该用户已经注册过");
        }
        ...
        int shareId;
        try {
            shareId = userInviteService.saveMiniProgramShareRecord(miniProgramShareRecordAdd,sysUser);
            if ( shareId == 0) {
                log.warn("分享失败:"+gson.toJson(miniProgramShareRecordAdd));
                return new ResponseEnvelope(false, "分享失败");
            }
        }catch (Exception e){
            log.error("分享数据异常"+e);
            return new ResponseEnvelope(false,"分享数据异常");
        }

        return new ResponseEnvelope(true, "分享成功", "shareId:"+shareId);
     ...
     * @date 2018/4/27 0027PM 6:02
     */
    @RequestMapping(value = "/myinvitelist", method = RequestMethod.GET)
    @ResponseBody
    public ResponseEnvelope getMyInviteList(UserInviteSearch userInviteSearch,HttpServletRequest request) {
        //获取当前登录用户
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        if (loginUser == null) {
        ...
        int count;
        try {
            count = userInviteService.getMyUserInviteCount(loginUser.getId(),userInviteSearch);
            if(count==0){
                log.warn("未获取到邀请列表记录数信息"+loginUser.getId());
                return new ResponseEnvelope(true,"未获取到邀请列表记录数信息");
            }

        }catch (Exception e){
            log.error("获取到邀请列表记录数据异常"+e);
            return new ResponseEnvelope(true,"未获取到邀请列表记录数据异常");
        }


        //查询我的邀请列表
        List<UserInviteListVo> userInviteListVoList;
        try {
            userInviteListVoList =  userInviteService.getMyUserInviteList(loginUser.getId(),userInviteSearch);
            if(null == userInviteListVoList || userInviteListVoList.size() == 0){
                log.warn("未获取到邀请列表信息"+loginUser.getId());
                return new ResponseEnvelope(true,"未获取到邀请列表信息");
            }
        }catch (Exception e){
            log.error("查询我的邀请列表数据异常:"+e);
            return new ResponseEnvelope(false,"查询我的邀请列表数据异常");
        }


        //处理用户头像(如果用户没有上传头像,根据性别用默认头像显示)
        String boyDefaultPic = sysUserService.getUserDefaultPic(UserDefaultPicContants.BOY_PIC_PATH);
        String girlDefaultPic = sysUserService.getUserDefaultPic(UserDefaultPicContants.GIRL_PIC_PATH);
        if(StringUtils.isNotBlank(boyDefaultPic) && StringUtils.isNotBlank(girlDefaultPic)){
            userInviteListVoList =  userInviteListVoList.stream().filter(userInviteListVo -> {
                if(StringUtils.isBlank(userInviteListVo.getUserPicPath())){
                    if(userInviteListVo.getSex() == null || userInviteListVo.getSex().intValue() == UserDefaultPicContants.BOY_PIC_PATH
                            || userInviteListVo.getSex().intValue() == UserDefaultPicContants.DEFAULT_PIC_PATH){
                        userInviteListVo.setUserPicPath(boyDefaultPic);

                    }else if(userInviteListVo.getSex() != null && userInviteListVo.getSex().intValue() == UserDefaultPicContants.GIRL_PIC_PATH){
                        userInviteListVo.setUserPicPath(girlDefaultPic);

                    }
                }
                if(userInviteListVo.getStatus().intValue()==UserInviteStatusNameContants.IS_SIGN){
                    userInviteListVo.setStatusName(UserInviteStatusNameContants.IS_SIGN_NAME);
                }else if(userInviteListVo.getStatus().intValue()==UserInviteStatusNameContants.NO_SIGN){
                    userInviteListVo.setStatusName(UserInviteStatusNameContants.NO_SIGN_NAME);
                }else {
                    userInviteListVo.setStatusName(UserInviteStatusNameContants.SIGNING_NAME);
                }
        ...
        return new ResponseEnvelope(true,"success",userInviteListVoList,count);
    }
}
