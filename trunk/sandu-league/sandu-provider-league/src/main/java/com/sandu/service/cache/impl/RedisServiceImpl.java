package com.sandu.service.cache.impl;


import com.google.gson.Gson;
import com.sandu.api.cache.service.RedisService;
import com.sandu.api.util.cache.SerializeUtil;
...
public class RedisServiceImpl implements RedisService {
    ...
    public Long getSizeOfList(String key) {
        ...
        try {
            ...
            log.error("getSizeOfList {} = {}", key, size);
        } finally {
    ...
    private synchronized ShardedJedisPool initPool() {
        ...
        if (shardedJedisPool == null) {
            shardedJedisPool = context.getBean(ShardedJedisPool.class);
            if (shardedJedisPool == null) {
            } else {
            }
        } else {
...
//				 value = jedis.brpop(2,key);
                ...
                log.error("getBrpopList size= " + size);
                log.info("getObjectList {} = {}", key, value);
            }
        } catch (Exception e) {
    ...
    public long mapIncrby(String mapName, String key, Long incrbyNumber) {
        ...
        try {
            ...
            log.debug("mapIncrby-----", mapName, key, incrbyNumber);
        } catch (Exception e) {
            log.error("mapIncrby----", e, mapName, key, incrbyNumber);
        } finally {
            returnResource(jedis);
        }
        return result;
