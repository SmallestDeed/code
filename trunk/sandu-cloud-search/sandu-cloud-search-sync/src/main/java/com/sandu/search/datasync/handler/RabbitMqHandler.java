    ...
    public void consumerMessage(String message) {
        ...
        switch (moduleType) {
            ...
            case QueueMessage.RECOMMENDATION_MODULE :
                //数据兼容处理(对象中包含对象，需要重解析)
                String jsonStr = null;
                List objectList = (List) queueMessage.getObject();
                if (null != objectList && 0 < objectList.size()) {
                    if (!(objectList instanceof RecommendationPlanIndexMappingData)) {
                        jsonStr = JsonUtil.toJson(queueMessage.getObject());
                    }
                }
                ...
                if (QueueMessage.ADD_ACTION == actionType || QueueMessage.UPDATE_ACTION == actionType) {
                    //新增|更新
                    messageConsumerStatus = recommendationMessageHandler.addAndUpdate(JsonUtil.fromJson(null == jsonStr ? queueMessage.getObject().toString() : jsonStr, new TypeToken<List<RecommendationPlanIndexMappingData>>() {
                    }.getType()));
                }
                break;
            default:
