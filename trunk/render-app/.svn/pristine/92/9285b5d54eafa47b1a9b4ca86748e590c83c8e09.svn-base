...
public class DesignPlanStoreReleaseController {
    ...
    public Object get(@ModelAttribute SceneDataSearch search,HttpServletRequest request){
        ...
        if(Objects.equals(SceneDataSearch.PlatformType.MINIPROGRAM,platformType)){
        ...
        }else if(Objects.equals(SceneDataSearch.PlatformType.PC2B,platformType) || Utils.isEmpty(platformType)){
        ...
        try {
            ...
            switch (search.getSceneType()) {
                ...
                case SceneTypeConstant.STORE_RELEASE_SCENE:
                    ...
                    break;
                case SceneTypeConstant.PC_COVER_SINGLE_SCENE:
                    //PC端推荐方案封面单场景查看
                    roamSceneDataVo = panoramaService.getBrowseVr720SenceData(search);
                    res.setObj(roamSceneDataVo);
                    break;
                default:
            ...
            //填充自定义分享文案
            if (roamSceneDataVo != null ) {
                roamSceneDataVo.setDesc(!Utils.isBlank(desc)? desc : roamSceneDataVo.getDesc());
                roamSceneDataVo.setShareTitle(!Utils.isBlank(title)? title : "3D全景");
            }
            if (storeSingleDataVo != null) {
                storeSingleDataVo.setDesc(!Utils.isBlank(desc)? desc : storeSingleDataVo.getDesc());
                storeSingleDataVo.setShareTitle(!Utils.isBlank(title)? title : "3D全景");
            }
            if (designPlanStoreReleaseVo != null) {
                designPlanStoreReleaseVo.setDesc(desc);
                designPlanStoreReleaseVo.setDesc(!Utils.isBlank(desc)? desc : designPlanStoreReleaseVo.getDesc());
                designPlanStoreReleaseVo.setShareTitle(!Utils.isBlank(title)? title : "3D全景");
            }
        }catch(BizException e1){
            logger.error("get请求出现异常情况:",e1);
            res.setSuccess(false);
