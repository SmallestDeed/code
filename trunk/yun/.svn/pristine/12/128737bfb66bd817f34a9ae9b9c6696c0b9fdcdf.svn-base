package com.sandu.pay.alipay.service.impl;
...
import com.sandu.pay.alipay.model.TradeStatus;
import com.sandu.pay.alipay.model.builder.AlipayTradeCancelRequestBuilder;
...
import com.sandu.pay.alipay.util.Utils;
...
abstract class AbsAlipayTradeService extends AbsAlipayService implements AlipayTradeService {
    ...
    public AlipayF2FPrecreateResult tradePrecreate(AlipayTradePrecreateRequestBuilder builder) {
        ...
        request.putOtherTextParam("app_auth_token", builder.getAppAuthToken());
        request.setBizContent(builder.toJsonString());
        //////System.out.println("auth_token:"+request.getApiMethodName());
        //////System.out.println("json:"+builder.toJsonString());
        log.info("trade.precreate bizContent:" + request.getBizContent());

		AlipayTradePrecreateResponse response = null;
		// 需要验签的方式
		// response = (AlipayTradePrecreateResponse) getResponse(client,request);

		AlipayParser<AlipayTradePrecreateResponse> parser = null;
		parser = new ObjectJsonParser<AlipayTradePrecreateResponse>(request.getResponseClass());
		Map<String, Object> resultMap = new HashMap<String, Object>();
		try {
			resultMap = ScanPayUtil.requestScanpay(builder);
			response = parser.parse((String) resultMap.get("rsp"));
    ...
    protected AlipayTradeQueryResponse loopQueryResult(AlipayTradeQueryRequestBuilder builder) {
        ...
        for (int i = 0; i < AlipayConfig.getMax_query_retry(); i++) {
            Utils.sleep(AlipayConfig.getQuery_duration());
    ...
    protected void asyncCancel(final AlipayTradeCancelRequestBuilder builder) {
        executorService.submit(new Runnable() {
            @Override
            public void run() {
                for (int i = 0; i < AlipayConfig.getMax_cancel_retry(); i++) {
                    Utils.sleep(AlipayConfig.getCancel_duration());
