...
//        tpm.init(); 
	...
	public void doFilter(ServletRequest arg0, ServletResponse arg1, FilterChain arg2)
		...
		authorization = request.getHeader("Authorization") != null ? (String) request.getHeader("Authorization") : "";
			...
			if(!stateFlag){//如果stateFlag为false则直接返回
				return;
			}
			...
			if(StringUtils.isBlank(signflat)){
				resposeMessageToCustomer(response,"系统标识缺失");
				return;
			}
			...
			if(StringUtils.isBlank(appkey)){
				logger.error("appKey不存在      appKey = " + appkey);
				resposeMessageToCustomer(response,"appKey不存在");
				return;
			}

			//从缓存中获取信息,获取不出来默认用户信息已过期
//			logger.error("signflat + appkey === " + signflat + appkey);
