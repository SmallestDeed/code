...
//		Integer userStatusType = model.getUserStatusType();
        ...
        groupSearch.setAppId(model.getAppId());
        groupSearch.setPlatFormCode(platformCode);
        LoginUser loginUser = sysUser.toLoginUser();
        if (loginUser == null) {
...
// 		}
        ...
        DesignPlanRecommendedProduct designPlanProduct = null;
        // 设置主产品属性过滤查询条件
        if (planProductId != null && planProductId > 0) {
            //先查推荐方案 gaoj- 2018.06.22-03：05
            designPlanProduct = designPlanRecommendedProductServiceV2.get(planProductId);
            ...
            if(designPlanProduct == null) {
                ...
                if (scene != null) {
                    designPlanProduct = scene.recommendedCopy();
                } else {
                    //不查草图方案
...
//				this.setAuthorizedConfigSearchParam(productTypeValue, smallTypeValue, groupSearch, loginUser);
            ...
            if (total > 0 || byBrandIdsTotal > 0) { // 获取组基本信息，包括 res_file, res_pic
                ...
                if (StringUtils.isEmpty(groupSearch.getAppId()) && groupResultList != null && groupResultList.size() > 0) {
                    // 处理 组装产品组中的产品列表
    ...
    @Override
    public Object searchProductGroup2(MobileSearchProductModel model, String mediaType) {
        // FIXME: 参数不一定能获取到，要去看看
        ProductCategoryRel productCategoryRel = new ProductCategoryRel();
        Integer limit = model.getLimit();
        Integer start = model.getStart();
...
//		Integer userStatusType = model.getUserStatusType();
        ...
        Integer userType = userStatusType;
        GroupProductSearch groupSearch = this.initGroupProductSearchFromCommon(null, productCategoryRel, userStatusType,userType);
        // 初始化
        if ( groupSearch == null ) {
        ...
        if(designPlanProduct != null) {
            designPlanId = designPlanProduct.getPlanRecommendedId();
            templateProductId = ""; // Didn't used it in this search.
            queryType = "";// Didn't used it in this search.
            groupType = "";// Didn't used it in this search.
            type = "";// Didn't used it in this search.
            designTempletId = 0;// Didn't used it in this search.
        }
        logger.error("Begin call the service searchProductGroupV2" + model.getAppId() + "getPlatformCode" + model.getPlatformCode());
        if(StringUtils.isNotEmpty(model.getAppId())) {
            //platform is miniprogram
            groupSearch.setAppId(model.getAppId());
            groupSearch.setPlatFormCode(model.getPlatformCode());
            logger.error("Begin call the service searchProductGroupV2" + model.getAppId() + "getPlatformCode" + model.getPlatformCode());
        }
        logger.error("Begin call the service searchProductGroupV2");
        return commonAppSearchService.searchProductGroupV2( productCategoryRel,  houseType,  designPlanId,
                ...
                groupType, type, structureId, designTempletId, userStatusType,  loginUser2,  groupSearch,  mediaType, null);
    }
     ...
     * @return
     */
    private GroupProductSearch initGroupProductSearchFromCommon (HttpServletRequest request,
    ...
    private void setProductVisibilityRangeParam(String productTypeValue, String smallTypeValue, GroupProductSearch groupSearch
		...
		if (Utils.isExternalUser(userType) || (PLATFORM_CODE_MINI_PROGRAM.equalsIgnoreCase(groupSearch.getPlatFormCode()) && userType.intValue() == 8)) {
			// 获取用户点击的产品类型
