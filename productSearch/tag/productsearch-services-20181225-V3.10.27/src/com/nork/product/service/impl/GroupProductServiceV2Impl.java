...
public class GroupProductServiceV2Impl implements GroupProductServiceV2 {
	...
	public GroupProductSearch configSearchFor(GroupProductSearch groupSearch, String  productTypeValue, String  smallTypeValue) {
		...
		groupSearch.setStructureState(GROUP_SEARCH_TYPE.normal_group.ordinal());	/*结构组合没有数据就查询普通组合*/
	...
	public List<SearchProductGroupResult> getGroupListByProduct(GroupProductSearch search, Integer userID, Integer structureId,Integer usertype,String versionType) {
		logger.debug("search getCompositeType== " + search.getCompositeType());
...
//					else {}
				...
				productGroup.setBrandId(baseInfo.getBrandId());
				productGroup.setBrandName(baseInfo.getBrandName());
				productGroup.setProductId(baseInfo.getProductId());
...
//		this.productGroupDao.getProductInfoInTheGroupByGroupIDAndModelIDType(map)
		...
		Map<String, Map<String, String>> productCodeAndMarkMap = new HashMap<>(); // 用来缓存1、材质，是可以替换的还是不可以替换的。2、product code 3、typemark 4、small_typemark
		...
		for (ProductInfoInGroupModel productInfoInGroupModel : allProductInfoList) {
			...
			String splitTexturesInfo = productInfoInGroupModel.getSplitTexturesChooseInfo();
			if (StringUtils.isEmpty(splitTexturesInfo)) {
				splitTexturesInfo = productInfoInGroupModel.getSplitTexturesInfo();
			}

			codeAndMarkMap.put(PRODUCT_CODE_AND_MARK_MAP_KEY_split_textures_info, splitTexturesInfo);
			codeAndMarkMap.put(PRODUCT_CODE_AND_MARK_MAP_KEY_material_pic_ids, productInfoInGroupModel.getMaterialPicIds());
			productCodeAndMarkMap.put(productInfoInGroupModel.getGroupID()+"_"+productInfoInGroupModel.getProductID(), codeAndMarkMap);
			...
			Double salePrice = productInfoInGroupModel.getSalePrice();
			groupPriceMap.put(groupID, groupPriceMap.get(groupID) + (salePrice == null ? 0 : salePrice)); // 价格加上去
			...
			//判断是否是多维材质
			if(Utils.isBlank(productInfoInGroupModel.getSplitTexturesInfo())){
				detail.setIsMultidimensional(0);
			}else{
				detail.setIsMultidimensional(1);
			}
			//添加床沿单位(存储单位为mm,换算为cm)
			detail.setBedsideDimensions(null == productInfoInGroupModel.getBedsideDimensions() ? null : (new Double(productInfoInGroupModel.getBedsideDimensions())/100));
		...
		for (SearchProductGroupResult groupResult : groupList) {
			...
			if ( list != null && !list.isEmpty() ) {
				...
				for (SearchProductGroupDetail searchProductGroupDetail : list) {
					Integer productID = searchProductGroupDetail.getProductId();
					Map<String, String> caizhiMap = productCodeAndMarkMap.get(searchProductGroupDetail.getProductGroupId()+"_"+productID);
		...
		//需要返回的产品属性map
		Map<Integer, Map<String, String>> returnProductAttrMap = new HashMap<>();
...
//						String attributeValueId = (String)map.get("attributeValueId"); // 现在不需要
					...
					//需要返回出去的产品属性集合
					Map<String,String> returnPropertyMap = new HashMap<String,String>();
					String code = (String)map.get("code");
					String parentCode = (String)map.get("parentCode");
					if(!Utils.isBlank(parentCode)){
						if(Objects.equals("structureSign",parentCode)){
							returnPropertyMap.put(parentCode,code);
						}else{
							returnPropertyMap.put(parentCode,propValue);
						}
					}
					returnProductAttrMap.put(productID,returnPropertyMap);
				}
			}
			else {}
		}
		long time7 = System.currentTimeMillis();
		System.out.println("point============================处理产品属性用时:"+(time7-time6));
		...
		for ( List<SearchProductGroupDetail> detailsInEveryGroup : listMap.values()) {
			...
			for (SearchProductGroupDetail searchProductGroupDetail : detailsInEveryGroup) {
				long time_7_1 = System.currentTimeMillis();
				Integer productID = searchProductGroupDetail.getProductId();
				Integer groupId = searchProductGroupDetail.getProductGroupId();
				Map<String,String> map = returnProductAttrMap.get(productID);
				/***在组合产品查询列表 中 增加产品属性****/
				...
				System.out.println("point============================查询填充属性用时:"+(time_7-time_7_1));
				// 进行 规则 service 的调用
				Map<String, String> codeAndMarkmap = productCodeAndMarkMap.get(groupId+"_"+productID);

				//TODO:整理下
				Map<String,String> rulesMap = null;
				Map<Object,Object>paramsMap =new HashMap<Object,Object>();
				paramsMap.put("productId", productID);
				paramsMap.put("designPlanId", null);
				...
				if(rulesMap == null){
					rulesMap = designRulesService.getRulesSecondaryList(String.valueOf(productID), codeAndMarkmap.get(PRODUCT_CODE_AND_MARK_MAP_KEY_productcode),
					...
					if(Utils.enableRedisCache()){
						//为适应PC端存储的缓存value 而存储ResponseEnvelope对象.....
						responseEnvelope = new ResponseEnvelope();
						responseEnvelope.setObj(rulesMap);
						//超时时间设为3小时
						CommonCacher.addAll(ModuleType.DesignRules, "getRulesSecondaryList", paramsMap, responseEnvelope , 3*60*60);
					}
				}
				searchProductGroupDetail.setRulesMap(rulesMap);
				long time_8 = System.currentTimeMillis();
				System.out.println("point============================查询填充规则用时:"+(time_8-time_7));
		...
		System.out.println("point============================填充产品属性和查询规则总用时:"+(time8-time7));
...
//				group.setBrandName(groupMainProductBrandNameMap.get(groupID));
		...
		System.out.println("point============================查询组合产品信息总用时:"+(time9-time1));
		return listMap;
	}
...
//						String attributeValueId = (String)map.get("attributeValueId"); // 现在不需要
		...
		for ( List<SearchProductGroupDetail> detailsInEveryGroup : listMap.values()) {  
			for (SearchProductGroupDetail searchProductGroupDetail : detailsInEveryGroup) {
				...
				searchProductGroupDetail.setPropertyMap(map);
				// 进行 规则 service 的调用
				Map<String, String> codeAndMarkmap = productCodeAndMarkMap.get(searchProductGroupDetail.getProductGroupId()+"_"+productID);
...
//				group.setBrandName(groupMainProductBrandNameMap.get(groupID));
	...
	@Override
	public List<SearchProductGroupInfoResult> getGroupInfoListBySearch(GroupProductSearch search, Integer userID) {

		List<SearchProductGroupInfoResult> groupInfoResults = productGroupDao.getGroupListBySearchParam(search);

		List<Integer> groupIDSList = new ArrayList<>();
/*
		groupIDSList = groupInfoResults.stream().map(SearchProductGroupInfoResult::getGroupId).collect(Collectors.toList());
*/
		for (SearchProductGroupInfoResult infoResult : groupInfoResults){
			if(infoResult != null && infoResult.getGroupId() != null){
				groupIDSList.add(infoResult.getGroupId());
			}
		}
		// 根据 userid 和 productgroupid 获取收藏
		Map<String, Object> params = new HashMap<>();
...
//			List<Map<String, Integer>> collectedGroupList = this.productGroupDao.getProductGroupUserCollectedUsingUnion(params);
		...
		if (collectedGroupList != null && collectedGroupList.size() > 0) {
			...
			// 遍历 resultList ，设置用户是否已经收藏
			for (SearchProductGroupInfoResult groupResult : groupInfoResults) {
				Integer groupID = groupResult.getGroupId();
				if (tempMap.get(groupID) != null) {
					// 收藏了
					groupResult.setCollectState(GROUP_COLLECTED_STATUS.collected.ordinal());
