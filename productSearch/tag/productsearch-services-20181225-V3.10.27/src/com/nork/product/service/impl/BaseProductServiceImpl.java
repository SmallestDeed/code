				...
				for (SplitTextureInfoDTO splitTextureInfoDTO : splitTextureList) {
					...
					if (StringUtils.equals("choose", type) || StringUtils.equals("all", type)) {
						...
						if (defaultId != null && defaultId > 0) {
							ResTexture resTexture = resTextureService.getIncludeDeleted(defaultId);
					...
					logger.error("------baseProduct的SplitTextureInfo信息格式错误,SplitTextureInfo:" + splitTexturesInfo + "  ,Exception:" + e);
				}
				return map;
			}
...
//				}
	...
	@Override
	public Integer getPracticableProductInfo(Integer productId, Integer fullPaveLength) throws BizException {
		String exceptionLogPrefix = "获取多模型适应产品失败";
		
		// 参数验证 ->start
		if(productId == null) {
			String log = "prarms error: productId = null";
			logger.error(LOGPREFIX + log);
			throw new BizException(exceptionLogPrefix + "(" + log + ")");
		}
		if(fullPaveLength == null) {
			String log = "prarms error: fullPaveLength = null";
			logger.error(LOGPREFIX + log);
			throw new BizException(exceptionLogPrefix + "(" + log + ")");
		}
		BaseProduct baseProduct = this.get(productId);
		if(baseProduct == null) {
			String log = "baseProduct not found; baseProductId = " + productId;
			logger.error(LOGPREFIX + log);
			throw new BizException(exceptionLogPrefix + "(" + log + ")");
		}
		if(baseProduct.getProductBatchId() == null || baseProduct.getProductBatchId().intValue() == 0) {
			// 非多模型产品
		...
		//add by xiaoxc-20181229 增加高度匹配
		Integer productHeight = 0;
		if (!StringUtils.isEmpty(baseProduct.getProductHeight())) {
			productHeight = Integer.valueOf(baseProduct.getProductHeight());
		}

		List<BaseProduct> baseProductList = this.getBaseProductListByProductBatchId(baseProduct.getProductBatchId(), fullPaveLength, productHeight);
		if(Lists.isEmpty(baseProductList)) {
			String log = "未找到模型组产品";
			logger.error(LOGPREFIX + log);
			throw new BizException(exceptionLogPrefix + "(" + log + ")");
		}
		
		return baseProductList.get(0).getId();
	}

	@Override
	public List<BaseProduct> getBaseProductListByProductBatchId(Integer productBatchId, Integer fullPaveLength, Integer productHeight) throws BizException {
		String exceptionLogPrefix = "获取产品组所有产品信息失败";
		
		// 参数验证 ->start
		if(productBatchId == null) {
			String log = "params error: productBatchId = null";
			logger.error(LOGPREFIX + log);
			throw new BizException(exceptionLogPrefix + "(" + log + ")");
		}
		if (fullPaveLength == null || productHeight == null) {
			String log = "params error: fullPaveLength = null || productHeight = null";
			logger.error(LOGPREFIX + log);
			throw new BizException(exceptionLogPrefix + "(" + log + ")");
		}
		// 参数验证 ->end
		
		return baseProductMapper.getBaseProductListByProductBatchId(productBatchId, fullPaveLength, productHeight);
	}
