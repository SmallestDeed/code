...
public class SearchForCommonApp {
    ...
    public Object searchProductGroupV2(HttpServletRequest request,
        ...
        //显示我的组合列表，检测是否为当前登录用户
        if(productCategoryRel.getUserId() != null){
            if(!productCategoryRel.getUserId().equals(loginUser2.getId())){
                return new ResponseEnvelope<>(false, "请求用户非当前登录用户,请重新登录", productCategoryRel.getMsgId());
            }
        }
...
//        String platformCode = request.getHeader("Platform-Code");
        ...
        return commonAppSearchService.searchProductGroup( productCategoryRel,  houseType,  designPlanId,
                planProductId, spaceCommonId,templateProductId, productTypeValue,  smallTypeValue,  queryType,
                groupType, type, structureId, designTempletId, userStatusType,  loginUser2,  groupSearch,  mediaType, searchType);
...
//        loginUser.setId(48);
    ...
    @RequestMapping(value = "/getGroupProductResultById")
    @ResponseBody
    public Object getGroupProductResultById(Integer id, String msgId){
        if(id == null){
            return new ResponseEnvelope<>(false,"缺失参数！");
        }
        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        if(loginUser == null || loginUser.getId() == null){
            return new ResponseEnvelope<>(false,"登录失效，请重新登录！");
        }
        System.out.println(loginUser);
        if(Utils.isBlank(loginUser.getMediaType())){
            loginUser.setMediaType("3");
        }
        SearchProductGroupResult productGroupResult = commonAppSearchService.getGroupProductResultById(id,loginUser);

        return new ResponseEnvelope<>(true,productGroupResult,msgId);
    }
}
