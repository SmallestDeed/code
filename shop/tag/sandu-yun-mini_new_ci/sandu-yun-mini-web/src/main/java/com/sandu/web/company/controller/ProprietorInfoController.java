...
public class ProprietorInfoController {
    ...
    private ResponseEnvelope checkPhone(String phone, String code) {
        ...
        //String map = (String) redisTemplate.opsForHash().get(SESSION_SMS_CODE, phone);
        ...
        if (verifyCount <= 0) {//是否失效
            ...
            //redisTemplate.opsForHash().delete(SESSION_SMS_CODE, phone);
            resopnse.setSuccess(false);
        ...
        if (!code.equals(yzm)) {//匹配验证码
            ...
            //redisTemplate.opsForHash().put(SESSION_SMS_CODE, phone, gson.toJson(smsVo));
            resopnse.setSuccess(false);
