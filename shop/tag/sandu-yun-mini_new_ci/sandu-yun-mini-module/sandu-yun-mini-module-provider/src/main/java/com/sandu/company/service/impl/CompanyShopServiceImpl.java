...
public class CompanyShopServiceImpl implements CompanyShopService {
	...
	private final static Cache<String, List<ShopPopularityInfoVO>> LOCAL_CACHE_SHOP_POPULARITY_INFO= CacheBuilder
			.newBuilder()
			.expireAfterWrite(1, TimeUnit.HOURS)
			.build();
	private final static String LOCAL_CACHE_SHOP_POPULARITY_INFO_KEY="LOCAL_CACHE_SHOP_POPULARITY_INFO_KEY";
	...
	public CompanyShopDetailVo get(CompanyShopDetailQuery query) {
		...
		if (detailVo != null) {
			...
			//设计师数量
			if (query.getPlatformValue() != null) {
			...
			// 转换参数 企业店铺装修方式:1.半包,2.全包,3.包软装,4.清水
			List<String> businessShopDecorationList = this.getBusinessShopDecorationList(detailVo.getBusinessShopDecorationType());
			detailVo.setBusinessShopDecorationList(businessShopDecorationList);

			try {
				Map<String, Object> map = nodeInfoBizService.getNodeData((int) detailVo.getId(), new Byte("6"), 0);
				Integer virtualView = map.get("setVirtualViewNum") == null ? 0 : (Integer)map.get("setVirtualViewNum");
				detailVo.setVisitCount(detailVo.getVisitCount() + virtualView);
			}catch (Exception e) {
				logger.error("get node info fail!!!");
			}
		}
		return detailVo;
	}
	...
	public Page<CompanyShopVo> list(CompanyShopQuery query) throws ElasticSearchException {
		...
		for (CompanyShopVo vo : lstVo) {
			...
			try {
				Map<String, Object> map = nodeInfoBizService.getNodeData((int) vo.getId(), new Byte("6"), 0);
				Integer virtualView = map.get("setVirtualViewNum") == null ? 0 : (Integer)map.get("setVirtualViewNum");
				vo.setVisitCount(vo.getVisitCount() + virtualView);
			}catch (Exception e) {
				logger.error("get node info fail!!!");
			}
			//查询工程案例集合
			ProjectCaseQuery projectCaseQuery = new ProjectCaseQuery();
			projectCaseQuery.setShopId((int)vo.getId());
			List<ProjectCaseVo> shopProjectCaseList = this.getShopProjectCaseList(projectCaseQuery);
			vo.setShopProjectCaseList(shopProjectCaseList);

			// 转换参数 企业店铺装修方式:1.半包,2.全包,3.包软装,4.清水
			List<String> businessShopDecorationList = this.getBusinessShopDecorationList(vo.getBusinessShopDecorationType());
			vo.setBusinessShopDecorationList(businessShopDecorationList);
		}
		page.setList(lstVo);
	...
	// 转换参数 企业店铺装修方式:1.半包,2.全包,3.包软装,4.清水
	private List<String> getBusinessShopDecorationList(String businessShopDecorationType) {
		if (StringUtils.isNotBlank(businessShopDecorationType)) {
            String[] split = businessShopDecorationType.split(",");
            List<String> businessShopDecorationList = new ArrayList<>();
            for (String s : split) {
                if (StringUtils.isNotBlank(s)) {
                    switch (s) {
                        case "1" :
                        	businessShopDecorationList.add("半包");
                        	break;
                        case "2" :
                        	businessShopDecorationList.add("全包");
                        	break;
                        case "3" :
                        	businessShopDecorationList.add("包软装");
							break;
                        case "4" :
                        	businessShopDecorationList.add("清水");
							break;
                    }
                }
            }
            return businessShopDecorationList;
        }
        return null;
	}

	@Override
	public Page<CompanyShopVo> getShopList(CompanyShopQuery query) {
		Long startTime = System.currentTimeMillis();
		//根据店铺类型得出用户类型
		setUserTypeByShopType(query);
		Page<CompanyShopVo> page = new Page<CompanyShopVo>();
		...
		page.setCount(count);
		page.setPageSize(query.getPageSize());
		List<CompanyShopVo> lstVo=companyShopDao.findFontPageListNew(query);
		logger.info("店铺列表sql查询耗时：{}, count:{}" , (System.currentTimeMillis() - startTime), count);
		startTime = System.currentTimeMillis();
		if (null == lstVo || 0 == lstVo.size()) {
			return page;
		...
		List<Long> shopIds = lstVo.stream().map(lst -> lst.getId()).collect(Collectors.toList());
		List<ShopPlanInfoPo.PlanPicInfo> infoList = companyShopDao.selectPlanPicPath(shopIds);
		Map<Long, List<ShopPlanInfoPo.PlanPicInfo>> infoMap = infoList.stream().collect(Collectors.groupingBy(ShopPlanInfoPo.PlanPicInfo::getShopId));
		logger.info("店铺方案数据sql查询耗时：{}, count:{}" , (System.currentTimeMillis() - startTime), infoList.size());
		startTime = System.currentTimeMillis();

		for (CompanyShopVo vo : lstVo) {
			List<ShopPlanInfoPo.PlanPicInfo> planInfo = infoMap.get(vo.getId());
			if (planInfo != null && planInfo.size() > 0){
				vo.setPlanTotal(planInfo.size());
				vo.setShopPlanCount(planInfo.size());
				vo.setLstResPic(planInfo);
			}
			vo.setBusinessTypeName(ShopUtils.getBusinessTypeName(vo.getBusinessType()));
			vo.setPraiseRatePercent(DigitalUtils.FloatToPercent(vo.getPraiseRate()));
			// 分类信息
			vo.setCategoryNames(this.getCategoryNames(vo));
			vo.setFirstCategoryNames(this.getFirstCategoryNames(vo));
			vo.setPlanTotal(vo.getLstResPic() == null? 0 : vo.getLstResPic().size());
			vo.setShopPlanCount(vo.getLstResPic() == null? 0 : vo.getLstResPic().size());
			// 获取沟通对象名称
			getCommunicationObj(vo);
			//获取封面图片资源
			String coverResIds = vo.getCoverResIds();
...
//			}
	...
	@Override
	public Integer getShopProjectCaseCount(ProjectCaseQuery query) {
		return companyShopDao.getShopProjectCaseCount(query);
	}

	@Override
	public ProjectCaseVo getProjectCaseDetail(Integer caseId) {
		return companyShopDao.getProjectCaseDetail(caseId);
	}

	@Override
	public List<Integer> getNearestContactShopId(Integer userId, Integer start, Integer limit, List<Integer> businessTypeList) {
		if (userId != null) {
			return companyShopDao.getNearestContactShopId(userId, start, limit, businessTypeList);
		}else {
			return null;
		}
	}
...
//				vo.setFirstCategoryNames(getFirstCategoryNames(vo));
	...
	private List<ShopPopularityInfoVO> getShopPopularityInfoData(CompanyShopQuery query) {
		...
		this.setUserTypeByShopType(query);
	...
	@Override
	public Page <ShopPopularityInfoVO> getAllShopPopularityList(CompanyShopQuery query) {
		List<ShopPopularityInfoVO> shopPopularityInfoVOList = new ArrayList <>();
		Page<ShopPopularityInfoVO> page = new Page<>();
		 try {
			 shopPopularityInfoVOList = LOCAL_CACHE_SHOP_POPULARITY_INFO.get(LOCAL_CACHE_SHOP_POPULARITY_INFO_KEY, new Callable<List<ShopPopularityInfoVO>>() {
		         @Override
		         public List<ShopPopularityInfoVO> call() throws Exception {
		             return getShopPopularityInfoData(query);
		         }
		     });
		...
		page.setList(shopPopularityInfoVOList);
		return page;
	}
		...
		if (list != null && list.size() > 0) {
			list.forEach(projectCaseVo -> {
				...
				if ("detail".equals(query.getPageType())) {
					...
					long aa = projectCaseVo.getCreateDate().getTime();
					long time = System.currentTimeMillis() - projectCaseVo.getCreateDate().getTime();
					long releaseDay = time / (1000 * 60 * 60 * 24);
					projectCaseVo.setReleaseDay((int)releaseDay);
					//获取详情时修改案例浏览数量
